...
public partial class fin_base_Pocketmenu : System.Web.UI.MasterPage
{
    string Cstr, str, co_cd = "", mbr = "", hname, br_name, btnval, yr, val1, tab_name, mhd, uname, cond, squery, col2,
        bval, get_qstr, url, usr_dept, frm_grp, frm_formID, frm_ulevel, frm_CDT1, frm_CDT2;
    ...
    protected void btnbackup_ServerClick(object sender, EventArgs e)
    {
        try
        {
            ...
            string dmpfile = HttpRuntime.AppDomainAppPath + "\\backups\\" + co_cd + "" + DateTime.Now.ToString("ddMMyyyyhhmmss") + ".dmp";
            string logfile = HttpRuntime.AppDomainAppPath + "\\backups\\" + co_cd + "" + DateTime.Now.ToString("ddMMyyyyhhmmss") + ".log";
            string exp_cmd = "exp PD" + co_cd + "/"+co_cd+"PD file='" + dmpfile + "' GRANTS=Y log=" + logfile + "";
            ...
            p.Start();
            fgen.msg("-", "AMSG", "Backup Started in Background ! Please Continue your Work !!");
    ...
    public void fill_val()
    {
        ...
        lblbrcode1.InnerText = mbr.Trim();

        lblbrname.InnerText = br_name;
        ...
        if (fgen.Make_date(frm_CDT1,"dd/MM/yyyy").ToString("yyyy") == fgen.Make_date(frm_CDT1, "dd/MM/yyyy").ToString("yyyy"))
            lblyearS.InnerText = "(" + yr.Split('-')[0] + ")";
        else lblyearS.InnerText = "(" + yr + ")";
        //lblbr.Text = mbr;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (url.Contains("STR"))
        {
            if (Request.QueryString["STR"].Length > 0)
            {
                ...
                //Response.Redirect("~/home/dashboard?str=" + get_qstr + "&guid=" + EncryptDecrypt.Encrypt(get_qstr), false);
                //return;
                fgenMV.Fn_Set_Mvar("BDQSTR", "QSTR", get_qstr);
    ...
    public string GetHtml(string MyGuid)
    {
        cli = 0;
        ...
        //dtparent = sgen.fill_icon_gridMVC(co_cd, tab_name, cond, MyGuid);//for developement
        dtparent.AcceptChanges();
        Session["dtparent"] = dtparent;

        DataTable dt0 = dtparent.AsEnumerable().Where(w => Convert.ToInt64(w["mlevel"]) == 1 && Convert.ToString(w["visi"]) == "Y").Select(s => s).CopyToDataTable();
            ...
            if (!dr["submenu"].ToString().Equals("Y") && dr["web_action"].ToString().Trim().Length > 2)
            {
                ...
                if (dr["web_action"].ToString().Trim().ToLower().Equals("prnfile"))
                {
                    html = html + "<li id='l" + cli + "' class='nav-item'> " +
                        "<a  onclick='liclick(this)' href='javascript:void(0);'  prd='" + (dr["prd"].ToString() + dr["brn"].ToString()).Trim().ToUpper() + "' onclick='ShowRPT(this);' id='m" + dr["id"].ToString() + "' >" +
                      dr["text"].ToString() + "</a></li>";
                }
                else if (dr["web_action"].ToString().Trim().ToLower().Equals("withfoo"))
                {
                    html = html + "<li id='l" + cli + "'> <a onclick='liclick(this)' href='javascript:void(0);'  prd='" + (dr["prd"].ToString() + dr["brn"].ToString()).Trim().ToUpper() + "' onclick='withfoo(this);' id='m" + dr["id"].ToString() + "' >" +
                      dr["text"].ToString() + "</a></li>";
                }
                else
                {
                    html = html + "<li id='l" + cli + "' class='nav-item'> <a onclick='liclick(this)' href='" + dr["web_action"].ToString() + "?guid=" + EncryptDecrypt.Encrypt(MyGuid) +
                "&mid=" + EncryptDecrypt.Encrypt(dr["id"].ToString()) + "&STR=" + MyGuid + "@" + dr["id"].ToString().Trim() + "' id='m" + dr["id"].ToString() + "' class='nav-link' >" +
                "<i class='fas fa-file-alt nav-icon'></i><p>" + dr["text"].ToString() + "</p></a></li>";
                }
            }
            else
            {
                ...
                html = html + "<li id='l" + cli + "' class='nav-item'> <a href='#' id='a" + cli + "'  class='nav-link'>" +
                    "<i class='nav-icon fas fa-folder-open'></i><p>" + dr["text"].ToString() + "" +
    ...
    private void makemenu(DataTable dtparent, string module3, string module1, long level, string MyGuid, string param)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                dtstatuswise = dtparent.Select("submenuid='" + module1 + "' and mlevel='" + level + "' and param='" + param + "'").CopyToDataTable();
            }
            ...
            foreach (DataRow dr in dtstatuswise.Rows)
            {
                ...
                if (!dr["submenu"].ToString().Equals("Y") && dr["web_action"].ToString().Trim().Length > 2)
                {
                    ...
                    if (dr["web_action"].ToString().Trim().ToLower().Equals("prnfile"))
                    {
                        html = html + "<li id='l" + cli + "'> <a onclick='liclick(this)' href='javascript:void(0);'  prd='" + (dr["prd"].ToString() + dr["brn"].ToString()).Trim().ToUpper() + "' onclick='ShowRPT(this);' id='m" + dr["id"].ToString() + "' >" +
                                       dr["text"].ToString() + "</a></li>";
                    }
                    else if (dr["web_action"].ToString().Trim().ToLower().Equals("withfoo"))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            html = html + "<i class='nav-icon fas fa-blank'></i>";
                        }
                        html = html + "<i class='fas fa-file-alt nav-icon'></i>";
                        html = html + "<p>" + dr["text"].ToString() + "</p></a></li>";
                    }
                }
                else
                {
                    ...
                    html = html + "<i class='nav-icon fas fa-folder-open'></i>";
                    html = html + "<p>" + dr["text"].ToString() + "" +
                               " <i class='fas fa-angle-left right'></i></p></a>";
                    html = html + "<ul id='u" + cli + "' class='nav nav-treeview'>";
                    makemenu(dtparent, dr["form"].ToString().Trim(), dr["submenuid"].ToString().Trim(), Convert.ToInt64(dr["mlevel"].ToString()) + 1, MyGuid, dr["param"].ToString().Trim());
                    html = html + " </ul>";
                }
            }
        }
