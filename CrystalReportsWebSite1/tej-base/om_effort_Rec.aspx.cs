...
public partial class om_effort_Rec : System.Web.UI.Page
{
    ...
    void setColHeadings()
    {
        ...
        for (int sR = 0; sR < sg1.Columns.Count; sR++)
        {
            ...
            for (int K = 0; K < sg1.Rows.Count; K++)
            {
                ...
                ((TextBox)sg1.Rows[K].FindControl("sg1_t2")).Attributes.Add("readonly", "readonly");
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_TASK":
                ...
                mrow = fgen.make_int(fgenMV.Fn_Get_Mvar(frm_qstr, "U_CMD_HF1"));
                SQuery = "SELECT a.stagec,b.Name,a.Stagec AS CODE FROM itwstage a,type b where b.id='#' and b.id2='TS' and trim(a.stagec)=trim(b.type1) and a.branchcd!='DD' and trim(icode)='" + sg1.Rows[mrow].Cells[13].Text + "' ORDER BY b.Name ";
                //SQuery = "SELECT Type1,Name,Type1 AS CODE,id2 as Ref FROM Type WHERE id='#' and id2='TS' ORDER BY Name ";
                break;
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    //SQuery = "Select a.*,b.text from " + frm_tabname + " a,FIN_MSYS b where trim(a.frm_name)=trim(b.id) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f2"] = dt.Rows[i]["cl_name"].ToString().Trim();
                            sg1_dr["sg1_f3"] = "-";
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = "-";

                            sg1_dr["sg1_t1"] = dt.Rows[i]["wcode"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["ef_name"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["whours"].ToString().Trim();
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_TASK":
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_HF1", hf1.Value);
                    //----------------------------
                    hffield.Value = "SG1_ROW_TASK";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Task", frm_qstr);
                }
                break;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["icode"] = sg1.Rows[i].Cells[13].Text;
                oporow["wcode"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text;
                oporow["whours"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
                oporow["remarks"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text;
                if (edmode.Value == "Y")
                {
