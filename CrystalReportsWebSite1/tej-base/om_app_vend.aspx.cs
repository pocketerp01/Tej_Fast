...
public partial class om_app_vend : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                //pop2
                SQuery = "SELECT type1 as Fstr,NAME,TYPE1  as Code FROM TYPE WHERE ID='A'  order by Name";
                break;
            case "SG1_ROW_ADD":
            case "SG1_ROW_ADD_E":
                col1 = "";
                ...
                SQuery = "SELECT Icode AS FSTR,Iname AS Item_Name,Cpartno,Cdrgno,unit,ent_by,Icode FROM Item where branchcd!='DD' and length(Trim(nvl(deac_by,'-')))<2  and length(Trim(icode))>4 " + col1 + " ORDER BY Iname ";
                break;
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t13"] = dt.Rows[i]["NUM1"].ToString().Trim();
                            sg1_dr["sg1_t14"] = dt.Rows[i]["NUM2"].ToString().Trim();
                ...
                case "Print_E":
                    ...
                    string rptname = "std_App_Price_Reg_base";
                    if (chk_opt == "Y")
                    //Member GCC Country
                    {
                        rptname = "std_App_Price_Reg_base_intl";
                    }
                    SQuery = "SELECT 'Approved Price List' AS HEADER,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy')||'-'||decode(a.app_by,'-','(Unapp)','(App)') as vchdate,a.acode,b.aname,a.icode,c.iname,c.cpartno,c.unit,a.irate,a.disc,(a.irate*((100-a.disc)/100))- a.discamt as net_rate,a.pexc as cgst,a.pcess as sgst,a.ptax as igst,a.jwrate,a.pfchg as pf_chrg,a.row_text as des,a.remark,a.srno FROM appvendvch A,famst B ,item C  WHERE TRIM(A.ICODE)=TRIM(c.ICODE) AND TRIM(A.aCODE)=TRIM(b.ACODE) AND TRIM(A.BRANCHCD)='" + frm_mbr + "' and TRIM(A.TYPE)='" + frm_vty + "' and TRIM(A.vchnum)||TO_CHAR(A.vchdate,'DD/MM/YYYY') in (" + col1 + ") ORDER BY A.SRNO ";
                    fgen.Fn_Print_Report(frm_cocd, frm_qstr, frm_mbr, SQuery, "std_App_Price_Reg_base", rptname);
                    break;
                case "TACODE":
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["NUM1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t13")).Text);
                oporow["NUM2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t14")).Text);
