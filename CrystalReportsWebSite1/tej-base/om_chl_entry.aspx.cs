...
public partial class om_chl_entry : System.Web.UI.Page
{
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                switch (lbl1a.Text)
                {
                    ...
                    case "21":
                    case "23":
                        SQuery = "SELECT distinct a.Acode as FStr,a.Aname as Party_Name,a.Acode,a.Addr1,a.Addr2,a.GST_No,a.Staten from famst a where a.branchcd!='DD' and length(trim(nvl(a.deac_by,'-'))) <2 and substr(a.acode,1,2) in ('06') /*and length(Trim(A.staten))>2 and a.ccode='T'*/ order by a.Aname ";
                        break;
                    default:
                        string chk_Req = "";
                        chk_Req = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(upper(OPT_ENABLE)) as fstr from FIN_RSYS_OPT where OPT_ID='W0048'", "fstr");
                        if (chk_Req == "Y")
                        {
                            SQuery = "select to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum as Fstr,a.vchnum as Req_no,to_char(a.vchdate,'dd/mm/yyyy') as req_dt,b.AName as Supplier,trim(a.acode) as Supp_Cd,trim(A.stage) as WIP_Stg,sum(a.iqty_chl)-sum(a.issued) as Pending_Qty,max(a.ent_by) As Request_by,round(sysdate-a.vchdate,0) as Pend_Days  from (SELECT acode,'-' as stage,vchnum,vchdate,icode,ent_by,'-' as jobno,vchdate as jobdt,req_qty as iqty_chl,0 as issued from wb_iss_req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and nvl(closed,'-')!='Y' union all SELECT acode,'-' as stage,genum,gedate,icode,null as entby,'-' as invno,vchdate,0 as iqty_chl,iqtyout as issued from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,famst b where trim(A.acode)=trim(B.acode) group by to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum,b.aName,trim(a.acode),a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),a.vchdate,trim(A.stage) having sum(a.iqty_chl)-sum(a.issued)>0 order by fstr";
                        }
                        else
                        {
                            SQuery = "SELECT distinct a.Acode as FStr,a.Aname as Party_Name,a.Acode,a.Addr1,a.Addr2,a.GST_No,a.Staten from famst a where a.branchcd!='DD' and length(trim(nvl(a.deac_by,'-'))) <2 and substr(a.acode,1,2) in ('02','05','06','14','15','16') and length(Trim(A.staten))>2 order by a.Aname ";
                        }
                        break;
                }
            ...
            case "TICODE":
                ...
                SQuery = "SELECT TYPE1 AS FSTR,NAME,TYPE1 AS CODE FROM TYPE where id='G' and type1 like '2%' ORDER BY NAME ";
            ...
            case "SG1_ROW_DT":
                ...
                SQuery = "SELECT a.Icode,c.Aname as Customer,a.icode as erpcode,a.irate,b.Cpartno AS Part_no,b.Cdrgno AS Drg_no,to_char(a.orddt,'dd/mm/yyyy') as Ord_Dt,a.ordno,to_char(a.orddt,'yyyymmdd') as VDD FROM " + frm_tabname + "  a, item b ,famst c WHERE trim(A.Acode)=trim(c.acode) and trim(A.icode)=trim(B.icode) and a.branchcd='" + frm_mbr + "' and a.icat<>'Y' and trim(a.icode) = '" + curr_itm + "' and length(Trim(nvl(b.deac_by,'-')))<=1 and length(Trim(nvl(b.hscode,'-')))>1  ORDER BY to_char(a.orddt,'yyyymmdd') desc  ";
                //SQuery = "Select sysdate,sysdate as a,sysdate as b from dual";
                SQuery = batchStkQuery(curr_itm, "");
                break;
            ...
            case "SG3_ROW_ADD_E":
                ...
                if ((frm_cocd == "AGRM" || frm_cocd == "KESR") && (lbl1a.Text == "29" || lbl1a.Text == "23" || lbl1a.Text == "22")) col1 = "'-'";
                switch (lbl1a.Text)
                {
                    case "21":
                    case "22":
                    case "23":
                        SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit,a.hscode,0 AS BAL,NULL AS btchno,NULL AS BTCHDT,a.irate from item a where  length(trim(nvl(a.deac_by,'-'))) <2 AND LENGTH(tRIM(a.ICODE))>=8 and trim(A.icode) not in (" + col1 + ") and nvl(TRIM(hscode),'-')!='-' order by a.Iname ";
                        if (frm_cocd == "AGRM")
                        {
                            //if (ddRejn.SelectedValue.ToString().Trim() == "M") SQuery = "select A.ICODE||A.BTCHNO||A.BTCHDT||A.BAL AS FSTR,B.INAME AS ITEM_NAME,A.ICODE,B.CPARTNO,B.CDRGNO,B.UNIT,A.BTCHNO,A.BTCHDT,A.PRODUCED,A.MATL_OUT AS ISSUED,A.BAL,B.HSCODE,B.IRATE from (SELECT BRANCHCD,VCHNUM as btchno,VCHDATE as btchdt,SUM(IQTYIN) AS PRODUCED,SUM(IQTYOUT) AS MATL_OUT,SUM(IQTYIN)-SUM(IQTYOUT) AS BAL,icode FROM (select BRANCHCD,trim(btchno) as vchnum,TO_CHAr(VCHDATE,'DD/MM/YYYY') AS VCHDATE,IQTYIN,0 AS IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE!='39' AND STORE='Y' UNION ALL SELECT BRANCHCD,trim(BTCHNO),trim(BTCHDT),0 AS QTYIN,IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + lbl1a.Text + "' AND STORE='Y' and trim(vchnum)||To_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "') GROUP BY BRANCHCD,VCHNUM,VCHDATE,icode HAVING SUM(IQTYIN)-SUM(IQTYOUT)>0 ) A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) ORDER BY A.ICODE,A.BTCHNO";
                            if (ddRejn.SelectedValue.ToString().Trim() == "W") SQuery = "select A.ICODE||A.BTCHNO||A.BTCHDT||A.BAL AS FSTR,B.INAME AS ITEM_NAME,A.ICODE,B.CPARTNO,B.CDRGNO,B.UNIT,A.BTCHNO,A.BTCHDT,A.PRODUCED,A.MATL_OUT AS ISSUED,A.BAL,B.HSCODE,B.IRATE from (SELECT BRANCHCD,VCHNUM as btchno,VCHDATE as btchdt,SUM(IQTYIN) AS PRODUCED,SUM(IQTYOUT) AS MATL_OUT,SUM(IQTYIN)-SUM(IQTYOUT) AS BAL,icode FROM (select BRANCHCD,trim(btchno) as vchnum,TO_CHAr(VCHDATE,'DD/MM/YYYY') AS VCHDATE,IQTYIN,0 AS IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND STORE='W' UNION ALL SELECT BRANCHCD,trim(BTCHNO),trim(BTCHDT),0 AS QTYIN,IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + lbl1a.Text + "' AND STORE='Y' and trim(vchnum)||To_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "') GROUP BY BRANCHCD,VCHNUM,VCHDATE,icode HAVING SUM(IQTYIN)-SUM(IQTYOUT)>0 ) A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) and nvl(TRIM(B.hscode),'-')!='-' ORDER BY A.ICODE,A.BTCHNO";
                        }
                        break;
                    case "24":
                        SQuery = "select trim(a.Fstr) as fstr,b.Iname as Item_Name,trim(a.ERP_code) as ERP_code,b.Cpartno as Part_no,a.irate as Prate,b.Cdrgno,(a.Qtyord)-(a.Soldqty) as Bal_Qty,b.Unit,b.hscode,trim(a.Fstr) as PO_link,(a.Qtyord) as Ord_Qty,(a.Soldqty) as Rcv_Qty from (select fstr,ERP_code,max(irate) as irate,sum(Qtyord) as Qtyord,sum(Soldqty) as Soldqty  from (SELECT trim(icode)||'-'||to_ChaR(vchdate,'YYYYMMDD')||'-'||vchnum||'-'||lpad(trim(to_Char(type,'9999')),4,'0') as fstr,trim(Icode) as ERP_code,iqtyout as Qtyord,0 as Soldqty,irate from rgpmst where branchcd='" + frm_mbr + "' and type in ('21','23') and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl7.Text.Trim() + "' union all SELECT trim(icode)||'-'||to_ChaR(rgpdate,'YYYYMMDD')||'-'||rgpnum||'-'||trim(ordlineno) as fstr,trim(Icode) as ERP_code,0 as Qtyord,iqty_chl as qtyord,0 as irate from ivoucherp where branchcd='" + frm_mbr + "' and type='00' and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl7.Text.Trim() + "')  group by fstr,ERP_code having sum(Qtyord)-sum(Soldqty)>0  )a,item b where trim(a.erp_code)=trim(B.icode) and nvl(TRIM(B.hscode),'-')!='-'  order by B.Iname,trim(a.fstr)";
                        break;
                    //case "25":
                    ...
                    //        "b.Unit,b.hscode,trim(a.Fstr) as PO_link,(a.Qtyord) as Ord_Qty,(a.Soldqty) as Rcv_Qty from (select fstr,ERP_code,max(irate) as irate,sum(Qtyord) as Qtyord,sum(Soldqty) as Soldqty  " +
                    //        "from (SELECT trim(icode)||'-'||to_ChaR(vchdate,'YYYYMMDD')||'-'||vchnum||'-'||lpad(trim(to_Char(type,'9999')),4,'0') as fstr,trim(Icode) as ERP_code,0 as Qtyord,iqtyout as Soldqty" +
                    //        ",irate from rgpmst where branchcd='" + frm_mbr + "' and type in ('25') and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl4.Text.Trim() + "'" +
                    //        " union all SELECT trim(icode)||'-'||to_ChaR(rgpdate,'YYYYMMDD')||'-'||rgpnum||'-'||trim(ordlineno) as fstr,trim(Icode) as ERP_code,iqty_chl as Qtyord,0 as qtyord,0 as irate " +
                    //        "from ivoucherp where branchcd='" + frm_mbr + "' and type='00' and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl4.Text.Trim() + "')  group by fstr," +
                    //        "ERP_code having sum(Qtyord)-sum(Soldqty)>0  )a,item b where trim(a.erp_code)=trim(B.icode) and nvl(TRIM(B.hscode),'-')!='-'  order by B.Iname,trim(a.fstr)";
                    //    break;
                    case "27":
                        SQuery = "select trim(a.Fstr) as fstr,b.Iname as Item_Name,trim(a.ERP_code) as ERP_code,b.Cpartno as Part_no,a.irate as Prate,b.Cdrgno,(a.Qtyord)-(a.Soldqty) as Bal_Qty,b.Unit,b.hscode,trim(a.Fstr) as PO_link,(a.Qtyord) as Ord_Qty,(a.Soldqty) as Rcv_Qty from (select fstr,ERP_code,max(irate) as irate,sum(Qtyord) as Qtyord,sum(Soldqty) as Soldqty  from (SELECT trim(icode)||'-'||to_ChaR(vchdate,'YYYYMMDD')||'-'||vchnum||'-'||lpad(trim(to_Char(type,'9999')),4,'0') as fstr,trim(Icode) as ERP_code,iqtyout as Qtyord,0 as Soldqty,irate from rgpmst where branchcd='" + frm_mbr + "' and type in ('21','23') and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl7.Text.Trim() + "' union all SELECT trim(icode)||'-'||to_ChaR(rgpdate,'YYYYMMDD')||'-'||rgpnum||'-'||trim(ordlineno) as fstr,trim(Icode) as ERP_code,0 as Qtyord,iqty_chl as qtyord,0 as irate from ivoucherp where branchcd='" + frm_mbr + "' and type='00' and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(Acode)='" + txtlbl7.Text.Trim() + "')  group by fstr,ERP_code having sum(Qtyord)-sum(Soldqty)>0  )a,item b where trim(a.erp_code)=trim(B.icode) and nvl(TRIM(B.hscode),'-')!='-' order by B.Iname,trim(a.fstr)";
                        break;
                    case "29":
                        SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit from item a where  length(trim(nvl(a.deac_by,'-'))) <2 AND LENGTH(tRIM(a.ICODE))>=8 and trim(A.icode) not in (" + col1 + ") and nvl(TRIM(A.hscode),'-')!='-' order by a.Iname ";
                        if (frm_cocd == "AGRM" && frm_mbr == "01")
                            SQuery = "select A.ICODE||A.BTCHNO||A.BTCHDT||A.BAL AS FSTR,B.INAME AS ITEM_NAME,A.ICODE,B.CPARTNO,B.CDRGNO,B.UNIT,A.BTCHNO,A.BTCHDT,A.PRODUCED,A.MATL_OUT AS ISSUED,A.BAL,B.HSCODE,B.IRATE from (SELECT BRANCHCD,VCHNUM as btchno,VCHDATE as btchdt,SUM(IQTYIN) AS PRODUCED,SUM(IQTYOUT) AS MATL_OUT,SUM(IQTYIN)-SUM(IQTYOUT) AS BAL,icode FROM (select BRANCHCD,trim(btchno) as vchnum,TO_CHAr(VCHDATE,'DD/MM/YYYY') AS VCHDATE,IQTYIN,0 AS IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND STORE='Y' UNION ALL SELECT BRANCHCD,trim(BTCHNO),trim(BTCHDT),0 AS QTYIN,IQTYOUT,icode FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='29' AND STORE='Y' and trim(vchnum)||To_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "') GROUP BY BRANCHCD,VCHNUM,VCHDATE,icode HAVING SUM(IQTYIN)-SUM(IQTYOUT)>0 ) A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) and nvl(TRIM(B.hscode),'-')!='-' ORDER BY A.ICODE,A.BTCHNO";
                        break;
                    default:
                        SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit,a.hscode,0 AS BAL,NULL AS btchno,NULL AS BTCHDT,a.irate from item a where  length(trim(nvl(a.deac_by,'-'))) <2 AND LENGTH(tRIM(a.ICODE))>=8 and trim(A.icode) not in (" + col1 + ") and nvl(TRIM(A.hscode),'-')!='-' order by a.Iname ";
                        break;
                }
            ...
            case "Print_Challan":
                SQuery = "SELECT 'YES' AS FSTR,'YES' AS CHOICE_,'BASIC FORMAT' AS PRINT FROM DUAL UNION ALL SELECT 'NO' AS FSTR,'NO' AS CHOICE_,'GST FORMAT' AS PRINT FROM DUAL";
                break;
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        string chk_freeze = "";
        chk_freeze = fgen.Fn_chk_doc_freeze(frm_qstr, frm_cocd, frm_mbr, "W1032", txtvchdate.Text.Trim());
        if (chk_freeze == "1")
        {
        ...
        if (fgen.make_double(txtlbl5.Text) < 1 && lbl1a.Text != "29" && lbl1a.Text != "22")
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + lbl5.Text;
        }
        if (txtlbl6.Text.Trim().Length < 2 && lbl1a.Text != "29")
        {
            reqd_nc = reqd_nc + 1;
        ...
        if (sg2.Rows.Count > 1)
        {
            string chk_opt = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(upper(OPT_ENABLE)) as fstr from FIN_RSYS_OPT where OPT_ID='W0001'", "fstr");
            if (chk_opt == "Y")
            {
                reelGridQty();
        ...
        frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
        if (frm_vty == "23" || frm_vty == "21" || frm_vty == "26")
        {
            double chk1 = 0;
            foreach (GridViewRow gr3 in sg3.Rows)
            {
                chk1 += fgen.make_double(((TextBox)gr3.FindControl("sg3_t1")).Text.ToString().Trim());
            }
            if (chk1 == 0)
            {
                fgen.msg("-", "AMSG", "Dear " + frm_uname + ", '13' Cannot Save without entering Qty of Rcv Material!!");
                return;
            }
        ...
        if (frm_cocd != "MULT")
        {
            foreach (GridViewRow gr1 in sg1.Rows)
            {
                ...
                {
                    if (gr1.Cells[13].Text.Trim().Substring(0, 2) == "70")
                    {
                        string btch = ((TextBox)gr1.FindControl("sg1_t10")).Text.Trim() + ((TextBox)gr1.FindControl("sg1_t11")).Text.Trim();
                        SQuery = batchStkQuery(gr1.Cells[13].Text.Trim(), btch);
                        col1 = fgen.seek_iname(frm_qstr, frm_cocd, SQuery, "bal");
                        if (fgen.make_double(col1) < fgen.make_double(((TextBox)gr1.FindControl("sg1_t1")).Text.Trim()))
                        {
                            fgen.msg("-", "AMSG", "Batch Stock is exceed!!'13'Batch Stock is : " + col1 + "'13' Out Qty : " + ((TextBox)gr1.FindControl("sg1_t1")).Text.Trim());
                            return;
                        }
                    }
                }
    ...
    string checkStockQty()
    {
        ...
        foreach (DataRow drQty1 in dtQty1.Rows)
        {
            ...
            frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
            col1 = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, drQty1["fstr"].ToString().Trim(), txtvchdate.Text.Trim(), false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'", ddRejn.SelectedValue.ToString().Trim());
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            //sg1_dr["sg1_f4"] = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, dt.Rows[i]["Icode"].ToString().Trim(), txtvchdate.Text.Trim(), false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'", ddRejn.SelectedValue.ToString().Trim());
                ...
                case "Print_Challan":
                    ...
                    if (Request.Cookies["REPLY"].Value.ToString().Trim() == "Y")
                        col1 = "YES";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL1", print_type.Value);
                ...
                case "Print_E":
                    ...
                    print_type.Value = col1;
                    hffield.Value = "Print_Challan";
                    fgen.msg("-", "CMSG", "Do you Want Baisc Format to Print'13'" + lblheader.Text + "'13'(No for Taxable Format)");
                    break;
                ...
                case "TACODE":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl72.Text = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT STATEnM FROM TYPE WHERE ID='B' AND TYPE1='" + frm_mbr + "'", "STATEnM");
                        txtlbl73.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select staten from famst where trim(upper(acode))=upper(Trim('" + txtlbl4.Text + "'))", "staten");

                        txtlbl8.Text = dt.Rows[i]["Ind_by"].ToString().Trim();
                        txtlbl9.Text = frm_uname;
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t8"] = "";
                            double fd_rate;
                            fd_rate = fgen.make_double(fgen.seek_iname(frm_qstr, frm_cocd, "select round(sum(iqtyin*nvl(irate,ichgs))/sum(iqtyin),3) as irate from ivoucher where branchcd='" + frm_mbr + "' and type like '0%' and vchdate>=to_Date('" + inv_St_dt + "','dd/mm/yyyy') and vchdate<=to_Date('" + txtvchdate.Text.Trim() + "','dd/mm/yyyy') and trim(upper(icode))=Trim('" + dt.Rows[i]["erp_Code"].ToString().Trim() + "') and store in ('Y','N')", "irate"));
                            sg1_dr["sg1_t5"] = fd_rate;

                            dt3 = new DataTable();
                            dt3 = fgen.getdata(frm_qstr, frm_cocd, "select a.icode,a.iname,a.cpartno,a.cdrgno,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7 from item a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.icode) ='" + dt.Rows[i]["erp_Code"].ToString().Trim() + "' ");
                            if (dt3.Rows.Count > 0)
                            {
                                if (txtlbl72.Text.Trim().ToUpper() == txtlbl73.Text.Trim().ToUpper())
                                {
                                    sg1_dr["sg1_t7"] = dt3.Rows[i]["num4"].ToString().Trim();
                                    sg1_dr["sg1_t8"] = dt3.Rows[i]["num5"].ToString().Trim();
                                }
                                else
                                {
                                    sg1_dr["sg1_t7"] = dt3.Rows[i]["num6"].ToString().Trim();
                                    sg1_dr["sg1_t8"] = "0";
                                }
                            }
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        switch (lbl1a.Text)
                        {
                            ...
                            case "23":
                                ...
                                SQuery = "SELECT '-' AS PO_NO,A.*,A.ITEM_NAME AS INAME,b.num4,b.num5,b.num6,b.num7 FROM (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in (" + col1 + ") order by A.ITEM_NAME";
                                break;
                            case "24":
                            ...
                            case "29":
                                ...
                                if (frm_cocd == "AGRM" && frm_mbr == "01")
                                    SQuery = "SELECT '-' AS PO_NO,A.*,A.ITEM_NAME AS INAME,b.num4,b.num5,b.num6,b.num7 FROM (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in (" + col1 + ")";
                                break;
                            default:
                                SQuery = "select '-' as po_no,'-' as fstr,A.ITEM_NAME AS INAME,a.*,b.num4,b.num5,b.num6,b.num7 FROM (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in (" + col1 + ")";
                                break;
                        }
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_f4"] = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, dt.Rows[d]["Icode"].ToString().Trim(), txtvchdate.Text.Trim(), false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'", ddRejn.SelectedValue.ToString().Trim());
                            ...
                            string app_rt = "";
                            app_rt = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT irate FROM ivoucher WHERE branchcd='" + frm_mbr + "' and type like '0%' and vchdate<=to_DaTE('" + txtvchdate.Text.Trim() + "','dd/mm/yyyy') and trim(Icode)='" + dt.Rows[d]["icode"].ToString().Trim() + "' and store='Y' order by vchdate desc", "irate");
                            if (app_rt == "0")
                            {
                            ...
                            if (frm_cocd == "AGRM" && lbl1a.Text == "29" && frm_mbr == "01")
                            {
                                sg1_dr["sg1_t1"] = dt.Rows[d]["bal"].ToString().Trim();
                                sg1_dr["sg1_t10"] = dt.Rows[d]["btchno"].ToString().Trim();
                                sg1_dr["sg1_t11"] = dt.Rows[d]["btchdt"].ToString().Trim();
                            }
                            sg1_dr["sg1_t12"] = "0";
                            sg1_dr["sg1_t13"] = "0";
                ...
                case "SG1_ROW_TAX":
                    ...
                    break;
                case "SG1_ROW_DT":
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t10")).Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2").ToString().Trim().Replace("&amp", "");
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t11")).Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3").ToString().Trim().Replace("&amp", "");
                    break;
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select a.Vchnum as CHL_No,to_char(a.vchdate,'dd/mm/yyyy') as Dated,c.Aname as Supplier,b.Iname as Item_Name,b.cpartno as Part_No,a.iqtyout as Chl_Qty,a.Irate,b.unit,b.hscode,a.Desc_,a.icode,a.ent_by,a.ent_Dt from " + frm_tabname + " a, item b,famst c where a.branchcd='" + frm_mbr + "'  and a.type='" + frm_vty + "' and a." + doc_df.Value + " " + PrdRange + " and trim(a.icode)=trim(b.icode) and trim(a.acode)=trim(c.acode) and a.acode like '" + party_cd + "%' and a.icode like '" + part_cd + "%' order by a." + doc_df.Value + ",a." + doc_nf.Value + ",a.morder ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (frm_cocd == "AGRM")
            {
                ((TextBox)e.Row.FindControl("sg1_t9")).Style["display"] = "none";
            }
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_DT":
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Batch", frm_qstr);
                    //fgen.Fn_open_dtbox("Select Batch", frm_qstr);
    ...
    protected void btnlbl7_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Thru ", frm_qstr);
    }
    ...
    void save_ivch()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["exc_time"] = txtlbl6.Text.Trim();
                ...
                if (frm_vty == "29")
                {
                    oporow["desc_"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim() + " " + ((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text.Trim();
                }
                ...
                oporow["REC_ISS"] = "C";
                oporow["store"] = ddRejn.SelectedValue.ToString().Trim();
                oporow["inspected"] = "Y";
    ...
    string batchStkQuery(string erpcode, string cond)
    {
        string condi;
        if (cond.Length > 0)
        {
            condi = "trim(btchno)||to_Char(vchdate,'dd/mm/yyyy') = '" + cond + "'";
        }
        else
        {
            condi = "1=1";
        ...
        return "select * from (SELECT BRANCHCD,VCHNUM as btchno,VCHDATE as btchdt,SUM(IQTYIN) AS PRODUCED,SUM(IQTYOUT) AS MATL_OUT,SUM(IQTYIN)-SUM(IQTYOUT) AS BAL FROM (select BRANCHCD,trim(btchno) as vchnum,TO_CHAr(VCHDATE,'DD/MM/YYYY') AS VCHDATE,IQTYIN,0 AS IQTYOUT FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND STORE='Y' and trim(icode)='" + erpcode + "' and " + condi + " UNION ALL SELECT BRANCHCD,trim(BTCHNO),trim(BTCHDT),0 AS QTYIN,IQTYOUT FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='29' AND STORE='Y' and trim(icode)='" + erpcode + "' and trim(vchnum)||To_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "') GROUP BY BRANCHCD,VCHNUM,VCHDATE HAVING SUM(IQTYIN)-SUM(IQTYOUT)>0 order by VCHNUM)";
    }
