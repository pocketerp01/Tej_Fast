...
public partial class om_sms_msg : System.Web.UI.Page
{
    ...
    public void enablectrl()
    {
       ...
       // btnexit.Visible = true; btncancel.Visible = false; btnhideF.Enabled = true; btnhideF_s.Enabled = true;
        ...
        btnnew.Disabled = false; btnexit.Visible = true;
    ...
    public void disablectrl()
    {
        ...
        btnnew.Disabled = true; btnexit.Visible = false;
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                ...
                break;
            case "TO":
                SQuery = "select userid as fstr, username as name,userid,deptt from evas order by username";
                break;
            case "CC":
                SQuery = "select userid as fstr, username as name,userid,deptt from evas order by username";
                break;
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            ...
            frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(vchnum) AS vchnum FROM " + frm_tabname + " where branchcd='" + frm_mbr + "' and substr(type,1,2)='10'  ORDER BY VCHDATE DESC  ", 6, "vchnum");
            MsgNo.Value = frm_vnum;
    ...
    protected void btnedit_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            ...
            {
                CheckBox chk1 = (CheckBox)gr1.FindControl("chk");
                if (chk1.Checked == true)
                {
                    if (col1.Length > 0)
                    {
                        col1 = col1 + ",'" + gr1.Cells[4].Text.Trim();
                    }
                    else
                    {
                        col1 = gr1.Cells[4].Text.Trim();
                    }
                }
            }
            ...
            if (col1.Length > 0)
            {
                cmd_query = "update " + frm_tabname + " set msgseen='Y'  where trim(branchcd)||trim(type)||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') = '"+ col1 +"' ";
                fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                fgen.msg("-", "AMSG", "This Message Marked as Seen, Sent to History Folder!");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit":
                    ...
                    break;
                case "TO":
                    TxtToCode.Value = col1;
                    TxtTo.Value = col2;
                    TxtTo.Focus();
                    break;
                case "CC":
                    TxtCCCode.Value = col1;
                    TxtCC.Value = col2;
                    TxtCC.Focus();
                    break;
                case "Print_E":
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(vchnum) AS VCH FROM mailbox2 where branchcd='" + frm_mbr + "' and type='10'  order by vchdate desc ", 6, "VCH");
                    ...
                    if (edmode.Value == "Y")
                    {
                        cmd_query = "update " + frm_tabname + " set branchcd='DD' where branchcd='" + frm_mbr + "' and type='10' and vchnum||to_char(vchdate,'dd/mm/yyyy')='" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "' ";
                        fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                    ...
                    if (edmode.Value == "Y")
                    {
                        cmd_query = "delete from " + frm_tabname + " where branchcd='DD' and type='10' and  vchnum||to_char(vchdate,'dd/mm/yyyy')='" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'  ";
                        fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            //fgen.send_mail(frm_cocd, "Tejaxo ERP", "info@pocketdriver.in", "", "", "Hello", "test Mail");
                             fgen.msg("-", "AMSG","Message Delivered!");
                        }
                        else
                        {
    ...
    void save_fun()
    {
        ...
        oporow["vchnum"]    = MsgNo.Value.ToString();
        oporow["vchdate"]   = DateTime.Now.ToShortDateString();
        oporow["Type"]      = frm_vty;
        oporow["msgto"]     = TxtToCode.Value;
        oporow["msgfrom"]   = frm_uname;
        oporow["msgtxt"]    = TxtMsg.Value + " Orig Msg To " + TxtTo.Value.ToString();
        oporow["Msgdt"]     = frm_UserID;
        oporow["msgseen"]   = "N";
        oporow["attachm"]   = TxtAttach2.Text;
        oDS.Tables[0].Rows.Add(oporow);

        if (TxtCC.Value.Length > 0)
        {
            string cccode = ""; string cccode1 = "";
            cccode = TxtCCCode.Value.ToString();
            string[] cccodelist = cccode.Split(',');

            foreach (string code in cccodelist)
            {
                string cccodename = "";
                oporow = oDS.Tables[0].NewRow();
                oporow["BRANCHCD"] = frm_mbr;
                ...
                oporow["vchdate"] = DateTime.Now.ToShortDateString();
                oporow["Type"] = frm_vty;
                cccode1 = code.Replace("`", "");
                cccode1 = code.Replace("'", "");
                oporow["msgto"] = cccode1;
                cccodename = fgen.seek_iname(frm_qstr, frm_cocd, "select username from evas where userid='" + cccode1 + "'", "username");
                oporow["msgfrom"] = frm_uname;
                oporow["msgtxt"] = TxtMsg.Value + " Orig Msg To " + cccodename;
                oporow["Msgdt"] = frm_UserID;
                oporow["msgseen"] = "N";
                oporow["attachm"] = TxtAttach2.Text;
                oDS.Tables[0].Rows.Add(oporow);
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[0].HeaderStyle.Width = 20;
    ...
    public void create_tab()
    {
        ...
        //sg1_dt.Columns.Add(new DataColumn("chk", typeof(Int32)));
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        col1 = fgen.seek_iname(frm_qstr, frm_cocd, "select vchnum from mailbox2 where type='10' and trim(upper(vchnum))='" + MsgNo.Value + "' order by vchdate desc", "vchnum");

        if (col1 != "0" && edmode.Value != "Y")
        {
            fgen.msg("-", "AMSG", "Msg no already Exist!!");
            return;
        }
    ...
    protected void BtnCC_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "CC";
        make_qry_4_popup();
    ...
    {
        string filePath = Label19.Text.Substring(Label19.Text.ToUpper().IndexOf("UPLOAD"), Label19.Text.Length - Label19.Text.ToUpper().IndexOf("UPLOAD"));
    ...
    protected void BtnAttach2_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload2.HasFile)
        {
            ...
            filepath = filepath + "_" + frm_mbr + "_" + "DI" + "_" + MsgNo.Value.Trim()  + "_" + FileUpload2.FileName;
            //  Attch.PostedFile.SaveAs(filepath);
            FileUpload2.PostedFile.SaveAs(filepath);
            filepath = Server.MapPath("~/tej-base/UPLOAD/") + "_" + frm_mbr + "_" + "20" + "_" + MsgNo.Value.Trim() + "~" + FileUpload2.FileName;
            FileUpload2.PostedFile.SaveAs(filepath);
            Label19.Text = filepath;
    ...
    protected void Btnhisout_ServerClick(object sender, EventArgs e)
    {
        hffield.Value = "List";
        SQuery = "Select distinct vchnum||to_char(vchdate,'dd/mm/yyyy') as fstr,Msgseen as Seen,Replace(MsgTo,'`','') as MsgTo,to_char(vchdate,'dd/mm/yyyy') as Dated  from mailbox2 where VCHDATE " + DateRange + " AND trim(msgfrom)='" + frm_uname + "' order by Dated desc";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        fgen.Fn_open_rptlevel("List of Outgoing Messages!", frm_qstr);
        hffield.Value = "-";
    }
    protected void BtnhisIn_ServerClick(object sender, EventArgs e)
    {
        SQuery = "Select Msgseen as Seen,Vchdate as Dated,Msgfrom  from mailbox2 where VCHDATE " + DateRange + " AND trim(msgto)='" + frm_UserID + "' order by vchdate desc";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        fgen.Fn_open_rptlevelIMG("List of Incoming Messages", frm_qstr);
        hffield.Value = "-";
    }
    ...
    public void Fill_sg1() {
        ...
        dt = new DataTable();
        SQuery = "Select * from mailbox2 where VCHDATE BETWEEN TO_DATe('" + DateTime.Now.AddDays(-15).ToShortDateString() + "','DD/MM/YYYY') AND  TO_DATe('" + DateTime.Now.AddDays(1).ToShortDateString() + "','DD/MM/YYYY') AND trim(msgto)='" + frm_UserID.Trim() + "' and msgseen='N' order by vchdate desc";
        dt = fgen.getdata(frm_qstr,frm_cocd,SQuery);
        if (dt.Rows.Count > 0) {
            create_tab();
            for (int i = 0; i < dt.Rows.Count; i++) {
                ...
                sg1_dr = sg1_dt.NewRow();
                sg1_dr["sg1_f1"] = dt.Rows[i]["msgfrom"].ToString().Trim();
                sg1_dr["sg1_f2"] = dt.Rows[i]["msgtxt"].ToString().Trim();
                sg1_dr["sg1_f3"] = Convert.ToDateTime(dt.Rows[i]["vchdate"].ToString().Trim()).ToShortDateString();
                sg1_dr["sg1_f4"] = dt.Rows[i]["branchcd"].ToString().Trim() + dt.Rows[i]["Type"].ToString().Trim() + dt.Rows[i]["vchnum"].ToString().Trim() + Convert.ToDateTime(dt.Rows[i]["vchdate"].ToString().Trim()).ToShortDateString();
                sg1_dr["sg1_f5"] = dt.Rows[i]["attachm"].ToString().Trim();
                sg1_dr["sg1_f6"] = dt.Rows[i]["msgdt"].ToString().Trim();
                sg1_dt.Rows.Add(sg1_dr);
            }
