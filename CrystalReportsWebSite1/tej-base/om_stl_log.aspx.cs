...
public partial class om_stl_log : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "Software Training Library";
    ...
    public void enablectrl()
    {
        ...
        ddpurpose.Disabled = false;
        ddvertical.Disabled = false;
    }
    //------------------------------------------------------------------------------------
    ...
    public void set_Val()
    {
        ...
        doc_df.Value = "STLDT";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "WB_STL_LOG";
        switch (Prg_Id)
        {
            case "F94101":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "TG");
                typePopup = "N";
                break;
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "USR":
                ...
                break;
            case "PERSON":
                //SQuery = "SELECT DISTINCT USERNAME ,USERNAME AS COCD,FULL_NAME AS company_name FROM EVAS WHERE NVL(USERNAME,'-')='FINS' ORDER BY USERNAME";
                SQuery = "select trim(upper(Cont_no)) as fstr, trim(upper(Cont_Name)) as Client_Person_name,trim(upper(Cont_Email)) as Email,trim(upper(Cont_no)) as Mobile_no,count(*) as Requests from wb_css_log  where branchcd!='DD' and UPPER(TRIM(Ccode))='FINS' and length(Trim(Cont_email))>4 group by trim(upper(Cont_no)), trim(upper(Cont_Name)),trim(upper(Cont_Email)),trim(upper(Cont_no)) order by trim(upper(Cont_Name)) ";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            fgen.msg("-", "AMSG", "Title /Subject not entered !!");
            return;
        }
        ...
        if (txtAttch.Text.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Attache a File !!");
            return;
        }
        ...
        {
            fgen.msg("-", "AMSG", " Please Enter Purpose Achieved !!");
            return;
        }
        if (txtWrkRmk.Text.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Pre Requisites !!");
            return;
        }
        string mandField = "";
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        ddvertical.Value = dt.Rows[i]["EVERTICAL"].ToString().Trim();
                        ddpurpose.Value = dt.Rows[i]["EPURPOSE"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT a.STLNO as STL_NO,to_char(A.STLDT,'dd/mm/yyyy') as STL_Dt,a.Evertical as Vertical_Name,a.Eicon,a.CCODE as Client_Code,a.Emodule as Module_Name,a.Remarks,a.wrkRmk,a.Ent_Dt,a.Ent_by,to_chaR(a.sTLDT,'YYYYMMDD') as ACT_DT FROM " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.STLdt " + PrdRange + " order by a.STLno ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                ...
                try
                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "STL No " + frm_vnum + "'13'We appreciate your Entry to Finsys STL.Aiming for Systematic Working");
                        }
                        else
                        {
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F94101":
                SQuery = "SELECT 'TG' AS FSTR,'Software Training Guide' as NAME,'TG' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "TG");
                break;
