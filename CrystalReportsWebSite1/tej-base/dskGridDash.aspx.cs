...
public partial class dskGridDash : System.Web.UI.Page
{
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                catch
                {
                    ...
                    Response.Redirect("~/tej-base/" + "desktop_wt" + ".aspx?STR=" + frm_qstr, false);
                }
            }
        }
    }
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            HtmlGenericControl msgDiv = (HtmlGenericControl)e.Row.FindControl("chart1");

            string mystr = "<script type='text/javascript'>$(document).ready(function () {Highcharts.theme = {colors: ['#03AAF1', '#4CE210', '#F74500', '#FFA804', '#40D550', '#F15C00', '#9035EA', '#F7437C', '#0017FF'],chart: {backgroundColor: {linearGradient: [0, 0, 500, 500],stops: [[0, 'rgb(255, 255, 255)'],[1, 'rgb(240, 240, 255)']]},},title: {style: {color: '#000',font: 'bold 16px Trebuchet MS Verdana, sans-serif'}},subtitle: {style: {color: '#666666',font: 'bold 12px Trebuchet MS Verdana, sans-serif'}},legend: {itemStyle: {font: '9pt Trebuchet MS, Verdana, sans-serif',color: 'black'},itemHoverStyle:{color: 'gray'}   }};Highcharts.setOptions(Highcharts.theme);$('#VIPIN').highcharts({chart: {type: 'column'},title: {text: 'Invoices(Rs Lakhs)'},subtitle: {text: 'HOTPACK PACKAGING INDUSTRIES LLC'},tooltip: {formatter: function() {return this.point.name + ' ('+ this.y + ') ' + ' ';}},plotOptions: {column : {dataLabels: {enabled: true,formatter: function () {return this.point.name + ': (' + this.y + ') ';},style: {color: (Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black' }}}},series: [{data: [{ name : 'Jan', y : 51.98 } , { name : 'Feb', y : 35.85 } ]}],xAxis: {categories: [ 'Jan', 'Feb' ],crosshair: true},});});</script>";

            string str = mystr.Trim().Replace("VIPIN", msgDiv.ClientID);
            ScriptManager.RegisterStartupScript(this, this.GetType(), msgDiv.ClientID + "_", str, false);
    ...
    void makequery()
    {
        allQuery[0] = new string[2];
        allQuery[0][0] = "Sales Information (Last 100)";
        SQuery = "SELECT * FROM (SELECT TRIM(A.VCHNUM) AS INV_NO,TO_cHAR(A.VCHDATE,'DD/MM/YYYY') AS INV_DT,B.ANAME AS CUSTOMER,c.iname as product,c.cpartno as partno,a.iqtyout as qty,to_char(a.irate,'99,99,99,999.99') as rate,TO_CHAR(A.iamount,'99,99,99,999.99') AS INV_VALUE,A.ENT_BY,TO_CHAR(A.ENT_dT,'DD/MM/YYYY HH24:MI') AS ENT_dT,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD FROM IVOUCHER A,FAMST B,ITEM C WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND TRIM(A.ICODE)=TRIM(C.ICODe) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '4%' AND A.VCHDATE between TO_dATE('" + fromdt + "','dd/mm/yyyy') and to_Date('" + todt + "','dd/mm/yyyy') order by VDD desc,a.vchnum desc) WHERE ROWNUM<101 ";
        allQuery[0][1] = SQuery;

        allQuery[1] = new string[2];
        allQuery[1][0] = "MRR Information (Last 100)";
        SQuery = "SELECT * FROM (SELECT TRIM(A.VCHNUM) AS MRR_NO,TO_cHAR(A.VCHDATE,'DD/MM/YYYY') AS MRR_DT,B.ANAME AS VENDOR,c.iname as product,c.cpartno as partno,a.IQTYIN as qty,to_char(a.irate,'99,99,99,999.99') as rate,TO_CHAR(A.iamount,'99,99,99,999.99') AS MRR_VALUE,A.ENT_BY,TO_CHAR(A.ENT_dT,'DD/MM/YYYY HH24:MI') AS ENT_dT,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD FROM IVOUCHER A,FAMST B,ITEM C WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND TRIM(A.ICODE)=TRIM(C.ICODe) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '0%' AND A.VCHDATE between TO_dATE('" + fromdt + "','dd/mm/yyyy') and to_Date('" + todt + "','dd/mm/yyyy') order by VDD desc,a.vchnum desc) WHERE ROWNUM<101 ";
        SQuery = "SELECT SYSDATE,'-' AS VDD FROM DUAL";
        allQuery[1][1] = SQuery;

        allQuery[2] = new string[2];
        allQuery[2][0] = "Sales Order Information (Last 100)";
        SQuery = "SELECT * FROM (SELECT TRIM(A.ORDNO) AS SO_NO,TO_cHAR(A.ORDDT,'DD/MM/YYYY') AS SO_DT,B.ANAME AS CUSTOMER,c.iname as product,c.cpartno as partno,a.QTYORD as qty,to_char(a.irate,'99,99,99,999.99') as rate,TO_CHAR(A.TOTAL,'99,99,99,999.99') AS SO_VALUE,A.ENT_BY,TO_CHAR(A.ENT_dT,'DD/MM/YYYY HH24:MI') AS ENT_dT,TO_CHAR(A.ORDDT,'YYYYMMDD') AS VDD FROM SOMAS A,FAMST B,ITEM C WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND TRIM(A.ICODE)=TRIM(C.ICODe) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '4%' AND A.ORDDT between TO_dATE('" + fromdt + "','dd/mm/yyyy') and to_Date('" + todt + "','dd/mm/yyyy') order by VDD desc,a.ORDNO desc) WHERE ROWNUM<101 ";
        allQuery[2][1] = SQuery;

        allQuery[3] = new string[2];
        allQuery[3][0] = "Receipt Information (Last 100)";
        SQuery = "SELECT * FROM (select a.vchnum as cheque_no,to_char(a.vchdate,'dd/MM/yyyy') as cheque_Date,b.aname as party, to_char(round((a.dramt-a.cramt),2),'99,99,99,999.99') as value,A.ENT_BY,TO_CHAR(A.ENT_DATE,'DD/MM/YYYY HH24:MI') AS ENT_dT,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD from voucher a ,famst b where trim(a.acode)=trim(B.acode) and a.branchcd='" + frm_mbr + "' and substr(a.type,1,1)='2' and substr(a.acode,1,2) in('05','06') AND A.VCHDATE  between TO_dATE('" + fromdt + "','dd/mm/yyyy') and to_Date('" + todt + "','dd/mm/yyyy') order by a.vchnum) WHERE ROWNUM<101 ";
        allQuery[3][1] = SQuery;

        allQuery[4] = new string[2];
        allQuery[4][0] = "Payment Information (Last 100)";
        SQuery = "SELECT * FROM (select a.vchnum as cheque_no,to_char(a.vchdate,'dd/MM/yyyy') as cheque_Date,b.aname as party, to_char(round((a.dramt-a.cramt),2),'99,99,99,999.99') as value,A.ENT_BY,TO_CHAR(A.ENT_DATE,'DD/MM/YYYY HH24:MI') AS ENT_dT,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD from voucher a ,famst b where trim(a.acode)=trim(B.acode) and a.branchcd='" + frm_mbr + "' and substr(a.type,1,1)='1' and substr(a.acode,1,2) in('16') AND A.VCHDATE  between TO_dATE('" + fromdt + "','dd/mm/yyyy') and to_Date('" + todt + "','dd/mm/yyyy') order by a.vchnum) WHERE ROWNUM<101 ";
        allQuery[4][1] = SQuery;
    }
    void showdata()
    {
        index = fgen.make_int(fgenMV.Fn_Get_Mvar(frm_qstr, "U_COUNTER"));
        if (index > 4)
        {
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_COUNTER", "1");
            Response.Redirect("~/tej-base/" + "desktop_wt" + ".aspx?STR=" + frm_qstr, false);
        ...
        else
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dt.Columns.Add("MYGR", typeof(string));
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        dr["MYGR"] = "<script type='text/javascript'>$(document).ready(function () {Highcharts.theme = {colors: ['#03AAF1', '#4CE210', '#F74500', '#FFA804', '#40D550', '#F15C00', '#9035EA', '#F7437C', '#0017FF'],chart: {backgroundColor: {linearGradient: [0, 0, 500, 500],stops: [[0, 'rgb(255, 255, 255)'],[1, 'rgb(240, 240, 255)']]},},title: {style: {color: '#000',font: 'bold 16px Trebuchet MS Verdana, sans-serif'}},subtitle: {style: {color: '#666666',font: 'bold 12px Trebuchet MS Verdana, sans-serif'}},legend: {itemStyle: {font: '9pt Trebuchet MS, Verdana, sans-serif',color: 'black'},itemHoverStyle:{color: 'gray'}   }};Highcharts.setOptions(Highcharts.theme);$('#VIPIN').highcharts({chart: {type: 'column'},title: {text: 'Invoices(Rs Lakhs)'},subtitle: {text: 'HOTPACK PACKAGING INDUSTRIES LLC'},tooltip: {formatter: function() {return this.point.name + ' ('+ this.y + ') ' + ' ';}},plotOptions: {column : {dataLabels: {enabled: true,formatter: function () {return this.point.name + ': (' + this.y + ') ';},style: {color: (Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black' }}}},series: [{data: [{ name : 'Jan', y : 51.98 } , { name : 'Feb', y : 35.85 } ]}],xAxis: {categories: [ 'Jan', 'Feb' ],crosshair: true},});});</script>";
                    }
                }
            }
            ...
            sg1.DataSource = dt;
            sg1.DataBind();
            for (int s = 0; s < sg1.Rows.Count; s++)
            {
                for (int j = 0; j < sg1.Rows[s].Cells.Count; j++)
                {
                    if (sg1.HeaderRow.Cells[j].Text.ToUpper().Contains("VALUE") || sg1.HeaderRow.Cells[j].Text.ToUpper().Contains("RATE") || sg1.HeaderRow.Cells[j].Text.ToUpper().Contains("QTY"))
                    {
                        sg1.Rows[s].Cells[j].Text = sg1.Rows[s].Cells[j].Text + "&nbsp;&nbsp;";
                        sg1.Rows[s].Cells[j].HorizontalAlign = HorizontalAlign.Right;
                    }
                    sg1.Rows[s].Cells[j].ToolTip = sg1.Rows[s].Cells[j].Text;
                    if (sg1.Rows[s].Cells[j].Text.Trim().Length > 35)
                    {
                        sg1.Rows[s].Cells[j].Text = sg1.Rows[s].Cells[j].Text.Substring(0, 35);
                    }
                }
            }
            ...
            str.Append("}, 120000);});");
            index++;
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_COUNTER", index.ToString());
