...
public partial class frmUmst : System.Web.UI.Page
{
    ...
    void setColHeadings()
    {
        ...
        switch (Prg_Id)
        {
            case "*******":
                ...
                break;
            case "F47125":
                lblheader.Text = "Supervisior Master";
                break;
        }
        if (Prg_Id == "*******")
        {
        ...
        switch (btnval)
        {
            ...
            default:
                if (btnval == "Edit" || btnval == "Del" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct branchcd||id||trim(type1) as fstr,type1 as Person_code,name as Person_name,acref as email_id from typegrp where branchcd='" + frm_mbr + "' and id='SE' order by type1 desc";
                break;
        }
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            ...
            frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select substr(max(type1),2,3) as vch from typegrp where id='SE' and branchcd='" + frm_mbr + "'", 3, "vch");
            txtUserIDNo.Value = "E" + frm_vnum;
            disablectrl();
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtUserName.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Enter User Name!!");
            return;
        }
        col1 = fgen.seek_iname(frm_qstr, frm_cocd, "select name from " + frm_tabname + " where trim(upper(name))='" + txtUserName.Value.ToUpper().Trim() + "'", "name");
        if (col1 != "0" && edmode.Value != "Y")
        {
            fgen.msg("-", "AMSG", "User Name already Exist!!");
            return;
        }
        if (txtEmail.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Enter Email ID!!");
            return;
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select * from " + frm_tabname + " where branchcd||id||trim(type1)='" + col1 + "' ";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtUserIDNo.Value = dt.Rows[0]["type1"].ToString().Trim();
                        txtUserName.Value = dt.Rows[0]["name"].ToString().Trim();
                        txtEmail.Value = dt.Rows[0]["acref"].ToString().Trim();
                        txtMobile.Value = dt.Rows[0]["acref3"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select userid as fstr,username,userid as code,emailid,contactno from EVAS order by userid desc,username";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            ...
            if (col1 == "Y")
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                frm_vnum = "E" + fgen.next_no(frm_qstr, frm_cocd, "select substr(max(type1),2,3) as vch from typegrp where id='SE' and branchcd='" + frm_mbr + "'", 3, "vch");
                    ...
                    if (edmode.Value == "Y")
                    {
                        ...
                        cmd_query = "update typegrp set branchcd='DD' where id='SE' and trim(type1)='" + txtUserIDNo.Value.Trim() + "'";
                    ...
                    if (edmode.Value == "Y")
                    {
                        ...
                        cmd_query = "delete from typegrp where branchcd='DD' and id='SE' and trim(type1)='" + txtUserIDNo.Value.Trim() + "'";
                        fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
    ...
    void save_fun()
    {
        ...
        oporow["name"] = txtUserName.Value.Trim().ToUpper();

        oporow["acref"] = txtEmail.Value.Trim().ToUpper();
        oporow["acref3"] = txtMobile.Value.Trim();
