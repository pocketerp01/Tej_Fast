...
public partial class prod_entry2 : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "JOBNO":
                ...
                break;
            case "STAGE":
                SQuery = "SELECT TYPE1 AS FSTR,NAME,TYPE1 AS CODE FROM TYPE WHERE ID='1' and type1 LIKE '6%' and type1 not in ('6R','6H','6G') ORDER BY TYPE1";
                break;
            case "MACHINE":
            ...
            case "ERPCODE":
                ...
                SQuery = "SELECT A.VCHNUM||A.VCHDATE||A.ICODE AS FSTR, A.VCHNUM AS JONO,A.VCHDATE AS JODT,B.INAME AS PRODUCT,A.ICODE ERPCODE,SUM(A.JCQTY) AS JC_QTY,SUM(A.PRODQTY) AS PROD_QTY,SUM(A.JCQTY)-SUM(A.PRODQTY) AS BAL FROM (SELECT TRIM(ORDNO) AS VCHNUM,TO_cHAR(ORDDT,'DD/MM/YYYY') AS VCHDATE,TRIM(ICODE) AS ICODE,QTYORD AS JCQTY,0 AS PRODQTY FROM SOMAS WHERE BRANCHCD='" + frm_mbr + "' AND TYPE LIKE '4%' AND ORDDT " + DateRange + " UNION ALL SELECT TRIM(INVNO),TO_cHAR(INVDATE,'DD/MM/YYYY'),TRIM(ICODE),0 AS QTY,IQTYIN FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' AND VCHDATE " + DateRange + " AND TRIM(STAGE)='" + txtStage.Value.Trim().Split('~')[0] + "') A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) GROUP BY A.VCHNUM||A.VCHDATE||A.ICODE,A.VCHNUM,A.VCHDATE,B.INAME,A.ICODE HAVING SUM(A.JCQTY)-SUM(A.PRODQTY)>0 ORDER BY A.VCHNUM,A.VCHDATE";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "MACHINE":
                    ...
                    break;
                case "ERPCODE":
                    hf1.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3").ToString().Trim().Replace("&amp", "");
                    var trcno = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL8").ToString().Trim().Replace("&amp", "");
                    ...
                    {
                        {
                            ...
                            txtIcode.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3").ToString().Trim().Replace("&amp", "");
                    ...
                    if (col1.Length > 1)
                    {
                        ...
                        if (txtStage.Value.Length > 2)
                        {
                            ...
                            if (stage != "61" && stage != "69")
                            {
                                ...
                                //sg1_dr["sg1_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, wipquery(txtIcode.Value.ToString().Trim(), txtlbl51v.Text.Trim()), "BAL");
                                sg1_dr["sg1_f5"] = fgen.SeekWipStock(frm_cocd, frm_qstr, frm_mbr, txtIcode.Value.ToString().Trim(), txtStage.Value.Split('~')[0], txtlbl51v.Text.Trim(), fromdt, todt, " and REVIS_NO='" + txtlbl51v.Text + "' and type || trim(vchnum) || to_Char(vchdate, 'dd/mm/yyyy') != '" + frm_vty + txtvchnum.Value.Trim() + txtvchdate.Value.Trim() + "'");
    ...
    void calc(string bomno)
    {
        ...
        {
            if (txtStage.Value.Trim().Split('~')[0] == "61" || txtStage.Value.Trim().Split('~')[0] == "69")
            {
                dt = new DataTable();
