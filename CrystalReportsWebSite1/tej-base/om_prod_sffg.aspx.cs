...
public partial class om_prod_sffg : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                SQuery = "SELECT type1 as Fstr,NAME,TYPE1  as Code FROM TYPE WHERE ID='1' and type1 like '6%' order by Name";
                break;
            case "TICODE":
                //pop2
                //SQuery = "select trim(acode) as acode,sum(cntr) as cntr from (SELECT ACODE,1 AS CNTR FROM FAMST WHERE substr(Acode,1,2) in ('02','05','06') and length(trim(nvl(deac_by,'-'))) <2  and length(Trim(nvl(STATEN,'-')))>1 UNION ALL SELECT DISTINCT ACODE,-1 AS CNTR FROM APPVENDVCH WHERE BRANCHCD='" + frm_mbr + "' and type='10') group by trim(Acode) having sum(cntr)>0 ";
                //SQuery = "SELECT a.Acode as FStr,a.Aname as Supplier,a.Acode,a.Addr1,a.Addr2,a.GST_No,a.Staten from famst a,("+ SQuery +")b where trim(A.acodE)=trim(B.acode) and substr(a.Acode,1,2) in ('02','05','06','16') and length(trim(nvl(a.deac_by,'-'))) <2 order by a.Aname ";

                //SQuery = "select to_Char(a.vchdate,'yyyymmdd')||a.vchnum as Fstr,a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as Plan_dt,b.Name as WIPStage_Name,sum(a.plan_qty)-sum(a.issued) as Pending_Qty  from (SELECT acode,vchnum,vchdate,a1 as plan_Qty,0 as issued from prod_Sheet where branchcd='" + frm_mbr + "' and type='11' and vchdate " + DateRange + "  union all SELECT stage,jobno,jobdt,0 as iqtychl,req_qty as issued from wb_iss_Req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,type b where b.id='1' and trim(A.acode)=trim(B.type1) group by to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum,b.Name,trim(a.acode),a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),a.vchdate having sum(a.plan_qty)-sum(a.issued)>0 order by fstr";


                SQuery = "SELECT distinct a.Icode as Fstr,b.Iname,b.Cpartno,b.Cdrgno,a.Icode as Code FROM itemosp a, item b where trim(A.icode)=trim(B.icode) order by b.Iname";
                break;
            case "SG1_ROW_ADD":
            case "SG1_ROW_ADD_E":
                col1 = "";
    ...
    void newCase(string vty)
    {
        ...
        lbl1a.Text = vty;
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' and orddr "+ DateRange +" ", 6, "VCH");
        txtvchnum.Text = frm_vnum;
        txtvchdate.Text = fgen.Fn_curr_dt(frm_cocd, frm_qstr);
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl7a.Text = dt.Rows[0]["CURRENCY"].ToString().Trim();
                        txtlbl7.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select type1 from type where id='A' and trim(upper(name))=upper(Trim('" + txtlbl7a.Text + "'))", "type1");

                        txtlbl8.Text = dt.Rows[0]["FRT_TEXT"].ToString().Trim();
                        txtlbl9.Text = dt.Rows[0]["rcflag"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["row_text"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["irate"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["disc"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["discamt"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["JWRATE"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["PFCHG"].ToString().Trim();
                ...
                case "Print_E":
                    ...
                    SQuery = "SELECT 'Approved Price List' AS HEADER, B.INAME AS ITEM_NAME,B.CPARTNO,B.HSCODE,C.ANAME AS Supplier ,A.* FROM POMAS A,ITEM B ,famst C  WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND TRIM(A.aCODE)=TRIM(c.ACODE) AND TRIM(A.BRANCHCD)='" + frm_mbr + "' and TRIM(A.TYPE)='" + frm_vty + "' and TRIM(A.vchnum)||TO_CHAR(A.vchdate,'DD/MM/YYYY') in (" + col1 + ") ORDER BY A.SRNO ";
                    fgen.Fn_Print_Report(frm_cocd, frm_qstr, frm_mbr, SQuery, "std_pr", "std_pr");
                    break;
                case "TACODE":
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["CURRENCY"] = txtlbl7a.Text;
                
                oporow["frt_TEXT"] = txtlbl8.Text;
                oporow["rcflag"] = txtlbl9.Text;

                oporow["icode"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["SRNO"] = i;

                oporow["row_text"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text;
                oporow["irate"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);
                oporow["disc"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
                oporow["discamt"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text);


                oporow["JWRATE"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text);
                oporow["PFCHG"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text);
                ...
                oporow["PTAX"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text);
