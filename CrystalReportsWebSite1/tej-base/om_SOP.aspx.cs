...
public partial class om_SOP : System.Web.UI.Page
{
    ...
    public void enablectrl()
    {
        ...
        create_tab(); //btnrfq.Enabled = false;
        sg1_add_blankrows();        
    ...
    public void disablectrl()
    {
        ...
        btnicode.Enabled = true; //btnrfq.Enabled = true;
    ...
    public void set_Val()
    {
        ...
        frm_tabname = "inspmst";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "SO");
        lblheader.Text = "SOP";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                SQuery = "SELECT TRIM(A.ICODE) AS FSTR,I.INAME AS SUBGRP_NAME,A.ICODE AS SUBGRP_CODE FROM (SELECT DISTINCT SUBSTR(TRIM(ICODE),0,4) AS ICODE,1 AS QTY,TRIM(ICODE) AS CODE FROM ITEM WHERE LENGTH(TRIM(ICODE))=8 UNION ALL SELECT DISTINCT TRIM(COL1) AS ICODE,-1 AS QTY,TRIM(ICODE) AS CODE FROM INSPMST WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='SO') A, ITEM I WHERE TRIM(A.ICODE)=TRIM(I.ICODE) GROUP BY A.ICODE,I.INAME HAVING SUM(QTY)>0 ORDER BY A.ICODE";
                break;

            case "Item":
                SQuery = "SELECT TRIM(A.ICODE) AS FSTR,I.INAME AS ITEM,A.ICODE AS ITEM_CODE FROM (SELECT DISTINCT TRIM(ICODE) AS ICODE,1 AS QTY FROM ITEM WHERE LENGTH(TRIM(ICODE))=8 AND SUBSTR(TRIM(ICODE),0,4)='" + txtSubGrpCode.Text.Trim() + "' UNION ALL SELECT DISTINCT TRIM(ICODE) AS ICODE,-1 AS QTY FROM INSPMST WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='SO' AND TRIM(COL1)='" + txtSubGrpCode.Text.Trim() + "') A, ITEM I WHERE TRIM(A.ICODE)=TRIM(I.ICODE) GROUP BY A.ICODE,I.INAME HAVING SUM(QTY)>0 ORDER BY A.ICODE";
                break;
            ...
            case "Print_E":
                SQuery = "SELECT DISTINCT TRIM(A.BRANCHCD)||TRIM(A.TYPE)||TRIM(A.VCHNUM)||TO_CHAR(A.VCHDATE,'DD/MM/YYYY')||TRIM(A.COL1)||TRIM(A.ICODE) AS FSTR,A.COL1 AS SUBGRP_CODE,I.INAME AS SUBGRP_NAME,A.ICODE AS ITEM_CODE,B.INAME AS ITEM,A.VCHNUM AS TEMP_NO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS TEMP_DATE,TITLE AS HEADER,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD FROM " + frm_tabname + " A,ITEM I,ITEM B WHERE TRIM(A.COL1)=TRIM(I.ICODE) AND TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.SRNO='1' AND LENGTH(TRIM(I.ICODE))=4 ORDER BY VDD DESC,A.VCHNUM DESC,A.COL1 DESC";
                break;

            default:
                frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD" )
                    SQuery = "SELECT DISTINCT TRIM(A.VCHNUM)||TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS FSTR,A.COL1 AS SUBGRP_CODE,trim(I.INAME) AS SUBGRP_NAME,trim(A.ICODE) AS ITEM_CODE,trim(B.INAME) AS ITEM,trim(A.VCHNUM) AS TEMP_NO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS TEMP_DATE,trim(TITLE) AS HEADER,TO_CHAR(A.VCHDATE,'YYYYMMDD') AS VDD FROM " + frm_tabname + " A,ITEM I,ITEM B WHERE TRIM(A.COL1)=TRIM(I.ICODE) AND TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.SRNO='1' AND LENGTH(TRIM(I.ICODE))=4 ORDER BY VDD DESC,trim(A.VCHNUM) DESC,A.COL1 DESC";
                break;
        }
    ...
    void newCase(string vty)
    {
        ...
        lbl1a.Text = vty;
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' AND COL1='" + txtSubGrpCode.Text.Trim() + "' ", 6, "VCH");
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtSubGrpCode.Text == "-" || txtSubGrpCode.Text == "")
        {
            fgen.msg("-", "AMSG", "Please Select Sub Group"); btnlbl7.Focus(); return;
        }
        if (txtIcode.Text == "-" || txtIcode.Text == "")
        {
            fgen.msg("-", "AMSG", "Please Select Item"); btnicode.Focus(); return;
        }
        //if (sg1.Rows.Count <= 1)
        //{
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtIcode.Text = dt.Rows[i]["frm_name"].ToString().Trim();
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,to_char(a.ent_Dt,'dd/mm/yyyy') as pent_dt,i.iname,b.iname as item from " + frm_tabname + " a,item i,item b where trim(a.col1)=trim(i.icode) and trim(a.icode)=trim(b.icode) and length(trim(i.icode))=4 and trim(a.branchcd)||trim(a.type)||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' order by a.srno";                    
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtSubGrpCode.Text = dt.Rows[0]["COL1"].ToString().Trim();
                        txtSubGrpName.Text = dt.Rows[0]["iname"].ToString().Trim();
                        txtIcode.Text = dt.Rows[0]["ICODE"].ToString().Trim();
                        txtIname.Text = dt.Rows[0]["item"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f2"] = "-";
                            sg1_dr["sg1_f3"] = "-";
                            sg1_dr["sg1_t1"] = dt.Rows[i]["col3"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["TITLE"].ToString().Trim();
                            sg1_dr["sg1_t3"] = "-";
                            sg1_dr["sg1_t4"] = "-";
                            sg1_dr["sg1_t5"] = "-";
                            sg1_dr["sg1_t6"] = "-";
                ...
                case "TACODE":
                    ...
                    txtSubGrpCode.Text = col1;
                    txtSubGrpName.Text = col2;
                    btnicode.Focus();
                    break;
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.col1 as subgrp_code,i.iname as subgrp_name,a.icode as item_code,b.iname as item,a.vchnum as temp_no,to_char(a.vchdate,'dd/mm/yyyy') as temp_date,title as header,to_char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,item i,item b where trim(a.col1)=trim(i.icode) and trim(a.icode)=trim(b.icode) and a.branchcd='" + frm_mbr + "' and a.type='SO' and vchdate " + PrdRange + " and length(trim(i.icode))=4 order by vdd desc,a.vchnum desc,a.col1,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                ...
                if (col1 == "Y" && Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (save_it == "Y")
                            {
                                // BECAUSE AS PER NEOP, THE NEW NUMBER IS GENERATED ON THE BASIS OF SUB GROUP THATS WHY I WROTE THESE LINES
                                i = 0;
                                do
                                {
                                    frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(vchnum)+" + i + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND col1='" + txtSubGrpCode.Text.Trim() + "'", 6, "vch");
                                    pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + frm_vty + frm_vnum + System.DateTime.Now.ToString("dd/MM/yyyy") + txtSubGrpCode.Text.Trim(), frm_mbr, frm_vty, frm_vnum, txtvchdate.Text.Trim(), txtSubGrpCode.Text.Trim(), frm_uname);
                                }
                                while (pk_error == "Y");
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[20].HeaderStyle.Width = 1000;            
        }
    }
    //------------------------------------------------------------------------------------
        ...
        switch (var)
        {
            case "SG1_RMV":
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgen.msg("-", "CMSG", "Are You Sure!! You Want to Remove This Row From The List");
                }
                break;

            case "SG1_ROW_ADD":
                // SG1_ROW_ADD_E IS NOT REQUIRED FOR THIS FORM
                if (index < sg1.Rows.Count - 1)
                {
                    //hf1.Value = index.ToString();
                    //fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_HF1", hf1.Value);
                ...
                else
                {
                    ...
                    // NO POPUP IS REQUIRED THAT'S WHY THESE SET OF LINES ARE WRITTEN HERE
                    dt = new DataTable();
                    if (ViewState["sg1"] != null)
                    {
                        sg1_dt = new DataTable();
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().Length > 1)
            {
                ...
                oporow["acode"] = "-";
                oporow["title"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper();
                oporow["col1"] = txtSubGrpCode.Text.Trim();
                mq0 = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper();
                int i1 = Regex.Matches(mq0, @"[a-zA-Z]").Count;
                ...
                {
                    oporow["col2"] = "NOHEADER";
                }
                oporow["col3"] = mq0.Trim().ToUpper();
                oporow["srno"] = i + 1;
                oporow["NUM1"] = 0;
                oporow["NUM2"] = 0;
                oporow["NUM3"] = 0;
                ...
                oporow["NUMWT"] = 0;
                oporow["BEND_WT"] = 0;
                oporow["REMARK2"] = txtrmk.Text.Trim();
                //-------------------------------------------
                if (edmode.Value == "Y")
                {
