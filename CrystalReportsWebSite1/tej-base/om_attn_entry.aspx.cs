...
public partial class om_attn_entry : System.Web.UI.Page
{
    ...
    public void disablectrl()
    {
        ...
        btnlbl7.Enabled = true; btnprint.Disabled = true;
    }
    //------------------------------------------------------------------------------------
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                SQuery = "select distinct trim(branchcd) as fstr,branchcd from empmas where branchcd='" + frm_mbr + "' and grade='" + txtlbl4.Text.Trim() + "' order by branchcd";
                break;
            ...
            default:
                ...
                {
                    if (Convert.ToInt32(col1) > 3)
                    {
                        yr = frm_myear;
                    }
                    else
                    {
                        int i = Convert.ToInt32(frm_myear) + 1;
                        yr = Convert.ToString(i);
                    }
                    //    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Entry_no,to_char(a.vchdate,'dd/mm/yyyy') as Entry_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.grade='" + hf2.Value + "' and to_char(a.vchdate,'mm/yyyy')='" + col1 + "/" + yr + "' order by a.vchnum desc";
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Entry_no,to_char(a.vchdate,'dd/mm/yyyy') as Entry_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.grade='" + hf2.Value + "' and to_char(a.vchdate,'mm/yyyy')='" + col1 + "/" + yr + "' order by a.vchnum desc";
                }
                else if (btnval == "Print_E" || btnval == "List_E")
                {
                    ...
                    hf3.Value = col1 + "/" + yr;
                    //SQuery = "select distinct  trim(a.branchcd)||trim(a.grade)||trim(a.empcode) as fstr,b.name ,a.empcode,b.fhname from attn a,empmas b where trim(a.branchcd)||trim(a.grade)||trim(a.empcode) =trim(b.branchcd)||trim(b.grade)||trim(b.empcode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.grade='" + hf2.Value + "'  and to_char(a.vchdate,'mm/yyyy')='" + col1 + "/" + yr + "' order by name";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (edmode.Value == "")
        {
            SQuery = "SELECT distinct vchnum  FROM attn WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='10' and grade='" + txtlbl4.Text.Trim() + "' and to_char(vchdate,'dd/mm/yyyy')='" + txtvchdate.Text + "'";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "SELECT a.*,B.NAME,B.DEPTT_TEXT,B.DESG_TEXT,B.DTJOIN,C.NAME AS GRADE_NAME FROM ATTN A ,EMPMAS B,TYPE C WHERE TRIM(A.EMPCODE)=TRIM(B.EMPCODE) AND TRIM(A.GRADE)=TRIM(B.GRADE) AND TRIM(A.BRANCHCD)=TRIM(B.BRANCHCD) AND TRIM(A.GRADE)=TRIM(C.TYPE1) AND C.ID='I' and trim(a.branchcd)||trim(A.type)||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t10"] = dt.Rows[i]["tot_ded"].ToString().Trim();
                ...
                case "Print_E":
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL4", col1); //FSTR VALUE
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL5", hf3.Value);
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL6", hf4.Value);
                    fgenMV.Fn_Set_Mvar(frm_qstr, "REPID", "F85101");
                ...
                case "TACODE":
                    ...
                    else
                    {
                        ...
                        txtlbl4a.Text = col2;
                        //===========================for grid
                        SQuery = "select  EMPCODE AS COL1,NAME AS COL2, DEPTT_TEXT AS COL3,DESG_TEXT AS COL4,TO_CHAR(DTJOIN,'dd/MM/yyyy') AS COL6,ENT_DT,ENT_BY from empmas where BRANCHCD='" + frm_mbr + "' AND LENGTH(TRIM(LEAVING_DT))<5 AND grade='" + txtlbl4.Text.Trim() + "' and branchcd='" + frm_mbr + "' and substr(nvl(trim(appr_by),'-'),1,3)='[A]' order by empcode";//
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
                        ...
                        {
                            fgen.msg("-", "AMSG", "No Approved Employee for this Grade '" + col1 + "'!!Only Approved Employee Will Showing!!");
                            return;
                        }
                        //   btnlbl7.Focus();
                    }
                ...
                case "TICODE":
                    if (col1.Length <= 0) return;
                    txtlbl7.Text = col2;
                    SQuery = "select  EMPCODE AS COL1,NAME AS COL2, DEPTT_TEXT AS COL3,DESG_TEXT AS COL4,TO_CHAR(DTJOIN,'dd/MM/yyyy') AS COL6,ENT_DT,ENT_BY from empmas where BRANCHCD='" + frm_mbr + "' AND LENGTH(TRIM(LEAVING_DT))<5 AND grade='" + txtlbl4.Text.Trim() + "' and branchcd='" + txtlbl7.Text.Trim() + "'  order by empcode";//and substr(nvl(trim(appr_by),'-'),1,3)='[A]'
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0)
                    {
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.HeaderRow.Cells[25].Text = "DownTime(Min)";
            sg1.Columns[26].HeaderStyle.Width = 130;
            sg1.HeaderRow.Cells[26].Text = "DownTime1(Min)";
            sg1.Columns[27].HeaderStyle.Width = 150;
            sg1.HeaderRow.Cells[27].Text = "Tot_DownTime(Min)";
        }
    }
    //------------------------------------------------------------------------------------
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 1)
            {
                ...
                oporow["Tot_ded"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text);
                oporow["REFR"] = 0;
                oporow["SHFTINHR"] = 0;
                oporow["SHFTINMIN"] = 0;
                oporow["EXTINHR"] = 0;
    ...
    protected void sg1_t5_TextChanged(object sender, EventArgs e)
    {
        ...
        if (DT1 == "1")
        {
            ...
            else
            {
                ...
                //        return;
                //    }
                //    if (Convert.ToInt32(dtfromh) == 24 && Convert.ToInt32(dtfromm) >= 0)
                //    {
                //        fgen.msg("-", "AMSG", "Please enter valid Time in Time Out(Hrs)!!");
                //        return;
                //    }
    ...
    public void Cal()
    {
        ...
        for (int i = 0; i < sg1.Rows.Count; i++)
        {
            ...
            tot_dedn = food + dedn;
            ((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text = tot_dedn.ToString();
        }
    }
    //------------------------------------------------------------------------------------
}