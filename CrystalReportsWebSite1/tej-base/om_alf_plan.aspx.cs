...
public partial class om_alf_plan : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                SQuery = "SELECT userid AS FSTR,trim(username) as Person_Name,userid,emailid,trim(Full_Name) AS Full_Name FROM evas where branchcd!='DD' and username!='-' and userid<='000052'  order by Username";
                break;
            case "TICODE":
                //pop2
            ...
            case "SG1_ROW_ADD_E":
                ...
                SQuery = "SELECT userid AS FSTR,Full_Name AS Client_Name,username as CCode FROM evas where branchcd!='DD' and username!='-' and userid>'000052' and trim(userid) not in (select trim(Tcode) from wb_ALF_PLAN where branchcd!='DD' and to_char(ALFdt,'yyyymm')=to_char(to_DaTE('" + txtvchdate.Text  + "','dd/mm/yyyy'),'yyyymm')) order by Username";
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.ALFNO)||to_Char(a.ALFDT,'dd/mm/yyyy') as fstr,a.ALFNO as Plan_no,to_char(a.ALFdt,'dd/mm/yyyy') as plan_Dt,b.UserName,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.ALFdt,'yyyymmdd') as vdd,a.tcode from " + frm_tabname + " a,evas b where trim(A.tcode)=trim(B.userid) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and ALFdt " + DateRange + " order by vdd desc,a.ALFno desc";
                break;
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.full_name,b.username,b.contactno,b.emailid,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,evas b where trim(a.ccode)=trim(b.userid) and a.branchcd||a.type||trim(a.ALFno)||to_Char(a.ALFdt,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtvchnum.Text = dt.Rows[0]["ALFno"].ToString().Trim();
                        txtvchdate.Text = Convert.ToDateTime(dt.Rows[0]["ALFdt"].ToString().Trim()).ToString("dd/MM/yyyy");
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["VISIT_DT"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.ALFNO as Plan_no,to_char(a.ALFdt,'dd/mm/yyyy') as Plan_Dt,c.Username as Plan_by,b.Full_name,b.Username as Client_Code,a.VISIT_dT as VISIT_dT,a.Remarks,a.Naration,a.CCode,a.Ent_by,a.ent_Dt ,to_Char(a.ALFdt,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,evas b,evas c where trim(A.ccode)=trim(B.userid) and trim(A.tcode)=trim(c.userid) and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and ALFdt " + PrdRange + " order by vdd ,a.ALFno ,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["TYPE"] = frm_vty;
                oporow["ALFNO"] = frm_vnum;
                oporow["ALFDT"] = txtvchdate.Text.Trim();

                oporow["SRNO"] = i;
                oporow["tcode"] = txtlbl4.Text;

                oporow["CCODE"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["VISIT_DT"] = fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim(), vardate);
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F92101":
                SQuery = "SELECT 'AL' AS FSTR,'ALF Plan' as NAME,'AL' AS CODE FROM dual";
                break;
