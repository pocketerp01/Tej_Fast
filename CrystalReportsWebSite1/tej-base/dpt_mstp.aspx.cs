...
public partial class dpt_mstp : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            case "TYPECODE":
                ...
                {
                    SQuery = "select * from (select sum(Valu) as fstr,coded as Code_No_Available,max(name) as Code_Name,(case when sum(Valu)>0 then 'Code Available' else 'Code Already Used' end) as Code_Status from (select lpad(trim(to_char(rownum,'99')),2,'0') as coded,1 as Valu,null as name from (select rowid,rownum from ico_tab order by id) where rownum<100 union all select type1,-1 as coded,name from type where id='" + frm_vty + "' and type1 like '" + frm_fchar + "%') group by coded ) where Code_No_Available like '" + frm_fchar + "%' order by Code_No_Available ";
                }

                else
                {
                    SQuery = "select sum(Valu) as fstr,coded as Code_No_Available,max(name) as Code_Name,(case when sum(Valu)>0 then 'Code Available' else 'Code Already Used' end) as Code_Status from (select lpad(trim(to_char(rownum,'99')),2,'0') as coded,1 as Valu,null as name from (select rowid,rownum from ico_tab order by id) where rownum<100 union all select type1,-1 as coded,name from type where id='" + frm_vty + "') group by coded  order by Coded";
                }
                break;
            case "ACTGCODE":
                switch (comb_char)
        ...
        {
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_XID", "ERP");
    ...
    void newCase(string vty)
    {
        ...
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE tBRANCHCD='" + frm_mbr + "' AND ID='" + frm_vty + "' AND type1 like '" + frm_fchar + "%' ", 6, "VCH");
    ...
    {
        Response.Redirect("~/dir-main/desktop.aspx?STR=" + frm_qstr);
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                mqry = "delete from wsr_ctrl a where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'";
                fgen.execute_cmd(frm_qstr, frm_cocd, mqry);
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl5.Value = dt.Rows[i]["stform"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            {
                case "F10101":
                    SQuery = "select distinct a.type1 as Type_Codes,a.Name as Type_Name,a.Acode as Account_Code,a.type1 as Type_Code,b.aname as Account_Name,a.MEnt_by,to_char(a.Ment_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd,a." + doc_nf.Value + " as Opt_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as Opt_Dt from " + frm_tabname + " a left outer join famst b on trim(A.acode)=trim(B.acode) where a.tbranchcd='" + frm_mbr + "' and trim(a.ID)='" + frm_vty + "' and trim(a.type1) like '" + frm_fchar + "%'  order by a.type1";
                    break;
                case "F70176":
                    SQuery = "select distinct a.type1 as Type_Codes,a.Name as Type_Name,a.Acode as Account_Code,a.type1 as Type_Code,b.aname as Account_Name,a.MEnt_by,to_char(a.Ment_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd,a." + doc_nf.Value + " as Opt_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as Opt_Dt from " + frm_tabname + " a where  a.tbranchcd='" + frm_mbr + "' and trim(a.ID)='" + frm_vty + "' and trim(a.type1) like '" + frm_fchar + "%'  order by a.type1";
                    break;
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                if (i > 20)
                                {
                                    ...
                                    frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + 1 + " as vch from " + frm_tabname + " where tbranchcd='" + frm_mbr + "' and ID='" + frm_vty + "' ", 6, "vch");
                                    pk_error = "N";
                                    i = 0;
                                }
                                i++;
    ...
    void save_fun()
    {
        ...
        oporow["stform"] = txtlbl5.Value.ToUpper().Trim();
