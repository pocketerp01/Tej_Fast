...
public partial class om_Min_Wages : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        frm_tabname = "WB_PTAX";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "MW");
        lblheader.Text = "Minimum Wages";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        SQuery = "";
        dt = new DataTable();
        SQuery = "SELECT * FROM WB_PTAX WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='MW' AND STATEN='" + txtlbl7.Text.Trim() + "' AND GRADE='" + txtlbl4.Text + "' AND ICAT='N' AND VCHDATE BETWEEN TO_DATE('" + txtvchdate.Text + "','DD/MM/YYYY') AND TO_DATE('" + txtlbl4a.Text + "','DD/MM/YYYY')";
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        if (edmode.Value != "Y")
        {
            if (dt.Rows.Count > 0)
            {
                fgen.msg("-", "AMSG", "On this State an Entry is Already Activated first Deactivate that Entry!!"); return;
            }
        }
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f3"] = "-";
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = dt.Rows[i]["ED_NAME"].ToString().Trim();
                            sg1_dr["sg1_f6"] = dt.Rows[i]["CATEGORY"].ToString().Trim();        
                            sg1_dr["sg1_t2"] = dt.Rows[i]["MTH01"].ToString().Trim();                         
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                       // sg1_add_blankrows();
                        ViewState["sg1"] = sg1_dt;
                ...
                case "STATE":
                    ...
                    if (col1 == "") return;
                       txtlbl7.Text = col1;
                       txtlbl7a.Text = col2;
                    SQuery = "SELECT * FROM WB_PTAX WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='MW' AND VCHDATE " + DateRange + " AND STATEN='" + txtlbl7.Text.Trim() + "' AND ICAT='N'";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0)
                    {
                        txtlbl7.Text = "";
                    ...
                    //===========================
                        ...
                        mq0 = "SELECT DISTINCT TYPE1,NAME,ID FROM TYPEGRP WHERE ID='KL' AND BRANCHCD='" + frm_mbr + "'  ORDER BY TYPE1";
                        dt = new DataTable();
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            for (i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                sg1_dr["sg1_f3"] = "";
                                sg1_dr["sg1_f4"] = "";
                                sg1_dr["sg1_f5"] = dt.Rows[i]["TYPE1"].ToString().Trim();
                                sg1_dr["sg1_f6"] = dt.Rows[i]["NAME"].ToString().Trim();
                                sg1_dr["sg1_t2"] = "-";
                                sg1_dt.Rows.Add(sg1_dr);
                            }
                            sg1_add_blankrows();
                ...
                case "GRADE":
                    if (col1 == "") return;
                    dt = new DataTable();
                    SQuery = "SELECT * FROM WB_PTAX WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='MW' AND VCHDATE " + DateRange + " AND GRADE='" + col1 + "' AND STATEN='" + txtlbl7.Text.Trim() + "' AND ICAT='N'";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0)
                    {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f5"] = dt.Rows[i]["ED_NAME"].ToString().Trim();  
                            sg1_dr["sg1_f6"] = dt.Rows[i]["CATEGORY"].ToString().Trim();
                            sg1_dr["sg1_t2"] =  fgen.make_double(dt.Rows[i]["MTH01"].ToString().Trim());                                                  
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        ...
                        sg1.DataBind();
                        dt.Dispose(); sg1_dt.Dispose();
                        ((TextBox)sg1.Rows[z].FindControl("sg1_t3")).Focus();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            //SQuery = "sELECT distinct trim(a.Vchnum) as Entry_no,to_char(a.Vchdate,'dd/mm/yyyy') as Entry_Dt,a.grade,b.name as grade_name,a.ed_name,TO_CHAR(a.eff_from,'DD/MM/YYYY') AS effective_from,TO_CHAR(a.eff_to,'DD/MM/YYYY') AS effective_upto,trim(a.staten) as state,c.name as state_name,a.sal_frm,a.sal_upto,a.mth04 as april,a.mth05 as may,a.mth06 as june,a.mth07 as july,a.mth08 as august,a.mth09 as september,a.mth10 as october,a.mth11 as november,a.mth12 as december,a.mth01 as january,a.mth02 as february,a.mth03 as march,a.m_tot as total,a.ent_by,to_char(a.Ent_Dt,'dd/mm/yyyy') as ent_dt,to_char(a.vchdate,'yyyymmdd') as vdd FROM " + frm_tabname + " a, type b,type c WHERE trim(a.grade)=trim(b.type1) and trim(a.staten)=trim(c.type1) and a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' and b.id='I' and c.id='{' and b.type1 like '0%' AND a.VCHDATE " + PrdRange + " ORDER BY vdd DESC,entry_No DESC";
            SQuery = "sELECT distinct trim(a.Vchnum) as Entry_no,to_char(a.Vchdate,'dd/mm/yyyy') as Entry_Dt,a.grade,b.name as grade_name,a.ed_name,TO_CHAR(a.eff_from,'DD/MM/YYYY') AS effective_from,TO_CHAR(a.eff_to,'DD/MM/YYYY') AS effective_upto,trim(a.staten) as state,c.name as state_name,a.mth01 as MIN_WAGES,a.ent_by,to_char(a.Ent_Dt,'dd/mm/yyyy') as ent_dt,to_char(a.vchdate,'yyyymmdd') as vdd FROM WB_PTAX a, type b,type c WHERE trim(a.grade)=trim(b.type1) and trim(a.staten)=trim(c.type1) and a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='MW' and b.id='I' and c.id='{' and b.type1 like '0%' AND a.VCHDATE " + PrdRange + " ORDER BY vdd DESC,entry_No DESC";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            ...
            oporow["ED_FLD"] = "DED10";
            oporow["ED_NAME"] = sg1.Rows[i].Cells[17].Text.Trim();
            oporow["CATEGORY"] = sg1.Rows[i].Cells[18].Text.Trim(); 
            oporow["MTH01"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper());          
...
///ADD ONE MORE FIELD IN TABLE
/// alter table WB_PTAX ADD CATEGORY VARCHAR2(30);