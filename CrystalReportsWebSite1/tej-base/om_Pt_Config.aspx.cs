...
public partial class om_Pt_Config : System.Web.UI.Page
{
        ...
        switch (btnval)
        {
            ...
            default:
                ...
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    SQuery = "SELECT distinct trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_dt,a.grade,b.name as grade_name,a.staten as statecode,t.name as state,a.ENT_BY ,to_char(a.ENT_dt,'dd/mm/yyyy') as ENT_dt FROM " + frm_tabname + " A,type b,type t WHERE trim(a.grade)=trim(b.type1) and trim(a.staten)=trim(t.type1) and t.ID='{' and A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' and b.id='I' and b.type1 like '0%' AND A.VCHDATE  " + DateRange + " and a.icat='N' ORDER BY A.VCHNUM DESC";
                break;
        }
    ...
    void newCase(string vty)
    {
        ...
        fgen.EnableForm(this.Controls);
        btnlbl7.Focus();
        txtlbl5.Text = frm_uname;
        txtlbl6.Text = vardate;
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            fgen.msg("-", "AMSG", "Please Select State !!"); btnlbl7.Focus();
            return;
        }
        ...
        if (txtGender.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Gender");
            txtGender.Focus();
            return;
        }
        SQuery = "";
        dt = new DataTable();
        SQuery = "SELECT * FROM WB_PTAX WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='11' AND GRADE='" + txtlbl4.Text.Trim() + "' AND STATEN='" + txtlbl7.Text.Trim() + "' AND ICAT='N' AND VCHDATE BETWEEN TO_DATE('" + txtvchdate.Text + "','DD/MM/YYYY') AND TO_DATE('" + txtlbl4a.Text + "','DD/MM/YYYY')";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtefffrom.Text = Convert.ToDateTime(dt.Rows[i]["EFF_FROM"].ToString().Trim()).ToString("dd/MM/yyyy");
                        ...
                        txtlbl5.Text = frm_uname;
                        txtlbl6.Text = vardate;
                        txtGender.Text = dt.Rows[0]["category"].ToString().Trim();
                        create_tab();
                        sg1_dr = null;
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t4"] = dt.Rows[i]["MTH04"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["MTH05"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["MTH06"].ToString().Trim();
                            sg1_dr["sg1_t7"] = dt.Rows[i]["MTH07"].ToString().Trim();
                            sg1_dr["sg1_t8"] = dt.Rows[i]["MTH08"].ToString().Trim();
                            sg1_dr["sg1_t9"] = dt.Rows[i]["MTH09"].ToString().Trim();
                            sg1_dr["sg1_t10"] = dt.Rows[i]["MTH10"].ToString().Trim();
                            sg1_dr["sg1_t11"] = dt.Rows[i]["MTH11"].ToString().Trim();
                            sg1_dr["sg1_t12"] = dt.Rows[i]["MTH12"].ToString().Trim();
                            sg1_dr["sg1_t13"] = dt.Rows[i]["MTH01"].ToString().Trim();
                            sg1_dr["sg1_t14"] = dt.Rows[i]["MTH02"].ToString().Trim();
                            sg1_dr["sg1_t15"] = dt.Rows[i]["MTH03"].ToString().Trim();
                ...
                case "GRADE":
                    if (col1 == "") return;
                    dt = new DataTable();
                    SQuery = "SELECT * FROM WB_PTAX WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='11' AND VCHDATE " + DateRange + " AND GRADE='" + col1 + "' AND STATEN='" + txtlbl7.Text.Trim() + "' AND nvl(ICAT,'-')!='Y'";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0)
                    {
                        fgen.msg("-", "AMSG", "On this Grade an Entry is Already Activated first Deactivate that Entry!!"); return;
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,to_char(a.ent_dt,'dd/mm/yyyy') as pend_dt from " + frm_tabname + " a where  a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.morder";
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    dt2 = new DataTable();
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl5.Text = dt.Rows[0]["ent_by"].ToString();
                        txtlbl6.Text = dt.Rows[0]["pend_dt"].ToString();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "sELECT distinct trim(a.Vchnum) as Entry_no,to_char(a.Vchdate,'dd/mm/yyyy') as Entry_Dt,a.grade,b.name as grade_name,a.ed_name,TO_CHAR(a.eff_from,'DD/MM/YYYY') AS effective_from,TO_CHAR(a.eff_to,'DD/MM/YYYY') AS effective_upto,trim(a.staten) as state,c.name as state_name,(case when a.category='M' then 'MALE' else 'FEMALE' end) as gender,a.sal_frm,a.sal_upto,a.mth04 as april,a.mth05 as may,a.mth06 as june,a.mth07 as july,a.mth08 as august,a.mth09 as september,a.mth10 as october,a.mth11 as november,a.mth12 as december,a.mth01 as january,a.mth02 as february,a.mth03 as march,a.m_tot as total,a.ent_by,to_char(a.Ent_Dt,'dd/mm/yyyy') as ent_dt,to_char(a.vchdate,'yyyymmdd') as vdd FROM " + frm_tabname + " a, type b,type c WHERE trim(a.grade)=trim(b.type1) and trim(a.staten)=trim(c.type1) and a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' and b.id='I' and c.id='{' and b.type1 like '0%' AND a.VCHDATE " + PrdRange + " ORDER BY vdd DESC,entry_No DESC";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            ...
            else
            {
                if (col1 == "Y" && Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (edmode.Value == "Y")
                        {
                            cmd_query = "update " + frm_tabname + " set branchcd='DD' where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_vty + txtvchnum.Text + txtvchdate.Text + "'";
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            ...
            oporow["STATEN"] = txtlbl7.Text.Trim().ToUpper();
            ...
            oporow["CATEGORY"] = txtGender.Text.Trim().ToUpper();

            oporow["ED_FLD"] = "DED10";
            oporow["ED_NAME"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper();
            oporow["SAL_FRM"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper());
            oporow["SAL_UPTO"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().ToUpper());
            oporow["MTH04"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper());
            oporow["MTH05"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim().ToUpper());
            oporow["MTH06"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.Trim().ToUpper());
            oporow["MTH07"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.Trim().ToUpper());
            oporow["MTH08"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text.Trim().ToUpper());
            oporow["MTH09"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text.Trim().ToUpper());
            oporow["MTH10"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text.Trim().ToUpper());
            oporow["MTH11"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t11")).Text.Trim().ToUpper());
            oporow["MTH12"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t12")).Text.Trim().ToUpper());
            oporow["MTH01"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t13")).Text.Trim().ToUpper());
            oporow["MTH02"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t14")).Text.Trim().ToUpper());
            oporow["MTH03"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t15")).Text.Trim().ToUpper());
            oporow["M_TOT"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t16")).Text.Trim().ToUpper());
