...
public partial class om_css_log : System.Web.UI.Page
{
    ...
    string pk_error = "Y", chk_rights = "N", DateRange, PrdRange, cond, cond1;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "Customer Support System";
    ...
    public void enablectrl()
    {
        ...
        btnlist.Disabled = false; btnMonthlyList.Disabled = false;
        btnprint.Disabled = false; btnNewList.Disabled = false;
        create_tab();
        create_tab2();
        create_tab3();
        create_tab4();
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "MonthlyList":
                ...
                break;
            case "PERSON":
                SQuery = "select trim(upper(Cont_no)) as fstr, trim(upper(Cont_Name)) as Client_Person_name,trim(upper(Cont_Email)) as Email,trim(upper(Cont_no)) as Mobile_no,count(*) as Requests from wb_css_log  where branchcd!='DD' and UPPER(TRIM(Ccode))='" + txtlbl4.Value.ToUpper().Trim() + "' and length(Trim(Cont_email))>4 group by trim(upper(Cont_no)), trim(upper(Cont_Name)),trim(upper(Cont_Email)),trim(upper(Cont_no)) order by trim(upper(Cont_Name)) ";
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as CSS_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as CSS_Dt,a.ccode as company,a.Eicon as Subjects,substr(a.remarks,1,60) as Remarks,a.dir_comp,a.Last_Action,a.cont_name as person,a.cont_no as contact_no, a.Ent_by,a.ent_Dt,nvl(trim(a.fapp_by),'-') as css_clrby,to_char(a.fapp_dt,'dd/mm/yyyy') as css_clrdt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='" + lbl1a_Text + "' " + cond + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            fgen.msg("-", "AMSG", "Company Code not entered !!");
            return;
        }
        if (txtlbl8.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", "Icon Name / Path not entered !!");
            return;
        }
        if (txtlbl2.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact Name !!");
            return;
        }
        if (txtlbl3.Value.Length != 10)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact No. !!");
            return;
        }
        if (txtlbl5.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact Email ID !!");
            return;
        }

        if (fgen.isValidMailID(txtlbl5.Value.Trim()) == false)
        {
            fgen.msg("-", "AMSG", " Please Enter Correct Email ID !!");
            return;
        }

        if (txtrmk.Text.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Your Remarks !!");
            return;
        }

        if (txtstarttime.Text.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Your Start Time !!");
            return;
        }

        if (txtendtime.Text.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Your End Time !!");
            return;
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtstarttime.Text = dt.Rows[i]["start_time"].ToString().Trim();
                        txtendtime.Text = dt.Rows[i]["end_time"].ToString().Trim();
                        if (txtstarttime.Text.Length > 2)
                        {
                            DateTime StartTimee = DateTime.Parse(txtstarttime.Text);

                            DateTime EndTimee = DateTime.Parse(txtendtime.Text); // It converts this to 1600.

                            TimeSpan tss = EndTimee - StartTimee;

                            time_taken.Value = tss.TotalMinutes.ToString();
                        }
                        ...
                        if (dt.Rows[i]["dir_comp"].ToString().Trim() == "N") ddWorkDone.Value = "NO";
                        else ddWorkDone.Value = "YES";

                        txtWrkRmk.Text = dt.Rows[i]["wrkRmk"].ToString().Trim();
                        time_taken.Value = dt.Rows[i]["time_taken"].ToString().Trim();

                        txtRoot.Text = dt.Rows[i]["ROOT"].ToString().Trim();
                        txtCorrective.Text = dt.Rows[i]["CORRECTIVE"].ToString().Trim();
                        txtPreventive.Text = dt.Rows[i]["PREVENTIVE"].ToString().Trim();
                        txtSolvedBy.Text = dt.Rows[i]["SOLVEDBY"].ToString().Trim();
                        txtClrBy.Text = dt.Rows[i]["fapp_by"].ToString().Trim();
                        if (dt.Rows[i]["fapp_dt"].ToString().Trim().Length > 2)
                        {
                            txtClrDt.Text = Convert.ToDateTime(dt.Rows[i]["fapp_dt"].ToString().Trim()).ToString("dd/MM/yyyy");
                        }
                        Txtent_by.Text = dt.Rows[0]["ent_by"].ToString().Trim();
                        Txtent_dt.Text = dt.Rows[0]["ent_dt"].ToString();
                        ...
                        if (fgen.make_double(frm_ulvl) < 3) txtlbl9.Disabled = false;
                ...
                case "NewList":
                    hf1.Value = col1;
                    hffield.Value = "NewList_E";
                    fgen.Fn_open_prddmp1("Select Date for List", frm_qstr);
                    break;

                case "MonthlyList":
                    hf1.Value = col1;
                    hffield.Value = "MonthlyList_E";
                    fgen.Fn_open_prddmp1("Select Date for List", frm_qstr);
                    break;
            }
        }
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        else if (hffield.Value == "NewList_E")
        {
            ...
            if (frm_ulvl == "3") cond = " and trim(a.ccode)='" + frm_uname + "'";
            if (CSR.Length > 1) cond = " and trim(a.ccode)='" + CSR + "'";
            if (hf1.Value.Length > 1) cond1 = " and a.CCODE='" + hf1.Value + "'";
            SQuery = "SELECT a.CSSNO as CSS_NO,to_char(A.CSsDT,'dd/mm/yyyy') as CSS_Dt,a.CCODE as Client_Code,a.Remarks,a.ent_by,a.wrkrmk,a.root,a.corrective,a.preventive,a.solvedby,a.dir_comp as work_done,a.cont_name,a.cont_no,to_chaR(a.CSSDT,'YYYYMMDD') as CSS_DTd FROM " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.cssdt " + PrdRange + " " + cond + " " + cond1 + " and a.cssdt>=to_Date('01/10/2017','dd/mm/yyyy') order by CSS_DTd,a.cssno";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);

            DataRow dr1 = null;
            dt4 = new DataTable();
            dt4.Columns.Add("Css_No", typeof(string));
            mq0 = "";
            if (dt.Rows.Count > 0)
            {
                DataView view1 = new DataView(dt);
                dt2 = new DataTable();
                dt2 = view1.ToTable(true, "css_no", "css_dt");

                foreach (DataRow dr in dt2.Rows)
                {
                    DataView view2 = new DataView(dt, "css_no='" + dr["css_no"].ToString().Trim() + "' and css_dt='" + dr["css_dt"].ToString().Trim() + "'", "", DataViewRowState.CurrentRows);
                    dt3 = new DataTable();
                    dt3 = view2.ToTable();
                    mq0 = "";
                    for (i = 0; i < dt3.Rows.Count; i++)
                    {
                        ...
                        mq0 = "Sir/ Madam,";
                        mq0 += "\\n";
                        mq0 += "Finsys Online Support Given On : " + dt3.Rows[i]["CSS_Dt"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "CSS No. : " + dt3.Rows[i]["css_no"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Company Login Id : " + dt3.Rows[i]["Client_Code"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Query : " + dt3.Rows[i]["Remarks"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Solution : " + dt3.Rows[i]["wrkrmk"].ToString().Trim();
                        mq0 += "\\n";
                        if (dt3.Rows[i]["root"].ToString().Trim().Length > 3)
                        {
                            mq0 += "Root Cause : " + dt3.Rows[i]["root"].ToString().Trim();
                            mq0 += "\\n";
                        }
                        if (dt3.Rows[i]["Corrective"].ToString().Trim().Length > 3)
                        {
                            mq0 += "Corrective Action : " + dt3.Rows[i]["corrective"].ToString().Trim();
                            mq0 += "\\n";
                        }
                        if (dt3.Rows[i]["Preventive"].ToString().Trim().Length > 3)
                        {
                            mq0 += "Preventive Action Suggestion : " + dt3.Rows[i]["preventive"].ToString().Trim();
                            mq0 += "\\n";
                        }
                        if (dt3.Rows[i]["work_done"].ToString().Trim() == "Y")
                        {
                            mq0 += "Query Done Successfully";
                        }
                        else
                        {
                            mq0 += "Query Pending";
                        ...
                        mq0 += "\\n";
                        mq0 += "Customer's Person : : " + dt3.Rows[i]["cont_name"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Phone No. : " + dt3.Rows[i]["cont_no"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Call Received By : " + dt3.Rows[i]["ent_by"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Solved By : " + dt3.Rows[i]["solvedby"].ToString().Trim();
                        mq0 += "\\n";
                        mq0 += "Regards,";
                        mq0 += "\\n";
                        mq0 += "Alka Tiwari";
                        mq0 += "\\n";
                        mq0 += "Tejaxo ERP Support Center";
                        dr1["Css_No"] = mq0;
                        dt4.Rows.Add(dr1);
                        dt4.Rows.Add(); // FOR ADDING A BLANK ROW BEFORE NEXT CSS NO.
                    }
                }
            }
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", "-");
            Session["send_dt"] = dt4;
            fgen.Fn_open_rptlevelJS("List of " + lblheader.Text.Trim() + " In New Format for the Period of " + fromdt + " to " + todt, frm_qstr);
            hffield.Value = "-";
        }
        else if (hffield.Value == "MonthlyList_E")
        {
            PrdRange = fgenMV.Fn_Get_Mvar(frm_qstr, "U_PRDRANGE");
            fromdt = fgenMV.Fn_Get_Mvar(frm_qstr, "U_MDT1");
            todt = fgenMV.Fn_Get_Mvar(frm_qstr, "U_MDT2");
            ...
            if (CSR.Length > 1) cond = " and trim(a.ccode)='" + CSR + "'";
            if (hf1.Value.Length > 1) cond1 = " and a.CCODE='" + hf1.Value + "'";
            SQuery = "SELECT a.CSSNO as CSS_NO,to_char(A.CSsDT,'dd/mm/yyyy') as CSS_Dt,a.CCODE as Client_Code,b.full_name as comp_name,a.Remarks,a.ent_by,a.wrkrmk,a.root,a.corrective,a.preventive,a.solvedby,a.dir_comp as work_done,a.cont_name,a.cont_no,to_chaR(a.CSSDT,'YYYYMMDD') as CSS_DTd FROM " + frm_tabname + " a,evas b where trim(a.CCODE)=trim(b.username) and b.allowigrp='FINCOCD' and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.cssdt " + PrdRange + " " + cond + " " + cond1 + " and a.cssdt>=to_Date('01/10/2017','dd/mm/yyyy') and nvl(trim(a.req_type),'-')!='Visit' order by CSS_DTd,a.cssno";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);

            string mq1 = "", mq2 = "";
            mq1 = "SELECT trim(CLIENT_CODE) as CLIENT_CODE,SUM(work_done) as work_done,SUM(NO_work_done) as NO_work_done FROM (SELECT a.CSSNO as CSS_NO,to_char(A.CSsDT,'dd/mm/yyyy') as CSS_Dt,a.CCODE as Client_Code,(CASE WHEN NVL(TRIM(a.dir_comp),'-')='Y' THEN 1 END) as work_done,(CASE WHEN NVL(TRIM(a.dir_comp),'-')='N' THEN 1 END) as NO_work_done FROM WB_CSS_LOG a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.cssdt " + PrdRange + " " + cond + " " + cond1 + " and a.cssdt>=to_Date('01/10/2017','dd/mm/yyyy') and nvl(trim(a.req_type),'-')!='Visit')GROUP BY trim(CLIENT_CODE)";
            ...
            if (dt.Rows.Count > 0)
            {
                DataView view1 = new DataView(dt);
                dt2 = new DataTable();
                dt2 = view1.ToTable(true, "Client_Code", "comp_name");

                foreach (DataRow dr in dt2.Rows)
                {
                    DataView view2 = new DataView(dt, "Client_Code='" + dr["Client_Code"].ToString().Trim() + "' and comp_name='" + dr["comp_name"].ToString().Trim() + "'", "", DataViewRowState.CurrentRows);
                    dt3 = new DataTable();
                    dt3 = view2.ToTable();
                    mq0 = "";
                    dr1 = dt4.NewRow();
                    for (i = 0; i < dt3.Rows.Count; i++)
                    {
                        if (i == 0)
                        {
                            mq0 = "Sir/ Madam,";
                            mq0 += "\\n";
                            mq0 += "Finsys Online Support Given From : " + fromdt + " To " + todt;
                            mq0 += "\\n";
                            mq0 += "Company Login Id : " + dr["Client_Code"].ToString().Trim();
                            mq0 += "\\n";
                            mq0 += "Customer Name : " + dr["comp_name"].ToString().Trim();
                            mq0 += "\\n";
                            mq0 += "\\n";
                            mq0 += "Summary : ";
                            mq0 += "\\n";
                            mq0 += "Summary of CSS (Customer Support System) Queries Created In Monitoring Database";
                            mq0 += "\\n";
                            mq0 += "Total CSS Created : " + dt3.Rows.Count;
                            mq0 += "\\n";
                            mq0 += "Solved And Marked Closed : " + fgen.seek_iname_dt(dt1, "client_code='" + dr["client_code"].ToString().Trim() + "'", "work_done");
                            mq0 += "\\n";
                            mq2 = fgen.seek_iname_dt(dt1, "client_code='" + dr["client_code"].ToString().Trim() + "'", "NO_work_done");
                            if (mq2 == "")
                            {
                                mq0 += "Currently Open : 0";
                            }
                            else
                            {
                                mq0 += "Currently Open : " + mq2;
                            }
                            mq0 += "\\n";
                            mq0 += "\\n";
                            mq0 += "Short Summary of the Above is as Follows :";
                        }
                        mq0 += "\\n";
                        mq0 += "CSS No. : " + dt3.Rows[i]["css_no"].ToString().Trim();
                        mq0 += "\\n";
            ...
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", "-");
            Session["send_dt"] = dt4;
            fgen.Fn_open_rptlevelJS("Monthly List of " + lblheader.Text.Trim() + " In New Format for the Period of " + fromdt + " to " + todt, frm_qstr);
            hffield.Value = "-";
        }
        else
        {
            Checked_ok = "Y";
            //-----------------------------
            if (ddWorkDone.Value.Substring(0, 1) == "Y")
            {
                if (fgen.make_double(time_taken.Value) == 0)
                {
                    Checked_ok = "N";
                    fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Please Fill the Time Taken to clear This Task !!");
                }
            }
            string last_entdt;
            //checks
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + frm_vty + frm_vnum + frm_CDT1, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Value.Trim()).ToString("dd/MM/yyyy"), "", frm_uname);
                                if (i > 20)
                                {
    ...
    void save_fun()
    {
        ...
        oporow[doc_df.Value] = Convert.ToDateTime(txtvchdate.Value.Trim()).ToString("dd/MM/yyyy");
        oporow["SRNO"] = i;
        oporow["CCODE"] = txtlbl4.Value.ToUpper().Trim();
        oporow["EMODULE"] = ddModule.Value;
        oporow["EICON"] = txtlbl8.Value.ToUpper().Trim();
        oporow["REQ_TYPE"] = txtlbl9.Value;
        oporow["ISS_TYPE"] = ddIssueType.Value;
        ...
        oporow["dir_comp"] = ddWorkDone.Value.Substring(0, 1);
        oporow["wrkRmk"] = txtWrkRmk.Text.Trim();        
        oporow["ROOT"] = txtRoot.Text.Trim();
        oporow["CORRECTIVE"] = txtCorrective.Text.Trim();
        oporow["PREVENTIVE"] = txtPreventive.Text.Trim();
        oporow["SOLVEDBY"] = txtSolvedBy.Text.Trim();
        oporow["FAPP_BY"] = txtClrBy.Text.Trim().ToUpper();
        oporow["start_time"] = txtstarttime.Text.Trim();
        oporow["end_time"] = txtendtime.Text.Trim();
        oporow["time_taken"] = fgen.make_double(time_taken.Value);
        if (txtClrDt.Text.Trim().Length > 2)
        {
            oporow["FAPP_DT"] = txtClrDt.Text.Trim().ToUpper();
        }
        if (edmode.Value == "Y")
        {
    ...
    protected void btnAtt_Click(object sender, EventArgs e)
    {
        ...
        if (Attch.HasFile)
        {
            ...
            filepath = Server.MapPath("~/tej-base/UPLOAD/") + txtlbl4.Value.Trim() + "_" + txtvchnum.Value.Trim() + frm_CDT1.Replace(@"/", "_") + "~" + Attch.FileName;
            Attch.PostedFile.SaveAs(filepath);
    ...
    protected void btnNewList_ServerClick(object sender, EventArgs e)
    {
        hffield.Value = "NewList";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Customer", frm_qstr);
    }

    protected void AlterQuery()
    {
        //ALTER TABLE WB_CSS_LOG ADD ROOT VARCHAR2(250);
        //ALTER TABLE WB_CSS_LOG ADD CORRECTIVE VARCHAR2(250);
        //ALTER TABLE WB_CSS_LOG ADD PREVENTIVE VARCHAR2(250);
        //ALTER TABLE WB_CSS_LOG ADD SOLVEDBY VARCHAR2(20);
    }

    protected void btnMonthlyList_ServerClick(object sender, EventArgs e)
    {
        hffield.Value = "MonthlyList";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Customer", frm_qstr);
    }
    protected void txtendtime_TextChanged(object sender, EventArgs e)
    {
        if (txtstarttime.Text.Length > 2)
        {
            ...
            DateTime EndTime = DateTime.Parse(txtendtime.Text); // It converts this to 1600.

            TimeSpan ts = EndTime - StartTime;

            time_taken.Value = ts.TotalMinutes.ToString();
        }
    }
}