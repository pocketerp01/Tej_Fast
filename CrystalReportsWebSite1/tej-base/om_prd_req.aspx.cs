...
public partial class om_prd_req : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                ...
                SQuery = "select Acref,name as Wip_Stage ,Acref as Stg_code,Type1 as Srno from typegrp where branchcd='"+ frm_mbr+"' and id='WI' order by type1";
            ...
            default:
                ...
                switch (Prg_Id)
                {
                    case "F39211":
                    case "F40211":
                        SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as entry_no,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as entry_Dt,b.AName as Supplier,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where trim(a.acode)=trim(B.acode) and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' AND a." + doc_df.Value + " " + DateRange + " order by vdd desc,a." + doc_nf.Value + " desc";
                        break;
                }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["req_qty"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["req_wt"].ToString().Trim();
                ...
                case "TACODE":
                    ...
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0 && plan_base == "Y")
                    {
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["req_Qty"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());
                oporow["req_wt"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim());
                ...
                if (doc_addl.Value == "Y")
                {
                    ...
                    oporow["jobdt"] = fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text.Trim(), vardate);

                }
                else
                {

                    oporow["jobno"] = txtlbl5.Text.Trim();
                    oporow["jobdt"] = fgen.make_def_Date(txtlbl6.Text.Trim(), vardate);
