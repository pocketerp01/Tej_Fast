...
public partial class findCr : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                break;
            case "TACODE":
                SQuery = "select acode,aname as customer,acode as code from famst where trim(nvl(GRP,'-')) in ('05','06') order by acode";
                break;
            case "TICODE":
                ...
                SQuery = "select distinct a.icode,B.INAME as product,a.icode as erpcode,B.CPARTNO as partname,a.ponum,to_Char(a.podate,'dd/mm/yyyy') as podate,a.invno as invno,to_char(a.invdate,'dd/mm/yyyy') as invdate from ivoucher a,item b where trim(a.icode)=trim(B.icode) and a.branchcd='" + frm_mbr + "' and a.type like '0%' and a.vchdate between (sysdate-600) and sysdate and trim(A.acode)='" + txtacode.Value.Trim() + "' order by a.icode";
                //SQuery = "SELECT DISTINCT A.ICODE,B.INAME AS PRODUCT,A.ICODE AS ERPCODE FROM POMAS A,ITEM B WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '5%' AND A.ORDDT between (sysdate-600) and sysdate  AND TRIM(A.ACODE)='" + txtacode.Value.Trim() + "' AND NVL(A.APP_BY,'-')!='-' ORDER BY A.ICODE";
                break;
            case "TRCODE":
                SQuery = "select acode,aname as customer,acode as code from famst where trim(Acode) like '3%' order by acode";
                break;
            case "DNCN":
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtRcode.Value.Trim().Length < 2)
        {
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                //fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.acode)='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4") + "' and a.type in ('" + frm_vty + "','31','32') ");
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.type)||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'  ");
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.type)||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') in (select DISTINCT a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr from ivoucherp A WHERE A.BTCHNO='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "')");
                // Deleing data from voucher Table
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from voucher a where a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') in (select DISTINCT a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr from ivoucherp A WHERE A.BTCHNO='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "')");
                // Deleing data from ivoucherp Table
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from ivoucherp a where A.BTCHNO='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "' AND A.TYPE IN ('58','59') AND A.BRANCHCD='" + frm_mbr + "'");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    break;
                case "LIST_E":
                    SQuery = "SELECT A.VCHNUM AS ENTRYNO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS ENTRYDT,A.COL33 AS cust_pono,A.COL2 AS inv_no,a.col3 as inv_dt,a.acode as partycode,c.aname as party,b.icode as erpcode,d.iname as product,a.col6 as qty,a.col7 as oldrate,a.col8 as newrate,a.col9 as rate_diff,a.col13 as pono,a.col14 as pordt,a.col15 as inv_value,to_number(a.col16)+to_number(a.col17) as tax_per,a.ent_by,a.ent_dt,B.VCHNUM AS VCH_NO,TO_CHAR(b.VCHDATE,'DD/MM/YYYY') AS VCH_DT,B.TYPE AS NOTE_TYPE,B.BRANCHCD AS B_CODE FROM SCRATCH2 A,ivoucherp B,famst c,item d WHERE TRIM(A.ACODE)||TRIM(A.COL2)||TO_CHAR(TO_DATE(A.COL3,'DD/MM/YYYY'),'DD/MM/YYYY')||TRIM(A.COL33)||trim(a.col5)=TRIM(B.ACODE)||TRIM(B.INVNO)||TO_CHAR(B.INVDATE,'DD/MM/YYYY')||TRIM(B.LOCATION)||trim(b.icode) and trim(A.acode)=trim(c.acode) and trim(b.icode)=trim(d.icode) AND a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' and b.type in ('31','32') ORDER BY A.COL33";
                    SQuery = "SELECT A.VCHNUM AS ENTRYNO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS ENTRYDT,A.COL33 AS cust_pono,A.COL2 AS inv_no,a.col3 as inv_dt,a.acode as partycode,c.aname as party,b.icode as erpcode,B.PURPOSE as product,B.EXC_57F4,a.col6 as qty,a.col7 as oldrate,a.col8 as newrate,a.col9 as rate_diff,a.col13 as pono,a.col14 as pordt,a.col15 as inv_value,to_number(a.col16)+to_number(a.col17) as tax_per,a.ent_by,a.ent_dt,B.VCHNUM AS VCH_NO,TO_CHAR(b.VCHDATE,'DD/MM/YYYY') AS VCH_DT,b.iamount as amt_wot,b.spexc_amt as amt,B.TYPE AS NOTE_TYPE,B.BRANCHCD AS B_CODE FROM SCRATCH2 A,ivoucherp B,famst c WHERE TRIM(A.ACODE)||TRIM(A.COL2)||TO_CHAR(TO_DATE(A.COL3,'DD/MM/YYYY'),'DD/MM/YYYY')||TRIM(A.COL33)||trim(a.col5)||trim(a.col6)=TRIM(B.ACODE)||TRIM(B.INVNO)||TO_CHAR(B.INVDATE,'DD/MM/YYYY')||TRIM(B.LOCATION)||trim(b.icode)||b.iqty_chl AND A.BRANCHCD||TRIM(A.VCHNUM)||TO_CHAR(A.VCHDATE,'DD/MM/YYYY')=TRIM(B.BTCHNO) and trim(A.acode)=trim(c.acode) AND a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' and b.type in ('31','32') ORDER BY A.COL33";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                    fgen.Fn_open_rptlevel("List of " + lblheader.Text.Trim() + " for the Period of " + fromdt + " to " + todt, frm_qstr);
                    break;
                case "LIST_E2":
                    SQuery = "SELECT A.BRANCHCD,A.INVNO,A.INVDATE,A.ACODE AS CODE,B.ANAME AS PARTY,A.ICODE AS ERPCODE,C.INAME AS PRODUCT,C.CPARTNO,SUM(A.AMT) AS UPLD_AMT,SUM(A.AMT2) AS IVCH_AMT,SUM(A.AMT3) AS VCH_AMT FROM (SELECT BRANCHCD,TRIM(COL11) AS INVNO,TRIM(COL12) AS INVDATE,TRIM(ACODE) AS ACODE,TRIM(ICODE) AS ICODE,SUM(IS_NUMBER(col6*col9)) AS AMT,0 AS AMT2,0 AS AMT3 FROM SCRATCH2 WHERE branchcd||type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' group by branchcd,trim(col11),trim(Col12),trim(acode),trim(icodE) UNION ALL  SELECT a.BRANCHCD,TRIM(a.INVNO),TO_CHAR(a.INVDATE,'DD/MM/YYYY') AS INVATE,TRIM(a.ACODE) AS ACODE,TRIM(a.ICODE) AS ICODE,0 AS AMT,SUM(a.IAMOUNT) AS AMT2,sum(b.dramt+b.cramt) AS AMT3 FROM ivoucherp a ,voucher b where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode) =b.branchcd||b.type||trim(b.vchnum)||to_char(b.vchdate,'dd/mm/yyyy')||trim(b.Rcode) and a.BRANCHCD='" + frm_mbr + "' and a.type in ('31','32') and a.btchno='" + frm_mbr + col1 + "' and b.srno='50' group by a.branchcd,TRIM(a.INVNO),TO_CHAR(a.INVDATE,'DD/MM/YYYY'),TRIM(a.ACODE),TRIM(a.ICODE)) A,FAMST B,ITEM C WHERE TRIM(A.ACODE)=TRIM(b.ACODE) AND TRIM(A.ICODE)=TRIM(C.ICODE) GROUP BY A.BRANCHCD,A.INVNO,A.INVDATE,A.ACODE,B.ANAME,A.ICODE,C.INAME,C.CPARTNO ORDER BY A.INVNO,A.INVDATE,A.ACODE,A.ICODE";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                    fgen.Fn_open_rptlevel("List of " + lblheader.Text.Trim() + " for the Period of " + fromdt + " to " + todt, frm_qstr);
                    break;
                case "Print_E":
    ...
    void fillItems()
    {
        ...
        SQuery = "SELECT a.*,a.iqtyout as dnqty,0 as cust_rej,b.iname,b.cpartno,b.unit,B.HSCODE,a.exc_rate,a.cess_percent,a.iopr,c.num4,c.num5,c.num6,c.num7 FROM ivoucherp a,item b,typegrp c WHERE trim(a.icode)=trim(B.icode) and trim(b.hscode)=trim(c.acref) and a.BRANCHCD='" + frm_mbr + "' and a.type like '4%' and a.vchdate " + PrdRange + " and trim(a.acode)='" + txtacode.Value + "' and trim(a.icode)='" + txtIcode.Value + "' and trim(A.ponum)||to_char(a.podate,'dd/mm/yyyy')='" + txtponum.Value + txtPodt.Value + "' and c.id='T1' order by a.vchnum ";
        if (frm_cocd == "BONY") SQuery = "SELECT a.*,a.iqtyout as dnqty,0 as cust_rej,A.PURPOSE AS iname,A.EXC_57F4 AS cpartno,b.unit,B.HSCODE,a.exc_rate,a.cess_percent,a.iopr,c.num4,c.num5,c.num6,c.num7 FROM ivoucherp a,item b,typegrp c WHERE trim(a.icode)=trim(B.icode) and trim(b.hscode)=trim(c.acref) and a.BRANCHCD='" + frm_mbr + "' and a.type like '4%' and a.vchdate " + PrdRange + " and trim(a.acode)='" + txtacode.Value + "' and trim(a.icode)='" + txtIcode.Value + "' and trim(A.finvno)='" + txtcustPo.Text + "' AND TRIM(A.EXC_57F4)='" + txtCpartno.Value.Trim() + "' and c.id='T1' order by a.vchnum ";
        if (frm_cocd == "SAIP") SQuery = "SELECT a.*,(Case when nvl(a.st_modv,0)>0 and nvl(a.st_modv,0)<>nvl(a.iqtyout,0) then nvl(a.st_modv,0) else a.iqtyout end ) as dnqty,nvl(et_paid,0) as cust_rej,A.PURPOSE AS iname,A.EXC_57F4 AS cpartno,b.unit,B.HSCODE,a.exc_rate,a.cess_percent,a.iopr,c.num4,c.num5,c.num6,c.num7 FROM ivoucherp a,item b,typegrp c WHERE trim(a.icode)=trim(B.icode) and trim(b.hscode)=trim(c.acref) and a.BRANCHCD='" + frm_mbr + "' and a.type like '4%' and a.vchdate " + PrdRange + " and trim(a.acode)='" + txtacode.Value + "' and trim(a.icode)='" + txtIcode.Value + "' and trim(A.finvno)='" + txtcustPo.Text + "' and c.id='T1' order by a.vchnum ";
        SQuery = "SELECT A.*,B.INAME,B.CPARTNO,B.HSCODE,c.num4,c.num5,c.num6,c.num7 FROM ivoucher A,ITEM B,typegrp c WHERE TRIM(a.ICODE)=TRIM(b.ICODE) AND trim(b.hscode)=trim(c.acref) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '0%' AND A.vchdate " + PrdRange + " AND TRIM(A.ACODE)='" + txtacode.Value.Trim() + "' AND TRIM(A.ICODE)='" + txtIcode.Value.Trim() + "' and c.id='T1' ORDER BY A.vchnum,A.vchdate";
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        if (dt.Rows.Count > 0)
        {
            i = 1;
            foreach (DataRow dr in dt.Rows)
            {
                ...
                sg1_dr["sg1_h7"] = fgen.make_double(dr["iqtyin"].ToString().Trim());
                ...
                sg1_dr["sg1_f2"] = Convert.ToDateTime(fgen.make_def_Date(dr["invdate"].ToString().Trim(), vardate)).ToString("dd/MM/yyyy");

                sg1_dr["sg1_F3"] = dr["ponum"].ToString().Trim();
                sg1_dr["sg1_F4"] = Convert.ToDateTime(fgen.make_def_Date(dr["podate"].ToString().Trim(), vardate)).ToString("dd/MM/yyyy");
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT A.COL33 AS BATCH_NO,A.COL34 AS SERIAL_NO,A.COL35 AS BATCH_DATE,A.COL2 AS PART_NO,A.COL1 AS PART_NAME,A.COL3 AS PO_NO,A.COL12 AS INVNO,A.COL22 AS INV_DATE,A.COL13 AS SRV_NO,A.COL14 AS QTY,A.COL16 AS OLD_RATE,A.COL26 AS NEW_RATE,(TO_NUMBER(A.COL26)-TO_NUMBER(A.COL16)) AS DIFF,A.COL17 AS BASIC_AMT,A.COL18 AS CGST,A.COL19 AS SGST,A.COL20 AS IGST,(TO_NUMBER(A.COL17)+TO_NUMBER(A.COL18)+TO_NUMBER(A.COL19)+TO_NUMBER(A.COL20)) AS TOTAL,A.COL29 AS HSCODE,B.VCHNUM AS VCH_NO,TO_CHAR(b.VCHDATE,'DD/MM/YYYY') AS VCH_DT FROM SCRATCH2 A,ivoucherp B WHERE TRIM(A.ACODE)||TRIM(a.ICODE)||TRIM(A.COL12)||TO_CHAR(TO_DATE(A.COL22,'DD/MM/YY'),'DD/MM/YYYY')=TRIM(B.ACODE)||TRIM(B.ICODE)||TRIM(B.INVNO)||TO_CHAR(B.INVDATE,'DD/MM/YYYY') AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE " + DateRange + " and a.num10>0 ORDER BY A.COL33";
            SQuery = "SELECT A.VCHNUM AS ENTRYNO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS ENTRYDT,A.COL33 AS cust_pono,A.COL2 AS inv_no,a.col3 as inv_dt,a.acode as partycode,c.aname as party,a.icode as erpcode,d.iname as product,a.col6 as qty,a.col7 as oldrate,a.col8 as newrate,a.col9 as rate_diff,a.col13 as pono,a.col14 as pordt,a.col15 as inv_value,to_number(a.col16)+to_number(a.col17) as tax_per,a.ent_by,a.ent_dt,B.VCHNUM AS VCH_NO,TO_CHAR(b.VCHDATE,'DD/MM/YYYY') AS VCH_DT,B.TYPE AS NOTE_TYPE,B.BRANCHCD AS B_CODE FROM SCRATCH2 A,ivoucherp B,famst c,item d WHERE TRIM(A.ACODE)||TRIM(a.ICODE)||TRIM(A.COL2)||TO_CHAR(TO_DATE(A.COL3,'DD/MM/YY'),'DD/MM/YYYY')=TRIM(B.ACODE)||TRIM(B.ICODE)||TRIM(B.INVNO)||TO_CHAR(B.INVDATE,'DD/MM/YYYY') and trim(A.acode)=trim(c.acode) and trim(a.icode)=trim(d.icode) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE " + DateRange + " and b.type in ('31','32') and a.num10>0  ORDER BY A.COL33";

            SQuery = "SELECT A.VCHNUM AS ENTRYNO,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS ENTRYDT,A.COL33 AS cust_pono,A.COL2 AS inv_no,a.col3 as inv_dt,a.acode as partycode,c.aname as party,b.icode as erpcode,B.PURPOSE as product,B.EXC_57F4,a.col6 as qty,a.col7 as oldrate,a.col8 as newrate,a.col9 as rate_diff,a.col13 as pono,a.col14 as pordt,a.col15 as inv_value,to_number(a.col16)+to_number(a.col17) as tax_per,a.ent_by,a.ent_dt,B.VCHNUM AS VCH_NO,TO_CHAR(b.VCHDATE,'DD/MM/YYYY') AS VCH_DT,b.iamount as amt_wot,b.spexc_amt as amt,B.TYPE AS NOTE_TYPE,B.BRANCHCD AS B_CODE FROM SCRATCH2 A,ivoucherp B,famst c WHERE TRIM(A.ACODE)||TRIM(A.COL2)||TO_CHAR(TO_DATE(A.COL3,'DD/MM/YYYY'),'DD/MM/YYYY')||TRIM(A.COL33)||trim(a.col5)=TRIM(B.ACODE)||TRIM(B.INVNO)||TO_CHAR(B.INVDATE,'DD/MM/YYYY')||TRIM(B.LOCATION)||trim(b.icode) AND A.BRANCHCD||TRIM(A.VCHNUM)||TO_CHAR(A.VCHDATE,'DD/MM/YYYY')=TRIM(B.BTCHNO) and trim(A.acode)=trim(c.acode) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE " + DateRange + " and b.type in ('31','32') and a.num10>0 ORDER BY A.COL33";
        ...
        else if (hffield.Value == "ListS")
        {
            ...
            todt = fgenMV.Fn_Get_Mvar(frm_qstr, "U_MDT2");
            PrdRange = fgenMV.Fn_Get_Mvar(frm_qstr, "U_PRDRANGE");
            //SQuery = "select FSTR AS VAL1,SUM(AMT) AS UPL_AMT,SUM(AMT2) AS IVCH_AMT,SUM(AMT3) AS VCH_AMT FROM (SELECT branchcd||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') AS fstr ,SUM(IS_NUMBER(col6)*IS_NUMBER(col9)) AS AMT,0 AS AMT2,0 AS AMT3 from scratch2 where branchcd='" + frm_mbr + "' and type='"+frm_vty+"' and vchdate " + DateRange + " group by branchcd||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') union all SELECT a.btchno,0 AS AMT,SUM(a.IAMOUNT) AS AMT2,sum(b.dramt+b.cramt) AS AMT3 FROM ivoucherp a ,voucher b where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode) =b.branchcd||b.type||trim(b.vchnum)||to_char(b.vchdate,'dd/mm/yyyy')||trim(b.Rcode) and a.BRANCHCD='" + frm_mbr + "' and a.type in ('31','32') and a.vchdate " + DateRange + " and b.srno='50' group by a.btchno) GROUP BY FSTR ORDER BY FSTR";
            SQuery = "select FSTR AS VAL1,SUM(AMT) AS UPL_AMT,SUM(AMT2) AS IVCH_AMT,SUM(AMT3) AS VCH_AMT FROM (SELECT branchcd||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') AS fstr ,SUM(IS_NUMBER(col6)*IS_NUMBER(col9)) AS AMT,0 AS AMT2,0 AS AMT3 from scratch2 where branchcd='" + frm_mbr + "' and type='"+frm_vty+"' and vchdate " + DateRange + " group by branchcd||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') union all SELECT a.branchcd||trim(a.col1) as fstr,0 AS AMT,SUM(a.IAMOUNT) AS AMT2,sum(b.dramt+b.cramt) AS AMT3 FROM ivoucherp a ,voucher b where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode) =b.branchcd||b.type||trim(b.vchnum)||to_char(b.vchdate,'dd/mm/yyyy')||trim(b.Rcode) and a.BRANCHCD='" + frm_mbr + "' and a.type in ('31','32') and a.vchdate " + DateRange + " and b.srno='50' group by a.branchcd||trim(a.col1)) GROUP BY FSTR ORDER BY FSTR";
                ...
                if (saveTo == "Y")
                {
                    ...
                    if (qty > 0)
                    {
                        #region Complete Save Function
                        {
                            ...
                            oDS1 = new DataSet();
                            oporow1 = null;
                            oDS1 = fgen.fill_schema(frm_qstr, frm_cocd, "ivoucherp");
                            ...
                            if (fgen.make_double(drw["sg1_h10"].ToString().Trim()) > 0) nVty = "32";
                            else nVty = "31";

                            oporow1["TYPE"] = nVty;

                            frm_vnum = fgen.Fn_next_doc_no(frm_qstr, frm_cocd, "IVOUCHERP", "VCHNUM", "VCHDATE", branchcd, nVty, txtvchdate.Text.Trim(), frm_uname, frm_formID);

                            string vinvno = drw["sg1_f1"].ToString().Trim().PadLeft(6, '0');
                            string vinvdate = fgen.make_def_Date(Convert.ToDateTime(drw["sg1_f2"].ToString().Trim()).ToString("dd/MM/yyyy"), vardate);

                            string batchNo = vinvno + vinvdate;
                            ...
                            if (nVty == "31") invRmrk = "Debit Note Against PO No. :" + drw["sg1_f3"].ToString().Trim();
                            else invRmrk = "Credit Note Against PO No. :" + drw["sg1_f3"].ToString().Trim();
                            ...
                            fgen.save_data(frm_qstr, frm_cocd, oDS1, "ivoucherp");
    ...
    protected void btnupload_Click(object sender, EventArgs e)
    {
        ...
        {
            fgen.msg("-", "AMSG", "Please Select Supplier First!!");
        }
    }
