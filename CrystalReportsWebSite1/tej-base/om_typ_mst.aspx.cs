...
public partial class om_typ_mst : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, "Select id,type1,name,ent_by,to_char(ent_dt,'dd/mm/yyyy') as ent_Dt from " + frm_tabname + " where id='" + frm_vty + "' order by type1 ");
        ...
        switch (btnval)
        {
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as Mst_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as MSt_Dt,a.Name as Mst_Name,a.type1 as MSt_Code, a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and trim(a.id)='" + lbl1a_Text + "' " + cond + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date,to_Char(a.app_Dt,'dd/mm/yyyy') As mapp_date from " + frm_tabname + " a where a.branchcd||trim(a.ID)||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + mv_col + "' ";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtrmk.Text = dt.Rows[i]["Name"].ToString().Trim();

                        //txtlbl4.Value = dt.Rows[i]["Type1"].ToString().Trim();

                        ddAnalysis.Value = dt.Rows[i]["typedpt"].ToString().Trim();

                        txtlbl2.Value = dt.Rows[i]["suppfld1"].ToString().Trim();
                        txtlbl3.Value = dt.Rows[i]["suppfld2"].ToString().Trim();
                        txtlbl5.Value = dt.Rows[i]["suppfld3"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT a.Id,a.Type1,a.Name,a.Typedpt,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_Dtd,a.Edt_by,to_char(a.edt_Dt,'dd/mm/yyyy') as Edt_Dtd FROM " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.ID='" + frm_vty + "'  order by a.type1 ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        oporow["ID"] = lbl1a_Text;
        oporow[doc_nf.Value] = frm_vnum;
        oporow[doc_df.Value] = txtvchdate.Value.Trim();
        oporow["type1"] = frm_vnum.Substring(2, 4);
        oporow["suppfld1"] = txtlbl2.Value.ToUpper().Trim();
        oporow["suppfld2"] = txtlbl3.Value.ToUpper().Trim();
        oporow["suppfld3"] = txtlbl5.Value.ToUpper().Trim();

        oporow["typedpt"] = ddAnalysis.Value;

        oporow["Name"] = txtrmk.Text;
