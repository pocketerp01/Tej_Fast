...
public partial class om_so : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                txtlbl20.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(params) as params from controls where id='D39'", "params");
                txtlbl20a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where trim(acode)='" + txtlbl20.Text.Trim() + "'", "aname");
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //txtlbl5.Text = dt.Rows[i]["payment"].ToString().Trim();
                        //txtlbl6.Text = dt.Rows[i]["pdays"].ToString().Trim();
                        ...
                        txtlbl8.Text = dt.Rows[0]["check_by"].ToString().Trim();
                        txtlbl9.Text = dt.Rows[0]["app_by"].ToString().Trim();
                        ...
                        txtlbl15.Text = dt.Rows[i]["shipto"].ToString().Trim();
                        txtlbl16.Text = dt.Rows[i]["shipfrom"].ToString().Trim();
                        txtlbl17.Text = dt.Rows[i]["finaldst"].ToString().Trim();
                        txtlbl18.Text = dt.Rows[i]["destcount"].ToString().Trim();
                        txtlbl19.Text = dt.Rows[i]["bank_cd"].ToString().Trim();

                        txtlbl23a.Text = dt.Rows[i]["currency"].ToString().Trim();
                        txtlbl23.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select replacE(name,'''','`')  as name from type where id='A' and trim(upper(name))=upper(Trim('" + txtlbl23a.Text + "'))", "name");

                        txtlbl23b.Text = dt.Rows[i]["curr_Rate"].ToString().Trim();

                        txtlbl24.Text = dt.Rows[i]["basic"].ToString().Trim();
                        txtlbl26.Text = dt.Rows[i]["excise"].ToString().Trim();
                        txtlbl28.Text = dt.Rows[i]["cess"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t6"] = dt.Rows[i]["pexc"].ToString().Trim();
                            sg1_dr["sg1_t7"] = dt.Rows[i]["ptax"].ToString().Trim();
                            sg1_dr["sg1_t8"] = dt.Rows[i]["cdisc"].ToString().Trim();
    ...
    void save_fun()
    {
        ...
        if (fgen.make_double(txtlbl23b.Text) <= 0)
        {
            txtlbl23b.Text = "1";
        }
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["pordno"] = txtlbl2.Text.Trim().ToUpper();
                oporow["porddt"] = txtlbl3.Text.Trim().ToUpper();
                ...
                oporow["qtyord"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim();
                oporow["irate"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim();
                oporow["cdisc"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text.Trim());

                oporow["desc_"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim();

                oporow["WORK_ORDNO"] = "-";
                oporow["basic"] = txtlbl24.Text.Trim();
                ...
                oporow["iexc_addl"] = 0;
                oporow["pvt_mark"] = "-";
                //oporow["thru"] = txtpterms.Text.Trim().ToUpper();
                oporow["invno"] = "-";
                ...
                //oporow["desc9"] = fgen.seek_iname(co_cd, "select ciname from item where trim(icode)='" + sg1.Rows[i].Cells[3].Text.Trim() + "'", "ciname");
                oporow["inst1"] = 0;
                oporow["inst2"] = 0;
                oporow["inst3"] = 0;
                oporow["bcd"] = 0;
                ...
                //oporow["gmt_shade"] = txtdlvterms.Text.Trim().ToUpper();
                //oporow["busi_expect"] = txtbrand.Text.Trim().ToUpper();
                oporow["CO_ORIG"] = "-";

                oporow["ST_TYPE"] = lbl26.Text.Substring(0, 2);
                oporow["pexc"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.Trim();
                oporow["ptax"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.Trim();

                oporow["excise"] = txtlbl26.Text;
                oporow["cess"] = txtlbl28.Text;
