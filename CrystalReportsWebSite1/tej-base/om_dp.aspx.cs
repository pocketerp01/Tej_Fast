...
public partial class om_dp : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        frm_tabname = "inspvch";
        lblheader.Text = "Dim. And Visual Exam. Rep. For Dual Plate Check Valve";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "82");
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                // SQuery = "Select trim(icode)||trim(invno)||trim(desc_)  as fstr,invno as Wo_No,desc_ as Tag_no,IQTYIN AS Qty,to_char(vchdate,'dd/mm/yyyy') as Entry_Date,Ent_by,icode from ivoucherp where branchcd='" + frm_mbr + "' and type='15' and invno='" + txtlbl4.Text.Trim() + "' AND trim(ICODE)||trim(desc_) NOT IN (" + col1 + ") order by desc_";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtMaterial.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Material Code/Stock Code");
            txtMaterial.Focus();
            return;
        }
        if (txtDrawing.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Drawing_No & Rev_No");
            txtDrawing.Focus();
            return;
        }
        if (txtlbl2.Text == "-")
        ...
        {
            fgen.msg("-", "AMSG", "The Above Tested Products Are In Compliance With Customer Statutory And Regulatory Requirements '13' (Tab 2)");
            dd_Visual.Focus();
            return;
        }
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (i > 1)
            {
                ...
                //{
                //    fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Please Fill Item Code/Client Tag No. At Line No. " + sg1.Rows[i].Cells[12].Text.Trim());
                //    return;
                //}
            }
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtMaterial.Text = dt.Rows[0]["COL5"].ToString().Trim();
                        dd_Desg.SelectedItem.Text = dt.Rows[0]["COL3"].ToString().Trim();
                        txtFlange.Text = dt.Rows[0]["OBSV23"].ToString().Trim();
                        ...
                        txtTesting_Dt.Text = dt.Rows[0]["CONTPLAN"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t7"] = dt.Rows[i]["OBSV21"].ToString().Trim();
                            ...
                            sg1_dr["sg1_t16"] = dt.Rows[i]["OBSV22"].ToString().Trim();
                            sg1_dr["sg1_t17"] = dt.Rows[i]["OBSV17"].ToString().Trim();
                            sg1_dr["sg1_t18"] = dt.Rows[i]["OBSV18"].ToString().Trim();
                            sg1_dr["sg1_t19"] = dt.Rows[i]["OBSV19"].ToString().Trim();
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        ...
                        if (dt2.Rows.Count > 0)
                        {
                            ...
                            ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t16")).Text = dt2.Rows[0]["FACING"].ToString().Trim();
                        }
                    }
                    setColHeadings();
                    break;
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            if (dt2.Rows.Count > 0)
                            {
                                ...
                                sg1_dr["sg1_t16"] = dt2.Rows[0]["FACING"].ToString().Trim();
                            }
                            sg1_dr["sg1_t17"] = "";
                            sg1_dr["sg1_t18"] = "";
                            sg1_dr["sg1_t19"] = "";
            ...
            for (int sg1r = 0; sg1r < sg1.Rows.Count; sg1r++)
            {
                for (int j = 0; j < sg1.Columns.Count; j++)
                {
                    ...
                    if (sg1r == 0)
                    {
                        ...
                        sg1.Rows[sg1r].Cells[36].Enabled = true;

                        sg1.Rows[sg1r].Cells[29].Enabled = true;
                        sg1.Rows[sg1r].Cells[30].Enabled = true;
                        sg1.Rows[sg1r].Cells[31].Enabled = true;
                    ...
                    if (sg1r == 1)
                    {
                        ...
                        ((TextBox)sg1.Rows[1].FindControl("sg1_t1")).Text = "REQUIRED DIMENSIONS & FEATURE";
                        ((TextBox)sg1.Rows[1].FindControl("sg1_t1")).Enabled = false;
                        ((TextBox)sg1.Rows[1].FindControl("sg1_t16")).ReadOnly = false;
                    }
                }
            }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            //if (((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().Length > 1) // if this cond is removed then remove i!=0 from save also and from btnhidef_s
            //{
            ...
            oporow["COL5"] = txtMaterial.Text.Trim().ToUpper();
            ...
            oporow["OBSV19"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t19")).Text.Trim().ToUpper();
            oporow["CONTPLAN"] = txtTesting_Dt.Text.Trim();
            oporow["SAMPQTY"] = 0;
            oporow["FINISH"] = "-";
            oporow["OMAX"] = "-";
            oporow["OMIN"] = "-";
            ...
            oporow["OBSV21"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.Trim().ToUpper();
            oporow["OBSV22"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t16")).Text.Trim().ToUpper();
            oporow["OBSV23"] = txtFlange.Text.Trim().ToUpper();
    ...
    protected void FillGrid()
    {
        create_tab();
        //dt2 = new DataTable();
        //SQuery = "SELECT 'SIZE(MM)' AS HEADING FROM DUAL UNION ALL SELECT 'VALVE TAG NO.' AS HEADING FROM DUAL UNION ALL SELECT 'ITEM CODE/CLIENT TAG NO.' AS HEADING FROM DUAL UNION ALL SELECT 'OUTER DIA Ø A' AS HEADING FROM DUAL UNION ALL SELECT 'FACE TO FACE F/F' AS HEADING FROM DUAL UNION ALL SELECT 'INNER DIA Ø ID' AS HEADING FROM DUAL UNION ALL SELECT 'VALVE MIN FLANGE SHELL THICKNESS(T)' AS HEADING FROM DUAL UNION ALL SELECT 'RAISED FACE OD(D)' AS HEADING FROM DUAL UNION ALL SELECT 'RAISED FACE HEIGHT (RH)' AS HEADING FROM DUAL UNION ALL SELECT 'PCD' AS HEADING FROM DUAL UNION ALL SELECT 'NO. OF TAPPED HOLE & DIA' AS HEADING FROM DUAL UNION ALL SELECT 'SIZE OF TAPPED HOLE' AS HEADING FROM DUAL UNION ALL SELECT 'NO. OF THRU HOLES' AS HEADING FROM DUAL UNION ALL SELECT 'SIZE OF THRU HOLES' AS HEADING FROM DUAL UNION ALL SELECT 'TOTAL NO. OF HOLES' AS HEADING FROM DUAL UNION ALL SELECT 'FACING' AS HEADING FROM DUAL UNION ALL SELECT 'FINISH' AS HEADING FROM DUAL UNION ALL SELECT '-' AS HEADING FROM DUAL UNION ALL SELECT 'REMARKS' AS HEADING FROM DUAL";
        ...
        GridHeading.Add("ITEM CODE/CLIENT TAG NO.");
        GridHeading.Add("OUTER DIA Ø A");
        GridHeading.Add("FACE TO FACE F/F");
        GridHeading.Add("INNER DIA Ø ID");
        GridHeading.Add("VALVE MIN FLANGE SHELL THICKNESS(T)");
        GridHeading.Add("RAISED FACE OD(D)");
        GridHeading.Add("RAISED FACE HEIGHT (RH)");
        GridHeading.Add("PCD");
        GridHeading.Add("NO. OF TAPPED HOLE & DIA");
        GridHeading.Add("SIZE OF TAPPED HOLE");
        GridHeading.Add("NO. OF THRU HOLES");
        GridHeading.Add("SIZE OF THRU HOLES");
        GridHeading.Add("TOTAL NO. OF HOLES");
        GridHeading.Add("FACING");
        GridHeading.Add("FINISH");
        GridHeading.Add("-");
