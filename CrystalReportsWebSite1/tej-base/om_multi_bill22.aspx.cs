...
public partial class om_multi_bill22 : System.Web.UI.Page
{
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT ACODE AS PARTY_CODE,DRAMT AS DEBIT_AMOUNT,CRAMT AS CREDIT_AMOUNT,INVNO,TO_CHAR(INVDATE,'dd/MM/yyyy')as Invoice_date,to_char(ENT_DATE,'dd/MM/yyyy')as Entry_date, ENT_BY  FROM " + frm_tabname + " where VCHDATE " + PrdRange + " and branchcd='" + frm_mbr + "' and type='30' ORDER BY VCHDATE,vchnum";
            ...
            else
            {
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        fgen.save_data(frm_cocd, frm_qstr, oDS2, "famstbal");
                        //save_fun2();
    ...
    void save_fun()
    {
        ...
        if (dtW != null)
        {
            ...
            foreach (DataRow gr1 in dtW.Rows)
            {
                ...
                string chkname = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(branchcd)||trim(acode)||trim(invno)||trim(invdate) as fstr from recebal where trim(branchcd)||trim(acode)||trim(invno)||trim(invdate) ='" + gr1["fstr2"].ToString().Trim() + "'", "fstr");
               ...
               // if (chkname=="0")
                ...
                //else
                //{
                //    SQuery = "update " + frm_tabname + " set type='30',vchnum='" + txtvchnum.Text + "', vchdate=to_date('" + Convert.ToDateTime(txtvchdate.Text).ToString("dd/MM/yyyy") + "','dd/MM/yyyy'),";
                //    SQuery += "dramt='" + gr1["debitamt"].ToString().Trim() + "', cramt='" + gr1["creditamt"].ToString().Trim() + "', invno='" + gr1["invno"].ToString().Trim() + "',";
                //    SQuery += "invdate=to_date('" + Convert.ToDateTime(gr1["invdate"].ToString().Trim()).ToString("dd/MM/yyyy") + "','dd/MM/yyyy'),naration='Opening Balance', fctype='" + gr1["fctype"].ToString().Trim() + "',";
                //    SQuery += "fcrate='0',refnum='-', fcdramt='0',";
                //    SQuery += "fccramt='0',vchid='0', oscl='0',ent_by='" + frm_uname + "',ent_date=to_date('" + Convert.ToDateTime(txtvchdate.Text).ToString("dd/MM/yyyy") + "','dd/MM/yyyy')";
                //    SQuery += "where acode='" + gr1["partycode"].ToString().Trim() + "' and branchcd='00'";
        ...
        if (FileUpload1.HasFile)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    drn["invdate"] = dr[4].ToString().Trim();
                    drn["debitamt"] = dr[5].ToString().Trim();
                    drn["creditamt"] = dr[6].ToString().Trim();
                    drn["fctype"] = dr[7].ToString().Trim();
                    drn["NetAmount"] = Convert.ToDouble(dr[5].ToString().Trim()) - Convert.ToDouble(dr[6].ToString().Trim());
                    drn["dtsrno"] = count1;
  ...
  protected void btnvalidate_ServerClick(object sender, EventArgs e)
{
    ...
    dtn.Columns.Add("fstr2", typeof(string));
    for (int K = 0; K < dtn.Rows.Count;K++)
    {
        ...
        dtn.Rows[K]["fstr2"] = dtn.Rows[K]["PLANTCODE"].ToString().Trim() + dtn.Rows[K]["PARTYCODE"].ToString().Trim() + dtn.Rows[K]["INVNO"].ToString().Trim() + dtn.Rows[K]["INVDATE"].ToString().Trim();
    ...
    for (int i1 = 0; i1 < dtn.Rows.Count; i1++)
    {
        ...
        if (chkname3 != "0")
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", This party name " + chkname1 + "  already exist. Please update it ");
            flag = 1;
            app += "The opening balance for this partycode " + dtn.Rows[i1]["partycode"].ToString().Trim() + "already exists in database.,";
