...
public partial class om_ccm_act : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "CCM Action Taken";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "ASGN":
                ...
                break;
            case "ACTPOP":
                SQuery = "SELECT Name as FSTR ,Name AS Complaint_type,type1 AS Code FROM  typegrp where branchcd!='DD' and id='E2' order by Name";
                break;
            case "DEPPOP":
                SQuery = "SELECT Name as FSTR ,Name AS Department,type1 AS Code FROM  type where id='M' and type1 like '6%' order by Name";
                break;
    ...
    void newCase(string vty)
    {
        ...
        SQuery = "Select a.* from WB_CCM_LOG a where a.branchcd||a.type||trim(a.CCMNO)||to_Char(a.CCMDT,'dd/mm/yyyy')='" + frm_mbr + "CC" + col1 + "' ORDER BY A.SRNO";
        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        if (dt.Rows.Count > 0)
        {
            i = 0;
            txtvchnum.Value = dt.Rows[0]["CCMNO"].ToString().Trim();
            txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["cCMDT"].ToString().Trim()).ToString("dd/MM/yyyy");
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtvchnum.Value = dt.Rows[i]["CCMNO"].ToString().Trim();
                        txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["CCMDT"].ToString().Trim()).ToString("dd/MM/yyyy");


                        txtActn.Value = dt.Rows[i]["Curr_stat"].ToString().Trim();
                        txtAssgn.Value = dt.Rows[i]["Input_from"].ToString().Trim();
                        txtdeptt.Value = dt.Rows[i]["Act_mode"].ToString().Trim();
    ...
    void save_fun()
    {
        ...
        oporow["Cust_NAME"] = txtlbl4.Value.ToUpper().Trim();
        oporow["comp_type"] = ddModule.Value;
        oporow["Cdescr"] = txtlbl8.Value.ToUpper().Trim();

        oporow["Compcatg"] = txtlbl9.Value;
        oporow["compOccr"] = ddIssueType.Value;
        ...
        oporow["CCMNO"] = txtvchnum.Value.Trim();
        oporow["CCMDT"] = txtvchdate.Value.Trim();

        oporow["Curr_stat"] = txtActn.Value.Trim();
        oporow["Input_from"] = txtAssgn.Value;
        oporow["Act_mode"] = txtdeptt.Value;
        oporow["Next_Folo"] = ddnextfolo.Value;

        oporow["ORemarks"] = txtrmk.Text;
        ...
        fgen.execute_cmd(frm_qstr, frm_cocd, "update wb_CCM_LOG set CURR_STAT='" + txtActn.Value.Trim() + "',last_Action='" + txtdeptt.Value.Trim() + "',last_Actdt='" + txtDFdt.Value.Trim() + "' where branchcd||type||trim(CCMNO)||to_char(CCMDT,'dd/mm/yyyy')='" + frm_mbr + "CC" + txtvchnum.Value + txtvchdate.Value + "'");
    ...
    protected void btnActn_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "ACTPOP";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Action Status", frm_qstr);
    }

    protected void btn_deptt_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "DEPPOP";
        make_qry_4_popup();
