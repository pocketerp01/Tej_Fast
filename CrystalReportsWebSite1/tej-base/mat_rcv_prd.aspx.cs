...
public partial class mat_rcv_prd : System.Web.UI.Page
{
    ...
    string save_it;
    string Prg_Id;
    string pk_error = "Y", chk_rights = "N", DateRange, PrdRange, gate_link = "";
    ...
    public void set_Val()
    {
        ...
        if (frm_ulvl == "0")
        {
            txtvchnum.ReadOnly = false;
            txtlbl5.ReadOnly = false;
            txtlbl6.ReadOnly = false;
        }
    }
    //------------------------------------------------------------------------------------
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SFROM":
                ...
                break;
            case "STO":
                SQuery = "Select type1 as fstr,name as stage,type1 as code,'WIP' as stg from type where id='1' and type1 not in ('6G','6H','6R') order by type1";
                break;
            case "BTN_19":
            ...
            case "TACODE":
                ...
                if (gate_link == "Y")
                {
                    ...
                    SQuery = "SELECT DISTINCT A.vchnum||to_char(A.vchdate,'dd/mm/yyyy')||trim(a.Acode) as Fstr,A.VCHNUM AS GE_Number,to_Char(a.VCHDATE,'dd/mm/yyyy') as Ge_Date,B.Aname as Supplier,a.Invno as Inv_no,A.Refnum as Chl_no,a.Acode,b.Staten,a.prnum,to_Char(A.Invdate,'dd/mm/yyyy') as Inv_Dt,to_Char(a.vchdate,'yyyymmdd') as GE_Dt FROM IVOUCHERP a, famst b  WHERE TRIM(A.ACODE)=TRIM(B.ACODE)  and a.BRANCHCD='" + frm_mbr + "' AND a.VCHDATE  " + DateRange + " AND a.TYPE='00' and a.prnum='" + prcond + "' AND (a.VCHNUM||to_char(a.vchdate,'yyyymm')) IN (SELECT VCHNUM FROM (SELECT X.VCHNUM,SUM(X.aBC) AS CNT FROM (select distinct a.vchnum||to_char(a.vchdate,'yyyymm') as vchnum,a.type,1 AS ABC from ivoucherp a  where branchcd='" + frm_mbr + "' and a.VCHDATE   " + DateRange + " AND a.type='00' and a.vchnum<>'000000' UNION ALL select distinct a.GENUM||to_char(a.gedate,'yyyymm') as genum,a.type,1 AS ABC from ivoucher a where branchcd='" + frm_mbr + "' and substr(a.type,1,1)='0' and a.VCHDATE  " + DateRange + " AND a.vchnum<>'000000' ) X GROUP BY X.VCHNUM) WHERE CNT=1) order by to_Char(a.vchdate,'yyyymmdd') desc,A.VCHNUM desc ";
                }
                else
                {
                    SQuery = "SELECT distinct a.ACODE AS FSTR,b.ANAME AS PARTY,a.ACODE AS CODE,b.ADDR1,b.ADDR2,b.staten as state,Pay_num FROM somas a, FAMST b where a.branchcd='" + frm_mbr + "' and trim(nvl(a.ICAT,'-'))!='Y'  and trim(nvl(a.app_by,'-'))!='-' and trim(A.acode)=trim(B.acode) and length(Trim(nvl(b.deac_by,'-')))<=1 ORDER BY aname ";
                }
    ...
    protected void btnlbl70B_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "SFROM";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Stage ", frm_qstr);
    }
    protected void btnlbl71D_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "STO";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Stage ", frm_qstr);
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtlbl2.Text != "-")
        {
            if (edmode.Value == "Y")
            {
                chk_dupl = fgen.seek_iname(frm_qstr, frm_cocd, "select vchnum||'-'||to_char(vchdate,'dd/mm/yyyy') as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type like '0%' and vchdate " + DateRange + " and vchnum||to_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text + txtvchdate.Text + "' and trim(upper(acode))='" + txtlbl4.Text + "' and trim(upper(genum))='" + txtlbl2.Text + "'", "ldt");
            }
            else
            {
        ...
        if (txtlbl5.Text != "-")
        {
            if (edmode.Value == "Y")
            {
                chk_dupl = fgen.seek_iname(frm_qstr, frm_cocd, "select vchnum||'-'||to_char(vchdate,'dd/mm/yyyy') as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type like '0%' and vchdate " + DateRange + " and vchnum||to_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text + txtvchdate.Text + "' and trim(upper(acode))='" + txtlbl4.Text + "' and trim(upper(invno))='" + txtlbl5.Text + "'", "ldt");
            }
            else
            {
        ...
        if (edmode.Value == "Y")
        {
            ...
            chk_dupl = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(nvl(pname,'-')) as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and vchnum||to_char(vchdate,'dd/mm/yyyy')='" + txtvchnum.Text + txtvchdate.Text + "' and trim(upper(acode))='" + txtlbl4.Text + "'", "ldt");
            ...
            //chk_dupl = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(nvl(finvno,'-')) as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and vchnum||to_char(vchdate,'dd/mm/yyyy')='" + txtvchnum.Text + txtvchdate.Text + "' and trim(upper(acode))='" + txtlbl4.Text + "'", "ldt");
    ...
    void splitBatch()
    {
        ...
        double BatchNo = 0;
        frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
        BatchNo = fgen.make_double(frm_CDT1.Substring(8, 2) + frm_vty + fgen.next_no(frm_qstr, frm_cocd, "Select max(substr(kclreelno,5,10)) as vch from reelvch where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and vchdate " + DateRange + "", 6, "vch"));
        ...
        else
        {
            ...
            switch (btnval)
            {
                case "New":
                    ...
                    if (frm_cocd == "HIME")
                        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE like '" + frm_vty + "%' AND " + doc_df.Value + " " + DateRange + " ", 6, "VCH");
                    txtvchnum.Text = frm_vnum;
                    txtvchdate.Text = fgen.Fn_curr_dt(frm_cocd, frm_qstr);
                ...
                case "Print":
                    ...
                    else fgen.Fn_open_mseek("Select Entry No to Print", frm_qstr);
                    break;
                case "Edit_E":
                    //edit_Click
                    #region Edit Start
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl3.Text = Convert.ToDateTime(dt.Rows[0]["gedate"].ToString().Trim()).ToString("dd/MM/yyyy");
                        ...
                        txtlbl24.Text = dt.Rows[i]["cavity"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            if (frm_cocd == "HIME")
                            {
                                if (dt.Rows[i]["Icode"].ToString().Trim().Substring(0, 2) == "10")
                                {
                                    if (dt.Rows[i]["inspected"].ToString().Trim() != "Y") showItem = "Y";
                                    else showItem = "N";
                                }
                            }
                            if (showItem == "Y")
                            {
                                ...
                                sg1_dr["sg1_h1"] = "-";
                                sg1_dr["sg1_h2"] = "-";
                                sg1_dr["sg1_h3"] = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT PACKSIZE FROM ITEM WHERE TRIM(ICODe)='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "PACKSIZE");
                                sg1_dr["sg1_h4"] = "-";

                                sg1_dr["sg1_h5"] = dt.Rows[i]["INSPECTED"].ToString().Trim();
                                sg1_dr["sg1_h6"] = dt.Rows[i]["STORE"].ToString().Trim();
                                ...
                                sg1_dr["sg1_t22"] = dt.Rows[i]["revis_no"].ToString().Trim();
                                //sg1_dr["sg1_t23"] = dt.Rows[i]["stage"].ToString().Trim();
                                sg1_dr["sg1_t23"] = dt.Rows[i]["stage"].ToString().Trim() + "(" + fgen.seek_iname(frm_qstr, frm_cocd, "select name from type where type1='" + dt.Rows[i]["stage"].ToString().Trim() + "' and id='1'", "name") + ")";
                                sg1_dr["sg1_t24"] = dt.Rows[i]["iopr"].ToString().Trim() + "(" + fgen.seek_iname(frm_qstr, frm_cocd, "select name from type where type1='" + dt.Rows[i]["iopr"].ToString().Trim() + "' and id='1'", "name") + ")";
                ...
                case "SAVED":
                    ...
                    if (frm_cocd == "SVPL") hffield.Value = "SPrint_E";
                ...
                case "SPrint_E":
                    if (col1.Length < 2) return;
                    fgenMV.Fn_Set_Mvar(frm_qstr, "REPID", "S1002");
                    fgen.fin_invn_reps(frm_qstr);
                    break;
                case "TACODE":
                    //-----------------------------
                    ...
                    SQuery = "Select b.iname,nvl(b.cpartno,'-') as icpartno,nvl(b.cdrgno,'-') as icdrgno,nvl(b.unit,'-') as iunit,nvl(b.hscode,'-') as hscode,a.* from ivoucherp a,item b where trim(a.icode)=trim(B.icode) and a.branchcd='" + frm_mbr + "' and a.type like '00%' and a.vchnum||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode)='" + col1 + "' ORDER BY A.srno";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t14"] = dt.Rows[i]["ponum"].ToString().Trim();
                            sg1_dr["sg1_t15"] = dt.Rows[i]["ordlineno"].ToString().Trim();
                            try { sg1_dr["sg1_t16"] = Convert.ToDateTime(dt.Rows[i]["podate"].ToString().Trim()).ToString("dd/MM/yyyy"); }
                            catch { sg1_dr["sg1_t16"] = Convert.ToDateTime(dt.Rows[0]["podate"].ToString().Trim()).ToString("dd/MM/yyyy"); }
                            col3 = dt.Rows[i]["ponum"].ToString().Trim() + Convert.ToDateTime(dt.Rows[0]["podate"].ToString().Trim()).ToString("dd/MM/yyyy") + dt.Rows[i]["icode"].ToString().Trim();
                            dt4 = new DataTable();
                            dt4 = fgen.getdata(frm_qstr, frm_cocd, "SELECT PCESS,PEXC FROM POMAS WHERE BRANCHCD='" + frm_mbr + "' AND type like '5%' and trim(ordno)||to_char(orddt,'dd/mm/yyyy')||trim(icode) ='" + col3 + "'");
                            if (dt4.Rows.Count > 0)
                            {
                                if (dt4.Rows[0]["pexc"].ToString().Trim() != sg1_dr["sg1_t7"].ToString().Trim())
                                {
                                    sg1_dr["sg1_t7"] = dt4.Rows[0]["pexc"].ToString().Trim();
                                    sg1_dr["sg1_t8"] = dt4.Rows[0]["pcess"].ToString().Trim();
                                }
                            }
                            ...
                            sg1_dr["sg1_t22"] = dt.Rows[i]["revis_no"].ToString().Trim();
                            //sg1_dr["sg1_t23"] = dt.Rows[i]["STAGE"].ToString().Trim();
                            sg1_dr["sg1_t23"] = dt.Rows[i]["stagef"].ToString().Trim() + "(" + fgen.seek_iname(frm_qstr, frm_cocd, "select name from type where type1='" + dt.Rows[i]["stagef"].ToString().Trim() + "' and id='1'", "name") + ")";
                            sg1_dr["sg1_t24"] = dt.Rows[i]["iopr"].ToString().Trim() + "(" + fgen.seek_iname(frm_qstr, frm_cocd, "select name from type where type1='" + dt.Rows[i]["iopr"].ToString().Trim() + "' and id='1'", "name") + ")";
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            col3 += fgen.make_def_Date(mpo_Dt, vardate) + dt.Rows[d]["icode"].ToString().Trim();

                            dt4 = fgen.getdata(frm_qstr, frm_cocd, "SELECT PCESS,PEXC FROM POAMS WHERE BRANCHCD='" + frm_mbr + "' AND type like '5%' and trim(ordno)||to_char(orddt,'dd/mm/yyyy')||trim(icode) ='" + col3 + "'");
                            if (dt4.Rows.Count > 0)
                            {
                                if (dt4.Rows[0]["pexc"].ToString().Trim() != sg1_dr["sg1_t7"].ToString().Trim())
                                {
                                    sg1_dr["sg1_t7"] = dt.Rows[d]["pexc"].ToString().Trim();
                                    sg1_dr["sg1_t8"] = dt.Rows[d]["pcess"].ToString().Trim();
                                }
                            }
                ...
                case "SG2_ROW_ADD":
                    ...
                    if (ViewState["sg2"] != null)
                    {
...

                        {
                            ...
                            sg2_dr["sg2_t1"] = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(kclreelno) AS VCH FROM REELVCH WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' AND VCHDATE " + DateRange + " ", 6, "VCH");
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = fgen.makeRepQuery(frm_qstr, frm_cocd, "F25126", "branchcd='" + frm_mbr + "'", "a.type like '0%' and a.inspected='Y' ", PrdRange);
            if (frm_cocd == "HIME" && frm_uname == "ASHEESH")
                SQuery = "Select to_char(a.vchdate,'dd/mm/yyyy') as Doc_Dt,a.vchnum as Doc_no,a.acode as Party_Code,b.aname as Party_name,a.icode as Item_Code,c.iname as Item_name,c.unit as UOM,a.iqtyin as Qty_rcv,a.irate as Rate,a.iamount as Basic_Val,a.genum as Gate_Entry_No,to_char(a.gedate,'dd/mm/yyyy') as Gate_Entry_dt,a.invno as Inv_no,to_char(a.invdate,'dd/mm/yyyy') as Inv_Dt,a.potype,a.ponum,a.podate as podt,a.Desc_ as Remarks,c.hscode,c.hscode2,a.Ent_by as Entry_by,to_char(a.ent_dt,'dd/mm/yyyy') as Entry_dt,a.Edt_by as Edit_by,(case when nvl(a.edt_by,'-')!='-' then to_char(a.edt_dt,'dd/mm/yyyy') else '-' end) as Edit_dt,to_char(a.vchdate,'yyyymmdd') as VDD from ivoucher a , famst b, item c where a.branchcd='" + frm_mbr + "' and a.type like '0%' and a.inspected='Y' and a.vchdate  " + PrdRange + " and trim(A.acode)=trim(B.acode) and trim(A.icode)=trim(C.icode) and a.store not in ('R','W')  order by vdd,a.vchnum,a.morder";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                col3 = "and type='" + frm_vty + "'";
                                if (frm_cocd == "HIME") col3 = "and type like '" + frm_vty + "%'";

                                frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + i + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' " + col3 + " and " + doc_df.Value + " " + DateRange + "", 6, "vch");
                                ...
                                if (i > 20)
                                {
                                    ...
                                    frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + 1 + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' " + col3 + " and " + doc_df.Value + " " + DateRange + "", 6, "vch");
                                    pk_error = "N";
                                    i = 0;
                                }
                                i++;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["invdate"] = txtlbl6.Text.Trim();
                oporow["refnum"] = txtlbl8.Text.Trim();
                oporow["refdate"] = txtlbl9.Text.Trim();
                ...
                if (((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text.Trim() == "-")
                {
                    ...
                    oporow["btchdt"] = txtlbl6.Text.Trim();
                }
                ...
                if (po_dtls.Trim().Length > 6)
                {
                    ...
                    oporow["rtn_Date"] = po_dtls.Split('@')[2].ToString();
                }
                else
                {
                    oporow["potype"] = "-";
                ...
                oporow["revis_no"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t22")).Text.Trim();
                try
                {
                    oporow["stage"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t23")).Text.ToString().Split('(')[0].Trim());
                    oporow["IOPR"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t24")).Text.ToString().Split('(')[0].Trim());
                }
                catch (Exception EE)
                {
                }
                ...
                if (sg1.Rows[i].Cells[13].Text.Trim().Substring(0, 2) != "10")
                {
                    oporow["store"] = "W";
                    oporow["inspected"] = "Y";
                }
                ...
                {
                    if (sg1.Rows[i].Cells[4].Text.Trim() == "Y")
                    {
                        oporow["store"] = "Y";
                        oporow["inspected"] = "Y";
                    }
    ...
    void save_fun2()
    {
        ...
        oporow2["invdate"] = txtlbl6.Text.Trim();
    ...
    void Type_Sel_query()
    {
        ...
        SQuery = "SELECT type1 AS FSTR,NAME,type1 AS CODE FROM type where id='M' and type1 like '0%' AND TYPE1!='0W' order by type1";
    }
    //------------------------------------------------------------------------------------
    void setGST()
    {
    ...
    protected void btnRead_ServerClick(object sender, EventArgs e)
    {
        ...
        if (frm_cocd == "BONY" || frm_cocd == "SRPF")
        {
            ...
            dt2 = new DataTable();
            dt2 = fgen.getdata(frm_qstr, frm_cocd, "Select a.*,b.iname,b.cpartno,b.unit,b.cdrgno from finprim.scratch a,finprim.item b where trim(A.icode)=trim(B.icode) and a.branchcd||a.type||a.vchnum||to_Char(a.Vchdate,'dd/mm/yyyy')='" + txtBarCode.Value.Trim().Substring(0, 20) + "'");
            if (dt2.Rows.Count > 0)
            {
                #region for gridview 1
                if (ViewState["sg1"] != null)
    ...
    protected void btnCal_Click(object sender, EventArgs e)
    {
        ...
        {
            hf1.Value = "XXX";
            ((TextBox)gr.FindControl("sg1_t6")).Text = Math.Round(fgen.make_double(((TextBox)gr.FindControl("sg1_t2")).Text.ToString()) * fgen.make_double(((TextBox)gr.FindControl("sg1_t5")).Text.ToString()), 2).ToString();
        }
    }
}