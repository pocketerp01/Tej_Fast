...
public partial class om_jour_vch : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                switch (Prg_Id)
                {
                    case "F70122C":
                        if (frm_cocd == "SGRP")
                        {
                            SQuery = "Select Acode as fstr,Aname,Acode,Addr1,Addr2 from famst where substr(grp,1,2) in ('02','06','16') order by Aname";
                        }
                        else
                        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                switch (Prg_Id)
                {
                    case "F70122C":
                        if (frm_cocd == "SGRP")
                        {
                            SQuery = "select to_char(invdate,'yyyymmdd')||trim(acode)||trim(invno) As fstr,trim(invno) as Inv_no,to_char(invdate,'dd/mm/yyyy') as Inv_Date,sum(dramt) as dramt,sum(cramt) as cramt,sum(dramt)-sum(cramt) as Net_Amt,to_char(invdate,'yyyymmdd') as Invd,trim(acode) As Acc_code from (select acode,invno,invdate,dramt,cramt from recebal where branchcd='" + frm_mbr + "' and acode='" + txtlbl4.Text.Trim() + "' union all select acode,invno,invdate,dramt,cramt from voucher where branchcd='" + frm_mbr + "' and type like '%' and vchdate>=to_Date('01/01/2021','dd/mm/yyyy') and acode='" + txtlbl4.Text.Trim() + "') where " + col1 + " group by trim(acode),trim(invno),to_char(invdate,'dd/mm/yyyy'),to_char(invdate,'yyyymmdd')||trim(acode)||trim(invno),to_char(invdate,'yyyymmdd')  having sum(dramt)-sum(cramt) <>0 order by to_char(invdate,'yyyymmdd'),trim(invno) ";
                        }
                        else
                        {
                            SQuery = "select to_char(invdate,'yyyymmdd')||trim(acode)||trim(invno) As fstr,trim(invno) as Inv_no,to_char(invdate,'dd/mm/yyyy') as Inv_Date,sum(dramt) as dramt,sum(cramt) as cramt,sum(dramt)-sum(cramt) as Net_Amt,to_char(invdate,'yyyymmdd') as Invd,trim(acode) As Acc_code from (select acode,invno,invdate,dramt,cramt from recebal where branchcd='" + frm_mbr + "' and acode='" + txtlbl4.Text.Trim() + "' union all select acode,invno,invdate,dramt,cramt from voucher where branchcd='" + frm_mbr + "' and acode='" + txtlbl4.Text.Trim() + "') where " + col1 + " group by trim(acode),trim(invno),to_char(invdate,'dd/mm/yyyy'),to_char(invdate,'yyyymmdd')||trim(acode)||trim(invno),to_char(invdate,'yyyymmdd')  having sum(dramt)-sum(cramt) <>0 order by to_char(invdate,'yyyymmdd'),trim(invno) ";
                        }

                        break;
                    default:
                        //SQuery = "select a.Acode as fstr,a.ANAME as Account_Name,a.Acode as Ac_Code,a.Addr1 as Address,a.Addr2 as City,a.grp,b.Name,a.Payment from famst a left outer join ( select name,type1 from type where id='Z') b on trim(a.grp)=trim(b.type1) where a.branchcd='00'  order by Account_Name ";

                        SQuery = "select A.acode as fstr,replacE(ANAME,'''','`') as Account_Name,A.Addr1 as Address_l1,a.addr2 as Address_l2,a.acode as ERP_Acode,a.Grp,b.Name,a.deac_by,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as entry_Dt,a.Edt_by,a.edt_Dt,a.ent_Dt from famst a left outer join (Select type1,name from type where id='Z') b on trim(a.grp)=trim(B.type1) where a.branchcd!='DD' and length(Trim(nvl(a.deac_by,'-')))<=1  union all select A.acode as fstr,replacE(a.ANAME,'''','`') as Account_Name,A.Addr1 as Address_l1,a.addr2 as Address_l2,a.acode as ERP_Acode,a.Grp,b.Name,a.deac_by,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as entry_Dt,a.Edt_by,a.edt_Dt,a.ent_Dt from famst a ,(Select type1,name from type where id='Z') b,(Select trim(acode) as acode,sum(Dramt)-sum(Cramt) as tot from recdata where branchcd='" + frm_mbr + "' group by trim(AcodE) having sum(Dramt)-sum(Cramt)<>0)  c where trim(a.grp)=trim(B.type1)and trim(a.acode)=trim(c.acode) and length(Trim(nvl(a.deac_by,'-')))>1 ";
                        break;
                }
            ...
            default:
                if (btnval == "Edit_E" || btnval == "COPY_OLD")
                ...
                else if (btnval == "Del_E")
                    SQuery = "select distinct type||trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as JV_no,to_char(a.vchdate,'dd/mm/yyyy') as Vch_Dt,b.aname as Account,a.ent_by,to_char(a.ent_Date,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where trim(A.acode)=trim(B.acode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + DateRange + " order by vdd desc,a.vchnum desc";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (frm_cocd == "SGRP" && lbl1a.Text.Substring(0, 1) != "3" && lbl1a.Text.Substring(0, 2) != "58" && lbl1a.Text.Substring(0, 2) != "59")
        {
            string chk_opt = "";
            chk_opt = fgen.seek_iname(frm_qstr, frm_cocd, "select (Case when to_datE('" + txtvchdate.Text + "','dd/mm/yyyy')>=to_DatE('01/04/2021','dd/mm/yyyy') then 'NO' else 'OK' end) as res from dual ", "res");
            if (chk_opt == "NO")
            {
                fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Non JV Voucher after 1/4/2021 Cannot be Saved from This form !!");
                return;
            }
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            ...
            if (Prg_Id == "F70122C")
            {
                ...
                if (sg1.Rows[i].Cells[13].Text.Trim().Length > 2 && ((fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text) + fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text)) > fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text) + 2))
                {
                    Checked_ok = "N";
                    fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Amount Cannot Exceed Orignal Amount , See Line " + (i + 1) + "  !!'13'Amount Entered : " + (fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text) + fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text)) + "'13'Pending Amount Against the Bill : " + ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text);
                    i = sg1.Rows.Count;
                    return;
                }
            }
            ...
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Amount Not Filled Correctly at Line " + (i + 1) + "  !!");
                i = sg1.Rows.Count;
                return;
            ...
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 2 && sg1.Rows[i].Cells[17].Text.Trim().Length < 6)
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Reference A/C Code Not Filled Correctly at Line " + (i + 1) + "  !!");
                i = sg1.Rows.Count;
                return;
            }
        ...
        double totDrAmt = 0, totCrAmt = 0;
        foreach (GridViewRow gr in sg1.Rows)
        {
            totDrAmt += ((TextBox)gr.FindControl("sg1_t1")).Text.toDouble();
            totCrAmt += ((TextBox)gr.FindControl("sg1_t2")).Text.toDouble();
        }
        totCrAmt = totCrAmt.toDouble(2);
        totDrAmt = totDrAmt.toDouble(2);
        if (totDrAmt != totCrAmt)
        {
            fgen.msg("", "AMSG", "Total Debit and Credit amount is not matching, Please check again before saving'13' Dr Amount : " + totDrAmt + "'13' Cr Amount : " + totCrAmt + " !!");
            return;
        }
        int xx = 0;
        foreach (GridViewRow gr in sg1.Rows)
        {
            if (gr.Cells[17].Text.Trim().ToUpper().Replace("&NBSP;", "").Length < 6)
            {
                xx = gr.RowIndex;
            }
            else
            {
                col2 = gr.Cells[13].Text.Trim().ToUpper().Replace("&NBSP;", "");
                col3 = gr.Cells[14].Text.Trim().ToUpper().Replace("&NBSP;", "");
                if (sg1.Rows[xx].Cells[17].Text.ToUpper().Replace("&NBSP;", "").Length < 6)
                {
                    sg1.Rows[xx].Cells[17].Text = col2;
                    sg1.Rows[xx].Cells[18].Text = col3;
                }
            }
        }
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " a where a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
                // Deleing data from Sr Ctrl Table
                //fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.type)||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.type)||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
                // Saving Deleting History
                //fgen.save_info(frm_qstr, frm_cocd, frm_mbr, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2"), fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3"), frm_uname, frm_vty, lblheader.Text.Trim() + " Deleted");
                fgen.save_info(frm_qstr, frm_cocd, frm_mbr, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2"), fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3"), frm_uname, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1").Trim().Substring(0, 2), lblheader.Text.Trim() + " Deleted");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    //edit_Click
                    if (col1 == "") return;
                    editFunction(frm_mbr + lbl1a.Text + col1);
                    break;
                case "SAVED":
                    hffield.Value = "Print_E";
                    if (Request.Cookies["REPLY"].Value.ToString().Trim() == "Y")
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count - 1; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());
                            sg1_dr["sg1_t2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim());
                        ...
                        switch (Prg_Id)
                        {
                            case "F70122C":
                                ...
                                break;
                            default:
                                SQuery = "select a.*,a.account_name as aname,a.fstr as acode from (" + pop_qry + ") a where trim(a.fstr) in (" + col1 + ") order by a.account_name";
                                break;
                        }
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            if (1 == 1)
                            {
                                ...
                                sg1_dr["sg1_t2"] = "0";
                                sg1_dr["sg1_t3"] = "";
                                sg1_dr["sg1_t4"] = DateTime.Now.ToString("dd/MM/yyyy");
                                sg1_dr["sg1_t5"] = "";

                                switch (Prg_Id)
                                {
                                    case "F70122C":
                                        sg1_dr["sg1_f4"] = dt.Rows[d]["acode"].ToString().Trim();
                                        sg1_dr["sg1_f5"] = dt.Rows[d]["aname"].ToString().Trim();

                                        if (fgen.make_double(dt.Rows[d]["net_amt"].ToString().Trim()) > 0)
                                        {
                                            sg1_dr["sg1_t1"] = 0;
                                            sg1_dr["sg1_t2"] = fgen.make_double(dt.Rows[d]["net_amt"].ToString().Trim());
                                            sg1_dr["sg1_t5"] = fgen.make_double(dt.Rows[d]["net_amt"].ToString().Trim());
                                        }
                                        else
                                        {
                                            sg1_dr["sg1_t1"] = fgen.make_double(dt.Rows[d]["net_amt"].ToString().Trim()) * -1;
                                            sg1_dr["sg1_t2"] = 0;
                                            sg1_dr["sg1_t5"] = fgen.make_double(dt.Rows[d]["net_amt"].ToString().Trim()) * -1;
                                        }


                                        sg1_dr["sg1_t3"] = dt.Rows[d]["inv_no"].ToString().Trim();
                                        sg1_dr["sg1_t4"] = dt.Rows[d]["inv_date"].ToString().Trim();

                                        break;
                                    default:
                                        sg1_dr["sg1_f4"] = last_acd;
                                        sg1_dr["sg1_f5"] = last_anm;
                                        break;
                                }
                ...
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (oDS.Tables[0].Rows.Count > 0)
                        {
                            ...
                            {
                                myCheckDr += drCheck["DRAMT"].ToString().toDouble();
                                myCheckCr += drCheck["CRAMT"].ToString().toDouble();
                            }
                        }
                        myCheckDr = myCheckDr.toDouble(2);
                        myCheckCr = myCheckCr.toDouble(2);
                        if (myCheckCr != myCheckDr)
                        {
                            btnsave.Disabled = false;
                        ...
                        else
                        {
                            ...
                            if (save_it == "Y")
                            {
                                ...
                                string doc_is_ok = "";
                                frm_vnum = fgen.Fn_next_doc_no(frm_qstr, frm_cocd, frm_tabname, doc_nf.Value, doc_df.Value, frm_mbr, lbl1a.Text, txtvchdate.Text.Trim(), frm_uname, Prg_Id);
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            cmd_query = "update " + frm_tabname + " set branchcd='DD' where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + lbl1a.Text + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'";
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            cmd_query = "delete from " + frm_tabname + " where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='DD" + lbl1a.Text + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'";
                            fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                        ...
                        fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL1", "'" + frm_mbr + lbl1a.Text + frm_vnum + txtvchdate.Text.Trim() + "'");
    ...
    public void create_tab()
    {
        ...
        sg1_dt.Columns.Add(new DataColumn("sg1_t1", typeof(double)));
        sg1_dt.Columns.Add(new DataColumn("sg1_t2", typeof(double)));
    ...
    void save_fun()
    {
        ...
        {
            dr1 = dtLr.NewRow();
            dr1["sg1_f1"] = sg1.Rows[i].Cells[13].Text.ToString().Trim();


            dr1["sg1_t1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
            dr1["sg1_t2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);
        ...
        int largamtPassAmt = (dtLr.Compute("max(sg1_t1)", string.Empty)).ToString().toInt();

        int largamtManualAmt = (dtLr.Compute("max(sg1_t2)", string.Empty)).ToString().toInt();


        DataView dv = new DataView(dtLr, "", "sg1_t1 desc", DataViewRowState.CurrentRows);
        if (dv.Count > 0)
            largest_value_acode = dv[0].Row["sg1_f1"].ToString();

        DataView dv1 = new DataView(dtLr, "", "sg1_t2 desc", DataViewRowState.CurrentRows);
        if (dv1.Count > 0)
            largest_value_rcode = dv1[0].Row["sg1_f1"].ToString();
        ...
        for (i = 0; i <= sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                //{
                //    if (Convert.ToDouble(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim()) > largest_value_amt)
                //    {
                //        largest_value_amt = Convert.ToDouble(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());
                ...
                oporow["dramt"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().toDouble();
                oporow["Cramt"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().toDouble();
                if (((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().toDouble() > 0)
                {
                    oporow["RCODE"] = largest_value_rcode;
                }
                else
                {
                ...
                oporow["tfcdr"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().toDouble();
                oporow["tfccr"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().toDouble();

                oporow["invno"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim()).ToUpper();
                oporow["invdate"] = Convert.ToDateTime(fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text, vardate).ToString()).ToString("dd/MM/yyyy");

                my_nars = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim();
                if (frm_formID == "F70122C")
                {
                    oporow["ADDL_NARR"] = my_nars.Trim().ToUpper();
                ...
                else
                {
                    if (my_nars.Length > 1)
                    {
                        oporow["naration"] = my_nars.Trim().ToUpper();
                    }
                    else
                    {
                        oporow["naration"] = txtRmk.Text.Trim().ToUpper();
                ...
                if (txtlbl4.Text == "001" && lbl1a.Text == "30")
                {
                    oporow["CCENT"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim();
                    oporow["naration"] = my_nars;
                ...
                if (edmode.Value == "Y")
                {
                    ...
                    oporow["ent_date"] = ViewState["entdt"];
    ...
    void Type_Sel_query()
    {
        ...
        switch (Prg_Id)
        {
            ...
            default:
                if (frm_cocd == "SGRP" && fgen.make_double(frm_ulvl) <= 2)
                {
                    SQuery = "SELECT type1 AS FSTR,name as NAME,type1 AS CODE FROM TYPE WHERE ID='V' ORDER BY TYPE1 ";
                }
                else
                {
                    SQuery = "SELECT type1 AS FSTR,name as NAME,type1 AS CODE FROM TYPE WHERE ID='V' AND TYPE1 in ('30','33','37','38') ORDER BY TYPE1 ";
    ...
    void editFunction(string fstr)
    {
        ...
        {
            string chk_opt = "";
            chk_opt = fgen.seek_iname(frm_qstr, frm_cocd, "select (Case when to_datE('" + col1.Trim().Right(10) + "','dd/mm/yyyy')>=to_DatE('01/04/2021','dd/mm/yyyy') then 'NO' else 'OK' end) as res from dual ", "res");
            if (chk_opt == "NO")
            {
                fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Non JV Voucher after 01/04/2021 Cannot be Edited from This form !!");
                return;
            }
        }
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                fgen.msg("-", "AMSG", "Approved Voucher Cannot be Edited, Contact HOD/Admin");
                return;
            }
            ...
            txtlbl6.Text = dt.Rows[i]["edtd_Dt"].ToString().Trim();
            ...
            for (i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sg1_dr["sg1_t5"] = (frm_formID == "F70122C" ? dt.Rows[i]["ADDL_NARR"].ToString().Trim() : dt.Rows[i]["naration"].ToString().Trim());
