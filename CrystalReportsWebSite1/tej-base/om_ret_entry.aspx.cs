...
public partial class om_ret_entry : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                if (doc_addl.Value == "Y")
                {
                    SQuery = "select to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum as Fstr,a.vchnum as Req_no,to_char(a.vchdate,'dd/mm/yyyy') as req_dt,b.Name as Deptt_Name,trim(a.acode) as Dept_Cd,trim(A.stage) as WIP_Stg,sum(a.iqty_chl)-sum(a.issued) as Pending_Qty,max(a.ent_by) As Request_by,round(sysdate-a.vchdate,0) as Pend_Days from (SELECT acode,stage,vchnum,vchdate,icode,ent_by,jobno,jobdt,req_qty as iqty_chl,0 as issued from wb_iss_req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and nvl(closed,'-')!='Y' union all SELECT acode,stage,refnum,refdate,icode,null as entby,invno,invdate,0 as iqty_chl,iqtyin as issued from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,type b where b.id='M' and trim(A.acode)=trim(B.type1) group by to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum,b.Name,trim(a.acode),a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),a.vchdate,trim(A.stage) having sum(a.iqty_chl)-sum(a.issued)>0 order by fstr";
                }
                else
                {
                    SQuery = "select to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum as Fstr,a.vchnum as Req_no,to_char(a.vchdate,'dd/mm/yyyy') as req_dt,b.Name as Deptt_Name,trim(a.acode) as Dept_Cd,trim(A.stage) as WIP_Stg,sum(a.iqty_chl)-sum(a.issued) as Pending_Qty,max(a.ent_by) As Request_by,round(sysdate-a.vchdate,0) as Pend_Days  from (SELECT acode,stage,vchnum,vchdate,icode,ent_by,'-' as jobno,vchdate as jobdt,req_qty as iqty_chl,0 as issued from wb_iss_req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and nvl(closed,'-')!='Y' union all SELECT acode,stage,refnum,refdate,icode,null as entby,'-' as invno,vchdate,0 as iqty_chl,iqtyin as issued from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,type b where b.id='M' and trim(A.acode)=trim(B.type1) group by to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum,b.Name,trim(a.acode),a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),a.vchdate,trim(A.stage) having sum(a.iqty_chl)-sum(a.issued)>0 order by fstr";
                }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        string chk_freeze = "";
        chk_freeze = fgen.Fn_chk_doc_freeze(frm_qstr, frm_cocd, frm_mbr, "W1034", txtvchdate.Text.Trim());
        if (chk_freeze == "1")
        {
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "TACODE":
                    ...
                    if (doc_addl.Value == "Y")
                    {
                        SQuery = "select a.vchnum||to_Char(a.vchdate,'dd/mm/yyyy')||trim(a.icode) as Fstr,b.Iname,b.Cpartno,b.cdrgno,b.unit,sum(a.iqty_chl)-sum(a.issued) as Pending,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,trim(a.icode) as ERP_Code,max(a.ent_by) As Ind_by,max(a.desc_) As desc_,trim(a.jobno) as JobNo,to_char(a.jobdt,'dd/mm/yyyy') as jobdt,trim(a.acode) as acode,trim(a.stage) As wstage from (SELECT acode,stage,vchnum,vchdate,icode,ent_by,jobno,jobdt,req_qty as iqty_chl,0 as issued,desc_ from wb_iss_req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and nvl(closed,'-')!='Y' union all SELECT acode,stage,refnum,refdate,icode,null as entby,invno,invdate,0 as iqty_chl,iqtyin as issued,null as desc_ from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,item b where trim(A.icode)=trim(B.icode) and to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum='" + col1 + "' group by a.vchnum||to_Char(a.vchdate,'dd/mm/yyyy')||trim(a.icode),b.Iname,b.Cpartno,b.cdrgno,b.unit,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),trim(a.icode),trim(a.jobno),to_char(a.jobdt,'dd/mm/yyyy'),trim(a.acode),trim(a.stage)  having sum(a.iqty_chl)-sum(a.issued)>0  order by b.iname";
                    }
                    else
                    {
                        SQuery = "select a.vchnum||to_Char(a.vchdate,'dd/mm/yyyy')||trim(a.icode) as Fstr,b.Iname,b.Cpartno,b.cdrgno,b.unit,sum(a.iqty_chl)-sum(a.issued) as Pending,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,trim(a.icode) as ERP_Code,max(a.ent_by) As Ind_by,max(a.desc_) As desc_,max(a.jobno) as JobNo,to_char(max(a.jobdt),'dd/mm/yyyy') as jobdt,trim(a.acode) as acode,trim(a.stage) As wstage from (SELECT acode,stage,vchnum,vchdate,icode,ent_by,'-' as jobno,vchdate as jobdt,req_qty as iqty_chl,0 as issued,desc_ from wb_iss_req where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + " and nvl(closed,'-')!='Y' union all SELECT acode,stage,refnum,refdate,icode,null as entby,'-' as invno,null as invdate,0 as iqty_chl,iqtyin as issued,null as desc_ from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "' and vchdate " + DateRange + ")a,item b where trim(A.icode)=trim(B.icode) and to_Char(a.vchdate,'yyyymmdd')||trim(a.acode)||a.vchnum='" + col1 + "' group by a.vchnum||to_Char(a.vchdate,'dd/mm/yyyy')||trim(a.icode),b.Iname,b.Cpartno,b.cdrgno,b.unit,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy'),trim(a.icode),trim(a.acode),trim(a.stage)  having sum(a.iqty_chl)-sum(a.issued)>0  order by b.iname";
                    }
    ...
    protected void sg2_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (var)
        {
            ...
            case "SG2_ROW_ADD":
                ...
                SQuery = "SELECT distinct TRIM(A.ICODE) AS FSTR,B.INAME AS PRODUCT,A.ICODE AS ERPCODE,A.KCLREELNO,A.COREELNO,B.OPRATE1,B.OPRATE3,B.UNIT,a.irate FROM REELVCH A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) and" +
                    " trim(a.icode)||trim(a.kclreelno) not in (" + col1 + ") and substr(a.type,1,1)='3' and type <> '36'";
