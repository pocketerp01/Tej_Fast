...
public partial class om_css_act : System.Web.UI.Page
{
    ...
    string frm_tabname, frm_myear, frm_ulvl, frm_formID, frm_UserID, frm_CDT1, filepath, filename;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "CSS Action Reporting";
    ...
    public void set_Val()
    {
        ...
        switch (Prg_Id)
        {
            ...
            case "F60111":
                ...
                txtDFno.Disabled = true;
                txtDFdt.Disabled = true;
                txtAFno.Disabled = true;
                txtAFdt.Disabled = true;
                txtAssgn.Disabled = true;
                ddStatus.Disabled = true;
                txtStage.Disabled = true;
                break;
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        cond = "";
        if (frm_ulvl != "0") cond = " and upper(trim(a.ent_by))='" + frm_uname.ToUpper() + "'";
        switch (btnval)
        {
            ...
            case "New":
                SQuery = "select CSSno||CSSdt||trim(ccode)||dsrno as fstr,CSSno as CSS_no,CSSdt as CSS_Dt,ccode as Client_code,max(Emodule) as Module_name,max(eicon) as Sup_Topic,max(Asg_asys) as Asg_Analysis,max(priority) as Asg_hrs from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Asg_Asys,eicon,Priority, 1 as d,dsrno from WB_CSS_ASG where branchcd='" + frm_mbr + "' and type='DF' AND (UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' or UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED') and upper(asg_agt)=upper('" + frm_uname + "') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emodule,null as Asg_Asys,null as eicon,null as Priority, -1 as d,dsrno from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and upper(asg_agt)=upper('" + frm_uname + "') AND UPPER(TASK_COMPL)='Y' ) group by CSSno||CSSdt||trim(ccode)||dsrno,CSSno,CSSdt,ccode having sum(d)>0 order by CSSno,CSSdt ";
                SQuery = "select CSSno||CSSdt||trim(ccode)||dsrno as fstr,CSSno as CSS_no,CSSdt as CSS_Dt,ccode as Client_code,max(Emodule) as Module_name,max(eicon) as Sup_Topic,max(Asg_asys) as Asg_Analysis,max(priority) as Asg_hrs,max(ent_by) as Asg_by,max(Remarks) as remarks from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Asg_Asys,eicon,Priority, 1 as d,dsrno,ent_by,Remarks from WB_CSS_ASG where branchcd='" + frm_mbr + "' and type='DF' AND (UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' or UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED') and upper(asg_agt)=upper('" + frm_uname + "') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emodule,null as Asg_Asys,null as eicon,null as Priority, -1 as d,dsrno,null as ent_by,null as Remarks from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and upper(asg_agt)=upper('" + frm_uname + "') AND UPPER(TASK_COMPL)='Y' ) group by CSSno||CSSdt||trim(ccode)||dsrno,CSSno,CSSdt,ccode having sum(d)>0 order by CSSno,CSSdt ";
                break;
            case "Edit":
            case "Del":
            case "Print":
                ...
                break;
            case "STATUS":
                SQuery = "Select name as fstr,Name as Action_taken,type1 as code from wb_type_mst where ID='CS' and upper(trim(typedpt))=upper(trim('" + ddStatus.Value + "')) order by TYPE1 ";
                break;
            default:
                //and upper(a.asg_agt)=upper('" + frm_uname + "')
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as ACT_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as ACT_Dt,a.csSno as CSSno,to_char(a.cssdt,'dd/mm/yyyy') as CSSdt,a.ccode as company,a.Emodule,a.Eicon,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,Remarks,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='AC' " + cond + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    void newCase(string vty)
    {
        ...
        SQuery = "Select a.* from WB_CSS_ASG a where a.branchcd||a.type||trim(a.CSSNO)||to_Char(a.CSSDT,'dd/mm/yyyy')||TRIM(A.CCODE)||trim(A.dsrno)='" + frm_mbr + "DF" + col1 + "' ORDER BY A.SRNO";
        dt = new DataTable();
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            txtDFno.Value = dt.Rows[0]["DSRNO"].ToString().Trim();
            txtDFdt.Value = Convert.ToDateTime(dt.Rows[0]["DSRDT"].ToString().Trim()).ToString("dd/MM/yyyy");
            ...
            txtAssgn.Value = dt.Rows[0]["ASG_AGT"].ToString().Trim();
            ddStatus.Value = dt.Rows[0]["asg_asys"].ToString().Trim();
            TxtPriority.Value = dt.Rows[0]["priority"].ToString().Trim();
            txt_asg_rmk.Text = dt.Rows[0]["remarks"].ToString().Trim();
        ...
        txtAFno.Value = frm_vnum;
        txtAFdt.Value = fgen.Fn_curr_dt(frm_cocd, frm_qstr);
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtStage.Value.Trim().Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Enter Stage !!");
            return;
        }
        if (txtDone.Value.Trim().Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Date of Workdone !!");
            return;
        }
        ...
        if (Txt_TDONE.Value.ToUpper().Trim() != "Y" && Txt_TDONE.Value.ToUpper().Trim() != "N")
        {
            fgen.msg("-", "AMSG", "Please Task Done (Y)/ Not Done (N)");
            return;
        }

        if (Txt_NEXTDT.Value.Trim().Length < 2 && Txt_TDONE.Value.ToUpper().Trim() == "N")
        {
            fgen.msg("-", "AMSG", "Please Put Next Target Date if Task Done='N' ");
            return;
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtAFno.Value = dt.Rows[0]["" + doc_nf.Value + ""].ToString().Trim();
                        txtAFdt.Value = Convert.ToDateTime(dt.Rows[0]["" + doc_df.Value + ""].ToString().Trim()).ToString("dd/MM/yyyy");

                        txtrmk.Text = dt.Rows[i]["remarks"].ToString().Trim();

                        Txt_TDONE.Value = dt.Rows[i]["TASK_COMPL"].ToString().Trim();
                        Txt_NEXTDT.Value = dt.Rows[i]["NEXT_TGT_DATE"].ToString().Trim();


                        txtDFno.Value = dt.Rows[i]["DSRNO"].ToString().Trim();
                        txtDFdt.Value = Convert.ToDateTime(dt.Rows[0]["DSRDT"].ToString().Trim()).ToString("dd/MM/yyyy");
                        ...
                        txtStage.Value = dt.Rows[i]["ACT_STATUS"].ToString().Trim();
                        txtDone.Value = Convert.ToDateTime(dt.Rows[i]["ACT_DATE"].ToString().Trim()).ToString("yyyy-MM-dd");

                        SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date,to_Char(a.app_Dt,'dd/mm/yyyy') As mapp_date from WB_CSS_ASG a where a.branchcd||a.type||trim(a.csSno)||to_Char(a.csSdt,'dd/mm/yyyy')||trim(a.dsrno)='" + frm_mbr + "DF" + txtvchnum.Value + txtvchdate.Value + txtDFno.Value + "' ORDER BY A.SRNO";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            ddStatus.Value = dt.Rows[i]["asg_asys"].ToString().Trim();
                            TxtPriority.Value = dt.Rows[i]["priority"].ToString().Trim();
                            txt_asg_rmk.Text = dt.Rows[i]["remarks"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            cond = "";
            if (frm_ulvl != "0") cond = " and upper(trim(a.ent_by))='" + frm_uname.ToUpper() + "'";
            SQuery = "SELECT a.ActNO as Action_NO,to_char(A.ActDT,'dd/mm/yyyy') as Action_Dt,a.CSSNO as CSS_NO,to_char(A.CSSDT,'dd/mm/yyyy') as CSS_Dt,a.CCODE as Client_Code,a.Emodule as Module_Name,a.Task_compl,A.Asg_agt,a.Act_Status ,a.Remarks,a.Next_tgt_date,a.Ent_Dt,a.Ent_by,to_chaR(a.ACTDT,'YYYYMMDD') as ACT_DT  FROM WB_CSs_Act a where a.branchcd='" + frm_mbr + "'  and a.type like '" + frm_vty + "%' " + cond + " order by a." + doc_df.Value + ",a." + doc_nf.Value + ",a.srno ";
        ...
        else
        {
            ...
            if (col1 == "Y" && Checked_ok == "Y")
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + frm_vty + frm_vnum + frm_CDT1, frm_mbr, frm_vty, frm_vnum, txtAFdt.Value.Trim(), "", frm_uname);
                                if (i > 20)
                                {
                    ...
                    string subject = "Query Action Taken : " + txtlbl4.Value.Trim() + " : Customer Support Ticket [#" + txtvchnum.Value.Trim() + "] ";
                    string tfrBy = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT ENT_BY FROM WB_CSS_ASG WHERE TRIM(DSRNO)='" + txtDFno.Value.ToString() + "' AND TO_CHAR(DSRDT,'DD/MM/YYYY')='" + Convert.ToDateTime(txtDFdt.Value.ToString().Trim()).ToString("dd/MM/yyyy") + "'", "ent_by");
                    string emailTo = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT emailid FROM EVAS WHERE USERNAME ='" + tfrBy + "'", "emailid");

                    html_body = html_body + "Dear " + tfrBy + ", <br/><br/>";

                    html_body = html_body + "Action taken on a query that has been assign by You CSS No. [#" + txtvchnum.Value.ToString().Trim() + "], to : " + frm_uname + " <br/>";
                    html_body = html_body + "<b>Client Code</b> : " + txtlbl4.Value.ToString().Trim() + "<br/>";
                    html_body = html_body + "<b>Module</b> : " + ddModule.Value.ToString().Trim() + "<br/>";
                    html_body = html_body + "<b>Icon / Path</b> : " + txtlbl8.Value.ToString().Trim() + "<br/><br/>";

                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, "SELECT ENT_BY,ENT_DT,Remarks,filepath,filename FROM WB_cSS_LOG WHERE BRANCHCD='" + frm_mbr + "' and type='CS' and TRIM(CSSNO)||TO_cHAR(CSSDT,'DD/MM/YYYY')='" + txtvchnum.Value.ToString().Trim() + txtvchdate.Value.ToString().Trim() + "' ");
                    if (dt.Rows.Count > 0)
                    {
                        html_body = html_body + "<b>Log By</b> : " + dt.Rows[0]["ent_by"].ToString().Trim() + "<br/>";
                        html_body = html_body + "<b>Log Date</b> : " + Convert.ToDateTime(dt.Rows[0]["ent_dt"].ToString().Trim()).ToString("dd/MM/yyyy") + "<br/>";
                        html_body = html_body + "<b>Client Remarks</b> : " + dt.Rows[0]["remarks"].ToString().Trim() + "<br/><br/>";
                        filepath = dt.Rows[0]["filepath"].ToString().Trim();
                        filename = dt.Rows[0]["filename"].ToString().Trim();
                    }

                    html_body = html_body + "<b>Assign No</b> : " + txtDFno.Value.ToString().Trim() + "<br/>";
                    html_body = html_body + "<b>Assign Date</b> : " + Convert.ToDateTime(txtDFdt.Value.ToString().Trim()).ToString("dd/MM/yyyy") + "<br/>";
                    html_body = html_body + "<b>Assign Work</b> : " + fgen.seek_iname(frm_qstr, frm_cocd, "SELECT REMARKS FROM WB_CSS_aSG WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='DF' AND TRIM(DSRNO)||TO_CHAR(DSRDT,'DD/MM/YYYY')='" + txtDFno.Value.ToString().Trim() + Convert.ToDateTime(txtDFdt.Value.ToString().Trim()).ToString("dd/MM/yyyy") + "' ", "REMARKS") + "<br/><br/>";

                    html_body = html_body + "<u><b>Query Status</b></u> <br/><br/>";
                    html_body = html_body + "<b>Action</b> : " + txtrmk.Text.ToString().Trim() + "<br/>";
                    html_body = html_body + "<b>Action Date</b> : " + txtAFdt.Value.ToString().Trim() + "<br/>";
                    string tc = (Txt_TDONE.Value.ToString().Trim() == "Y") ? "Yes" : "No";
                    html_body = html_body + "<b>Task Completed</b> : " + tc + "<br/>";

                    html_body = html_body + "Regards<br/>";
                    html_body = html_body + "Development / Technical Support Team | Pocketdriver Limited | 9015-220-220 | <a href='http://www.Tejaxo.asia'>www.Tejaxo.asia</a> |";
                    html_body = html_body + "<br/>";
                    html_body = html_body + "<br>===========================================================<br>";
                    html_body = html_body + "This Email is Auto generated from the Finsys CSS.<br/> ";
                    ...
                    fgen.send_mail(frm_cocd, "Tejaxo ERP", emailTo, col2, "", subject, html_body, filepath);
                    #endregion
    ...
    void save_fun()
    {
        ...
        oporow[doc_df.Value] = txtAFdt.Value.Trim();

        oporow["DSRNO"] = txtDFno.Value.Trim();
        oporow["DSRDT"] = txtDFdt.Value.Trim();
        ...
        oporow["TASK_COMPL"] = Txt_TDONE.Value.ToUpper().Trim();
        oporow["NEXT_TGT_DATE"] = Txt_NEXTDT.Value.ToUpper().Trim();

        oporow["ASG_DPT"] = fgen.seek_iname(frm_qstr, frm_cocd, "select erpdeptt from evas where upper(trim(username))=upper(trim('" + txtAssgn.Value.Trim() + "'))", "erpdeptt");
        oporow["ASG_AGT"] = txtAssgn.Value.Trim();

        oporow["ACT_STATUS"] = txtStage.Value;
        oporow["ACT_DATE"] = Convert.ToDateTime(txtDone.Value).ToString("dd/MM/yyyy");
        ...
        fgen.execute_cmd(frm_qstr, frm_cocd, "update wb_CSS_ASG set TASK_COMPL='" + Txt_TDONE.Value.ToUpper().Trim() + "',last_Action=substr('" + txtStage.Value + " | " + txtrmk.Text.Trim() + "',1,45) ,last_Actdt='" + txtAFdt.Value.Trim() + "' where branchcd||type||trim(CSSNO)||to_char(CSSDT,'dd/mm/yyyy')||trim(dsrno)='" + frm_mbr + "DF" + txtvchnum.Value + txtvchdate.Value + txtDFno.Value + "'");
    }
    void save_fun2()
    {
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F60111":
                SQuery = "SELECT 'AC' AS FSTR,'Support Request Action Logging' as NAME,'AC' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "AC");
                break;
    ...
    {
        hffield.Value = "STATUS";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select STATUS", frm_qstr);
    }
    ...
    protected void btnAtt_Click(object sender, EventArgs e)
    {
        ...
        if (Attch.HasFile)
        {
            ...
            filepath = filepath + txtlbl4.Value.Trim() + "_" + txtvchnum.Value.Trim() + txtAFno.Value.Trim() + frm_CDT1.Replace(@"/", "_") + "~" + Attch.FileName;
