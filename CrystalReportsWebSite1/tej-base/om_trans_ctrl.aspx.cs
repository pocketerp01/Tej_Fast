...
public partial class om_trans_ctrl : System.Web.UI.Page
{
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                switch (Prg_Id)
                {
                    ...
                    case "F70720":
                        lblheader.Text = "Permit Credit Extension to Party";
                        break;
                    case "F70724":
                        lblheader.Text = "Block Transaction With Party";
                        break;
                    case "F75229":
                        lblheader.Text = "Transaction Master";
                        break;
                }
    ...
    public void set_Val()
    {
        ...
        switch (Prg_Id)
        {
            ...
            case "F70724":
                //lblheader.Text = "Block Transaction With Party";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "BL");
                typePopup = "N";
                break;
            case "F75229":
                //lblheader.Text = "VEhicle inp...cheklist";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "AT");
                typePopup = "N";
                break;
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        SQuery = "";
        string acodeCond = "trim(GRP) in ('16','05','06') and ";
        ...
        switch (btnval)
        {
            ...
            case "Print":
                ...
                break;
            case "USR":
                SQuery = "SELECT ACODE AS FSTR,ANAME AS PARTY,ACODE AS CODE,ADDR1,ADDR2,staten as state,Pay_num,trim(nvl(gst_no,'-')) as " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_TAXVAR") + " FROM famst where " + acodeCond + "  length(Trim(nvl(deac_by,'-')))<=1 and length(trim(nvl(staten,'-')))>1 and (case when length(nvl(showinbr,'-'))>1 then instr(nvl(showinbr,'-'),'" + frm_mbr + "') else 1 end)>0  ORDER BY aname ";
                break;
            case "PERSON":
                SQuery = "SELECT email AS FSTR,Person AS Contact_Name,Mobile,email FROM famst where " + acodeCond + "  length(Trim(nvl(deac_by,'-')))<=1 and trim(Acode)='" + txt_partyc.Value.Trim() + "' ORDER BY aname ";
                break;
            ...
            case "RSN":
                ...
                {
                    case "F70720":
                        SQuery = "SELECT Name as fstr ,Name AS Reaons,type1 as Code FROM Typegrp WHERE id='CE' ORDER BY Name";
                        break;
                    case "F70724":
                        SQuery = "SELECT Name as fstr ,Name AS Reaons,type1 as Code FROM Typegrp WHERE id='BL' ORDER BY Name";
                        break;
                }
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    lbl1a_Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
                SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as CTRL_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as CTRL_Dt,b.aname,a.Tr_Reason,a.Tr_upto,a.cont_name as person,a.cont_no as contact_no, a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where trim(A.acode)=trim(B.acode)and  a.branchcd='" + frm_mbr + "' and a.type='" + lbl1a_Text + "' and a.CTRLDT " + DateRange + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txt_partyc.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", "Company Code not entered !!");
            return;
        }
        if (txt_rsn.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", "Reason not entered !!");
            return;
        }


        if (txt_contn.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact Name !!");
            return;
        }
        if (txt_contp.Value.Length != 10)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact No. !!");
            return;
        }
        if (txt_email.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Contact Email ID !!");
            return;
        }
        if (txt_upto.Value.Length < 2)
        {
            fgen.msg("-", "AMSG", " Please Enter Validity Upto !!");
            return;
        }


        if (fgen.isValidMailID(txt_email.Value.Trim()) == false)
        {
        ...
        else
        {
            ...
            switch (btnval)
            {
                case "New":
                    ...
                    break;
                case "USR":
                    txt_partyc.Value = col1;
                    txt_partyn.Value = col2;

                    if (fgenMV.Fn_Get_Mvar(frm_qstr, "U_CLIENT_GRP") == "SG_TYPE")
                        txt_dues.Value = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT sum(dramt-cramt) as total from  RecData where branchcd!='DD' AND ACODE='" + col1 + "'", "total");
                    else
                        txt_dues.Value = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT acode,invno,invdate,dramt-cramt as total from  RecDataW_" + frm_mbr + " where branchcd='" + frm_mbr + "' AND ACODE='" + col1 + "'", "total");
                    break;
                case "RSN":
                    txt_rsn.Value = col1;
                    break;
                case "PERSON":
                    txt_contn.Value = col2;
                    txt_contp.Value = col3;
                    txt_email.Value = col1;
                    break;
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txt_partyc.Value = dt.Rows[i]["ACODE"].ToString().Trim();
                        txt_partyn.Value = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where acode='" + txt_partyc.Value + "'  ", "aname");
                        txt_dues.Value = dt.Rows[i]["curr_due"].ToString().Trim();

                        txt_rsn.Value = dt.Rows[i]["tr_reason"].ToString().Trim();
                        txt_upto.Value = dt.Rows[i]["tr_upto"].ToString().Trim();

                        txt_allo.Value = dt.Rows[i]["AMT_ALLOW"].ToString().Trim();

                        txt_contn.Value = dt.Rows[i]["CONT_NAME"].ToString().Trim();
                        txt_contp.Value = dt.Rows[i]["CONT_NO"].ToString().Trim();
                        txt_email.Value = dt.Rows[i]["CONT_EMAIL"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as CTRL_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as CTRL_Dt,b.aname,b.addr1,b.addr2,a.Tr_Reason,a.Tr_upto,a.cont_name as person,a.cont_no as contact_no, a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,a.App_by,to_char(a.app_Dt,'dd/mm/yyyy') as app_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where trim(A.acode)=trim(B.acode)and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.CTRLDT " + PrdRange + " order by vdd desc,a." + doc_nf.Value + " desc";
                ...
                try
                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "Trans No " + frm_vnum + "'13'We appreciate your Entry to Tejaxo");
                        }
                        else
                        {
    ...
    void save_fun()
    {
        ...
        oporow["ACODE"] = txt_partyc.Value.ToUpper().Trim();
        oporow["curr_due"] = fgen.make_double(txt_dues.Value.ToUpper().Trim());

        oporow["tr_reason"] = txt_rsn.Value.ToUpper().Trim();
        oporow["tr_upto"] = txt_upto.Value.ToUpper().Trim();

        oporow["AMT_ALLOW"] = txt_allo.Value.toDouble();

        oporow["CONT_NAME"] = txt_contn.Value.ToUpper().Trim();
        oporow["CONT_NO"] = txt_contp.Value.ToUpper().Trim();
        oporow["CONT_EMAIL"] = txt_email.Value.ToUpper().Trim();
    ...
    protected void btnAtt_Click(object sender, EventArgs e)
    {
        ...
        if (Attch.HasFile)
        {
            ...
            filepath = filepath + txt_partyc.Value.Trim() + "_" + txtvchnum.Value.Trim() + frm_CDT1.Replace(@"/", "_") + "~" + Attch.FileName;
            Attch.PostedFile.SaveAs(filepath);
            //Attch.PostedFile.SaveAs(Server.MapPath(@"tej-base/upload/" + txt_partyc.Value.Trim() + "_" + txtvchnum.Value.Trim() + frm_CDT1.Replace(@"/", "_") + "~" + Attch.FileName));
    ...
    protected void btnrsncd_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "RSN";
        make_qry_4_popup();
