...
public partial class prod_entryy : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "JOBNO":
                ...
                break;
            case "STAGE":
                SQuery = "SELECT TYPE1 AS FSTR,NAME,TYPE1 AS CODE FROM TYPE WHERE ID='1' and type1 LIKE '6%' ORDER BY TYPE1";
                break;
            case "MACHINE":
            ...
            case "ERPCODE":
                ...
                if (txtStage.Value.Length > 2)
                {
                    ...
                    cond = "";
                    if (stage == "69") cond = " AND STAGE!='61'";
                    else cond = " AND STAGE!='69'";

                    if (stage != "61")
                        SQuery = "SELECT A.BRANCHCD||TRIM(A.ICODE)||TRIM(A.PURPOSE)||TRIM(A.TC_NO)||TRIM(A.BTCHNO) AS FSTR,TRIM(A.ICODE) AS ERPCODE,B.INAME AS PRODUCT,B.CPARTNO AS CODE,A.PURPOSE AS CTRLNO,A.TC_NO,A.BTCHNO,SUM(A.IQTYIN) AS QTY FROM (SELECT BRANCHCD,TRIM(ICODE) AS ICODE,(IQTYIN) as IQTYIN,trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND VCHDATE " + DateRange + " AND STAGE!='" + stage + "' " + cond + " AND STORE='W' union all SELECT BRANCHCD,TRIM(ICODE) AS ICODE,iqtyout as IQTYIN,trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE like '3%' and type!='3A' AND VCHDATE >to_date('01/04/2019','dd/mm/yyyy') AND STAGE='" + stage + "' " + cond + " AND STORE='Y' UNION ALL SELECT BRANCHCD,TRIM(ICODE) AS ICODE,-1*(IQTYIN+rej_rw),trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND VCHDATE>to_date('01/04/2010','dd/mm/yyyy') AND STAGE='" + stage + "' " + cond + " AND (STORE='W' or store='Y')) A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) GROUP BY TRIM(A.ICODE),B.INAME,B.CPARTNO,A.PURPOSE,A.TC_NO,A.BTCHNO,A.BRANCHCD||TRIM(A.ICODE)||TRIM(A.PURPOSE)||TRIM(A.TC_NO)||TRIM(A.BTCHNO) having sum(a.iqtyin)>0 order by a.purpose";
                    if (stage == "69") { }
                    //SQuery = "SELECT A.BRANCHCD||TRIM(A.ICODE)||TRIM(A.PURPOSE)||TRIM(A.TC_NO)||TRIM(A.BTCHNO) AS FSTR,TRIM(A.ICODE) AS ERPCODE,B.INAME AS PRODUCT,B.CPARTNO AS CODE,A.PURPOSE AS CTRLNO,A.TC_NO,A.BTCHNO,SUM(A.IQTYIN) AS QTY FROM (SELECT BRANCHCD,TRIM(ICODE) AS ICODE,IQTYIN as IQTYIN,trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND VCHDATE>TO_dATE('01/04/2010','DD/MM/YYYY') AND STAGE>='62' AND STORE='W' union all SELECT BRANCHCD,TRIM(ICODE) AS ICODE,IQTYout as IQTYIN,trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE like '3%' and type!='3A' AND VCHDATE>TO_dATE('01/04/2010','DD/MM/YYYY') AND STAGE='" + stage + "' AND STORE='Y'  UNION ALL SELECT BRANCHCD,TRIM(ICODE) AS ICODE,-1*(IQTYIN+rej_rw),trim(PURPOSE) as PURPOSE,trim(TC_NO) as TC_NO,trim(BTCHNO) as BTCHNO FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND VCHDATE>TO_dATE('01/04/2010','DD/MM/YYYY') AND STAGE='69' AND STORE='Y') A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) GROUP BY TRIM(A.ICODE),B.INAME,B.CPARTNO,A.PURPOSE,A.TC_NO,A.BTCHNO,A.BRANCHCD||TRIM(A.ICODE)||TRIM(A.PURPOSE)||TRIM(A.TC_NO)||TRIM(A.BTCHNO) having sum(a.iqtyin)>0 order by a.purpose";
                }
                break;
            case "VALIDATEBOM":
                SQuery = "Select trim(a.vchnum)||to_Char(A.vchdate,'dd/mm/yyyy')||trim(a.icode)||Trim(a.ibqty) as fstr,a.vchnum as bomno,to_char(A.vchdate,'dd/mm/yyyy') as bomdt,a.ibcode as erpcode,b.iname as product,a.ibqty as bomqty from itemosp a,item b where trim(a.ibcode)=trim(b.icode) and trim(a.icode)='" + txtIcode.Value.Trim() + "' order by a.ibcode";
                break;
            default:
                frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
            ...
            if (txtStage.Value.Split('~')[0] == "61")
            {
                ...
                foreach (GridViewRow gr in sg1.Rows)
                {
                    if (gr.Cells[13].Text.Trim().Length > 2)
                    {
                        ...
                        string ctrlno = ((TextBox)gr.FindControl("sg1_t3")).Text;
                        string btchno = ((TextBox)gr.FindControl("sg1_t4")).Text;
                        string idno = ((TextBox)gr.FindControl("sg1_t5")).Text;
                        cond = " and trim(vchnum)||to_char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Value + txtvchdate.Value + "'";
                        SQuery = "SELECT ICODE,SUM(BAL) AS qty FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(PURPOSE) AS PURPOSE,TRIM(BTCHNO) AS BTCHNO,TRIM(TC_NO) AS TC_NO,-1*IQTYOUT AS BAL FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' and trIm(icodE)='" + icode + "' and trim(purpose)='" + ctrlno + "' and trim(BTCHNO)||TRIM(TC_NO)='" + btchno + idno + "' " + cond + " UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(PURPOSE) AS PURPOSE,TRIM(BTCHNO) AS BTCHNO,TRIM(TC_NO) AS TC_NO,IQTYOUT AS BAL FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE LIKE '3%' AND TYPE!='39' AND STORE='Y' and trIm(icodE)='" + icode + "' and trim(purpose)='" + ctrlno + "' and trim(BTCHNO)||TRIM(TC_NO)='" + btchno + idno + "' ) GROUP BY ICODE,TRIM(BTCHNO),TRIM(Tc_NO),TRIM(PURPOSE) HAVING SUM(BAL)>0";
                        //SQuery = "SELECT ICODE,SUM(BAL) AS qty FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(PURPOSE) AS PURPOSE,IQTYIN AS BAL FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1)!='3' AND STORE='Y' and trIm(icodE)='" + icode + "' and trim(purpose)='" + ctrlno + "' " + cond + " UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(PURPOSE) AS PURPOSE,-1*IQTYOUT AS BAL FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE LIKE '3%' AND TYPE!='39' AND STORE='Y' and trim(icodE)='" + icode + "' and trim(purpose)='" + ctrlno + "' " + cond + " ) GROUP BY ICODE HAVING SUM(BAL)>0";
                        col1 = fgen.seek_iname(frm_qstr, frm_cocd, SQuery, "qty");
                ...
                {
                    if (fgen.make_double(txtOBSODmin.Text) <= 0)
                    {
                        fgen.msg("-", "AMSG", "OBS OD Min Not Entered!!");
                        return;
                    }
                    if (fgen.make_double(txtOBSODmax.Text) <= 0)
                    {
                        fgen.msg("-", "AMSG", "OBS OD Max Not Entered!!");
                        return;
                    }
                    if (fgen.make_double(txtOBSthikmin.Text) <= 0)
                    {
                        fgen.msg("-", "AMSG", "OBS Thickness Min Not Entered!!");
                        return;
                    }
                    if (fgen.make_double(txtOBSthikmax.Text) <= 0)
                    {
                        fgen.msg("-", "AMSG", "OBS Thickness Max Not Entered!!");
                        return;
                    }
                }
            }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ent_date,b.iname,b.cpartno from " + frm_tabname + " a,item b where trim(a.icode)=trim(B.icode) and a.branchcd||a.type||trim(a.VCHNUM)||TO_CHAR(a.VCHDATE,'DD/MM/YYYY')='" + mv_col + "' ";
                    ...
                    if (col1.Length > 1)
                    {
                        ...
                        if (txtStage.Value.Length > 2)
                        {
                            ...
                            if (stage != "61")
                            {
                                ...
                                txtControl.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL5").ToString().Trim().Replace("&amp", "");
                                txtIDNo.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL6").ToString().Trim().Replace("&amp", "");
                                txtBtchno.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL7").ToString().Trim().Replace("&amp", "");
                                ...
                                sg1_dr["sg1_f3"] = txtCpartno.Value.Trim();
                                sg1_dr["sg1_f4"] = "-";
                                sg1_dr["sg1_f5"] = "-";
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.HeaderRow.Cells[17].Text = "Ctrl No.";
    ...
    void save_fun_01()
    {
        ...
        foreach (GridViewRow gr in sg1.Rows)
        {
            if (fgen.make_double(((TextBox)gr.FindControl("sg1_t1")).Text) > 0)
            {
                ...
                oporow["REJ_RW"] = 0;
                rejqty += fgen.make_double(((TextBox)gr.FindControl("sg1_t2")).Text);
                if (fgen.make_double(((TextBox)gr.FindControl("sg1_t2")).Text) > 0)
                    fgcode = gr.Cells[13].Text.Trim();
        ...
        oporow["icode"] = txtIcode.Value.Trim();
        oporow["iqtyin"] = fgen.make_double(txtlbl51.Text.Trim());
        oporow["rej_rw"] = 0;
        oporow["iqtyout"] = "0";

        oporow["stage"] = txtStage.Value.Trim().Split('~')[0].ToString();
        oporow["store"] = "W";
        if (frm_cocd == "HIME" && frm_mbr == "01")
            oporow["store"] = "Y";
    ...
    protected void txtlbl51_TextChanged(object sender, EventArgs e)
    {
        ...
        make_qry_4_popup();
        if (frm_mbr == "01" && frm_cocd == "HIME") fgen.Fn_open_mseek("-", frm_qstr);
    ...
    void calc(string bomno)
    {
        ...
        {
            if (txtStage.Value.Trim().Split('~')[0] == "61")
            {
                dt = new DataTable();
                dt = fgen.getdata(frm_qstr, frm_cocd, "SELECT A.IBCODE,B.INAME,B.CPARTNO,B.UNIT,a.ibqty FROM ITEMOSP A,ITEM B where TRIM(A.IBCODE)=TRIM(B.ICODE) AND TRIM(A.ICODE)='" + txtIcode.Value.Trim() + "' and a.branchcd='" + frm_mbr + "' and trim(a.vchnum)||to_Char(A.vchdate,'dd/mm/yyyy')||trim(a.icode)||Trim(a.ibqty) in (" + bomno + ") order by a.ibcode ");
                if (dt.Rows.Count > 0)
                {
                    i = 0;
                    create_tab();
