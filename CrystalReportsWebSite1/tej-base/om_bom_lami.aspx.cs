...
public partial class om_bom_lami9 : System.Web.UI.Page
{
    ...
    void setColHeadings()
    {
        ...
        for (int sR = 0; sR < sg1.Columns.Count; sR++)
        {
            ...
            for (int K = 0; K < sg1.Rows.Count; K++)
            {
                ...
                if (((TextBox)sg1.Rows[K].FindControl("sg1_t1")).Text.toDouble() > 0)
                    sg1.Rows[K].BackColor = System.Drawing.Color.FromName("#fbfb2682");
            }
        ...
        if (sg1.Rows.Count > 1)
        {
            sg1.Rows[sg1.Rows.Count - 1].Cells[13].Text = "Total ";
            sg1.Rows[sg1.Rows.Count - 1].BackColor = System.Drawing.Color.GreenYellow;
        }
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "inspvch";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "B1");
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                {
                    SQuery = "SELECT Icode AS FSTR,Iname AS Item_Name,Cpartno,Cdrgno,Unit,Icode as ERp_Code,mat4 as density,mat5 as micron,oprate3 as gsm,Ent_by,Ent_Dt FROM item where length(Trim(nvl(deac_by,'-')))<=1 " + col1 + " and length(Trim(icode))>4 ORDER BY IName ";
                }
                else
                {
                    SQuery = "SELECT Icode AS FSTR,Iname AS Item_Name,Cpartno,Cdrgno,Unit,Icode as ERp_Code,mat4 as density,mat5 as micron,oprate3 as gsm,Ent_by,Ent_Dt FROM item where length(Trim(nvl(deac_by,'-')))<=1 and trim(icode)!='" + txtlbl4.Text + "'  " + col1 + " and length(Trim(icode))>4 ORDER BY IName ";
                }
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as BOM_no,to_char(a.vchdate,'dd/mm/yyyy') as BOM_Dt,b.IName as Product_Name,A.ACODE AS ERPCODE,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,Item b where trim(A.Acode)=trim(B.Icode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' order by vdd desc,a.vchnum desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        dtnx.Columns.Add("LAYER");
        dtnx.Columns.Add("LAYER_PER", typeof(double));
        double totTarget = 0;
        DataRow drx = null;
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            drx = dtnx.NewRow();
            drx["LAYER"] = ((DropDownList)sg1.Rows[i].FindControl("ddLayer")).SelectedItem.Text.ToUpper();
            drx["LAYER_PER"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.toDouble();
            totTarget += ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.toDouble();
            dtnx.Rows.Add(drx);
        }

        if (totTarget != 100)
        {
            fgen.msg("-", "AMSG", "Total % target is not equal to 100%, Please correct.");
            return;
        }

        DataView dv = new DataView(dtnx);
        dt = new DataTable();
        dt = dv.ToTable(true, "LAYER");
        foreach (DataRow drt in dt.Rows)
        {
            if (dtnx.Compute("sum(layer_per)", "LAYER='" + drt["layer"].ToString() + "'").ToString().toDouble() != 100)
            {
                fgen.msg("-", "AMSG", "For Layer : " + drt["layer"].ToString().Trim() + " '13'Item wise total : " + dtnx.Compute("sum(layer_per)", "LAYER='" + drt["layer"].ToString() + "'").ToString().toDouble() + "'13' Total Layer percentage can not be greater then 100 %");
                return;
            }
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,c.iname as Iname1,c.cpartno as cpartno1,c.cdrgno as cdrgno1,c.unit as unit1 from " + frm_tabname + " a,item c where trim(a.icode)=trim(c.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl2.Text = dt.Rows[i]["OBSV1"].ToString().Trim();
                        txtlbl3.Text = dt.Rows[i]["OBSV2"].ToString().Trim();
                        TextBox1.Text = dt.Rows[i]["OBSV3"].ToString().Trim();
                        TextBox4.Text = dt.Rows[i]["OBSV4"].ToString().Trim();
                        txtlbl5.Text = dt.Rows[i]["OBSV5"].ToString().Trim();
                        txtlbl6.Text = dt.Rows[i]["OBSV6"].ToString().Trim();
                        TextBox2.Text = dt.Rows[i]["OBSV7"].ToString().Trim();
                        txtlbl8.Text = dt.Rows[i]["OBSV8"].ToString().Trim();
                        TextBox3.Text = dt.Rows[i]["OBSV9"].ToString().Trim();

                        txtQty.Text = dt.Rows[i]["SAMPQTY"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            sg1_dr = sg1_dt.NewRow();
                            sg1_dr["sg1_srno"] = sg1_dt.Rows.Count + 1;
                            sg1_dr["sg1_h1"] = dt.Rows[i]["icode"].ToString().Trim();
                            sg1_dr["sg1_h2"] = dt.Rows[i]["iname1"].ToString().Trim();
                            sg1_dr["sg1_h3"] = "-";
                            sg1_dr["sg1_h4"] = "-";
                            sg1_dr["sg1_h5"] = "-";
                            sg1_dr["sg1_h6"] = "-";
                            ...
                            sg1_dr["hflayer"] = dt.Rows[i]["wono"].ToString().Trim();
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["QTY7"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["QTY1"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["QTY2"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["QTY3"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["QTY4"].ToString().Trim();

                            sg1_dr["sg1_t6"] = dt.Rows[i]["QTY5"].ToString().Trim();
                            sg1_dr["sg1_t7"] = dt.Rows[i]["QTY6"].ToString().Trim();

                            sg1_dr["sg1_t11"] = dt.Rows[i]["QTY8"].ToString().Trim();
                ...
                case "Print_C":
                    if (col1 == "") return;
                    if (Request.Cookies["REPLY"].Value == "Y") fgenMV.Fn_Set_Mvar(frm_qstr, "REPID", frm_formID);
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count - 1; i++)
                        {
                            ...
                            sg1_dr["hflayer"] = ((HiddenField)sg1.Rows[i].FindControl("hfLayer")).Value.Trim();
                            sg1_dt.Rows.Add(sg1_dr);
                        }

                        dt = new DataTable();
                        if (col1.Length > 8) SQuery = "select icode,iname,cpartno,cdrgno,unit,mat4 ,mat5 ,oprate3 from item where trim(icode) in (" + col1 + ")";
                        else SQuery = "select icode,iname,cpartno,cdrgno,unit,mat4 ,mat5 ,oprate3 from item where trim(icode)='" + col1 + "'";
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_t2"] = dt.Rows[d]["mat5"].ToString().Trim().toDouble();
                            sg1_dr["sg1_t3"] = dt.Rows[d]["mat4"].ToString().Trim().toDouble();
                            sg1_dr["sg1_t4"] = dt.Rows[d]["oprate3"].ToString().Trim().toDouble();
                            sg1_dr["sg1_t5"] = "100";
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t2")).Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL7").ToString().Trim().Replace("&amp", "").toDouble().ToString();
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t3")).Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL8").ToString().Trim().Replace("&amp", "").toDouble().ToString();
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t4")).Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL9").ToString().Trim().Replace("&amp", "").toDouble().ToString();
                    ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t5")).Text = "100";
                    setColHeadings();
                    break;
                case "SG1_ROW_ALT_E":
                    if (col1.Length <= 0)
    ...
    public void create_tab()
    {
        ...
        sg1_dt.Columns.Add(new DataColumn("hflayer", typeof(string)));
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                SQuery = "select type1 as fstr,name as layer,type1 as Code from TYPEGRP where id='XX' order by type1";
                dtx = new DataTable();
                dtx = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                if (dtx.Rows.Count <= 0)
                {
                    dtx = new DataTable();
                    dtx.Columns.Add("fstr");
                    dtx.Columns.Add("layer");
                    dtx.Columns.Add("type1");
            ...
            {
                dtx = new DataTable();
                dtx = (DataTable)ViewState["DSGT"];
            }
            if (dtx.Rows.Count > 0)
            {
                try
                {
                    DropDownList ddx = (DropDownList)e.Row.FindControl("ddLayer");
                    ddx.DataSource = dtx;
                    ddx.DataTextField = "layer";
                    ddx.DataValueField = "layer";
                    ddx.DataBind();
                }
                catch { }
            }
    ...
    void set_oldRow()
    {
        ...
        foreach (GridViewRow gr in sg1.Rows)
        {
            if (z < sg1.Rows.Count)
            {
                try
                {
                    ((DropDownList)gr.FindControl("ddLayer")).Items.FindByText(((HiddenField)gr.FindControl("hfLayer")).Value).Selected = true;
                }
                catch { }
                z++;
            }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["TITLE"] = "BOM CREATION SCREEN: LAMINATE";

                oporow["SRNO"] = i;

                oporow["Acode"] = txtlbl4.Text;
                oporow["icode"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["COL1"] = sg1.Rows[i].Cells[14].Text.Trim();

                oporow["COL2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.toDouble();
                oporow["COL3"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.toDouble();
                oporow["COL4"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble();

                oporow["qty7"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.toDouble();

                oporow["qty1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.toDouble();
                oporow["qty2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble();
                oporow["qty3"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.toDouble();
                oporow["qty4"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.toDouble();
                oporow["qty5"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.toDouble();
                oporow["qty6"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.toDouble();
                oporow["qty8"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t11")).Text.toDouble();

                oporow["OBSV1"] = txtlbl2.Text;
                oporow["OBSV2"] = txtlbl3.Text;
                oporow["OBSV3"] = TextBox1.Text;
                oporow["OBSV4"] = TextBox4.Text;
                oporow["OBSV5"] = txtlbl5.Text;
                oporow["OBSV6"] = txtlbl6.Text;
                oporow["OBSV7"] = TextBox2.Text;
                oporow["OBSV8"] = txtlbl8.Text;
                oporow["OBSV9"] = TextBox3.Text;

                oporow["SAMPQTY"] = txtQty.Text;

                oporow["wono"] = ((DropDownList)sg1.Rows[i].FindControl("ddLayer")).Text.ToUpper();
