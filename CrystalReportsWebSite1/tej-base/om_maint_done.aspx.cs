...
public partial class om_maint_done : System.Web.UI.Page
{
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                else
                {
                 ...
                 SQuery = "select DISTINCT trim(mch_code) as fstr,trim(Machine_Name) as Machine_Name,trim(Mch_Code) as Mch_Code,trim(Inst_no) as Inst_no,Plan_date as Plan_date,sum(Mcnt) as Mcnt  from (select acode,vchnum,vchdate,mchname as Machine_Name,mchcode as Mch_Code,'-' as Inst_no,maintdt as Plan_date,1 as Mcnt  from pmaint where branchcd='00' and type='66' and vchdate between to_date('01/04/2018','dd/mm/yyyy') and to_date('31/03/2019','dd/mm/yyyy') union all select acode,vchnum,vchdate,mchname as Machine_Name,mchcode as Mch_Code,'-' as Inst_no,spec4 as Plan_date,-1 as sss from pmaint where branchcd='00' and type='20' and vchdate between to_date('01/04/2018','dd/mm/yyyy') and to_date('31/03/2019','dd/mm/yyyy')) where acode='"+txtlbl4.Text+"' group by trim(Machine_Name),trim(Mch_Code),trim(Inst_no),Plan_date having sum(Mcnt)=1 order by trim(Machine_Name),trim(Mch_Code),trim(Inst_no),trim(Plan_date)";
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as Rec_No,a.vchdate as Rec_Dt,a.type,b.name as Section_Name,a.ent_by from pmaint a, (Select type1,name from type where id=':' and type1>'10') b where  trim(a.acode)=trim(b.type1) and a.type='" + frm_vty + "' and a.branchcd='" + frm_mbr + "' and a.vchnum<>'000000' and a.vchdate " + DateRange + " order by a.vchdate desc ,a.vchnum desc";
            ...
            switch (btnval)
            {
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        //SQuery = "select type1,name,rate from type where id='K' and trim(type1) in (" + col1 + ") order by type1";
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,a.mchname as name,a.mchcode as code,a.spec4 as last_done_dt,a.maintby as done_at, a.maintdt as done_dt,a.maintamt as amount_exp,a.appven as type,a.remarks from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + PrdRange + "  order by a.vchnum desc";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        ...
        else if (hffield.Value == "Print_E")
        {
            ...
            fgen.drillQuery(0, "select distinct trim(mchcode)||trim(mchname) as fstr,'-' as gstr, mchname as equipment_name,mchcode as equipment_code, srno as instances from pmaint where branchcd='" + frm_mbr + "' and type='20' and vchdate " + PrdRange + " " + PrdRange + "", frm_qstr);
