...
public partial class om_multi_balance : System.Web.UI.Page
{
    ...
    void save_fun()
    {
        ...
        if (dtW != null)
        {
            ...
            foreach (DataRow gr1 in dtW.Rows)
            {
                ...
                chkname4 = fgen.seek_iname(frm_qstr, frm_cocd, "select br_acode from famstbal where br_acode='" + gr1["fstr"].ToString().Trim() + "'", "br_acode");
                if(chkname4=="0")
                
                {
                    ...
                    oporow["br_acode"] = gr1["PLANTCODE"].ToString().Trim() + gr1["PARTYCODE"].ToString().Trim();

                    oporow2["grp"] = gr1["PARTYCODE"].ToString().Trim().Substring(0, 2);
                    oporow["YR_"+frm_myear+""] = gr1["AMOUNT"].ToString().Trim();
                    oporow["yr_2011"] = 0;
                    oporow["yr_2012"] = 0;
                    oporow["yr_2013"] = 0;
                    oporow["yr_2014"] = 0;
                ...
                {
                    SQuery = "update "+ frm_tabname +" set ";
                    SQuery += "yr_"+frm_myear+"=" + gr1["amount"].ToString().Trim() + "";
                   
                    SQuery += " where br_acode='" + gr1["fstr"].ToString().Trim() +"'";
        ...
        string excelConString = "";
            ...
            if (FileUpload1.HasFile)
            {
                ...
                try
                {
                    ...
                    dtn.Columns.Add("AMOUNT", typeof(double));
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                            ...
                            if ((fgen.padlc(Convert.ToInt32(dr[1].ToString().Trim()), 2) == "06") || (fgen.padlc(Convert.ToInt32(dr[1].ToString().Trim()), 2) == "05") || (fgen.padlc(Convert.ToInt32(dr[1].ToString().Trim()), 2) == "02") || (fgen.padlc(Convert.ToInt32(dr[1].ToString().Trim()), 2) == "16"))
                            {
                            ...
                            drn["amount"] = fgen.make_double(dr[3].ToString().Trim());
                            drn["dtsrno"] = count1;
  ...
  protected void btnvalidate_ServerClick(object sender, EventArgs e)
{
    ...
    {
        dtn.Rows[K]["fstr"] = dtn.Rows[K]["PLANTCODE"].ToString().Trim() + dtn.Rows[K]["PARTYCODE"].ToString().Trim();
    ...
    DataView view = new DataView(dtn);
    DataTable distinctValues = view.ToTable(true, "partycode");
  
    //checking duplicate values in dataview
    foreach (DataRow dr1 in distinctValues.Rows)
    {
        DataView view2 = new DataView(dtn, "partycode='" + dr1["partycode"].ToString().Trim() + "'", "", DataViewRowState.CurrentRows);
        dt2 = new DataTable();
        dt2 = view2.ToTable();
        if (dt2.Rows.Count == 1)
        {
    ...
    for (int i1 = 0; i1 < dtn.Rows.Count; i1++)
    {
        ...
        if (chkname2 == "0")
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", This party name " + chkname1 + "  already exist. Please update it ");
            flag = 1;
            app += "The plant code " + dtn.Rows[i1]["plantcode"].ToString().Trim() + " is not found in database.,";

            req = req + 1;

        }

        chkname4 = fgen.seek_iname(frm_qstr, frm_cocd, "select yr_"+frm_myear+" from famstbal where br_acode='" + dtn.Rows[i1]["fstr"].ToString().Trim() + "'", "yr_"+frm_myear+"");
        if (Convert.ToDouble(chkname4)>0)
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", This party name " + chkname1 + "  already exist. Please update it ");
            flag = 1;
            app += "Balance already existing, please delete the row from excel.";
            req = req + 1;
        ...
        if (dtn.Rows[i1]["partycode"].ToString().Length <0)
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Composition Account must be in Y/N format  ");
            flag = 1;


            app += " Party code must be entered. ";

            req = req + 1;
        }

        if (dtn.Rows[i1]["amount"].ToString().Length > 13)
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", This addr2 must be in 60 in length ");
            flag = 1;
            app += "AMOUNT can be upto 13 characters.,";

            req = req + 1;
        }
        if ((dtn.Rows[i1]["PLANTCODE"].ToString() =="06")||(dtn.Rows[i1]["PLANTCODE"].ToString() =="05")||(dtn.Rows[i1]["PLANTCODE"].ToString() =="02")||(dtn.Rows[i1]["PLANTCODE"].ToString() =="16"))
        {
            //fgen.msg("-", "AMSG", "Dear " + frm_uname + ", This primary  unit  must be in 10 in length and secondary unit  must be in 8 in length ");
            flag = 1;
            app += "For this ledger group kindly use bill by bill facility";
    ...
    protected void btnhelp_ServerClick(object sender, EventArgs e)
    {
        ...
        dt1 = fgen.getdata(frm_qstr, frm_cocd, "SELECT 'SRNO' AS SRNO,'PLANTCODE' AS PLANTCODE,'PARTYCODE' AS PARTYCODE,'AMOUNT' AS AMOUNT FROM DUAL");
