...
public partial class mat_issue_req : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                ...
                break;
            case "WIPSTG":
                SQuery = "SELECT TYPE1,NAME AS STATE ,TYPE1 AS CODE FROM TYPE WHERE ID='1' AND TYPE1 LIKE '6%' and type1 not in ('6R','6H') ORDER BY TYPE1";
                break;
            case "RWM":
                ...
                {
                    SQuery = " SELECT A.ibcode AS FSTR,B.INAME,A.ibcode,(case when trim(nvl(a.LINKAGE,'N')) = 'Y' then 'Main' else 'Alt' end) as Item_type,b.cpartno,b.cdrgno,b.unit,c.Closing_Stk as bal FROM ITEMOSP A,ITEM B," +
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            //sg1_dr["sg1_f4"] = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, dt.Rows[i]["icode"].ToString().Trim(), "31/03/2019", false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'");
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            else
                            {
                                for (int d = 0; d < dt.Rows.Count; d++)
                                {
                                    ...
                                    sg1_dr["sg1_t4"] = "";
                                    sg1_dr["sg1_t5"] = "";
                                    sg1_dr["sg1_t17"] = dt.Rows[d]["icode"].ToString().Trim();
                                    sg1_dr["sg1_t10"] = "-";
                                    sg1_dt.Rows.Add(sg1_dr);
                                }
                            }
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            //sg1_dr["sg1_f4"] = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, dt.Rows[d]["icode"].ToString().Trim(), "31/03/2019", false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'");                                                        
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                if (frm_vty.Equals("30") || frm_vty.Equals("39"))
                {
                ...
                if (ViewState["sg1"] != null)
                {
                    ...
                    for (int d = 0; d < dt2.Rows.Count; d++)
                    {
                        ...
                        if (dt3.Rows.Count > 0)
                        {
                            ...
                            //sg1_dr["sg1_f4"] = fgen.seek_istock(frm_qstr, frm_cocd, frm_mbr, dt2.Rows[d]["col3"].ToString().Trim(), txtvchdate.Text.Trim(), false, "closing_stk", " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "' AND BTCHNO='" + mbtchno + "'");
                            sg1_dr["sg1_f4"] = getStock(dt.Rows[d]["icode"].ToString().Trim(), dt.Rows[d]["CTRLNO"].ToString().Trim() + dt.Rows[d]["chl"].ToString().Trim() + dt.Rows[d]["BTCHNO"].ToString().Trim(), " and type||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy') !='" + frm_vty + txtvchnum.Text.Trim() + txtvchdate.Text.Trim() + "'");
