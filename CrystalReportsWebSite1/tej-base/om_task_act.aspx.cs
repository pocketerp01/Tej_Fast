...
public partial class om_task_act : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "Task Reporting Form";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "New":
                ...
                SQuery = "select TRCno||TRCdt  as fstr,TRCno as Task_No,TRCdt as Task_DT,CCode,Client_name,Task_type,tgt_Days,trcdd from (Select CCode,Client_Name,trim(Trcno) as TRCno,to_Char(Trcdt,'dd/mm/yyyy') as TRCdt,task_type,Tgt_Days,to_Char(TRCdt,'yyyymmdd') as TRCdd from WB_Task_LOG where branchcd='" + frm_mbr + "' and type='TR' and "+ add_filt +" and nvl(task_Close,'-')!='Y' )  order by Trcdd,tRCno";
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as TAC_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as TAC_Dt,a.Client_Name ,a.Task_type ,a.Client_Person ,a.TRCno ,to_char(a.TRCdt,'dd/mm/yyyy') as TRCdt,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='TA' order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    void newCase(string vty)
    {
        ...
        SQuery = "Select a.* from WB_Task_LOG a where a.branchcd||a.type||trim(a.TrcNO)||to_Char(a.TrcDT,'dd/mm/yyyy')='" + frm_mbr + "TR" + col1 + "' ORDER BY A.SRNO";
        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        if (dt.Rows.Count > 0)
        {
            i = 0;
            txtvchnum.Value = dt.Rows[0]["TrcNO"].ToString().Trim();
            txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["TRCDT"].ToString().Trim()).ToString("dd/MM/yyyy");


            txtlbl4.Value = dt.Rows[i]["CCODE"].ToString().Trim();
            ddModule.Value = dt.Rows[i]["Task_type"].ToString().Trim();
            txtlbl8.Value = dt.Rows[i]["Client_Name"].ToString().Trim();

            txtlbl9.Value = dt.Rows[i]["Tgt_days"].ToString().Trim();

            ddIssueType.Value = dt.Rows[i]["Tsubject"].ToString().Trim();
            
            txtlbl2.Value = dt.Rows[i]["TEAM_MEMBER"].ToString().Trim();
            txtlbl3.Value = dt.Rows[i]["CLIENT_PERSON"].ToString().Trim();
            txtlbl5.Value = dt.Rows[i]["CLIENT_PHONE"].ToString().Trim();
            Text_cl_rmk.Text = dt.Rows[i]["Oremarks"].ToString().Trim();
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (fgen.make_double(txtAssgn.Value) <= 0)
        {
            fgen.msg("-", "AMSG", "Please Enter Time Taken for this Task !!");
            return;
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtvchnum.Value = dt.Rows[i]["TRCNO"].ToString().Trim();
                        txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["tRCDT"].ToString().Trim()).ToString("dd/MM/yyyy");


                        ddAnalysis.Value = dt.Rows[i]["Curr_stat"].ToString().Trim();
                        txtAssgn.Value = dt.Rows[i]["Time_Taken"].ToString().Trim();
                        ddCmode.Value = dt.Rows[i]["Act_mode"].ToString().Trim();
                        ddnextfolo.Value = dt.Rows[i]["Next_Folo"].ToString().Trim();

                        txtrmk.Text = dt.Rows[i]["ORemarks"].ToString().Trim();

                        SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date,to_Char(a.app_Dt,'dd/mm/yyyy') As mapp_date from WB_Task_LOG a where a.branchcd||a.type||trim(a.TRCno)||to_Char(a.tRCdt,'dd/mm/yyyy')='" + frm_mbr + "TR" + txtvchnum.Value + txtvchdate.Value + "' ORDER BY A.SRNO";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT a." + doc_nf.Value + " as TAC_NO,to_char(A." + doc_df.Value + ",'dd/mm/yyyy') as TAC_Dt,a.TRCNO as Task_NO,to_char(A.TRCDT,'dd/mm/yyyy') as Task_Dt,a.Ccode,a.Client_name,a.Tgt_Days,a.Task_type,a.Team_member,a.Time_Taken,a.Act_mode,a.Curr_stat,a.Ent_Dt,a.Ent_by,to_chaR(a.TACDT,'YYYYMMDD') as LAC_DTd  FROM WB_Task_ACT a WHERE a.branchcd='" + frm_mbr + "' and A.TYPE='TA' AND A.TACDT " + PrdRange + "  ORDER BY LAC_DTd,A.TACNO";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        oporow["TASK_TYPE"] = ddModule.Value;
        oporow["Client_name"] = txtlbl8.Value.ToUpper().Trim();

        oporow["tgt_days"] = txtlbl9.Value;
        oporow["Tsubject"] = ddIssueType.Value;

        oporow["Team_Member"] = txtlbl2.Value.ToUpper().Trim();
        oporow["Client_person"] = txtlbl3.Value.ToUpper().Trim();
        oporow["Client_Phone"] = txtlbl5.Value.ToUpper().Trim();

        oporow["TRCNO"] = txtvchnum.Value.Trim();
        oporow["TRCDT"] = txtvchdate.Value.Trim();

        oporow["Curr_stat"] = ddAnalysis.Value.Trim();
        oporow["Time_Taken"] = txtAssgn.Value;
        ...
        string mhd = "";
        mhd = "update wb_Task_LOG set CURR_STAT='" + ddAnalysis.Value.Trim() + "',last_Action='" + ddCmode.Value.Trim() + "',last_Actdt='" + txtDFdt.Value.Trim() + "' where branchcd||type||trim(TRCNO)||to_char(TRCDT,'dd/mm/yyyy')='" + frm_mbr + "TR" + txtvchnum.Value + txtvchdate.Value + "'";
