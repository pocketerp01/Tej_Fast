...
public partial class om_vacation_req : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        frm_vty = "VC";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        lblheader.Text = "Vacation Request";
        frm_tabname = "WB_VACREQ";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_TABNAME", frm_tabname);
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "VC");
        lbl1a.Text = frm_vty;
    }
    //------------------------------------------------------------------------------------
    ...
    void newCase(string vty)
    {
        ...
        frm_vty = vty;
        lbl1a.Text = frm_vty;
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' and VACDT " + DateRange + "", 6, "VCH");
        txtvchnum.Value = frm_vnum;
        txtvchdate.Value = fgen.Fn_curr_dt(frm_cocd, frm_qstr);
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        lblfromtime.Value = dt.Rows[i]["VAC_TIME"].ToString().Trim(); //leavetime
                        ...
                        txtlbl9.Value = dt.Rows[i]["vreason1"].ToString().Trim();
                        lvfrom.Value = Convert.ToDateTime(dt.Rows[i]["vacfrom"].ToString().Trim()).ToString("yyy-MM-dd");
                        lvupto.Value = Convert.ToDateTime(dt.Rows[i]["vacupto"].ToString().Trim()).ToString("yyy-MM-dd");
                        ...
                        txtrmk.Text = dt.Rows[i]["vRemarks"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a." + doc_nf.Value + " as Vac_Req_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as vac_Req_Dt,b.Name,b.deptt_text as departemt,b.desg_text as designation,a.vacFROM as from_dt,A.vacUPTO as return_dt,a.CONT_NAME as alt_contact,A.CONT_NO as alt_contact_no,a.empcode,a.vac_time as vac_time,a.ret_time as return_time,a.tot_days as total_days,a.time_in_hrs, a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,a.app_by,(case when nvl(trim(a.app_by),'-')!='-' then to_char(a.app_Dt,'dd/mm/yyyy') else '-' end) as App_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,empmas b where a.branchcd||trim(A.empcode)=b.branchcd||trim(B.grade)||trim(B.empcode) and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vacdt " + PrdRange + " and a.ent_by='" + frm_uname + "' order by vdd desc,a." + doc_nf.Value + " desc";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        oporow["Vreason1"] = txtlbl9.Value.ToUpper().Trim();
        oporow["VACfrom"] = Convert.ToDateTime(lvfrom.Value.ToUpper().Trim()).ToString("dd/MM/yyyy");
        oporow["VACupto"] = Convert.ToDateTime(lvupto.Value.ToUpper().Trim()).ToString("dd/MM/yyyy");
        oporow["CONT_NAME"] = alt_cont_per.Value.ToUpper().Trim();
        oporow["CONT_NO"] = alt_cont_no.Value.ToUpper().Trim();
        oporow["VRemarks"] = txtrmk.Text;
        oporow["VAC_TIME"] = lblfromtime.Value.ToUpper().Trim(); //LEAVE TIME
