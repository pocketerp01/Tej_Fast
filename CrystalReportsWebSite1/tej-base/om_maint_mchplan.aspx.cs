...
public partial class om_maint_mchplan : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                // pquery = "select trim(icode) as icode,sum(cnt) as tot from (select icode,1 as cnt from item where length(trim(nvl(deac_by,'-')))<=1 and length(trim(icode))>4 union all select distinct icode,-1 as cnt from itwstage where branchcd!='DD' and type='" + lbl1a.Text + "') group by trim(icode) having sum(cnt)>0 ";
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    ...
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as Rec_No,to_char(a.vchdate,'dd/mm/yyyy') as Rec_Dt,a.type,b.name as Section_Name,a.ent_by from pmaint a, (Select type1,name from type where id=':' and type1>'10') b where  trim(a.acode)=trim(b.type1) and a.type='" + frm_vty + "' and a.branchcd='" + frm_mbr + "' and a.vchnum<>'000000' and a.vchdate " + DateRange + " order by a.vchnum desc";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    // SQuery = "select * from pmaint where branchcd='" + frm_mbr + "' and type='"+ frm_vty +"' and trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + col1 + "'";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = "-";
                            sg1_dr["sg1_t4"] = Convert.ToDateTime(dt.Rows[i]["MAINTDT"].ToString().Trim()).ToString("yyyy-MM-dd");
                            ...
                            //sg1_dr["sg1_t4"] = dt.Rows[i]["MAINTDT"].ToString().Trim();

                            //sg1_dr["sg1_t5"] = dt.Rows[i][""].ToString().Trim();
                            //sg1_dr["sg1_t6"] = dt.Rows[i][""].ToString().Trim();
                            //sg1_dr["sg1_t7"] = dt.Rows[i][""].ToString().Trim();
                            //sg1_dr["sg1_t8"] = dt.Rows[i][""].ToString().Trim();
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        sg1_add_blankrows();
                        ViewState["sg1"] = sg1_dt;
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
            SQuery = "select vchnum,to_char(vchdate,'dd/mm/yyyy') as vchdate,mchname,mchcode,maintby as To_be_done, maintdt as plandt,remarks,maintamt as tentative_amt,appven as type from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and vchdate " + PrdRange + " order by vchnum desc";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["MAINTDT"] = Convert.ToDateTime(fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper(), vardate)).ToString("dd/MM/yyyy");
                oporow["MAINTMTH"] = 0;
