...
public partial class MSEEK : System.Web.UI.Page
{
    DataTable dt;
    ...
    public void fill_data()
    {
        ...
        {
            if (frm_cocd == "SFLG") tkrow.Text = "1000";
            dt = new DataTable();
            if (tkrow.Text.Trim() == "" || tkrow.Text.Trim() == "0")
            { vartkrow = 1; }
            else
            { vartkrow = fgen.make_int(tkrow.Text.Trim()); }
            dt = fgen.getdata(frm_qstr, co_cd, "select * from ( " + query1 + " ) where rownum<=" + vartkrow + "");
            ViewState["sg1"] = dt;
    ...
    protected void imgproc_Click(object sender, ImageClickEventArgs e)
    {
        ...
        Value1 = removeduplicate(hf1.Value);
        Value2 = removeduplicate(hf2.Value);
        Value3 = removeduplicate(hf3.Value);
        Value4 = removeduplicate(hf4.Value);
        Value5 = removeduplicate(hf5.Value);
        Value6 = removeduplicate(hf6.Value);
        Value7 = removeduplicate(hf7.Value);
        Value8 = removeduplicate(hf8.Value);
        Value9 = removeduplicate(hf9.Value);
        Value10 = removeduplicate(hf10.Value);
        ...
        if (Value1.Length > 0)
        {
            ...
            switch (HCID)
            {
                ...
                case "IBOX":
                    ...
                    break;
                case "IBOXS":
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "abc", "closePopup('#ContentPlaceHolder1_btniBox');", true);
                    break;
            }
        }
    }
