...
public partial class om_disptgt : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "DT");
        lblheader.Text = "Target Planning";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "Print":
                ...
                break;
            case "PICK":
                SQuery = "Select a.acode as fstr,b.aname as customer,a.acode as code,count(a.icode) As Items from somas a, famst b where trim(A.Acode)=trim(B.Acode) and a.branchcd='" + frm_mbr + "' and a.type in('40','41','4A') and trim(a.icat)<>'Y' and trim(nvl(a.app_by,'-'))!='-' and to_DatE(to_char(delr_Date,'dd/mm/yyyy'),'dd/mm/yyyy')<=to_DaTE('" + txtvchdate.Text.Trim() + "','dd/mm/yyyy') and trim(A.acode) not in (Select distinct trim(Acode) from scratch2 where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and vchdate=to_DatE('" + txtvchdate.Text + "','dd/mm/yyyy')) group by b.aname,a.acode order by b.aname";
                break;

            default:
                frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "SELECT distinct trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_dt,trim(a.acode) as party_code,b.aname as party_name FROM " + frm_tabname + " A,famst b  WHere trim(a.acode)=trim(b.acode) and A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE  " + DateRange + " ORDER BY A.VCHNUM DESC";
                break;
        }
    ...
    protected void btnprint_ServerClick(object sender, EventArgs e)
    {
        hffield.Value = "Print";
        make_qry_4_popup();
        fgen.Fn_open_mseek("Select Entry for Print", frm_qstr);
    }
    //------------------------------------------------------------------------------------
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "PICK":
                    if (col1 == "") return;
                    SQuery = " Select DISTINCT b.aname,a.type,trim(a.acode) as acode,trim(a.icode) as icode,c.iname as ciname,c.CPARTNO,lpad(trim(c.mat1),4,'0') as itseq,c.packsize from somas a, famst b,item c where trim(A.icode)=trim(c.icode) and trim(A.Acode)=trim(B.Acode) and a.branchcd='" + frm_mbr + "' and a.type in('40','41','4A') and trim(a.icat)<>'Y' and trim(nvl(a.app_by,'-'))!='-' and trim(A.acode)='" + col1 + "' order by b.aname,itseq ";
                    dt = new DataTable();
                    dt3 = new DataTable();
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt3.Rows.Count; d++)
                        {
                            ...
                            sg1_dr = sg1_dt.NewRow();
                            sg1_dr["sg1_srno"] = sg1_dt.Rows.Count + 1;
                            sg1_dr["sg1_h1"] = dt3.Rows[d]["acode"].ToString();
                            sg1_dr["sg1_h2"] = dt3.Rows[d]["icode"].ToString();
                            sg1_dr["sg1_h3"] = dt3.Rows[d]["TYPE"].ToString();
                            sg1_dr["sg1_h4"] = "-";
                            sg1_dr["sg1_h5"] = "-";
                            sg1_dr["sg1_h6"] = "-";
                            sg1_dr["sg1_h7"] = "-";
                            ...
                            col2 = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT TRIM(ORDNO)||'~'||TO_CHAR(ORDDT,'DD/MM/YYYY') AS COL1 FROM SOMAS WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + dt3.Rows[d]["type"].ToString().Trim().ToUpper() + "' AND TRIM(aCODE)='" + dt3.Rows[d]["acode"].ToString().Trim().ToUpper() + "' AND TRIM(ICODE)='" + dt3.Rows[d]["icode"].ToString().Trim().ToUpper() + "' AND to_DatE(to_char(delr_Date,'dd/mm/yyyy'),'dd/mm/yyyy')<=to_DaTE('" + txtvchdate.Text.Trim() + "','dd/mm/yyyy') and trim(nvl(app_by,'-'))!='-' and trim(nvl(icat,'N'))!='Y' ORDER BY DELR_DATE DESC", "COL1");
                            if (col2.Length > 3)
                            {
                                sg1_dr["sg1_f1"] = col2.Split('~')[0];
                                sg1_dr["sg1_f2"] = col2.Split('~')[1];
                            }
                            sg1_dr["sg1_f3"] = dt3.Rows[d]["aname"].ToString().Trim().ToUpper();
                            sg1_dr["sg1_f4"] = dt3.Rows[d]["ciname"].ToString().Trim().ToUpper();
                            sg1_dr["sg1_f5"] = dt3.Rows[d]["itseq"].ToString().Trim().ToUpper();
                            sg1_dr["sg1_f6"] = "";
                            sg1_dr["sg1_t1"] = 0;
                            sg1_dr["sg1_t2"] = "";
                            sg1_dr["sg1_t3"] = "";
                            sg1_dr["sg1_t4"] = "";
                            sg1_dr["sg1_t5"] = "";
                            sg1_dr["sg1_t6"] = dt3.Rows[d]["packsize"].ToString().Trim();
                            sg1_dr["sg1_t7"] = "";
                            sg1_dr["sg1_t8"] = "";
                            sg1_dr["sg1_t10"] = "";
                            sg1_dr["sg1_t11"] = "";
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[15].HeaderStyle.Width = 250;
            sg1.Columns[16].HeaderStyle.Width = 250;
            sg1.Columns[17].HeaderStyle.Width = 120;
            sg1.Columns[18].HeaderStyle.Width = 1;
            ...
            sg1.Columns[24].HeaderStyle.Width = 1;
            sg1.Columns[25].HeaderStyle.Width = 1;
            sg1.Columns[26].HeaderStyle.Width = 1;
            sg1.Columns[27].HeaderStyle.Width = 1;
            sg1.Columns[28].HeaderStyle.Width = 1;
            sg1.Columns[29].HeaderStyle.Width = 1;
            sg1.Columns[30].HeaderStyle.Width = 1;
            sg1.Columns[31].HeaderStyle.Width = 1;
            sg1.Columns[32].HeaderStyle.Width = 1;
        }
    }
    //------------------------------------------------------------------------------------
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            ...
            oporow["acode"] = sg1.Rows[i].Cells[0].Text.Trim().ToUpper();
            oporow["icode"] = sg1.Rows[i].Cells[1].Text.Trim().ToUpper();
            oporow["COL45"] = sg1.Rows[i].Cells[2].Text.Trim().ToUpper();

            oporow["prate"] = 0;

            oporow["COL46"] = sg1.Rows[i].Cells[13].Text.Trim().ToUpper();
            oporow["COL47"] = sg1.Rows[i].Cells[14].Text.Trim().ToUpper();


            oporow["col1"] = sg1.Rows[i].Cells[15].Text.Trim().ToUpper();
            oporow["col2"] = sg1.Rows[i].Cells[16].Text.Trim().ToUpper();
            oporow["col3"] = sg1.Rows[i].Cells[17].Text.Trim().ToUpper();

            oporow["col4"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text;
            oporow["col5"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text;
            oporow["col6"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text;

            oporow["col7"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text;
            oporow["col8"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text;
            oporow["col9"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text;

            oporow["col10"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text;
            oporow["col11"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text;
            oporow["col12"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text;

            oporow["col16"] = sg1.Rows[i].Cells[3].Text.Trim().ToUpper();

            oporow["num1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble();
            oporow["num2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text.toDouble();


            //        Dim rst3 As ADODB.Recordset
            //Set rst3 = New ADODB.Recordset
            //'rst3.Open "select DISTINCT ICODE,ORDDT,irate as tot from somas where branchcd='" & mbr & "' and substr(type,1,1)='4' and icat<>'Y' and type<>'48' AND TRIM(ACODE)!='16L007' AND TRIM(ACODE)!='16A004' order by ICODE,orddt desc", consql, adOpenStatic, adLockReadOnly
            //rst3.Open "select DISTINCT a.ICODE,a.ORDDT,a.irate as tot from somas a, famst b where trim(a.acode)=trim(B.acode) and a.branchcd='" & mbr & "' and substr(a.type,1,1)='4' and a.icat<>'Y' and a.type<>'48' and trim(upper(b.del_note))<>'Y' order by a.ICODE,a.orddt desc", consql, adOpenStatic, adLockReadOnly
    ...
    protected void btnupload_Click(object sender, EventArgs e)
    {
        ...
        if (fupl.HasFile)
        {
            ...
            fileSavePath = "DT" + txtvchnum.Text + txtvchdate.Text.Replace("/", "_") + ext;
            lblU.Text = fileSavePath;
            fupl.SaveAs(AppDomain.CurrentDomain.BaseDirectory + "\\tej-base\\Upload\\" + fileSavePath);
        }
    }
    protected void btnPickCustomer_ServerClick(object sender, EventArgs e)
    {
        hffield.Value = "PICK";
        if (frm_ulvl == "M")
        {
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_COL1", frm_uname);
