...
public partial class om_maint_mach : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                {
                    SQuery = "SELECT type1 as fstr,NAME,TYPE1,ID  FROM TYPE WHERE ID=':' and type1>'10' order by type1";
                }
                break;
            case "TICODE":
                Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    if ( doc_addl.Value == "Y")
                    {
                        SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Sheet_no,to_char(a.vchdate,'dd/mm/yyyy') as Sheet_Dt,b.Name as Section_Name,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,typegrp b where b.id='WI' and trim(A.acode)=trim(B.acref) and a.branchcd='" + frm_mbr + "' and b.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' order by vdd desc,a.vchnum desc";
                    }
                    else
                    {
                        SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Sheet_no,to_char(a.vchdate,'dd/mm/yyyy') as Sheet_Dt,b.Name as Section_Name,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,type b where b.id=':' and trim(A.acode)=trim(B.type1) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' order by vdd desc,a.vchnum desc";
                    }
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select name from type where id=':' and trim(upper(type1))=upper(Trim('" + txtlbl4.Text + "'))", "name");
...
//                        txtrmk.Text = dt.Rows[0]["title"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["mchname"].ToString().Trim();
                            sg1_dr["sg1_t2"] = "";
                            sg1_dr["sg1_t3"] = dt.Rows[i]["spec1"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["spec2"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["spec3"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["spec4"].ToString().Trim();
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (doc_addl.Value == "Y")
                        {
                            txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select Name from Typegrp where branchcd='"+ frm_mbr +"' and id='WI' and trim(upper(acref))=upper(Trim('" + txtlbl4.Text + "'))", "name");
                        }
                        else
                        {
                            txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select Name from Type where id=':' and trim(upper(type1))=upper(Trim('" + txtlbl4.Text + "'))", "name");
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["mchname"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["mchcode"].ToString().Trim();
                            ...
                            sg1_dr["sg1_t7"] = dt.Rows[i]["locn"].ToString().Trim();
                            sg1_dr["sg1_t8"] = dt.Rows[i]["maintby"].ToString().Trim();
                            sg1_dr["sg1_t9"] = dt.Rows[i]["maintdt"].ToString().Trim();
                            sg1_dr["sg1_t10"] = dt.Rows[i]["maintmth"].ToString().Trim();

                            sg1_dr["sg1_t11"] = dt.Rows[i]["spec5"].ToString().Trim();
                            sg1_dr["sg1_t12"] = dt.Rows[i]["fasstno"].ToString().Trim();

                            sg1_dr["sg1_t13"] = dt.Rows[i]["itmremarks"].ToString().Trim();
                            sg1_dr["sg1_t14"] = dt.Rows[i]["mchtag"].ToString().Trim();
                            sg1_dr["sg1_t15"] = dt.Rows[i]["purch_Dt"].ToString().Trim();
                            sg1_dr["sg1_t16"] = dt.Rows[i]["purch_cost"].ToString().Trim();

                            sg1_dr["sg1_t17"] = dt.Rows[i]["capa_day"].ToString().Trim();
                            sg1_dr["sg1_t18"] = dt.Rows[i]["rcost_hr"].ToString().Trim();
                            sg1_dr["sg1_t19"] = dt.Rows[i]["ohcost_hr"].ToString().Trim();

                            sg1_dr["sg1_t20"] = dt.Rows[i]["mch_seq"].ToString().Trim();
                            sg1_dr["sg1_t21"] = dt.Rows[i]["conv_mach"].ToString().Trim();
                            
                            sg1_dr["sg1_t23"] = dt.Rows[i]["exp_date"].ToString().Trim();

                            if (dt.Rows[i]["icode"].ToString().Trim() == "1")
                            {
                                sg1_dr["sg1_t22"] = "Y";
                            }
                            else
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            if (doc_addl.Value == "Y")
            {
                SQuery = "select a.Vchnum as Sheet_no,to_char(a.vchdate,'dd/mm/yyyy') as Sheet_Dt,b.name as Section_Name,a.mchname as Machine_Name,a.mchcode as Machine_Code,a.spec2 as Trace_Code,a.capa_day,a.rcost_hr,a.ohcost_hr,a.purch_Dt,a.Spec3 ,a.Spec4,a.Ent_by,a.ent_Dt ,to_Char(a.vchdate,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,typegrp b where b.id='WI' and trim(A.acode)=trim(B.acref) and a.branchcd='" + frm_mbr + "' and b.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' order by vdd desc,a.vchnum desc";
            }
            else
            {
                SQuery = "select a.Vchnum as Sheet_no,to_char(a.vchdate,'dd/mm/yyyy') as Sheet_Dt,b.name as Section_Name,a.mchname as Machine_Name,a.mchcode as Machine_Code,a.spec2 as Trace_Code,a.capa_day,a.rcost_hr,a.ohcost_hr,a.purch_Dt,a.Spec3 ,a.Spec4,a.Ent_by,a.ent_Dt ,to_Char(a.vchdate,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,type b where b.id=':' and trim(A.acode)=trim(B.type1) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' order by vdd desc,a.vchnum desc";
    ...
    protected void btnlbl4_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Machine Group ", frm_qstr);
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Length > 1)
            {
                ...
                oporow["mchname"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text;
                oporow["mchcode"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text;

                if (((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Length <= 1)
                {
                    
                   oporow["mchcode"] = txtlbl4.Text + "/"+ (i+1).ToString().Trim().PadLeft(2,'0')   ;
                    //pad 2 char to be done.
                }

                oporow["spec1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text;
                oporow["spec2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text;
                oporow["spec3"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text;
                oporow["spec4"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text;

                oporow["locn"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text;
                oporow["maintby"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text;
                oporow["maintdt"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text;
                oporow["maintmth"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text);
                
                oporow["spec5"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t11")).Text;
                oporow["fasstno"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t12")).Text;
                oporow["itmremarks"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t13")).Text;
                oporow["mchTag"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t14")).Text;
                oporow["purch_Dt"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t15")).Text;
                oporow["purch_cost"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t16")).Text);
                oporow["capa_day"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t17")).Text);
                oporow["rcost_hr"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t18")).Text);
                oporow["ohcost_hr"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t19")).Text);

                oporow["mch_seq"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t20")).Text);
                oporow["conv_mach"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t21")).Text;

                oporow["icode"] = "0";
                if (((TextBox)sg1.Rows[i].FindControl("sg1_t22")).Text == "Y")
                {
                    oporow["icode"] = "1";
                }
                oporow["exp_date"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t23")).Text;
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F75165":
                SQuery = "SELECT '10' AS FSTR,'Machine Master' as NAME,'10' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "10");
                break;
