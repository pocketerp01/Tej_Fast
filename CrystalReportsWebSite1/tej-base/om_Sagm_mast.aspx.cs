...
public partial class om_Sagm_mast : System.Web.UI.Page
{
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    SQuery = "Select a.*, B.NAME from " + frm_tabname + " a,TYPEGRP b where trim(a.hscode)=trim(b.ACREF) AND B.ID='T1' AND A.FLAG='IM' and a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY a.SRNO";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "fstr", col1);
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    SQuery = "Select a.*, B.NAME from " + frm_tabname + " a,TYPEGRP b where trim(a.hscode)=trim(b.ACREF) AND B.ID='T1' AND A.FLAG='EX' and a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY a.SRNO";                    
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_h5"] = "-";
                            sg1_dr["sg1_h6"] = "-";
                            sg1_dr["sg1_f1"] = dt.Rows[i]["HSCODE"].ToString().Trim();
                            sg1_dr["sg1_f2"] = dt.Rows[i]["NAME"].ToString().Trim();
                            sg1_dr["sg1_t1"] = dt.Rows[i]["QTYIN"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["CIF_VAL"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["VAL_USD"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["WAST_PERC"].ToString().Trim();                            
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            for (i = 0; i < dt1.Rows.Count; i++)
                            {
                                ...
                                sg3_dr["sg3_f1"] = dt1.Rows[i]["HSCODE"].ToString().Trim();
                                sg3_dr["sg3_f2"] = dt1.Rows[i]["NAME"].ToString().Trim();
                                sg3_dr["sg3_t1"] = dt1.Rows[i]["QTYIN"].ToString().Trim();
                                sg3_dr["sg3_t2"] = dt1.Rows[i]["CIF_VAL"].ToString().Trim();
                                sg3_dr["sg3_t3"] = dt1.Rows[i]["VAL_USD"].ToString().Trim();                                
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT trim(a.vchnum) as doc_no ,to_char(a.vchdate,'dd/mm/yyyy') as doc_dt,a.srno,a.flag,a.hscode,b.name,a.licno as licence_no,to_char(a.licdt,'dd/mm/yyyy') as licence_dt, a.qtyin as qty,a.cif_val ,a.val_usd,a.wast_perc,trim(a.dgft_file) as dgft_file,a.val_add FROM WB_LICREC a , typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and a.branchcd!='DD' and a.type='10' and a.vchdate " + PrdRange + " order by a.flag desc ,a.srno ,trim(a.vchnum)";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            fgen.Fn_open_rptlevel("Master Entry for the Period " + fromdt + " to " + todt, frm_qstr);
        }
        else
        {
            Checked_ok = "Y";
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":             
                ...
                else
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_mseek("Select HSN Codes", frm_qstr);
                }
                break;
        }
    }
    ...
    protected void sg3_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg3.Columns[2].HeaderStyle.Width = 100;
            sg3.Columns[3].HeaderStyle.Width = 200;
            sg3.Columns[4].HeaderStyle.Width = 160;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["QTYIN"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper());
                oporow["CIF_VAL"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper());
                oporow["VAL_USD"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().ToUpper());
                oporow["WAST_PERC"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper());
                oporow["ACODE"] = "-";
                oporow["BALQTY"] = 0;
                oporow["ICODE"] = "-";
    ...
    void save_fun2()
    {
        for (i = 0; i < sg3.Rows.Count - 1; i++)
        {
            if (sg3.Rows[i].Cells[3].Text.Length > 2)
            {
                ...
                oporow["EXPVALID"] = txtlbl4a.Text.Trim().ToUpper();
                oporow["DGFT_FILE"] = txtlbl7.Text.Trim().ToUpper();
                oporow["VAL_ADD"] = txtlbl7a.Text.Trim().ToUpper();
                oporow["LICNO"] = txtlbl2.Text.Trim().ToUpper();
                oporow["LICDT"] = txtlbl3.Text.Trim().ToUpper();
                ...
                oporow["QTYIN"] = fgen.make_double(((TextBox)sg3.Rows[i].FindControl("sg3_t1")).Text.Trim().ToUpper());
                oporow["CIF_VAL"] = fgen.make_double(((TextBox)sg3.Rows[i].FindControl("sg3_t2")).Text.Trim().ToUpper());
                oporow["VAL_USD"] = fgen.make_double(((TextBox)sg3.Rows[i].FindControl("sg3_t3")).Text.Trim().ToUpper());
                if (edmode.Value == "Y")
                {
                    oporow["eNt_by"] = ViewState["entby"].ToString();
                    oporow["eNt_dt"] = ViewState["entdt"].ToString();
    ...
    public void cal()
    {
        try
        {
            ...
            {
                imqty += fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());
                imval += fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim());
                txtlbl5.Text = imqty.ToString().Trim();
                txtlbl6.Text = imval.ToString().Trim();
            }
            for (int sg1r = 0; sg1r < sg3.Rows.Count - 1; sg1r++)
            {
                expqty += fgen.make_double(((TextBox)sg3.Rows[sg1r].FindControl("sg3_t1")).Text.Trim());
                expval += fgen.make_double(((TextBox)sg3.Rows[sg1r].FindControl("sg3_t2")).Text.Trim());
