...
public partial class om_prod_bas : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                chk_opt = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(upper(OPT_ENABLE)) as fstr from FIN_RSYS_OPT_pw where branchcd='" + frm_mbr + "' and OPT_ID='W2019'", "fstr");
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_BATCH_INV", chk_opt);
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG3_ROW_ADD_E":
                //pop3
                string batch_inv = fgenMV.Fn_Get_Mvar(frm_qstr, "U_BATCH_INV");
                ...
                if (doc_addl.Value == "Y" || batch_inv == "Y")
                {
                    SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit,a.madeinbr from item a where  length(trim(nvl(a.deac_by,'-'))) <2 AND LENGTH(tRIM(a.ICODE))>=8 and substr(a.icode,1,1) in ('7','9') and (case when length(nvl(madeinbr,'-'))>1 then instr(nvl(madeinbr,'-'),'" + frm_mbr + "') else 1 end)>0 order by a.Iname ";
                }
                else
                {
                    SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit,a.madeinbr from item a where  length(trim(nvl(a.deac_by,'-'))) <2 AND LENGTH(tRIM(a.ICODE))>=8 and trim(A.icode) not in (" + col1 + ") and substr(a.icode,1,1) in ('7','9') and (case when length(nvl(madeinbr,'-'))>1 then instr(nvl(madeinbr,'-'),'" + frm_mbr + "') else 1 end)>0 order by a.Iname ";
                }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            Checked_ok = "N";
            fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Shift Not Filled Correctly !!");
            return;
        }
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            ...
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 2 && ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Length <= 1 && batch_inv == "Y")
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Batch Number Not Filled Correctly at Line " + (i + 1) + "  !!");
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl6.Text = Convert.ToDateTime(fgen.make_def_Date(dt.Rows[0]["invdate"].ToString().Trim(), vardate)).ToString("dd/MM/yyyy");
                        ...
                        txtlbl7a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT name FROM type WHERE id='M' and trim(type1)='" + txtlbl7.Text.Trim() + "'", "name");
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (col1.Trim().Length < 8) SQuery = "select '-' as po_no,'-' as fstr,a.Icode,a.iname,a.cpartno,a.cdrgno,a.irate,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7 from item a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.icode) in (" + col1 + ") order by a.iname";
                        else SQuery = "select '-' as po_no,'-' as fstr,a.Icode,a.iname,a.cpartno,a.cdrgno,a.irate,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7  from item a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.icode) in (" + col1 + ") order by a.iname";

                        if (frm_vty == "17")
                        {
                            SQuery = "select a.*,a.product as iname,a.partno as cpartno from (" + pop_qry + ") a where a.fstr in (" + col1 + ") order by a.product";
                        }
    ...
    protected void btnlbl4_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select " + lbl4.Text.Trim() + "", frm_qstr);
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["invdate"] = fgen.make_def_Date(txtlbl6.Text, vardate);
        ...
        if (frm_vty == "17")
        {
            for (i = 0; i < sg1.Rows.Count - 0; i++)
            {
                if (sg1.Rows[i].Cells[13].Text.Length > 2)
                {
                    ...
                    oporow["no_bdls"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim();
                    oporow["btchno"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper();
                    oporow["desc_"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim();
