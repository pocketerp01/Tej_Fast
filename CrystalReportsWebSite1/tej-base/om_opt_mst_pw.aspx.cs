...
public partial class om_opt_mst_pw : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "FIN_RSYS_OPT_PW";
        lblheader.Text = "Plant Level Config";
        ...
        string popsql;
        popsql = "insert into fin_rsys_opt_pw(branchcd,type,vchnum,vchdate,opt_id,opt_text,opt_enable,opt_param,ent_by,ent_dt)(Select '" + frm_mbr + "',type,vchnum,vchdate,opt_id,opt_text,opt_enable,opt_param,ent_by,ent_dt from fin_rsys_opt where branchcd='00' and opt_id>='W1000' and '" + frm_mbr + "'||type||vchnum not in (Select branchcd||type||vchnum from fin_rsys_opt_pw))";
        fgen.execute_cmd(frm_qstr, frm_cocd, popsql);

        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, "Select Opt_ID,Opt_text,Opt_Enable,Opt_Param,Opt_Param2,Opt_Start,ent_by,to_char(ent_dt,'dd/mm/yyyy') as ent_Dt,EDT_BY,EDT_DT from " + frm_tabname + " where BRANCHCD='" + frm_mbr + "' AND Type='OP' order by OPT_ID ");
        ...
        switch (btnval)
        {
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as Opt_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as Opt_Dt,a.Opt_text ,a.Opt_Enable,a.Opt_param,a.Opt_param2,a.Opt_Start,a.OPT_ID,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and trim(a.type)='" + lbl1a_Text + "' " + cond + " order by vdd desc,a." + doc_nf.Value + " ";
                break;
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (txtlbl3.Value == "" || txtlbl3.Value == "-" || txtlbl3.Value == "0")
                        { }
                        else
                        {
                            txtlbl3.Value = Convert.ToDateTime(dt.Rows[i]["OPT_PARAM2"].ToString().Trim()).ToString("yyyy-MM-dd");
                        }
                        if (dt.Rows[i]["OPT_START"].ToString().Trim().Length < 1)
                        { }
                        else
                        {
                            txtlbl5.Value = Convert.ToDateTime(fgen.make_def_Date(dt.Rows[i]["OPT_START"].ToString().Trim(), vardate)).ToString("yyyy-MM-dd");
                        }
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select Opt_ID,Opt_text,Opt_Enable,Opt_Param,Opt_Param2,Opt_Start,ent_by,to_char(ent_dt,'dd/mm/yyyy') as ent_Dt,EDT_BY,EDT_DT from " + frm_tabname + " where BRANCHCD!='DD' AND Type='OP' order by OPT_ID ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        if (txtlbl5.Value == "" || txtlbl5.Value == "0")
        { }
        else
        {
            oporow["OPT_Start"] = Convert.ToDateTime(txtlbl5.Value.ToUpper().Trim()).ToShortDateString();
        }
