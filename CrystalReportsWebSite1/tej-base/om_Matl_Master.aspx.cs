...
public partial class om_Matl_Master : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        lblheader.Text = "Paper Rate/ Material Master";//material master
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                {
                    if (gr.Cells[22].Text.Trim().Length > 1)
                    {
                        if (col1.Length > 0) col1 = col1 + "," + "'" + gr.Cells[22].Text.Trim() + "'";
                        else col1 = "'" + gr.Cells[22].Text.Trim() + "'";
                    }
                }
                if (col1.Length > 1)
                {
                    SQuery = "select icode as fstr,iname as Item,icode as erp_code from item where substr(trim(icode),1,2) in ('07','08','09') and trim(icode) not in (" + col1 + ") and length(trim(icode))=4 order by icode";
                }
                else
                {
                    SQuery = "select icode as fstr,iname as Item,icode as erp_code from item where substr(trim(icode),1,2) in ('07','08','09') and length(trim(icode))=4 order by icode";
                }
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,a.ID,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where a.branchcd='" + frm_mbr + "' and a.ID='" + frm_vty + "' and vchdate " + DateRange + " order by vdd desc,a.vchnum desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (sg1.Rows.Count <= 1)
        {
            fgen.msg("-", "AMSG", "No Material for Save!!'13'Please Select Some item in Grid First"); return;
        }
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_f8")).Text) == 0)//sg1.Rows[i].Cells[24].Text.Trim().Length > 0 &&
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Price Not Filled Correctly at Line " + (i + 1) + "  !!");
                i = sg1.Rows.Count;
                return;
            }          
        }    
    ...
    void newCase(string vty)
    {
        ...
        lbl1a.Text = vty;
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND ID='" + frm_vty + "' and vchdate " + DateRange + " ", 6, "VCH");
        ...
        //dt = new DataTable();
        //SQuery = "select trim(branchcd)||trim(id)||trim(vchnum)||to_char(vchdate,'dd/mm/yyyy') as fstr, num1,num2,num3,NAME from wb_master where branchcd='" + frm_mbr + "' and id='AR01' and vchdate " + DateRange + "";
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
              ...
              //  fgen.save_info(frm_qstr, frm_cocd, frm_mbr, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2"), fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3"), frm_uname, "MM01", lblheader.Text.Trim() + " Deleted");
                fgen.save_info(frm_qstr, frm_cocd, frm_mbr, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2"), fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3"), frm_uname, frm_vty.Substring(2, 2), lblheader.Text.Trim() + " Deleted");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.* from " + frm_tabname + " a where a.branchcd||a.ID||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = "-";
                            sg1_dr["sg1_f6"] = dt.Rows[i]["col2"].ToString().Trim();
                            sg1_dr["sg1_f7"] = dt.Rows[i]["col1"].ToString().Trim(); 
                            sg1_dr["sg1_f8"] = dt.Rows[i]["num1"].ToString().Trim();                           
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        sg1_add_blankrows();
                        ViewState["sg1"] = sg1_dt;
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        dt = new DataTable();                                          
                        SQuery = "select distinct icode,iname from item where trim(icode) in ('" + col1 + "')";                     
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);                     
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = "-";
                            sg1_dr["sg1_f6"] = dt.Rows[d]["icode"].ToString().Trim();
                            sg1_dr["sg1_f7"] = dt.Rows[d]["iname"].ToString().Trim();                           
                            sg1_dr["sg1_f8"] = "0";                                        
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                    }
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        ...
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[22].Text = dt.Rows[d]["icode"].ToString().Trim();
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[23].Text = dt.Rows[d]["iname"].ToString().Trim();                      
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select vchnum,to_char(vchdate,'dd/mm/yyyy') as vchdate,id,srno,col1 as code,col2 as name,nvl(num1,0) as price,ent_by,to_char(ent_Dt,'dd/mm/yyyy') as ent_dt  from wb_master WHERE branchcd='" + frm_mbr + "' and ID='" + frm_vty + "' and vchdate " + PrdRange + " order by vchnum desc,srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            ...
            else
            {
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (edmode.Value == "Y")
                        {
                            cmd_query = "update " + frm_tabname + " set branchcd='DD' where branchcd||ID||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'";
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            cmd_query = "delete from " + frm_tabname + " where branchcd||ID||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='DD" + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'";
                            fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                        }
                        else
                        {
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[23].HeaderStyle.Width = 300;
            sg1.Columns[24].HeaderStyle.Width = 100;
            //e.Row.Cells[21].Width = 50;
            //e.Row.Cells[22].Width = 300;
            //e.Row.Cells[23].Width = 50;
        }
    }
    //------------------------------------------------------------------------------------
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            ...
            oporow["SRNO"] = i + 1;
            oporow["col2"] = sg1.Rows[i].Cells[22].Text.Trim().ToUpper();//ICODE            
            oporow["col1"] = sg1.Rows[i].Cells[23].Text.Trim().ToUpper();//INAME
            oporow["num1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_f8")).Text.Trim());
            oporow["num2"] = 0;
            oporow["num3"] = 0;
            oporow["num4"] = 0;
            oporow["num5"] = 0;
    ...
    protected void btnC_ServerClick(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow gr in sg1.Rows)
        {
            for (int i = 1; i < 15; i++)
            {
                if (((TextBox)gr.FindControl("sg1_t" + i + "")).Text.ToString().toDouble() >= min && ((TextBox)gr.FindControl("sg1_t" + i + "")).Text.ToString().toDouble() <= max)
                {
                    ((TextBox)gr.FindControl("sg1_t" + i + "")).BackColor = Color.Yellow;
                    ((TextBox)gr.FindControl("sg1_t16")).Text = "Y";
                }
            }
        }
    }
