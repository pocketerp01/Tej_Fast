...
public partial class om_oms_plan : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD":
            case "SG1_ROW_ADD_E":
                SQuery = "SELECT acode AS FSTR,aname AS Client_Name,acode as CCode,substr(EMAIL,1,40) as emailid,mobile,person FROM famst where branchcd!='DD' and substr(acode,1,2)='16' order by acode,aname";
                if (frm_cocd == "TEST")
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.OPLNO)||to_Char(a.OPLDT,'dd/mm/yyyy') as fstr,a.OPLNO as Plan_no,to_char(a.opldt,'dd/mm/yyyy') as plan_Dt,b.UserName,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.opldt,'yyyymmdd') as vdd from " + frm_tabname + " a,evas b where trim(A.tcode)=trim(B.userid) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and opldt " + DateRange + " order by vdd desc,a.oplno desc";
                break;
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.aname full_name,b.acode username,b.mobile contactno,b.email emailid,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,famst b where trim(a.ccode)=trim(b.acode) and a.branchcd||a.type||trim(a.oplno)||to_Char(a.opldt,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    if (frm_cocd == "TEST")
                        SQuery = "Select a.*,b.full_name,b.username,b.contactno,b.emailid,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,evas b where trim(a.ccode)=trim(b.userid) and a.branchcd||a.type||trim(a.oplno)||to_Char(a.opldt,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        txtvchnum.Text = dt.Rows[0]["oplno"].ToString().Trim();
                        txtvchdate.Text = Convert.ToDateTime(dt.Rows[0]["opldt"].ToString().Trim()).ToString("dd/MM/yyyy");
                        ...
                        txtlbl4a.Text = dt.Rows[i]["full_name"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["Month_amt"].ToString().Trim();
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_t3"] = fgen.seek_iname(frm_qstr, frm_cocd, SQuery, "TOTAL_OUTSTANDING");
                            sg1_dr["sg1_t6"] = "";
                            sg1_dr["sg1_t7"] = "";
                            sg1_dr["sg1_t8"] = "";
                            sg1_dr["sg1_t9"] = "";
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["TYPE"] = frm_vty;
                oporow["OPLNO"] = frm_vnum;
                oporow["OPLDT"] = txtvchdate.Text.Trim();

                oporow["SRNO"] = i;
                oporow["tcode"] = txtlbl4.Text;

                oporow["CCODE"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["MONTH_AMT"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F93101":
                SQuery = "SELECT 'OP' AS FSTR,'O/S Collection Plan' as NAME,'OP' AS CODE FROM dual";
                break;
