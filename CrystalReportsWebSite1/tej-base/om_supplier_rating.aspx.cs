...
public partial class om_supplier_rating : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "Suppplier Performance Rating";
    ...
    public void set_Val()
    {
        ...
        {
            case "F97101":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "EQ");
                typePopup = "N";
                break;
            case "F15120":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "1S");
                typePopup = "N";
                break;
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "Print":
                ...
                break;
            case "USR":
                SQuery = "SELECT DISTINCT USERNAME ,USERNAME AS User_Code,FULL_NAME AS User_name FROM EVAS WHERE userid<'000052' and NVL(USERNAME,'-')!='-' ORDER BY USERNAME";
                break;

            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct a.vchnum||to_char(a.vchdate,'dd/mm/yyyy') as fstr, a.vchnum,a.vchdate,a.type,b.aname from SCRATCH a left outer join famst b on Trim(a.acode)=trim(b.acode) where  a.VCHDATE " + DateRange + " AND a.type='" + frm_vty + "' and a.branchcd='" + frm_mbr + "' order by a.vchdate desc ,a.vchnum desc";
                break;
        }
    ...
    void newCase(string vty)
    {
        ...
        if (col1 == "") return;
        frm_vty = vty;
        SQuery = "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' ";
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "New2")
        {
            PrdRange = fgenMV.Fn_Get_Mvar(frm_qstr, "U_PRDRANGE");
            col1 = Request.Cookies["REPLY"].Value.ToString().Trim();
            if (col1 == "Y")
            {
                SQuery = "Select acode ,'-' as icode,sum(iqty_chl) as supplies,sum(iqtyin) as daccept,sum(iexc_addl) as segn,sum(rej_rw) as rejn from ivoucher where branchcd='" + frm_mbr + "' and substr(type,1,2) in('02','03','05','07','09') and vchdate " + PrdRange + " and substr(acode,1,2)='06' AND STORE<>'R' group by acode order by acode";
            }
            else if (col1 == "N")
            {
                SQuery = "Select acode ,icode,sum(iqty_chl) as supplies,sum(iqtyin) as daccept,sum(iexc_addl) as segn,sum(rej_rw) as rejn from ivoucher where branchcd='" + frm_mbr + "' and substr(type,1,2) in('02','03','05','07','09') and vchdate " + PrdRange + " and substr(acode,1,2)='06' AND STORE<>'R' group by acode,icode order by acode,icode";
            }
            else
            {
                SQuery = "Select acode ,icode,sum(iqty_chl) as supplies,sum(iqtyin) as daccept,sum(iexc_addl) as segn,sum(rej_rw) as rejn from ivoucher where branchcd='" + frm_mbr + "' and substr(type,1,2) in('02','03','06','07','09','0J') and vchdate " + PrdRange + " and trim(acode) like '%%' and trim(icode) like '%%' AND STORE<>'R' group by acode,icode order by acode,icode";
            }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    sg4_dr["sg4_f1"] = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where acode='" + dt.Rows[i]["acode"].ToString().Trim() + "'", "aname");
                    sg4_dr["sg4_f2"] = "";
                    sg4_dr["sg4_f3"] = ctr;
                    if (col1 != "N")
                    {
                        sg4_dr["sg4_f4"] = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where acode='" + dt.Rows[i]["acode"].ToString().Trim() + "'", "aname");
                    }
                    else
                    {
                        sg4_dr["sg4_f4"] = fgen.seek_iname(frm_qstr, frm_cocd, "select iname from item where icode='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "iname");
                    }

                    if (frm_cocd == "NIRMPRAG" || frm_cocd == "RIKI" || frm_cocd == "JLAP" || frm_cocd == "ADPF")
                    {
                        if (col1 == "Y")
                        {
                            sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(total) as tot from schedule where branchcd='" + frm_mbr + "' and type in ('66','67') and vchdate " + PrdRange + " and trim(Acode)='" + dt.Rows[i]["acode"].ToString().Trim() + "' ", "tot");
                        }

                        else if (col1 == "N")
                        {
                            sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(total) as tot from schedule where branchcd='" + frm_mbr + "' and type in ('66','67') and vchdate " + PrdRange + " and trim(icode)='" + dt.Rows[i]["icode"].ToString().Trim() + "' ", "tot");
                        }

                        else
                        {
                            sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(total) as tot from schedule where branchcd='" + frm_mbr + "' and type in ('66','67') and vchdate " + PrdRange + " and trim(icode)='" + dt.Rows[i]["icode"].ToString().Trim() + "' and trim(Acode)='" + dt.Rows[i]["acode"].ToString().Trim() + "' ", "tot");
                        }
                    }

                    else
                    {
                        if ((frm_cocd == "CENL" || frm_cocd == "OPPL" || frm_cocd == "RIKI" || frm_cocd == "PMPL" || frm_cocd == "PRPL" || frm_cocd == "INFI" || frm_cocd == "MEGA") && (col1 == "A"))
                        {
                            sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(qtyord) as tot from pomas where branchcd='" + frm_mbr + "' and del_date " + PrdRange + " and substr(type,1,1)='5' and trim(Acode)='" + dt.Rows[i]["acode"].ToString().Trim() + "' and trim(icode)='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "tot");
                        }
                        else
                        {
                            if (frm_cocd == "PRPL" || frm_cocd == "PMPL" || frm_cocd == "OPPL" || frm_cocd == "XDIL" || frm_cocd == "MANU")
                            {
                                if (col1 == "Y")
                                {
                                    sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(qtyord) as tot from pomas where length(trim(delv_item))=10 and branchcd='" + frm_mbr + "' and to_date(trim(delv_item),'dd/mm/yyyy') " + PrdRange + " and substr(type,1,1)='5' and trim(Acode)='" + dt.Rows[i]["acode"].ToString().Trim() + "'", "tot");
                                }
                                else
                                {
                                    sg4_dr["sg4_f5"] = fgen.seek_iname(frm_qstr, frm_cocd, "select sum(qtyord) as tot from pomas where length(trim(delv_item))=10 and branchcd='" + frm_mbr + "' and to_date(trim(delv_item),'dd/mm/yyyy') " + PrdRange + " and substr(type,1,1)='5' and trim(Acode)='" + dt.Rows[i]["acode"].ToString().Trim() + "' and trim(icode)='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "tot");
                                }

                            }
                            else
                            {
                                sg4_dr["sg4_f5"] = dt.Rows[i]["supplies"].ToString().Trim();
                            }
                        }
                    }
                    sg4_dr["sg4_f6"] = dt.Rows[i]["supplies"].ToString().Trim();
                    sg4_dr["sg4_f7"] = dt.Rows[i]["supplies"].ToString().Trim();
                    sg4_dr["sg4_f8"] = dt.Rows[i]["supplies"].ToString().Trim();
                    sg4_dr["sg4_f9"] = (0.4 * ((fgen.make_double(dt.Rows[i]["supplies"].ToString().Trim()) / fgen.make_double(dt.Rows[i]["supplies"].ToString().Trim())) * 100)) + (0.5 * ((fgen.make_double(dt.Rows[i]["supplies"].ToString().Trim()) / fgen.make_double(dt.Rows[i]["supplies"].ToString().Trim())) * 100));
                    sg4_dr["sg4_f10"] = dt.Rows[i]["acode"].ToString().Trim();
                    sg4_dr["sg4_f11"] = dt.Rows[i]["icode"].ToString().Trim();
                    sg4_dr["sg4_f3"] = ctr;
                    sg4_dr["sg4_t1"] = "0";
                    sg4_dr["sg4_t2"] = dt.Rows[i]["supplies"].ToString().Trim();
                    sg4_dr["sg4_t3"] = dt.Rows[i]["daccept"].ToString().Trim();
                    sg4_dr["sg4_t4"] = dt.Rows[i]["segn"].ToString().Trim();
                    sg4_dr["sg4_t5"] = dt.Rows[i]["REJN"].ToString().Trim();
                    sg4_dr["sg4_t6"] = "0";
                    sg4_dr["sg4_t7"] = "0";
                    sg4_dr["sg4_t8"] = "0";
                    sg4_dr["sg4_t10"] = "-";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    SQuery = "select * from scratch where vchnum||to_char(vchdate,'dd/mm/yyyy')='" + col1 + "' and VCHDATE " + DateRange + " AND type='" + frm_vty + "' and branchcd='" + frm_mbr + "' order by col25 asc";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            sg4_dr = sg4_dt.NewRow();
                            sg4_dr["sg4_f3"] = sg4_dt.Rows.Count + 1;

                            sg4_dr["sg4_f1"] = dt.Rows[i]["col25"].ToString().Trim();
                            sg4_dr["sg4_f10"] = dt.Rows[i]["acode"].ToString().Trim();
                            sg4_dr["sg4_f11"] = dt.Rows[i]["icode"].ToString().Trim();
                            sg4_dr["sg4_f2"] = dt.Rows[i]["col16"].ToString().Trim();
                            sg4_dr["sg4_f4"] = dt.Rows[i]["col1"].ToString().Trim();
                            sg4_dr["sg4_f5"] = dt.Rows[i]["col2"].ToString().Trim();
                            sg4_dr["sg4_f6"] = dt.Rows[i]["col3"].ToString().Trim();
                            sg4_dr["sg4_f7"] = dt.Rows[i]["col11"].ToString().Trim();
                            sg4_dr["sg4_f8"] = dt.Rows[i]["col12"].ToString().Trim();
                            sg4_dr["sg4_f9"] = dt.Rows[i]["col13"].ToString().Trim();
                            sg4_dr["sg4_t1"] = dt.Rows[i]["col4"].ToString().Trim();
                            sg4_dr["sg4_t2"] = dt.Rows[i]["col5"].ToString().Trim();
                            sg4_dr["sg4_t3"] = dt.Rows[i]["col6"].ToString().Trim();
                            sg4_dr["sg4_t4"] = dt.Rows[i]["col7"].ToString().Trim();
                            sg4_dr["sg4_t5"] = dt.Rows[i]["col8"].ToString().Trim();
                            sg4_dr["sg4_t6"] = dt.Rows[i]["col9"].ToString().Trim();
                            sg4_dr["sg4_t7"] = dt.Rows[i]["col10"].ToString().Trim();
                            sg4_dr["sg4_t8"] = dt.Rows[i]["col30"].ToString().Trim();
                            sg4_dr["sg4_t10"] = dt.Rows[i]["col31"].ToString().Trim();
                            txtrmk.Text = dt.Rows[i]["Remarks"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        {
            PrdRange = fgenMV.Fn_Get_Mvar(frm_qstr, "U_PRDRANGE");
            SQuery = "SELECT a.CTRLNO as CTRL_NO,to_char(A.CTRLDT,'dd/mm/yyyy') as CTRL_Dt,a.CTRL_type,a.CTRL_spec,a.ACODE as Team_member,a.CTRL_Purpose,a.Ent_by,a.Ent_Dt,a.app_by,a.app_Dt,to_chaR(a.CTRLDT,'YYYYMMDD') as CTRLDTD FROM " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.CTRLdt " + PrdRange + " order by a.CTRLno ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            fgen.Fn_open_rptlevel("List of " + lblheader.Text.Trim() + " for the Period of " + fromdt + " to " + todt, frm_qstr);
            hffield.Value = "-";
        }
        else if (hffield.Value == "New2")
        {
            fgen.msg("-", "PMSG", "1. for Suppp.Wise'13'2. for Item.Wise'13'3. for Sel.App");
        }
        else
        {
            Checked_ok = "Y";
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + frm_vty + txtvchnum.Value.Trim() + frm_CDT1, frm_mbr, frm_vty, txtvchnum.Value.Trim(), txtvchdate.Value.Trim(), "", frm_uname);
                                if (i > 20)
                                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "Trans No " + txtvchnum.Value.Trim() + "'13'We appreciate your Entry to Finsys");
                        }
                        else
                        {
    ...
    public void create_tab4()
    {
        ...
        sg4_dt.Columns.Add(new DataColumn("sg4_f3", typeof(Int32)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f4", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f5", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f6", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f7", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f8", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f9", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f10", typeof(string)));
        sg4_dt.Columns.Add(new DataColumn("sg4_f11", typeof(string)));
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg4.Rows.Count - 1; i++)
        {
            ...
            oporow["SRNO"] = i;
            oporow["acode"] = sg4.Rows[i].Cells[9].Text.Trim();
            oporow["icode"] = sg4.Rows[i].Cells[10].Text.Trim();
            oporow["col1"] = sg4.Rows[i].Cells[3].Text.Trim();
            oporow["col2"] = sg4.Rows[i].Cells[4].Text.Trim();
            oporow["col3"] = sg4.Rows[i].Cells[5].Text.Trim();
            oporow["col4"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t1")).Text.Trim();
            oporow["col5"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t2")).Text.Trim();
            oporow["col6"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t3")).Text.Trim();
            oporow["col7"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t4")).Text.Trim();
            oporow["col8"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t5")).Text.Trim();
            oporow["col9"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t6")).Text.Trim();
            oporow["col10"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t7")).Text.Trim();
            oporow["col30"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t8")).Text.Trim();
            oporow["col11"] = sg4.Rows[i].Cells[6].Text.Trim();
            oporow["col12"] = sg4.Rows[i].Cells[7].Text.Trim();
            oporow["col13"] = sg4.Rows[i].Cells[8].Text.Trim();
            oporow["col31"] = ((TextBox)sg4.Rows[i].FindControl("sg4_t10")).Text.Trim();
            oporow["Remarks"] = txtrmk.Text;
            oporow["col14"] = fromdt + "to" + todt;
            oporow["col16"] = sg4.Rows[i].Cells[1].Text.Trim();
            oporow["col25"] = sg4.Rows[i].Cells[0].Text.Trim();
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F97101":
                SQuery = "SELECT 'EQ' AS FSTR,'Equipment List Logging' as NAME,'EQ' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "EQ");
                break;
    ...
    protected void sg4_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg4.Columns[0].HeaderStyle.Width = 300;
            sg4.Columns[3].HeaderStyle.Width = 300;
        }
    }
}