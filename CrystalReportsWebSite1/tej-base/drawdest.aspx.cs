...
//DEST
public partial class drawdest : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        lblheader.Text = "Drawing Destroy Entry";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "WB_DRAWREC";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "DD");
    ...
    //                query = "SELECT distinct branchcd||type||vchnum||to_char(vchdate,'DDMMYYYY') as fstr,vchnum as entry_no,to_char(vchdate,'dd/mm/yyyy') as entry_date,invno as issue_no,dtype as drawing_type,dno as drawing_no, to_char(rcpt_date,'dd/mm/yyyy') as issue_date,to_char(rdate,'dd/mm/yyyy') as return_target_date,to_char(issue_date,'dd/mm/yyyy') as actual_return_date,rno as revision_no,issue_by ,tno as delay_if_any,remarks as delay_reason,ent_by,ent_dt, to_char(vchdate,'YYYYMMDD') AS VDD from DRAWREC  where branchcd ='" + mbr + "' and type='DD' /*and  vchdate " + daterange + "*/ order by VDD desc,vchnum desc ";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "DT":
                ...
                SQuery = "SELECT distinct branchcd||type||vchnum||to_char(vchdate,'DDMMYYYY') as fstr," + mq1 + " vchnum as entry_no,to_char(vchdate,'dd/mm/yyyy') as entry_date,branchcd as br_code,rno as revision_no,ent_by,ent_dt, to_char(vchdate,'YYYYMMDD') AS VDD from " + frm_tabname + "  where  type='DE' /*and  vchdate " + DateRange + "*/ order by VDD desc,vchnum desc ";
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD" || btnval == "Print_E")
                    SQuery = "SELECT distinct branchcd||type||vchnum||to_char(vchdate,'DD/MM/YYYY') as fstr,vchnum as entry_no,to_char(vchdate,'dd/mm/yyyy') as entry_date,invno as issue_no,dtype as drawing_type,dno as drawing_no, to_char(rcpt_date,'dd/mm/yyyy') as issue_date,to_char(rdate,'dd/mm/yyyy') as return_target_date,to_char(issue_date,'dd/mm/yyyy') as actual_return_date,rno as revision_no,issue_by ,tno as delay_if_any,remarks as delay_reason,ent_by,ent_dt, to_char(vchdate,'YYYYMMDD') AS VDD from " + frm_tabname + "  where branchcd ='" + frm_mbr + "' and type='DD' /*and  vchdate " + DateRange + "*/ order by VDD desc,vchnum desc ";
    ...
    //    hfedmode.Value = "N";
    //    query = "select max(vchnum) as vch from DRAWREC where branchcd = '" + mbr + "' and type='DD' /*and vchdate " + daterange + "*/ ";
            ...
            case "SSEEK":
                switch (btnmode)
                {
                    case "ED":
                        ...
                        break;
                    case "DB":
                        headername = "Destroy By";
                        break;
                    case "SURE":
                        headername = "Confirmation for Deletion";
                        break;
                    case "Edit":
                        headername = "Edit Drawing Destroy  Master";
                        break;
                    case "Del":
                        headername = "Delete Drawing Destroy Master";
                        break;
                    default:
                        break;
                }
            ...
            switch (btnval)
            {
                ...
                case "ED":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtedate.Text = dt.Rows[0]["vchdate"].ToString().Trim().Substring(0, 10);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //sg1.DataBind();
                        //sg1.Visible = true;
                        //yaha upload wala show krna hai jab daata fill hoga
                    }
             #endregion
                    break;
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtdno.Text = dt.Rows[0]["dno"].ToString().Trim();
                        txtrno.Text = dt.Rows[0]["rno"].ToString().Trim();
                        txtdby.Text = dt.Rows[0]["issue_by"].ToString().Trim();
                        hf1.Value = dt.Rows[0]["finvno"].ToString().Trim();
    ...
    //                fgen.Tracking_Detail(co_cd, scode.Substring(0, 2), scode.Substring(4, 6), scode, uname, scode.Substring(2, 2), "DRAWING DESTROY DELETED", vardate, "");
    ...
    protected void btnAtt_Click(object sender, EventArgs e)
    {
        ...
        if (Attch.HasFile)
        {
            ...
            filepath = filepath + "_" + frm_mbr + "_" + "DD" + "_" + txtdocno.Text.Trim() + "_" + txtdate.Text.Replace(@"/", "_") + "~" + Attch.FileName;
            ...
            filepath = Server.MapPath("~/tej-base/UPLOAD/") + "_" + frm_mbr + "_" + "DD" + "_" + txtdocno.Text.Trim() + "_" + txtdate.Text.Replace(@"/", "_") + "~" + Attch.FileName;
            Attch.PostedFile.SaveAs(filepath);
                ...
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                            cmd_query = "delete from filetable where branchcd='" + frm_mbr + "' and type='DD' and vchnum='" + txtdocno.Text.Trim() + "' and to_DatE(to_char(vchdate,'dd/mm/yyyy'),'dd/mm/yyyy')=to_Date('" + txtdate.Text + "','dd/mm/yyyy')";
                            fgen.execute_cmd(frm_qstr, frm_cocd, cmd_query);
                        }
                        else
                        {
                            if (save_it == "Y")
                            {
                                ...
                                vchnum = "";
                                SQuery = "select max(vchnum) as vch from mailbox2 where branchcd = '" + frm_mbr + "' and type='10' /*and vchdate " + DateRange + "*/";
    ...
    void save_fun()
    {
        ...
        oporow["issue_by"] = txtdby.Text;
        oporow["rcpt_date"] = vardate;
    ...
    //            cmd = new OracleCommand("update DRAWREC  set branchcd='DD' where branchcd ='" + mbr + "' and type='DD' and vchnum='" + txtdocno.Text.Trim() + "' and to_DatE(to_char(vchdate,'dd/mm/yyyy'),'dd/mm/yyyy')=to_Date('" + hffielddt.Value + "','dd/mm/yyyy')", con);
    //            cmd.ExecuteNonQuery();
    ...
    //        if (edmode == "Y")
    //        {
    //            cmd = new OracleCommand("delete from DRAWREC  where branchcd='DD' and type='DD' and vchnum='" + txtdocno.Text.Trim() + "' and to_DatE(to_char(vchdate,'dd/mm/yyyy'),'dd/mm/yyyy')=to_Date('" + hffielddt.Value + "','dd/mm/yyyy')", con);
    //            cmd.ExecuteNonQuery();

    //            cmd = new OracleCommand("delete from filetable where branchcd='" + mbr + "' and type='DD' and vchnum='" + txtdocno.Text.Trim() + "' and to_DatE(to_char(vchdate,'dd/mm/yyyy'),'dd/mm/yyyy')=to_Date('" + hffielddt.Value + "','dd/mm/yyyy')", con);
    //            cmd.ExecuteNonQuery();
    //        }

    //        if (edmode == "Y")
    //            AlertMsg("AMSG", "Drawing Destroy No. " + txtdocno.Text + " Dated " + hffielddt.Value + " Updated Successfully.");
    //        else
    //            AlertMsg("AMSG", "Drawing Destroy No. " + txtdocno.Text + " Dated " + hffielddt.Value + " Saved Successfully.");
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        //{
        //    fgen.msg("-", "AMSG", "Please Enter Drawing Type");
        //    return;
        //}
    ...
    //    btnsave.Disabled = true;
    //    ScriptManager.RegisterClientScriptBlock(this, typeof(Page), "confirm", "<script>$(document).ready(function(){ MyConfirm2('Are you sure you want save Drawing Destroy Record?'); });</script>", false);
    ...
    protected void btndby_Click(object sender, ImageClickEventArgs e)
    {
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", "-");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_HF1", "-");
        hffield.Value = "DB";
