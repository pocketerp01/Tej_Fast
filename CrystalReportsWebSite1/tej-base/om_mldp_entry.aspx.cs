...
public partial class om_mldp_entry : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        switch (frm_formID)
        {
            ...
            case "F39111":
                ...
                if (frm_cocd == "SRPF") lblheader.Text = "Extrusion Production";
                frm_tabname2 = "PROD_SHEET";
                frm_tabname = "IVOUCHER"; frm_vty = "15";
                txtstage.Text = "61";
                frm_vty2 = fgenMV.Fn_Get_Mvar(frm_qstr, "U_MLD_PTYPE");

                if (frm_cocd == "AGRM") txtstage.Text = "62";
                ...
                break;
            case "F39106":
                lblheader.Text = "Vulcanization Production";
                ...
                txtShotHrs.Attributes.Add("readonly", "readonly");
                btnstage.Visible = false;
                Lunch_Excess.Visible = false;
                break;
        }
        if (sg1.Rows.Count > 0)
        {
            int widthMake = (sg1.Rows[0].Cells[i].Text.Length) * 10;
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "sg1_Row_Add_E":
                ...
                SQuery = "SELECT A.ICODE,B.INAME AS RAW_MAT,B.CPARTNO AS PARTNO,B.UNIT,A.BTCHNO AS BATCH_NO,sum(A.QTY) AS BAL,a.icode as code FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(IQTYOUT) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3','1') AND TYPE!='39' AND STORE='W' GROUP BY TRIM(ICODE),TRIM(BTCHNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*iqtyout) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' AND STORE='W' GROUP BY TRIM(ICODE),TRIM(BTCHNO) ) A,ITEM B," + frmTabItemOSP + " C WHERE TRIM(a.ICODE)=TRIM(B.ICODE) AND TRIM(A.ICODE)=TRIM(C.IBCODe) AND TRIM(C.ICODE)='" + txticode.Text.Trim() + "' group by a.icode,b.iname,b.cpartno,b.unit,a.btchno having sum(a.qty)>0 ORDER BY B.INAME ";
                string ticode = sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[3].Text.Trim();

                dt = new DataTable();
                dt = fgen.getdata(frm_qstr, frm_cocd, "SELECT ICODE,IBCODE FROM " + frmTabItemOSP + " WHERE TRIM(ICODE)='" + txticode.Text.Trim() + "'");
                string mq = "";
                foreach (DataRow dr in dt.Rows)
                {
                    mq += (mq.Length > 0 ? "," : "") + "'" + dr["ibcode"].ToString().Trim() + "'";
                }


                ticode = "and trim(a.icode)" + (ticode.Length > 2 ? "='" + ticode + "'" : mq.Length > 2 ? " in (" + mq + ")" : "like '%'");
                if (frm_cocd == "SRPF")
                {
                    SQuery = "SELECT A.ICODE,B.INAME AS RAW_MAT,B.CPARTNO AS PARTNO,B.UNIT,A.BTCHNO AS BATCH_NO,sum(A.QTY) AS BAL,a.icode as code FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(IQTYOUT) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' AND STORE='Y' GROUP BY TRIM(ICODE),TRIM(BTCHNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(KCLREELNO) AS BTCHNO,SUM(REELWOUT) AS QTY FROM REELVCH WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' GROUP BY TRIM(ICODE),TRIM(KCLREELNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*iqtyout) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' AND STORE='W' GROUP BY TRIM(ICODE),TRIM(BTCHNO) ) A,ITEM B WHERE TRIM(a.ICODE)=TRIM(B.ICODE)  group by a.icode,b.iname,b.cpartno,b.unit,a.btchno having sum(a.qty)>0 ORDER BY B.INAME ";
                    SQuery = "SELECT A.ICODE,B.INAME AS RAW_MAT,B.CPARTNO AS PARTNO,B.UNIT,A.BTCHNO AS BATCH_NO,sum(A.QTY) AS BAL,a.icode as code FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(nvl(pname,'-'))  AS BTCHNO,SUM(IQTYIN) AS QTY FROM WIPSTK WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='50' GROUP BY TRIM(ICODE),TRIM(nvl(pname,'-')) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(IQTYOUT) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' AND STORE='Y' GROUP BY TRIM(ICODE),TRIM(BTCHNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(KCLREELNO) AS BTCHNO,SUM(REELWOUT) AS QTY FROM REELVCH WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' GROUP BY TRIM(ICODE),TRIM(KCLREELNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*iqtyout) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' AND STORE='W' GROUP BY TRIM(ICODE),TRIM(BTCHNO) ) A,ITEM B WHERE TRIM(a.ICODE)=TRIM(B.ICODE)  group by a.icode,b.iname,b.cpartno,b.unit,a.btchno having sum(a.qty)>0 ORDER BY B.INAME ";
                }
                if (frm_formID == "F39106")
                {
                    SQuery = "SELECT A.ICODE,B.INAME AS PRODUCT,B.CPARTNO AS PARTNO,B.UNIT,A.BTCHNO AS BATCH_NO, (A.QTY) AS BAL,a.icode as code FROM (SELECT ICODE,BTCHNO,SUM(IQTYIN) AS IQTYIN,SUM(IQTYOUT) AS IQTYOUT,SUM(IQTYIN)-SUM(IQTYOUT) AS QTY FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,IQTYOUT AS IQTYIN,0 AS IQTYOUT FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='3A' AND STAGE='" + txtstage.Text.Trim() + "' UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,IQTYOUT AS IQTYIN,0 AS IQTYOUT FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='15' AND STAGE='63') GROUP BY ICODE,BTCHNO HAVING SUM(IQTYIN)-SUM(IQTYOUT)>0) A,ITEM B WHERE TRIM(A.ICODE)=TRIM(b.ICODE) ORDER BY A.BTCHNO";
                }
                break;
            case "MCH":
                ...
                break;
            case "SHIFT":
                SQuery = "SELECT TRIM(TYPE1) AS FSTR,NAME AS SHIFT,TYPE1 AS CODE,place as shft_min,round(case when IS_NUMBER(place)>0 then IS_NUMBER(place)/60 else 0 end) as shft_hrs FROM TYPE WHERE ID='D' AND TYPE1 LIKE '1%' ORDER BY code";
                break;
            case "STG":
                SQuery = "SELECT TRIM(TYPE1) AS FSTR,NAME AS STAGE,TYPE1 AS CODE FROM TYPE WHERE ID='1' AND TYPE1 LIKE '6%' ORDER BY NAME";
                break;
            case "TICODE":
                // replaced A.SHOTS_DAY to a.LP_PC
                mhd = (frm_cocd == "SRPF") ? "LP_PC" : "SHOTS_DAY";
                SQuery = "SELECT TRIM(A.ICODE) AS FSTR,B.INAME AS INAME,B.CPARTNO,B.UNIT,TRIM(A.ICODE)||'-'||TRIM(A.STATIONNO) AS CODE,C.INAME AS MLD_NAME,A.CAVITY,a." + mhd + " AS SHOTS FROM MACHMST A,ITEM B,ITEM C WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND TRIM(A.STATIONNO)=TRIM(c.ICODe) AND A.BRANCHCD = '" + frm_mbr + "' AND A.MCHNUM='" + txtmchcode.Text.Trim() + "' ORDER BY B.INAME";
                break;
            case "OPNAME":
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            frm_vnum = fgen.next_no(frm_qstr, frm_vnum, "SELECT MAX(vCHNUM) AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' AND VCHDATE " + DateRange + " AND STAGE='" + txtstage.Text.Trim() + "' ", 6, "VCH");
        ...
        dt3 = new DataTable();
        SQuery = "select NAME as h2_sg2,TYPE1 as h1_sg2,'-' as Obsv1 FROM TYPEWIP WHERE branchcd='" + frm_mbr + "' and ID='DTC" + txtstage.Text + "' ORDER BY TYPE1";
        if (frm_cocd == "AGRM") SQuery = "select NAME as h2_sg2,TYPE1 as h1_sg2,'-' as Obsv1 FROM TYPEWIP WHERE branchcd not in ('DD','88') and ID='DTC" + txtstage.Text + "' ORDER BY TYPE1";
        dt3 = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        sg2.DataSource = dt3;
        sg2.DataBind();

        dt3 = new DataTable();
        SQuery = "select NAME as h2_sg3,TYPE1 as h1_sg3,0 as SCRP1 FROM TYPEWIP WHERE branchcd='" + frm_mbr + "' and ID='RJC" + txtstage.Text + "' ORDER BY TYPE1";
        if (frm_cocd == "AGRM") SQuery = "select NAME as h2_sg3,TYPE1 as h1_sg3,0 as SCRP1 FROM TYPEWIP WHERE branchcd not in ('DD','88') and ID='RJC" + txtstage.Text + "' ORDER BY TYPE1";
        dt3 = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        SG3.DataSource = dt3;
        SG3.DataBind();
    }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (frm_formID == "F39106")
        {
            if (sg1.Rows.Count > 0)
            {
                txtOkProd.Text = ((TextBox)sg1.Rows[0].FindControl("sg1_t1")).Text;
                txtTotProd.Text = txtOkProd.Text;
            }
        }
        ...
        if (frm_formID != "F39106")
        {
            ...
            {
                if (gr.Cells[3].Text.Trim().Length < 2) continue;
                dt = new DataTable();
                string batchCond = "and nvl(btchno,'-')=" + (gr.Cells[7].Text.Trim() == "-" ? "'-'" : "'" + gr.Cells[7].Text.Trim() + "'");
                SQuery = "SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(IQTYOUT-IQTYIN) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3','1') and ICODE='" + gr.Cells[3].Text.Trim() + "' " + batchCond + " GROUP BY TRIM(ICODE),TRIM(BTCHNO)";
                SQuery = "SELECT A.ICODE,A.BTCHNO AS BATCH_NO,sum(A.QTY) AS QTY FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(KCLREELNO) AS BTCHNO,SUM(REELWOUT) AS QTY FROM REELVCH WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' and ICODE='" + gr.Cells[3].Text.Trim() + "' GROUP BY TRIM(ICODE),TRIM(KCLREELNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*iqtyout) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' AND STORE='W' and ICODE='" + gr.Cells[3].Text.Trim() + "' AND TRIM(VCHNUM)||TO_CHAR(VCHDATE,'DD/MM/YYYY')!='" + txtvchnum.Text + txtvchdate.Text + "' GROUP BY TRIM(ICODE),TRIM(BTCHNO) ) A WHERE TRIM(ICODE)='" + gr.Cells[3].Text.Trim() + "' " + batchCond + " group by a.icode,a.btchno having sum(a.qty)>0 ";
                SQuery = "SELECT A.ICODE,A.BTCHNO AS BATCH_NO,sum(A.QTY) AS QTY FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(nvl(pname,'-')) AS BTCHNO,SUM(IQTYIN) AS QTY FROM WIPSTK WHERE BRANCHCD='" + frm_mbr + "' AND type='50' and trim(ICODE)='" + gr.Cells[3].Text.Trim() + "' GROUP BY TRIM(ICODE),TRIM(nvl(pname,'-')) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(KCLREELNO) AS BTCHNO,SUM(REELWOUT) AS QTY FROM REELVCH WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1) IN ('3') AND TYPE!='39' and ICODE='" + gr.Cells[3].Text.Trim() + "' GROUP BY TRIM(ICODE),TRIM(KCLREELNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*iqtyout) AS QTY FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' AND STORE='W' and ICODE='" + gr.Cells[3].Text.Trim() + "' AND TRIM(VCHNUM)||TO_CHAR(VCHDATE,'DD/MM/YYYY')!='" + txtvchnum.Text + txtvchdate.Text + "' GROUP BY TRIM(ICODE),TRIM(BTCHNO) ) A WHERE TRIM(ICODE)='" + gr.Cells[3].Text.Trim() + "' " + batchCond + " group by a.icode,a.btchno having sum(a.qty)>0 ";
                dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                if (dt.Rows.Count <= 0)
                {
                    SQuery = "SELECT ICODE,BTCHNO,SUM(IQTYIN) AS QTY FROM (SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(IQTYOUT) AS IQTYIN FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND SUBSTR(TYPE,1,1)='3' AND TYPE!='39' and ICODE='" + gr.Cells[3].Text.Trim() + "' " + batchCond + " GROUP BY TRIM(ICODE),TRIM(BTCHNO) UNION ALL SELECT TRIM(ICODE) AS ICODE,TRIM(BTCHNO) AS BTCHNO,SUM(-1*IQTYIN) AS IQTYIN FROM IVOUCHER WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='39' and ICODE='" + gr.Cells[3].Text.Trim() + "' " + batchCond + " AND TRIM(VCHNUM)||TO_CHAR(VCHDATE,'DD/MM/YYYY')!='" + txtvchnum.Text + txtvchdate.Text + "' GROUP BY TRIM(ICODE),TRIM(BTCHNO)) GROUP BY  ICODE,BTCHNO";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                }
                if (dt.Rows.Count <= 0)
                {
                    fgen.msg("-", "AMSG", "Material not issued!!'13'Row No. " + gr.Cells[2].Text.Trim());
                    return;
                }
        ...
        {
            string tol = "";
            tol = fgen.getOption(frm_qstr, frm_cocd, "W2021", "OPT_PARAM");
            if ((fgen.make_double(double_val3, 2) > fgen.make_double(fgen.make_double(double_val1 + double_val2, 4) * (100 + fgen.make_double(tol.Trim())) / 100, 2)) || (fgen.make_double(double_val3, 2) < fgen.make_double(fgen.make_double(double_val1 + double_val2, 4) * (100 - fgen.make_double(tol.Trim())) / 100, 2))) 
            {
                fgen.msg("-", "AMSG", "Using Qty more/less than allowed BOM Qty!!'13'Bom Qty = " + (double_val1 + double_val2) + "'13'Qty with " + fgen.make_double(tol) + "% allowance Using = " + double_val3 + "' 13'Saving not Allowed ");
                return;
            }
        }
        ...
        double_val3 += fgen.make_double(txtLunch.Text);
        if (frm_cocd != "SRPF")
        {
        ...
        {
            txtbatchno.Text = txtstagename.Text.Substring(0, 3) + "/" + fgen.make_double(txtvchnum.Text).ToString() + "/" + frm_myear.Substring(2, 2);
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.iname,B.CPARTNO AS RCPARTNO,B.UNIT AS RUNIT,c.iname AS RINAME,c.cpartno,c.unit from " + frm_tabname + " a,item b,item c where trim(a.icode)=trim(b.icode) and trim(a.rcode)=trim(c.icode) and a.branchcd||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + col1 + "' and a.type in ('" + frm_vty + "','39') and a.stage='" + txtstage.Text.Trim() + "' order by a.srno ";
                    ViewState["fstr"] = col1;
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtLunch.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(mseq) as mseq from prod_sheet where branchcd||trim(type)||trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy')='" + frm_mbr + "62" + col1 + "'", "mseq");
                        create_tab();
                        sg1_dr = null;
                ...
                case "Print":
                    if (col1 == "") return;
                    fgenMV.Fn_Set_Mvar(frm_qstr, "REPID", fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID"));
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        if (hffield.Value == "List")
        {
            ...
            //ORIGINAL SQuery = "select a.* from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vchdate " + DateRange + " order by a.vchnum";

            SQuery = "Select a.type,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,a.rcode ,a.icode ,a.acode,a.O_DEPTT ,a.stage,a.mtime as intime,a.revis_no as out_time,a.et_topay as working_hrs,a.ipack as speed_min,a.cavity as actual_cavity,a.rlprc as target_pcs,a.shots as act_pcs,a.segment_ as shot_hrs,a.iqtyin,a.rej_rw,a.pname as operator,a.btchno,c.iname AS RINAME,P.MCHNAME from ivoucher a,item c,PMAINT P where trim(a.icode)=trim(c.icode) AND TRIM(A.acode)=trim(p.mchcode) and p.branchcd='" + frm_mbr + "' and p.type='10' and a.branchcd='" + frm_mbr + "' and a.type in ('15') and a.stage='" + txtstage.Text.Trim() + "' and a.store='W' and a.vchdate " + DateRange + "  order by a.vchnum,a.srno";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);

            string mq0 = "Select a.type,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,a.rcode ,a.icode ,a.acode ,a.O_DEPTT ,a.stage,a.mtime as intime,a.revis_no as out_time,a.et_topay as working_hrs,a.ipack as speed_min,a.cavity as actual_cavity,a.rlprc as target_pcs,a.shots as act_pcs,a.segment_ as shot_hrs,a.iqtyin,a.rej_rw,a.pname as operator,a.btchno,b.iname,B.CPARTNO AS RCPARTNO,B.UNIT AS RUNIT,c.iname AS RINAME,c.cpartno,c.unit,t.name from ivoucher a,item b,item c,type t where trim(a.icode)=trim(b.icode) and trim(a.rcode)=trim(c.icode) and trim(a.O_DEPTT)=trim(t.type1) and t.id='D' and a.branchcd='" + frm_mbr + "' and a.type in ('39') and a.stage='" + txtstage.Text.Trim() + "' and a.store='W' and a.vchdate " + DateRange + "  order by a.vchnum,a.srno";
            dt2 = new DataTable();
            dt2 = fgen.getdata(frm_qstr, frm_cocd, mq0);

            string mq1 = "Select a.type,a.vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,a.mseq from prod_sheet a where a.branchcd='" + frm_mbr + "' and a.type='62' and a.vchdate " + DateRange + "  order by a.vchnum,a.srno";
            DataTable dt4 = new DataTable();
            dt4 = fgen.getdata(frm_qstr, frm_cocd, mq1);

            dt3 = new DataTable();
            dt3.Columns.Add(new DataColumn("VCHNUM", typeof(string)));
            dt3.Columns.Add(new DataColumn("VCHDATE", typeof(string)));
            dt3.Columns.Add(new DataColumn("ICODE", typeof(string)));
            dt3.Columns.Add(new DataColumn("INAME", typeof(string)));
            dt3.Columns.Add(new DataColumn("COMP_CODE", typeof(string)));
            dt3.Columns.Add(new DataColumn("COMPNAME", typeof(string)));
            dt3.Columns.Add(new DataColumn("MCH_CODE", typeof(string)));
            dt3.Columns.Add(new DataColumn("MACHINE", typeof(string)));
            dt3.Columns.Add(new DataColumn("SHIFT_CODE", typeof(string)));
            dt3.Columns.Add(new DataColumn("SHIFT", typeof(string)));
            dt3.Columns.Add(new DataColumn("IN_TIME", typeof(string)));
            dt3.Columns.Add(new DataColumn("OUT_TIME", typeof(string)));
            dt3.Columns.Add(new DataColumn("BATCHNO", typeof(string)));
            dt3.Columns.Add(new DataColumn("COMP_BATCHNO", typeof(string)));
            dt3.Columns.Add(new DataColumn("WORKING_HRS", typeof(double)));
            dt3.Columns.Add(new DataColumn("SPEED_MIN", typeof(double)));
            dt3.Columns.Add(new DataColumn("ACTUAL_SPEED", typeof(double)));
            dt3.Columns.Add(new DataColumn("TARGET_PCS", typeof(double)));
            dt3.Columns.Add(new DataColumn("ACT_PCS", typeof(double)));
            dt3.Columns.Add(new DataColumn("SHOT_HRS", typeof(double)));
            dt3.Columns.Add(new DataColumn("TOTAL_PROD", typeof(double)));
            dt3.Columns.Add(new DataColumn("OKPROD", typeof(double)));
            dt3.Columns.Add(new DataColumn("TOTREJ", typeof(double)));
            dt3.Columns.Add(new DataColumn("OPERATOR", typeof(string)));
            dt3.Columns.Add(new DataColumn("lunch_tea_in_mins", typeof(string)));
            oporow = null;
            if (dt.Rows.Count > 0 && dt2.Rows.Count > 0)
            {
                foreach (DataRow dr in dt2.Rows)
                {
                    ...
                    oporow["mCH_code"] = dr["acode"].ToString().Trim();
                    oporow["machine"] = fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "MCHNAME");
                    oporow["icode"] = fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "icode");
                    oporow["iname"] = fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "riname");
                    oporow["comp_code"] = dr["icode"].ToString().Trim();
                    oporow["compname"] = dr["iname"].ToString().Trim();
                    oporow["COMP_BATCHNO"] = dr["btchno"].ToString().Trim();
                    oporow["batchno"] = fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "BTCHNO");
                    oporow["speed_min"] = fgen.make_double(dr["speed_min"].ToString().Trim());
                    oporow["ACTUAL_SPEED"] = fgen.make_double(dr["actual_cavity"].ToString().Trim());
                    oporow["act_pcs"] = fgen.make_double(dr["act_pcs"].ToString().Trim());
                    oporow["shift_code"] = dr["O_DEPTT"].ToString().Trim();
                    oporow["shift"] = dr["name"].ToString().Trim();
                    double TotProd = fgen.make_double(dr["actual_cavity"].ToString().Trim()) * fgen.make_double(dr["act_pcs"].ToString().Trim());
                    oporow["TOTAL_PROD"] = TotProd;
                    oporow["TOTREJ"] = fgen.make_double(fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "REJ_RW"));
                    oporow["OKPROD"] = fgen.make_double(fgen.seek_iname_dt(dt, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "IQTYIN"));
                    oporow["in_time"] = dr["intime"].ToString().Trim();
                    oporow["out_time"] = dr["out_time"].ToString().Trim();
                    oporow["target_pcs"] = fgen.make_double(dr["target_pcs"].ToString().Trim());
                    oporow["shot_hrs"] = fgen.make_double(dr["shot_hrs"].ToString().Trim());
                    oporow["operator"] = dr["operator"].ToString().Trim();
                    oporow["working_hrs"] = fgen.make_double(dr["working_hrs"].ToString().Trim());
                    oporow["lunch_tea_in_mins"] = fgen.seek_iname_dt(dt4, "vchnum='" + dr["vchnum"].ToString().Trim() + "' and vchdate='" + dr["vchdate"].ToString().Trim() + "'", "mseq");
                    dt3.Rows.Add(oporow);
                }
            }
            ...
            if (col1 == "Y")
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        do
                        {
                            frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(vchnum)+" + i + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and vchdate " + DateRange + " and stage='" + txtstage.Text.Trim() + "' ", 6, "vch");
    ...
    void save_fun2()
    {
        ...
        oporow["mseq"] = fgen.make_double(txtLunch.Text.Trim());
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[12].HeaderStyle.CssClass = "hidden";
            e.Row.Cells[13].CssClass = "hidden";
            sg1.Columns[13].HeaderStyle.CssClass = "hidden";
            e.Row.Cells[14].CssClass = "hidden";
            sg1.Columns[14].HeaderStyle.CssClass = "hidden";
            e.Row.Cells[15].CssClass = "hidden";
            sg1.Columns[15].HeaderStyle.CssClass = "hidden";
            e.Row.Cells[16].CssClass = "hidden";
            sg1.Columns[16].HeaderStyle.CssClass = "hidden";
        ...
        switch (var)
        {
            ...
            case "sg1_Row_Add":
                ...
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select RM/SF used in Production", frm_qstr);
                }
                else
                {
    ...
    {
        dt = new DataTable();
        SQuery = "SELECT A.IBCODE,B.INAME,b.cpartno,b.unit,A.IBQTY+A.IBWT AS IBQTY FROM " + frmItemOSP + " A,ITEM B WHERE TRIM(A.IBCODE)=TRIM(B.ICODE) AND TRIM(A.ICODE)='" + txticode.Text.Trim() + "' /*AND NVL(TRIM(A.IOMACHINE),'-')='-'*/ ORDER BY A.IBCODE";
        //if (frm_formID == "F39106") SQuery = "select icode as ibcode,iname,cpartno,unit from item where trim(icode)='" + txticode.Text.Trim() + "'";
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        create_tab();
        sg1_dr = null;
