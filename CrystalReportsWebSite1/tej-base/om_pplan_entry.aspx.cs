...
public partial class om_pplan_entry : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                SQuery = "SELECT 'SP' as fstr,'From Sales Plan' as Plan_source,'SP' AS CODE FROM dual union all SELECT 'DF','Direct Feeding' as PR_Type,'DF' AS CODE FROM dual ";
                break;
            case "TICODE":
                //pop2
            ...
            case "SG1_ROW_ADD_E":
                ...
                SQuery = "SELECT distinct a.Icode AS FSTR,a.Iname AS Item_Name,a.Cpartno,a.Cdrgno,a.unit,a.ent_by,a.Icode FROM Item a,itemosp b where trim(A.icode)=trim(B.icode) and a.branchcd!='DD' and length(Trim(a.deac_by))<2  and length(Trim(a.icode))>4 " + col1 + " ORDER BY a.Iname ";
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    if (lbl1a.Text == "10")
                    {
                        SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as Plan_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + DateRange + " order by vdd desc,a.vchnum desc";
                    }
                    else
                    {
                        SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as Projn_no,to_char(a.vchdate,'dd/mm/yyyy') as Projn_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + DateRange + " order by vdd desc,a.vchnum desc";
                    }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            Checked_ok = "N";
            fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Source of Sales Plan Not Filled Correctly !!");
            txtlbl4.Focus();
            return;
        }
        ...
        last_entdt = fgen.seek_iname(frm_qstr, frm_cocd, "select to_char(max(" + doc_df.Value + "),'dd/mm/yyyy') as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text + "'  and vchdate " + DateRange + " and vchdate<to_DaTE('" + txtvchdate.Text + "','dd/mm/yyyy')", "ldt");
        if (last_entdt == "0")
        { }
        else
        {
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,c.iname,c.cpartno as icpartno,c.cdrgno as icdrgno,c.unit as iunit,to_char(a.ent_Dt,'dd/mm/yyyy') as pent_dt,to_char(a.edt_Dt,'dd/mm/yyyy') as pedt_dt  from " + frm_tabname + " a,item c where trim(a.icode)=trim(c.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", lbl1a.Text);

                        txtlbl2.Text = dt.Rows[i]["ent_by"].ToString().Trim();
                        txtlbl3.Text = dt.Rows[i]["pent_Dt"].ToString().Trim();

                        txtlbl4.Text = dt.Rows[i]["dcode"].ToString().Trim();

                        if (txtlbl4.Text == "SP")
                        {
                            txtlbl4a.Text = "From Sales Plan";
                        }
                        else
                        {
                            txtlbl4a.Text = "Direct Feeding";
                        }
                        ...
                        dt2 = new DataTable();
                        SQuery = "select x.icode ,x.opening+nvl(y.cdbts,0)-nvl(y.ccdts,0) as Closing ,x.grp from (select a.icode,a.opening_bal+nvl(b.newop,0) opening,a.grp from  (select substr(f.icode,1,2) as grp,f.icode,sum(nvl(fb.YR_" + frm_myear + ",0)) as opening_bal from item f left outer join (select icode,YR_" + frm_myear + " from itembal where branchcd='" + frm_mbr + "') fb on trim(f.icode)=trim(fb.icode) where substr(f.icode,1,1)='9' and f.branchcd <>'DD' group by substr(f.icode,1,2),f.icode) a left outer join (select v.icode,nvl(sum(v.iqtyin),0)-nvl(sum(v.iqtyout),0) newop from ivoucher v where substr(v.icode,1,1)='9' and v.store='Y' and v.branchcd ='" + frm_mbr + "' and v.vchdate between TO_DATE('" + frm_CDT1 + "','DD/MM/YYYY') and TO_DATE('" + txtvchdate.Text.Trim() + "','DD/MM/YYYY') -1 group by v.icode  ) b on trim(a.icode)=trim(b.icode) ) x left outer join (select v.icode,sum(v.iqtyin) cdbts,sum(v.iqtyout)ccdts from ivoucher v where substr(v.icode,1,1)='9' and v.store='Y' and v.branchcd ='" + frm_mbr + "' and v.vchdate between TO_DATE('" + txtvchdate.Text.Trim() + "','DD/MM/YYYY') and TO_DATE('" + txtvchdate.Text.Trim() + "','DD/MM/YYYY') group by v.icode) y on trim(x.icode)=trim(y.icode)";
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["a1"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["a5"].ToString().Trim();

                            sg1_dr["sg1_t3"] = dt.Rows[i]["a2"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["a3"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["a4"].ToString().Trim();

                            sg1_dr["sg1_t6"] = dt.Rows[i]["a6"].ToString().Trim();
                            //sg1_dr["sg1_t7"] = dt.Rows[i]["a7"].ToString().Trim();
                            sg1_dr["sg1_t7"] = fgen.seek_iname_dt(dt2, "icode='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "Closing");
                            sg1_dr["sg1_t8"] = fgen.make_double(dt.Rows[i]["a6"].ToString()) - fgen.make_double(dt.Rows[i]["a7"].ToString());

                            sg1_dr["sg1_t9"] = dt.Rows[i]["a12"].ToString().Trim();
                            sg1_dr["sg1_t10"] = dt.Rows[i]["a13"].ToString().Trim();
                    ...
                    break;
                case "Print_E":
                    //fgen.Print_Report_ff(co_cd, frm_qstr, mbr, SQuery, "std_PR_REG", "std_PR_REG");
                    //fgen.fin_purc_reps(frm_cocd, frm_mbr, frm_UserID, "F1003", frm_CDT1, frm_mbr + frm_vty + col1);
                    break;
                case "TACODE":
                    ...
                    txtlbl4.Text = col1;
                    txtlbl4a.Text = col2;
                    if (txtlbl4.Text.Trim() == "SP")
                    {
                        SQuery = "Select b.iname,nvl(b.cpartno,'-') as icpartno,nvl(b.cdrgno,'-') as icdrgno,nvl(b.unit,'-') as iunit,trim(a.Icode) as Item_Code,sum(a.target) as Total_qty from mthlyplan a,item b where trim(a.icode)=trim(B.icode) and a.branchcd='" + frm_mbr + "' and a.type like '10%' and to_char(a.vchdate,'yyyymm')=to_char(to_datE('" + txtvchdate.Text + "','dd/mm/yyyy'),'yyyymm') group by b.iname,nvl(b.cpartno,'-'),nvl(b.cdrgno,'-'),nvl(b.unit,'-'),nvl(b.hscode,'-'),trim(a.Icode) order by b.iname";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            for (i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                sg1_dr["sg1_t3"] = dt.Rows[i]["Total_qty"].ToString().Trim();
                                sg1_dr["sg1_t4"] = "";
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as Plan_Dt,c.Iname as Item_Name,C.Cpartno,a.Target as Tgt_Qty,a.Icode,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,item c where trim(A.icode)=trim(c.icode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + PrdRange + " order by vdd ,a.vchnum ,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    protected void btnlbl4_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Source ", frm_qstr);
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["a1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
                oporow["a5"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);

                oporow["a2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
                oporow["a3"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text);
                oporow["a4"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text);

                oporow["a6"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text);
                oporow["a7"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text);

                oporow["a12"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text);
                oporow["a13"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text);



                fd_rate = 0;
                mqry = "Select irate from somas where branchcd='" + frm_mbr + "' and type like '4%' and type not in ('48','4F') and orddt<=to_DaTE('" + txtvchdate.Text + "','dd/mm/yyyy') and trim(upper(icode))='" + sg1.Rows[i].Cells[13].Text.Trim() + "' and trim(icat)<>'Y' and trim(App_by)!='-' order by icode,orddt desc";
                fd_rate = fgen.make_double(fgen.seek_iname(frm_qstr, frm_cocd, mqry, "irate"));
