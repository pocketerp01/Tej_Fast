...
public partial class om_hmas9 : System.Web.UI.Page
{
        ...
        switch (btnval)
        {
            ...
            default:
                ...
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy') as fstr,a.type,a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_dt,to_char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a WHERE a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' AND a.VCHDATE  " + DateRange + " ORDER BY vdd desc,entry_no DESC";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (edmode.Value == "")
        {
            SQuery = "select distinct trim(vchnum) as vchnum,to_char(vchdate,'dd/mm/yyyy') as vchdate,to_char(vchdate,'yyyymmdd') as vdd from payhol where branchcd='" + frm_mbr + "' and type='10' and vchdate " + DateRange + " order by vdd,vchnum desc";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
            if (dt.Rows.Count > 0)
            {
                fgen.msg("-", "AMSG", "Holiday Master is already created for This Year at Entry No." + dt.Rows[0]["vchnum"].ToString().Trim() + " Dated " + dt.Rows[0]["vchdate"].ToString().Trim());
                return;
            }
        }
        ...
        for (int i = 0; i < sg1.Rows.Count; i++)
        {
            ...
            {
                if (((TextBox)(sg1.Rows[i].FindControl("sg1_t2"))).Text == "-" || ((TextBox)(sg1.Rows[i].FindControl("sg1_t2"))).Text == "")
                {
                    fgen.msg("-", "AMSG", "Please Fill Valid Date At Line No. " + sg1.Rows[i].Cells[12].Text);
                    return;
                }
                else if (((TextBox)(sg1.Rows[i].FindControl("sg1_t2"))).Text.Length > 2)
                {
                    dhd = fgen.ChkDate(((TextBox)(sg1.Rows[i].FindControl("sg1_t2"))).Text);
                    if (dhd == 0)
                    {
                        fgen.msg("-", "AMSG", "Invalid Date At Line No. " + sg1.Rows[i].Cells[12].Text);
                        return;
                    }
                }
            }
    ...
    protected void btnlist_ServerClick(object sender, EventArgs e)
    {
        ...
        set_Val();
        hffield.Value = "List";
        SQuery = "select distinct trim(vchnum) as vchnum,to_char(vchdate,'dd/mm/yyyy') as vchdate,hol_title,hol_date,icat as done,ent_by,to_char(ent_dt,'dd/mm/yyyy') as ent_dt,srno from payhol where branchcd='" + frm_mbr + "' and type='10' and vchdate " + DateRange + " Order by srno";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f5"] = "-";
                            sg1_dr["sg1_f6"] = "-";
                            sg1_dr["sg1_t1"] = dt.Rows[i]["hol_title"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["hol_date"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["icat"].ToString().Trim();
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        for (int i = 0; i < 8; i++)
                        {
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[19].HeaderStyle.Width = 450;
        ...
        switch (var)
        {
            case "SG1_RMV":
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgen.msg("-", "CMSG", "Are You Sure!! You Want to Remove This Holiday From The List");
                }
                break;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count; i++)
        {
            if (((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().Length > 2)
            {
                ...
                oporow["srno"] = i + 1;
                oporow["hol_title"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper();
                oporow["hol_date"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper();
                oporow["icat"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().ToUpper();
                oporow["ucode"] = "-";
                oporow["uname"] = "-";
                oporow["ufreq"] = 0;
                oporow["tgtdt"] = "-";
