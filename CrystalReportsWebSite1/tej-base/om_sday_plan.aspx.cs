...
public partial class om_sday_plan : System.Web.UI.Page
{
    ...
    string save_it;
    string Prg_Id;
    string pk_error = "Y", chk_rights = "N", DateRange, PrdRange, ind_Ptype;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                doc_addl.Value = "N";
                chk_opt = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(upper(OPT_ENABLE)) as fstr from FIN_RSYS_OPT where OPT_ID='W0038'", "fstr");
                if (chk_opt == "Y")
                {
                    doc_addl.Value = "Y";
                }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG3_ROW_ADD_E":
                ...
                SQuery = "SELECT distinct a.Icode as FStr,a.Iname as Item_Name,a.Icode,a.cpartno,a.cdrgno,a.unit from item a,itemosp b where trim(a.icode)=trim(B.icode) and length(trim(nvl(a.deac_by,'-'))) <2  and substr(a.icode,1,1) in ('7','9') order by a.Iname ";
                ...
                {
                    if (txtlbl7.Text == "61")
                        SQuery = "select Distinct a.icode as fstr,b.iNAME as Item_Name,b.CPARTNO as Item_Code,a.Icode as ICODE,C.ORDNO,C.ORDDT,C.BAL_Q,b.unit,b.ent_by from itemosp a, item b,(SELECT TRIM(ICODE) AS ICODE,TRIM(ORDNO) AS ORDNO,TRIM(ORDDT) AS ORDDT,SUM(QTYORD)-SUM(PLAN) as BAL_Q FROM (SELECT ICODE,vchnum as ordno,TO_CHAR(vchdate,'DD/MM/YYYY') AS orddt,col1 as QTYORD,0 AS PLAN FROM purch_plan WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='10' AND to_char(vchdate,'yyyymm')='" + Convert.ToDateTime(txtvchdate.Text).ToString("yyyyMM") + "' and psectcd='" + txtlbl7.Text.Trim() + "' UNION ALL SELECT ICODE,JOB_NO,JOB_dT,0 AS QTYORD,A1 AS PLAN FROM PROD_SHEET WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='10' AND to_char(vchdate,'yyyymm')='" + Convert.ToDateTime(txtvchdate.Text).ToString("yyyyMM") + "') GROUP BY TRIM(ICODE),TRIM(ordno),TRIM(orddt) HAVING SUM(QTYORD)-SUM(PLAN)>0 ) C  WHERE TRIM(a.ICODe)=TRIM(c.ICODE) AND a.branchcd!='DD' and TRIM(A.ICODE)=TRIM(B.ICODE) and a.type='BM' order by b.iname ";
                    else
                        SQuery = "select Distinct a.icode as fstr,b.iNAME as Item_Name,b.CPARTNO as Item_Code,a.Icode as ICODE,C.ORDNO,C.ORDDT,C.BAL_Q,b.unit,b.ent_by from itemosp a, item b,(SELECT TRIM(ICODE) AS ICODE,TRIM(ORDNO) AS ORDNO,TRIM(ORDDT) AS ORDDT,SUM(QTYORD)-SUM(PLAN) as BAL_Q FROM (SELECT ICODE,vchnum as ordno,TO_CHAR(vchdate,'DD/MM/YYYY') AS orddt,num01 as QTYORD,0 AS PLAN FROM sl_plan WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='SL' AND to_char(vchdate,'yyyymm')='" + Convert.ToDateTime(txtvchdate.Text).ToString("yyyyMM") + "'  UNION ALL SELECT ICODE,JOB_NO,JOB_dT,0 AS QTYORD,A1 AS PLAN FROM PROD_SHEET WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='10' AND to_char(vchdate,'yyyymm')='" + Convert.ToDateTime(txtvchdate.Text).ToString("yyyyMM") + "') GROUP BY TRIM(ICODE),TRIM(ordno),TRIM(orddt) HAVING SUM(QTYORD)-SUM(PLAN)>0) C  WHERE TRIM(a.ICODe)=TRIM(c.ICODE) AND a.branchcd!='DD' and TRIM(A.ICODE)=TRIM(B.ICODE) and a.type='BM' order by b.iname ";
                }
            ...
            case "sg1_t5":
                SQuery = "select trim(mchcode) as fstr, Mchname ,mchcode as Code from pmaint where branchcd='" + frm_mbr + "' and type='10' order by mchname";
                SQuery = "SELECT DISTINCT TRIM(MCHNUM) as fstr,MCHNAME,round(is_number(CAVITY) * is_numbeR(shots_day)) as capicity,MCHNUM AS CODE FROM MACHMST WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='01' AND TRIM(ICODE)='" + (string)ViewState["icode"] + "' ORDER BY MCHNAME ";
                if (ind_Ptype == "01")
                    SQuery = "SELECT DISTINCT TRIM(MCHNUM) as fstr,MCHNAME,round(is_number(CAVITY) * is_numbeR(shots_day)) as capicity,MCHNUM AS CODE FROM MACHMST WHERE BRANCHCD='" + frm_mbr + "' ORDER BY MCHNAME ";
                break;
            case "EMPNAME":
                SQuery = "SELECT DISTINCT REMARKS2 AS FSTR,REMARKS2 AS NAME,'-' AS VAL FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' AND NVL(REMARKS2,'-')!='-' ORDER BY REMARKS2 ";
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as entry_no,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as entry_Dt,b.Name as Deptt,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,type b where trim(a.acode)=trim(B.type1) and b.id='1' and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' AND a." + doc_df.Value + " " + DateRange + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtlbl7.Text.Trim() == "61")
        {
            for (int i = 0; i < sg1.Rows.Count - 1; i++)
            {
                if (((TextBox)sg1.Rows[i].FindControl("sg1_t17")).Text.Trim() == "-" || ((TextBox)sg1.Rows[i].FindControl("sg1_t17")).Text.Trim() == "")
                {
                    fgen.msg("-", "AMSG", "Please Select Machine At Line No. " + sg1.Rows[i].Cells[12].Text.Trim());
                    return;
                }
            }
        }
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                if (frm_vty == "17")
                {
                    fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " a where a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + "3C" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
                }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT name FROM type WHERE id='D' and trim(type1)='" + txtlbl4.Text.Trim() + "'", "name");
                        ...
                        txtlbl8.Text = dt.Rows[i]["PREVCODE"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            //sg1_dr["sg1_t9"] = dt.Rows[i]["invdate"].ToString().Trim();                                                        
                            sg1_dr["sg1_t17"] = dt.Rows[i]["remarks"].ToString().Trim();

                            if (frm_vty != "17")
                            {
                                sg1_dr["sg1_t2"] = dt.Rows[i]["a2"].ToString().Trim();
                                sg1_dr["sg1_t3"] = dt.Rows[i]["a3"].ToString().Trim();
                                sg1_dr["sg1_t4"] = dt.Rows[i]["a4"].ToString().Trim();

                                sg1_dr["sg1_t5"] = dt.Rows[i]["remarks2"].ToString().Trim();
                            }
                        ...
                        // REEL TABLE
                        SQuery = "SELECT A.*,b.iname FROM REELVCH A,item b WHERE 1=2 and trim(a.icodE)=trim(B.icode) and a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' order by a.srno";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                ...
                case "sg1_t5":
                    if (col1.Length > 1)
                    {
                        if (frm_cocd == "AGRM")
                        {
                            ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t17")).Text = col2;
                            ViewState["hf1.Value"] = hf1.Value;
                            doc_addl.Value = col3;
                            hffield.Value = "sg1_t5value";
                            fgen.Fn_ValueBox("Please Enter Hour", frm_qstr);
                        }
                        else
                        {
                            ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t17")).Text = col2;
                            ViewState["hf1.Value"] = hf1.Value;
                            doc_addl.Value = col3;
                            if (ViewState["hf1.Value"] != null)
                                hf1.Value = (string)ViewState["hf1.Value"];
                            ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t1")).Text = Math.Round(doc_addl.Value.ToString().toDouble(2) * col1.toDouble(2), 2).ToString();

                            SQuery = "select sum(ibqty) as ibqty from itemosp where trim(icodE)='" + sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[13].Text.Trim() + "' AND SUBSTR(IBCODE,1,2) IN ('70','75') ";
                            col1 = fgen.seek_iname(frm_qstr, frm_cocd, SQuery, "ibqty");
                            ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t2")).Text = Math.Round(((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t1")).Text.ToString().toDouble() * col1.toDouble(), 4).ToString();

                            SQuery = "select distinct icode,ibcode from itemosp where trim(icodE)='" + sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[13].Text.Trim() + "' AND SUBSTR(IBCODE,1,2) IN ('70','75') ";
                            dt = new DataTable();
                            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                foreach (DataRow dr in dt.Rows)
                                {
                                    col1 = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT DISTINCT MAIN_ISSUE_NO FROM ITEMOSP WHERE TRIM(ICODE)='" + dr["ibcode"].ToString().Trim() + "'", "MAIN_ISSUE_NO");
                                    batchQty += col1.toDouble();
                                }

                                ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t4")).Text = Math.Round(((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t2")).Text.ToString().toDouble() / batchQty, 4).ToString();
                            }

                            if (ind_Ptype != "01")
                            {
                                ViewState["hf1.Value"] = hf1.Value;
                                hffield.Value = "EMPNAME";
                                make_qry_4_popup();
                ...
                case "EMPNAME":
                    ...
                    if (col1.Length > 1)
                    {
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t5")).Text = col1;
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t5")).Focus();
                    }
                    else ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t5")).Focus();
                    break;
                case "BTN_10":
                    if (col1.Length <= 0) return;
                    txtlbl10.Text = col2;
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        SQuery = "select '-' as po_no,'-' as fstr,a.Icode,a.iname,a.cpartno,a.cdrgno,a.irate,a.unit,a.hscode,0 as num4,0 as num5,0 as num6,0 as num7 from item a where trim(a.icode) in (" + col1 + ")";
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            ViewState["icode"] = dt.Rows[d]["icode"].ToString().Trim();
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[16].Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL5").ToString().Trim().Replace("&amp", "");
                ...
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            if (frm_vty == "17")
                            {
                                fgen.execute_cmd(frm_qstr, frm_cocd, "update " + frm_tabname + " set branchcd='DD' where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + "3C" + fgenMV.Fn_Get_Mvar(frm_qstr, "fstr") + "'");
                            }
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            if (frm_vty == "17")
                            {
                                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='DD" + "3C" + fgenMV.Fn_Get_Mvar(frm_qstr, "fstr") + "'");
                            }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["a1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());
                oporow["iqtyin"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim());

                oporow["PREVCODE"] = txtlbl8.Text;

                oporow["a2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim());
                oporow["a3"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim());
                oporow["a4"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim());

                oporow["REMARKS2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim().ToUpper();
                oporow["remarks"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t17")).Text.Trim().ToUpper();
                if (edmode.Value == "Y")
                {
