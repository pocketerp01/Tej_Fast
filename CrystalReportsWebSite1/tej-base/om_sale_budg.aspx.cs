...
public partial class om_sale_budg : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "96");
        //switch (Prg_Id)
        //{
        //    case "F99101":
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                SQuery = "SELECT type1 AS FSTR,name as Person_Name,type1 as code FROM typegrp where id='A' and type1 like '16%' and branchcd!='DD' order by name";
            ...
            SQuery = "SELECT acode AS FSTR,Aname AS Customer,acode as Code,addr1,addr2,gst_no FROM famst where branchcd!='DD' and trim(bssch)='"+ txtlbl4.Text +"' order by Aname";
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    ...
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as plan_Dt,b.name,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,typegrp b where b.id='A' and trim(A.acode)=trim(B.type1) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vchdate "+ DateRange + " order by vdd desc,a.vchnum desc";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    SQuery = "Select a.*,b.aname,b.staten,b.gst_no,b.person,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,famst b where trim(a.icode)=trim(b.acode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select name from typegrp where branchcd!='DD' and id='A' and trim(upper(type1))=upper(Trim('" + txtlbl4.Text + "'))", "name");
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f3"] = dt.Rows[i]["staten"].ToString().Trim();
                            sg1_dr["sg1_f4"] = dt.Rows[i]["gst_no"].ToString().Trim();
                            sg1_dr["sg1_f5"] = dt.Rows[i]["person"].ToString().Trim();



                            sg1_dr["sg1_t1"] = dt.Rows[i]["total"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["day1"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["day2"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["day3"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["day4"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["day5"].ToString().Trim();
                            sg1_dr["sg1_t7"] = dt.Rows[i]["day6"].ToString().Trim();
                            sg1_dr["sg1_t8"] = dt.Rows[i]["day7"].ToString().Trim();
                            sg1_dr["sg1_t9"] = dt.Rows[i]["day8"].ToString().Trim();
                            sg1_dr["sg1_t10"] = dt.Rows[i]["day9"].ToString().Trim();
                            sg1_dr["sg1_t11"] = dt.Rows[i]["day10"].ToString().Trim();
                            sg1_dr["sg1_t12"] = dt.Rows[i]["day11"].ToString().Trim();
                            sg1_dr["sg1_t13"] = dt.Rows[i]["day12"].ToString().Trim();
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (col1.Length > 6) SQuery = "select * from famst where trim(acode) in (" + col1 + ")";
                        else SQuery = "select * from famst where trim(acode)='" + col1 + "'";
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_f3"] = dt.Rows[d]["staten"].ToString().Trim();
                            sg1_dr["sg1_f4"] = dt.Rows[d]["gst_no"].ToString().Trim();
                            sg1_dr["sg1_f5"] = dt.Rows[d]["person"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.OPLNO as Plan_no,to_char(a.opldt,'dd/mm/yyyy') as Plan_Dt,c.Username as Plan_by,b.Full_name,b.Username as Client_Code,a.Month_Amt as Month_Tgt,a.Remarks,a.CCode,a.Ent_by,a.ent_Dt ,to_Char(a.opldt,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,evas b,evas c where trim(A.ccode)=trim(B.userid) and trim(A.tcode)=trim(c.userid) and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and opldt " + PrdRange + " order by vdd ,a.oplno ,a.srno";
            ...
            else
            {
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        fgen.save_Mailbox2(frm_qstr, frm_cocd, frm_formID, frm_mbr, lblheader.Text + " # " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR"), frm_uname, edmode.Value);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["total"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);

                oporow["day1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);
                oporow["day2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
                oporow["day3"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text);
                oporow["day4"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text);
                oporow["day5"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text);

                oporow["day6"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text);
                oporow["day7"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text);
                oporow["day8"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text);
                oporow["day9"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t10")).Text);
                oporow["day10"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t11")).Text);

                oporow["day11"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t12")).Text);
                oporow["day12"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t13")).Text);
    ...
    void Type_Sel_query()
    {
        ...
        switch (Prg_Id)
        {
            ...
            case "F47112":
                SQuery = "SELECT 'OP' AS FSTR,'Sales Budget' as NAME,'OP' AS CODE FROM dual";
