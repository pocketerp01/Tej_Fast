...
public partial class om_prt_prod : System.Web.UI.Page
{
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        //if (fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1").Length > 0 || fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2").Length > 0 || fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3").Length > 0)
        {
            ...
            switch (val)
            {
                ...
                case "F20132":
                    SQuery = "SELECT '" + fromdt + "' as FRMDATE,'" + todt + "' AS TODATE,TRIM(A.BRANCHCD) AS BRANCHCD,TRIM(A.VCHNUM) AS VCHNUM,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS VCHDATE,TRIM(A.ICODE) AS ICODE,A.SRNO,TRIM(A.ACODE) AS ACODE,(CASE WHEN A.PONUM='-' THEN '000000' ELSE A.PONUM END) AS PONUM,TO_CHAR(A.PODATE,'DD/MM/YYYY') AS PODATE,A.INVNO,TO_CHAR(A.INVDATE,'DD/MM/YYYY') AS INVDATE,a.type as grp,A.REFNUM,TO_CHAR(A.REFDATE,'DD/MM/YYYY') AS REFDATE,A.IQTY_CHL,A.NARATION,B.VCHNUM AS MRRNO,TO_CHAR(B.VCHDATE,'DD/MM/YYYY') AS MRRDATE,B.CESS_PU,B.DESC_,B.TYPE,B.MODE_TPT,I.INAME,I.UNIT,F.ANAME,TRIM(F.ADDR1)||TRIM(F.ADDR2) AS ADDRESS FROM FAMST F,ITEM I, IVOUCHERP A LEFT OUTER JOIN IVOUCHER B ON TRIM(A.BRANCHCD)||TRIM(A.VCHNUM)||TO_CHAR(A.VCHDATE,'DD/MM/YYYY')||TRIM(A.ACODE)||TRIM(A.ICODE) =TRIM(B.BRANCHCD)||TRIM(B.GENUM)||TO_CHAR(B.GEDATE,'DD/MM/YYYY')||TRIM(B.ACODE)||TRIM(B.ICODE)  AND SUBSTR(B.TYPE,0,1)='0'  WHERE TRIM(A.ICODE)=TRIM(I.ICODE) AND TRIM(A.ACODE)=TRIM(F.ACODE)  AND  A.BRANCHCD='" + mbr + "' AND A.TYPE='00' AND A.VCHDATE " + xprdrange + " ORDER BY A.SRNO";
                    fgen.Print_Report(co_cd, frm_qstr, mbr, SQuery, "std_GE_REG", "std_GE_REG");
                    break;
