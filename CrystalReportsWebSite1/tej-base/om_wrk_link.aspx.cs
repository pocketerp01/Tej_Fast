...
public partial class om_wrk_link : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                //pop1
                SQuery = "SELECT Type1,Name as Client_Name,Type1 AS CODE,ID2 as Ref FROM type WHERE id='#' and id2='CL' ORDER BY Name ";
            ...
            case "SG1_ROW_ADD_E":
                ...
                SQuery = "SELECT Type1,Name as Task_Name,Type1 AS CODE,ID2 as Ref FROM type WHERE id='#' and id2='TS' and trim(type1) not in (" + col1 + ")  ORDER BY Name ";
                break;
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD*":
                    ...
                    //if (col1 == "") return;
                    //clearctrl();
                    //SQuery = "Select a.*,b.Iname from " + frm_tabname + " a,FIN_MSYS b where trim(a.frm_name)=trim(b.id) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    //dt = new DataTable();
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.name from " + frm_tabname + " a,type b where trim(a.icode)=trim(b.type1) and b.id2='CL' and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_f2"] = dt.Rows[i]["remarks"].ToString().Trim();
                            sg1_dr["sg1_f3"] = "-";
                            sg1_dr["sg1_f4"] = "-";
                            sg1_dr["sg1_f5"] = "-";
                            ...
                            sg1_dr["sg1_t4"] = dt.Rows[i]["substg"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["machcd"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["scrpcd"].ToString().Trim();
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":
                ...
                else
                {
                    ...
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Item", frm_qstr);
                    //fgen.Fn_open_mseek("Select Effort", frm_qstr);
                }
                break;
        }
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["stagec"] = sg1.Rows[i].Cells[13].Text;
                oporow["remarks"] = sg1.Rows[i].Cells[14].Text;
                oporow["mtime"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
                oporow["mtime1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);
                

                oporow["opcode"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text;
                oporow["substg"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text;
                oporow["machcd"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text;
                oporow["scrpcd"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text;
    ...
    {
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        SQuery = "SELECT 'WL' AS FSTR,'Customer,Effort Target' as NAME,'WL' AS CODE FROM dual";
