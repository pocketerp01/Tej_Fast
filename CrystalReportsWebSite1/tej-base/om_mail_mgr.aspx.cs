...
public partial class om_mail_mgr : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "WB_mail_mgr";

        switch (Prg_Id)
        {
            case "F99119":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "MM");
                break;
            case "F99120":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "NF");
                break;
            case "F99123":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "MH");
                break;
            case "F99124":
                btnnew.Visible = false;
                btndel.Visible = false;
                frm_tabname = "TYPEGRP";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "ML");
                break;
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                {
                    case "F99119":
                        SQuery = "SELECT ID AS FSTR,trim(Text) as Report_Name,ID FROM FIN_MSYS where trim(id) not in (Select trim(rcode) from wb_mail_mgr where branchcd!='DD' and type='" + frm_vty + "') order by Text";
                        break;
                    case "F99120":
                        SQuery = "SELECT ID AS FSTR,trim(Text) as Event_Name,ID FROM FIN_MSYS where trim(id) not in (Select trim(rcode) from wb_mail_mgr where branchcd!='DD' and type='" + frm_vty + "') order by Text";
                        break;
                    case "F99123":
                        SQuery = "select userid as fstr,username as name,deptt from evas where branchcd!='DD' and length(Trim(nvl(close_by,'-')))<2 and length(Trim(userid))>4 order by name";
                        break;
                    case "F99124":
            ...
            case "SG1_ROW_ADD_E":
                ...
                col1 = " and TRIM(userid) not in ('" + txtlbl4.Text + "')";


                SQuery = "SELECT userid AS FSTR,Username ,Full_name,Userid as Code,Emailid FROM evas where branchcd!='DD' and length(Trim(nvl(close_by,'-')))<2 and length(Trim(userid))>4 " + col1 + " ORDER BY username ";
            ...
            default:
                ...
                //SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Mail_no,to_char(a.vchdate,'dd/mm/yyyy') as Mail_Dt,b.Text,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a,FIN_MSYS b where trim(A.rcode)=trim(B.ID) and a.branchcd='" + frm_mbr + "' and a.type='MH' order by vdd desc,a.vchnum desc";
                {
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.Vchnum as Mail_no,to_char(a.vchdate,'dd/mm/yyyy') as Mail_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and a.type='MH' order by vdd desc,a.vchnum desc";
                    switch (Prg_Id)
                    {
                        case "F99124":
                            SQuery = "select distinct trim(A.TYPE1) as fstr,a.TYPE1 as Mail_no,to_char(a.vchdate,'dd/mm/yyyy') as Mail_Dt,A.NAME AS MAIL_INFO,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a where a.branchcd!='DD' and a.id='ML' order by a.type1 desc";
                            break;
                    }
                }
                break;
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            ...
            frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(vCHNUM) AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='MH' AND VCHDATE " + DateRange + " ", 6, "VCH");
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " a where a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + "MH" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
                // Deleing data from Sr Ctrl Table
                //fgen.execute_cmd(frm_qstr, frm_cocd, "delete from wsr_ctrl a where a.branchcd||trim(a.type)||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");


                // Saving Deleting History
                fgen.save_info(frm_qstr, frm_cocd, frm_mbr, fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2"), fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3"), frm_uname, "MH", lblheader.Text.Trim() + " Deleted");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.full_name,b.username,b.contactno,b.emailid,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,evas b where trim(a.Ecode)=trim(b.userid) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    if (Prg_Id == "F99124")
                        SQuery = "Select a.*,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a where trim(a.type1)='" + col1 + "' AND A.ID='ML'";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (Prg_Id == "F99124")
                        {
                            txtlbl4.Text = dt.Rows[0]["type1"].ToString().Trim();
                            txtlbl4a.Text = dt.Rows[0]["name"].ToString().Trim();

                            col3 = dt.Rows[0]["ACREF"].ToString().Trim();
                            dt3 = new DataTable();
                            dt3.Columns.Add("ecode");
                            dt3.Columns.Add("username");
                            dt3.Columns.Add("full_name");
                            dt3.Columns.Add("contactno");
                            dt3.Columns.Add("EMAILID");
                            dt3.Columns.Add("REMARKS");
                            dt3.Columns.Add("Mail_Sent_Dt");                            
                            dt3.Columns.Add("mail_freq");
                            DataRow drx;
                            foreach (string mailToID in col3.Split(';'))
                            {
                                drx = dt3.NewRow();
                                drx["ecode"] = mailToID;
                                col2 = "";
                                col2 = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT USERNAME||'~'||FULL_NAME||'~'||CONTACTNO||'~'||EMAILID fstr FROM EVAS WHERE USERID='" + mailToID + "'", "fstr");
                                if (col2.Length > 6)
                                {
                                    drx["USERNAME"] = col2.Split('~')[0];
                                    drx["FULL_NAME"] = col2.Split('~')[1];
                                    drx["CONTACTNO"] = col2.Split('~')[2];
                                    drx["EMAILID"] = col2.Split('~')[3];
                                    //drx["mail_freq"] = mailToID;
                                    dt3.Rows.Add(drx);
                                }
                                //sg1_dr["sg1_f1"] = dt.Rows[i]["ecode"].ToString().Trim();
                                ...
                                //sg1_dr["sg1_t1"] = dt.Rows[i]["Mail_Freq"].ToString().Trim();
                                //sg1_dr["sg1_t2"] = dt.Rows[i]["Remarks"].ToString().Trim();
                                //sg1_dr["sg1_t3"] = dt.Rows[i]["Mail_Sent_Dt"].ToString().Trim();                                
                            }
                            dt = new DataTable();
                            dt = dt3;
                        }
                        else
                        {
                            ...
                            txtlbl4.Text = dt.Rows[i]["rcode"].ToString().Trim();
                            txtlbl4a.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select username from evas where trim(upper(userid))=upper(Trim('" + dt.Rows[i]["rcode"].ToString().Trim() + "'))", "username");
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        dt = new DataTable();
                        if (col1.Length > 6) SQuery = "select * from evas where trim(userid) in (" + col1 + ") order by username";
                        else SQuery = "select * from evas where trim(userid)='" + col1 + "' order by username";
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            //SQuery = "select a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as Plan_Dt,b.Full_name,b.Username as Client_Code,a.Month_Amt as Month_Tgt,a.Remarks,a.Naration,a.CCode,a.Ent_by,a.ent_Dt ,to_Char(a.opldt,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,evas b where trim(A.ccode)=trim(B.userid) and a.branchcd='" + frm_mbr + "' and a.type='MH' and opldt " + PrdRange + " order by vdd ,a.oplno ,a.srno";
            SQuery = "select a.vchnum as vchnum,to_char(a.vchdate,'dd/mm/yyyy') as vchdate,b.Full_name,b.Username as name,a.Remarks,a.Naration,a.Ent_by,a.ent_Dt ,a.srno from WB_mail_mgr a,evas b where trim(A.rcode)=trim(B.userid) and a.branchcd='" + frm_mbr + "' and a.type='MH' and a.vchdate " + PrdRange + " order by vchdate,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        ...
        else
        {
            ...
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , HOD Not Filled Correctly !!");
            }
            ...
            if (col1 == "Y" && Checked_ok == "Y")
            {
                if (Prg_Id == "F99124")
                {
                    ...
                    foreach (GridViewRow gr in sg1.Rows)
                    {
                        col1 += ";" + sg1.Rows[i].Cells[13].Text.Trim();
                    }
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (save_it == "Y")
                            {
                                ...
                                do
                                {
                                    frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + i + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='MH' ", 6, "vch");
                                    pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + "MH" + frm_vnum + frm_CDT1, frm_mbr, "MH", frm_vnum, txtvchdate.Text.Trim(), "", frm_uname);
                                    if (i > 20)
                                    {
                                        fgen.FILL_ERR(frm_uname + " --> Next_no Fun Prob ==> " + frm_PageName + " ==> In Save Function");
                                        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + 0 + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='MH' ", 6, "vch");
                                        pk_error = "N";
                                        i = 0;
                                    }
                                    i++;
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            cmd_query = "update " + frm_tabname + " set branchcd='DD' where branchcd||type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + "MH" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_FSTR") + "'";
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":
                ...
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Person", frm_qstr);
                }
                else
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_mseek("Select Person", frm_qstr);
    ...
    protected void btnlbl4_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select HOD ", frm_qstr);
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["SRNO"] = i;
                oporow["Rcode"] = txtlbl4.Text;

                oporow["ECODE"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["Mail_Freq"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
                oporow["REMARKS"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text;
                oporow["Mail_Sent_Dt"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text;
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F99119":
                SQuery = "SELECT 'MM' AS FSTR,'Mail Manager' as NAME,'MM' AS CODE FROM dual";
                break;
            case "F99120":
                SQuery = "SELECT 'NF' AS FSTR,'Notification Manager' as NAME,'NF' AS CODE FROM dual";
                break;
