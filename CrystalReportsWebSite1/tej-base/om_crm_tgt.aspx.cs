...
public partial class om_crm_tgt : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "FB");
        //switch (Prg_Id)
        //{
        //    case "F99101":
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                //pop1
                SQuery = "SELECT userid as fstr,username,userid from evas order by username";
                //SQuery = "SELECT '01' AS FSTR,'Income side' as Name,'01' as Code from dual union all SELECT '02' AS FSTR,'Expense side' as Name,'02' as Code from dual";
                break;
            case "TICODE": 
                //pop2
            ...
            case "SG1_ROW_ADD_E":
                if (txtlbl4.Text=="01")
                {
                    SQuery = "SELECT acode AS FSTR,Aname AS Account,acode as Code FROM famst where branchcd!='DD' and substr(acode,1,1)='2' order by Aname";
                }
                else
                {
                    SQuery = "SELECT acode AS FSTR,Aname AS Account,acode as Code FROM famst where branchcd!='DD' and substr(acode,1,1)>'2' order by Aname";
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.aname,b.acode,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt from " + frm_tabname + " a,famst b where trim(a.icode)=trim(b.acode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (txtlbl4.Text == "01")
                        {
                            txtlbl4a.Text = "Income Side";
                        }
                        else
                        {
                            txtlbl4a.Text = "Expense Side";
                        }
                        
                        
                        //fgen.seek_iname(frm_qstr, frm_cocd, "select typegrp from type where branchcd!='DD' and id='A' and trim(upper(userid))=upper(Trim('" + txtlbl4.Text + "'))", "name");
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.vchnum as Plan_no,to_char(a.vchdate,'dd/mm/yyyy') as Plan_Dt,c.Username as Plan_by,b.Full_name,b.Username as Client_Code,a.Month_Amt as Month_Tgt,a.Remarks,a.Naration,a.CCode,a.Ent_by,a.ent_Dt ,to_Char(a.vchdate,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,evas b,evas c where trim(A.ccode)=trim(B.userid) and trim(A.tcode)=trim(c.userid) and  a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + PrdRange + " order by vdd ,a.vchnum ,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F70119":
                SQuery = "SELECT 'OP' AS FSTR,'Expense Budget' as NAME,'OP' AS CODE FROM dual";
                break;
