...
//SS1======hppi
/// <summary>
/// Made by Suman.S for LRFP 27.03.2020
/// </summary>
public partial class om_cust_rej : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "BTN_19":
                break;
            case "CRCODE":
                SQuery = "SELECT Type1 AS FSTR,NAME AS Deptt,Type1 AS CODE FROM type where id='M' and type1 in('03','04') order by Name";
                break;
            case "TACODE":
                //pop1
                //SQuery = "SELECT ID,TEXT,ID AS CODE FROM FIN_MSYS WHERE NVL(TRIM(WEB_ACTION),'-')!='-' ORDER BY ID  ";
                SQuery = "SELECT 'MT','Purchase (Matl)' as PR_Type,'MT' AS CODE FROM dual union all SELECT 'JW','Job Work' as PR_Type,'JW' AS CODE FROM dual ";
                break;
            case "TICODE":
                //pop2
            ...
            case "SG1_ROW_ADD_E":
                ...
                SQuery = "SELECT Icode AS FSTR,Iname AS Item_Name,Cpartno,Cdrgno,unit,hscode,ent_by,Icode FROM Item where branchcd!='DD' and length(Trim(nvl(deac_by,'-')))<2  and length(Trim(icode))>4 " + col1 + " ORDER BY Iname ";
                break;
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as PR_no,to_char(a.vchdate,'dd/mm/yyyy') as PR_Dt,b.Name as Deptt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'dd/mm/yyyy') as vdd from " + frm_tabname + " a,type b where trim(A.T_deptt)=trim(B.type1) and b.id='M' and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + DateRange + " order by vdd desc,a.vchnum desc";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        string chk_po_made;
        chk_po_made = fgen.seek_iname(frm_qstr, frm_cocd, "select vchnum||' Dt.'||to_char(vchdate,'dd/mm/yyyy') As fstr from wb_cust_rej where branchcd='" + frm_mbr + "' and type='CR' and trim(vchnum)||to_char(pr_Dt,'dd/mm/yyyy')='" + txtvchnum.Text + orig_vchdt + "'", "fstr");
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl7.Text = dt.Rows[0]["T_deptt"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["iqtyrej"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["Remark"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select branchcd as brnch,vchnum as entry_no,Acode as ACode,Icode as Item_Code,to_char(vchdate,'dd/mm/yyyy') as Entry_date from wb_cust_rej  where branchcd='" + frm_mbr + "' and type='CR' and acode like '" + party_cd + "%' and icode like '" + part_cd + "%' and vchdate " + PrdRange + "  order by vchdate";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            fgen.Fn_open_rptlevel("Customer Rejection for the Period " + fromdt + " to " + todt, frm_qstr);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["acode"] = user_acode;
                oporow["T_deptt"] = txtlbl7.Text;
                oporow["icode"] = sg1.Rows[i].Cells[13].Text.Trim();
                oporow["iqtyrej"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text);
                ...
                if (txtlbl4.Text.Trim().ToUpper().Length > 1)
                {
                    oporow["prnum"] = txtlbl4.Text.Trim();
                }
                if (txtlbl7.Text.Trim().ToUpper().Length > 1)
                {
                    oporow["T_Deptt"] = txtlbl7.Text.Trim();
                }
    ...
    void Type_Sel_query()
    {
        SQuery = "SELECT 'CR' AS FSTR,'Customer Rejection' as NAME,'CR' AS CODE FROM dual";
    }
    //------------------------------------------------------------------------------------   
