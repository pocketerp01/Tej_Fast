...
public partial class om_splan_entry : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                SQuery = "SELECT 'SS' as fstr,'From Sales Schedules' as Plan_source,'SS' AS CODE FROM dual union all SELECT 'DF','Direct Feeding' as PR_Type,'DF' AS CODE FROM dual ";
                break;
            case "TICODE":
                //pop2
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if(txtlbl4.Text=="SS")
                        {
                            txtlbl4a.Text = "From Sales Schedules";
                        }
                        else
                        {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t1"] = dt.Rows[i]["cust"].ToString().Trim();
                            sg1_dr["sg1_t2"] = dt.Rows[i]["qtyupd"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["target"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["actual"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["prodn"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["wk1"].ToString().Trim();
                            sg1_dr["sg1_t7"] = dt.Rows[i]["wk2"].ToString().Trim();
                            sg1_dr["sg1_t8"] = dt.Rows[i]["wk3"].ToString().Trim();
                            sg1_dr["sg1_t9"] = dt.Rows[i]["wk4"].ToString().Trim();
                ...
                case "TACODE":
                    ...
                    txtlbl4.Text = col1;
                    txtlbl4a.Text = col2;
                    if (txtlbl4.Text.Trim() == "SS")
                    {
                        SQuery = "Select b.iname,nvl(b.cpartno,'-') as icpartno,nvl(b.cdrgno,'-') as icdrgno,nvl(b.unit,'-') as iunit,trim(a.Icode) as Item_Code,sum(a.total) as Total_qty from schedule a,item b where trim(a.icode)=trim(B.icode) and a.branchcd='" + frm_mbr + "' and a.type like '46%' and to_char(a.vchdate,'yyyymm')=to_char(to_datE('" + txtvchdate.Text + "','dd/mm/yyyy'),'yyyymm') group by b.iname,nvl(b.cpartno,'-'),nvl(b.cdrgno,'-'),nvl(b.unit,'-'),nvl(b.hscode,'-'),trim(a.Icode) order by b.iname";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["CUST"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text;
                oporow["qtyUPD"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text);
                oporow["TARGET"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
                oporow["ACTUAL"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text);
                oporow["PRODN"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text);
                oporow["WK1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text);
                oporow["WK2"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text);
                oporow["WK3"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text);
                oporow["WK4"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t9")).Text);
