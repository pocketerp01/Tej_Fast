...
public partial class om_inv_entry : System.Web.UI.Page
{
    ...
    string save_it;
    string pop_qry = ""; string mv_col;
    string newBranchcd = "", brCode = "";
    bool sdWorking = false;
    string Prg_Id;
    string pk_error = "Y", chk_rights = "N", DateRange, PrdRange;
    string frm_mbr, frm_vty, frm_vnum, frm_url, frm_qstr, frm_cocd, frm_uname, frm_PageName;
    string frm_tab_ivch, frm_myear, frm_ulvl, frm_formID, frm_UserID, frm_CDT1, cond = "", a95, a96;
    string frm_tab_sale;
    string frm_tab_vchr;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                doc_GST.Value = "Y";
                doc_hosopw.Value = "N";
                SQuery = "select opt_id,trim(upper(OPT_ENABLE)) as OPT_ENABLE from FIN_RSYS_OPT_PW where branchcd='" + frm_mbr + "' and OPT_ID in ('W1100','W2017','W2019','W2027') order by OPT_ID";
                dt = new DataTable();
                ...
                doc_hoso.Value = "N";
                doc_DAINV.Value = "N";
                SQuery = "select opt_id,trim(upper(OPT_ENABLE)) as OPT_ENABLE from FIN_RSYS_OPT where OPT_ID in ('W0052','W0044') order by OPT_ID";
                dt = new DataTable();
                dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                if (dt.Rows.Count > 0)
                {
                    for (i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        switch (chk_opt)
                        {
                            case "W0052":   //HO Based SO 
                                ...
                                break;
                            case "W0044":   //xx
                                if (chk_opt_yn == "Y") { doc_DAINV.Value = "Y"; }
                                break;
                        }
                    }
                }
                ...
                hfA95.Value = "";
                hfA96.Value = "";
                SQuery = "select id,trim(upper(ENABLE_YN)) as ENABLE_YN,PARAMS,PARAMS2 from CONTROLS where ID in ('A95','A96') order by ID";
                dt = new DataTable();
                dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                if (dt.Rows.Count > 0)
                {
                    for (i = 0; i < dt.Rows.Count; i++)
                    {
                        switch (dt.Rows[i]["ID"].ToString().Trim())
                        {
                            case "A95":
                                if (dt.Rows[i]["ENABLE_YN"].ToString().Trim() == "Y")
                                    hfA95.Value = dt.Rows[i]["PARAMS"].ToString().Trim();
                                break;
                            case "A96":
                                if (dt.Rows[i]["ENABLE_YN"].ToString().Trim() == "Y")
                                    hfA96.Value = dt.Rows[i]["PARAMS"].ToString().Trim();
                                break;
                        }
                    }
                }
    ...
    void setColHeadings()
    {
        ...
        if (frm_cocd != "MULT" && frm_cocd != "AGRM" && frm_cocd != "SAIA")
        {
            lblcashdicval.Style.Add("display", "none");
            lblcashdisc.Style.Add("display", "none");
            txtCashDisc.Style.Add("display", "none");
            txtCashDiscValue.Style.Add("display", "none");
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "BTN_18":
                SQuery = "Select Type1 as fstr,Name,type1 from type where id='<' order by name";
                if (frm_cocd.Trim() == "KRSM" || frm_cocd.Trim() == "TEJAXO")
                {
                    SQuery = "Select Type1 as fstr,Name,type1 from type where id='H' and substr(type1,1,1)='0' order by name";
                }
                break;
            ...
            case "TACODE":
                ...
                SQuery = "SELECT distinct a.ACODE AS FSTR,b.ANAME AS PARTY,a.ACODE AS CODE,b.ADDR1,b.ADDR2,b.ADDR3,b.staten as state,b.Pay_num,b.GST_no AS " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_TAXVAR") + " FROM somas a, FAMST b where " + ord_br_Str + " and a.type='" + frm_vty + "' and trim(nvl(a.ICAT,'-'))!='Y'  and trim(nvl(a.app_by,'-'))!='-' and trim(A.acode)=trim(B.acode) and length(Trim(nvl(b.deac_by,'-')))<=1 ORDER BY aname ";
                if ((Prg_Id == "F50106" && frm_cocd != "KRSM") || chkso.Checked)
                {
                    SQuery = "SELECT distinct a.ACODE AS FSTR,A.ANAME AS PARTY,a.ACODE AS CODE,A.ADDR1,A.ADDR2,A.ADDR3,A.staten as state,A.Pay_num,A.GST_no AS " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_TAXVAR") + " FROM FAMST A WHERE SUBSTR(ACODE,1,2)='16' ORDER BY A.ACODE,A.aname ";
                }
                if (lbl1a.Text.Substring(0, 2) == "47")
                {
                    SQuery = "SELECT distinct a.ACODE AS FSTR,A.ANAME AS PARTY,a.ACODE AS CODE,A.ADDR1,A.ADDR2,A.ADDR3,A.staten as state,A.Pay_num,A.GST_no AS " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_TAXVAR") + " FROM FAMST A WHERE SUBSTR(ACODE,1,2)='06' ORDER BY A.ACODE,A.aname ";
                }
                break;
            case "PICK_DA":
            ...
            case "SG3_ROW_ADD_E":
                ...
                SQuery = "select a.Fstr,max(a.Iname)as Item_Name,a.ERP_code,(case when length(trim(max(a.Cpartno)))>2 then max(a.Cpartno) else b.cpartno end) as Part_no,max(a.Irate) As Irate," +
                    "sum(a.Qtyord)-sum(a.Soldqty) as Balance_Qty,b.Unit,b.hscode,max(a.pordno) as PO_No,a.Fstr as SO_link,max(a.cdisc) as CDisc,max(a.iexc_Addl) as iexc_Addl,max(a.sd) as frt_pu," +
                    "max(a.ipack) as pkchg_pu,sum(a.Qtyord) as Qty_Ord,sum(a.Soldqty) as Sold_Qty,olineno,b.packsize as std_pack,max(sale_rep) as Sale_Rep,a.desc_,PAY_TERM from " +
                    "(SELECT to_ChaR(orddt,'YYYYMMDD')||'-'||ordno||'-'||trim(Icode) as fstr,pordno,(Case when length(trim(nvl(desc9,'-')))>1 then desc9 else ciname end) as Iname," +
                    "trim(Icode) as ERP_code,Cpartno,Irate,Qtyord,0 as Soldqty,nvl(Cdisc,0) as Cdisc,nvl(iexc_addl,0) as Iexc_Addl,nvl(sd,0) as sd ,nvl(ipack,0) as ipack,trim(nvl(cdrgno,'-')) as olineno" +
                    ",upper(trim(nvl(sale_rep,'-'))) as sale_rep,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from somas where " + ord_br_Str + " and type='" + lbl1a.Text.Substring(0, 2) + "' " +
                    "and trim(acode)='" + txtlbl4.Text + "' and trim(icat)!='Y' and trim(app_by)!='-'  union all SELECT to_ChaR(refdate,'YYYYMMDD')||'-'||ponum||'-'||trim(Icode) as fstr,null " +
                    "as pordno,null as Iname,trim(Icode) as ERP_code,null as Cpartno,0 as Irate,0 as Qtyord,iqtyout as Soldqty,0 as Cdisc,0 as iexc_Addl,0 as sd,0 as ipack,nvl(revis_no,'-') " +
                    "AS linno,null as sale_rep  ,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text.Substring(0, 2) + "' and " +
                    "trim(acode)='" + txtlbl4.Text + "')a,item b where trim(a.erp_code)=trim(B.icode)  group by a.olineno,a.fstr,a.ERP_code,b.unit,b.hscode,b.cpartno,b.packsize,a.desc_" +
                    " having (case when sum(a.Qtyord)>0 then sum(a.Qtyord)-sum(a.Soldqty) else max(a.irate) end)>0 ";
                if (Prg_Id == "F50106" && frm_cocd == "KRSM")
                {
                    if (frm_cocd == "KRSM" || frm_cocd == "TEJAXO") cond = "AND SUBSTR(A.ICODE,1,2)='98' ";
                    SQuery = SQuery + " union all " + "select a.ICODE AS FSTR,A.INAME as Item_Name,a.ICODE AS ERP_code,A.CPARTNO as Part_no,A.IRATE As Irate,0 as Balance_Qty,A.Unit,A.hscode,'-' as PO_No," +
                         "'-' as SO_link,0 as CDisc,0 as iexc_Addl,0 as frt_pu,0 as pkchg_pu,0 as Qty_Ord,0 as Sold_Qty,'-' olineno,A.packsize as std_pack,'-' as Sale_Rep,'-' desc_,'-' PAY_TERM from " +
                         "ITEM A WHERE LENGTH(TRIM(ICODE))>4 " + cond + "  order by Item_Name,fstr";
                }
                if (sdWorking)
                    SQuery = "select a.Fstr,max(a.Iname)as Item_Name,a.ERP_code,(case when length(trim(max(a.Cpartno)))>2 then max(a.Cpartno) else b.cpartno end) as Part_no,max(a.Irate) As Irate,sum(a.Qtyord)-sum(a.Soldqty) as Balance_Qty,b.Unit,b.hscode,max(a.pordno) as PO_No,a.Fstr as SO_link,max(a.cdisc) as CDisc,max(a.iexc_Addl) as iexc_Addl,max(a.sd) as frt_pu,max(a.ipack) as pkchg_pu,sum(a.Qtyord) as Qty_Ord,sum(a.Soldqty) as Sold_Qty,olineno,b.packsize as std_pack,'-' as sale_rep from (SELECT to_ChaR(orddt,'YYYYMMDD')||'-'||ordno||'-'||trim(Icode) as fstr,pordno,(Case when length(trim(nvl(desc9,'-')))>1 then desc9 else ciname end) as Iname,trim(Icode) as ERP_code,Cpartno,Irate,Qtyord,0 as Soldqty,nvl(Cdisc,0) as Cdisc,nvl(iexc_addl,0) as Iexc_Addl,nvl(sd,0) as sd ,nvl(ipack,0) as ipack,trim(nvl(cdrgno,'-')) as olineno from somas where " + ord_br_Str + " and type='" + lbl1a.Text.Substring(0, 2) + "' and trim(acode)='" + txtlbl4.Text + "' and trim(icat)!='Y' and trim(app_by)!='-'  union all SELECT to_ChaR(podate,'YYYYMMDD')||'-'||ponum||'-'||trim(Icode) as fstr,null as pordno,null as Iname,trim(Icode) as ERP_code,null as Cpartno,0 as Irate,0 as Qtyord,iqtyout as Soldqty,0 as Cdisc,0 as iexc_Addl,0 as sd,0 as ipack,nvl(revis_no,'-') AS linno  from ivoucher where branchcd='" + newBranchcd + "' and type='" + lbl1a.Text.Substring(0, 2) + "' and trim(acode)='" + txtlbl4.Text + "' and trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text + txtvchdate.Text + "')a,item b where trim(a.erp_code)=trim(B.icode)  group by a.olineno,a.fstr,a.ERP_code,b.unit,b.hscode,b.cpartno,b.packsize having (case when sum(a.Qtyord)>0 then sum(a.Qtyord)-sum(a.Soldqty) else max(a.irate) end)>0 order by Item_Name,a.fstr";


                if ((Prg_Id == "F50106" && frm_cocd != "KRSM") || chkso.Checked)
                {
                    cond = "AND (SUBSTR(A.ICODE,1,1)='9' OR SUBSTR(A.ICODE,1,2)='59')";
                    if (frm_cocd == "MASS" || frm_cocd == "MAST") cond = "";
                    if (frm_vty == "4B") cond = "";
                    if (frm_cocd == "MULT" || fgenMV.Fn_Get_Mvar(frm_qstr, "U_CLIENT_GRP") == "SG_TYPE") cond = "";
                    SQuery = "select a.ICODE AS FSTR,A.INAME as Item_Name,a.ICODE AS ERP_code,A.CPARTNO as Part_no,A.IRATE As Irate,0 as Balance_Qty,A.Unit,A.hscode,'-' as PO_No,'-' as SO_link,0 as CDisc,0 as iexc_Addl,0 as frt_pu,0 as pkchg_pu,0 as Qty_Ord,0 as Sold_Qty,A.packsize as std_pack,0 AS olineno,'-' as Sale_Rep from ITEM A WHERE LENGTH(TRIM(ICODE))>4 " + cond + " ORDER BY A.ICODE desc,A.INAME";
                }
                if (Prg_Id == "F50101" && !chkso.Checked)
                {

                    //SQuery = "select a.Fstr,max(a.Iname)as Item_Name,a.ERP_code,(case when length(trim(max(a.Cpartno)))>2 then max(a.Cpartno) else b.cpartno end) as Part_no,max(a.Irate) As Irate,sum(a.Qtyord)-sum(a.Soldqty) as Balance_Qty,b.Unit,b.hscode,max(a.pordno) as PO_No,a.Fstr as SO_link,max(a.cdisc) as CDisc,max(a.iexc_Addl) as iexc_Addl,max(a.sd) as frt_pu,max(a.ipack) as pkchg_pu,sum(a.Qtyord) as Qty_Ord,sum(a.Soldqty) as Sold_Qty,olineno,b.packsize as std_pack,max(sale_rep) as Sale_Rep,a.desc_,PAY_TERM from (SELECT to_ChaR(orddt,'YYYYMMDD')||'-'||ordno||'-'||trim(Icode) as fstr,pordno,(Case when length(trim(nvl(desc9,'-')))>1 then desc9 else ciname end) as Iname,trim(Icode) as ERP_code,Cpartno,Irate,Qtyord,0 as Soldqty,nvl(Cdisc,0) as Cdisc,nvl(iexc_addl,0) as Iexc_Addl,nvl(sd,0) as sd ,nvl(ipack,0) as ipack,trim(nvl(cdrgno,'-')) as olineno,upper(trim(nvl(sale_rep,'-'))) as sale_rep,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from somas where " + ord_br_Str + " and type='" + lbl1a.Text.Substring(0, 2) + "' and trim(acode)='" + txtlbl4.Text + "' and trim(icat)!='Y' and trim(app_by)!='-'  union all SELECT to_ChaR(podate,'YYYYMMDD')||'-'||ponum||'-'||trim(Icode) as fstr,null as pordno,null as Iname,trim(Icode) as ERP_code,null as Cpartno,0 as Irate,0 as Qtyord,iqtyout as Soldqty,0 as Cdisc,0 as iexc_Addl,0 as sd,0 as ipack,nvl(revis_no,'-') AS linno,null as sale_rep  ,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text.Substring(0, 2) + "' and trim(acode)='" + txtlbl4.Text + "')a,item b where trim(a.erp_code)=trim(B.icode)  group by a.olineno,a.fstr,a.ERP_code,b.unit,b.hscode,b.cpartno,b.packsize,a.desc_,a.PAY_TERM having (case when sum(a.Qtyord)>0 then sum(a.Qtyord)-sum(a.Soldqty) else max(a.irate) end)>0 ";
                    SQuery = "select a.Fstr,max(a.Iname)as Item_Name,a.ERP_code,(case when length(trim(max(a.Cpartno)))>2 then max(a.Cpartno) else b.cpartno end) as Part_no,max(a.Irate) As Irate," +
                        "sum(a.Qtyord)-sum(a.Soldqty) as Balance_Qty,b.Unit,b.hscode,max(a.pordno) as PO_No,max(a.porddt) as Po_Date,a.Fstr as SO_link,max(a.cdisc) as CDisc,max(a.iexc_Addl) as iexc_Addl" +
                        ",max(a.sd) as frt_pu,max(a.ipack) as pkchg_pu,sum(a.Qtyord) as Qty_Ord,sum(a.Soldqty) as Sold_Qty,olineno,b.packsize as std_pack,max(sale_rep) as Sale_Rep,a.desc_,max(PAY_TERM) PAY_TERM  from " +
                        "(SELECT to_ChaR(orddt,'YYYYMMDD')||'-'||ordno||'-'||trim(Icode) as fstr,pordno,to_char(porddt,'dd/mm/yyyy') as porddt,(Case when length(trim(nvl(desc9,'-')))>1 then desc9 else ciname end) " +
                        "as Iname,trim(Icode) as ERP_code,Cpartno,Irate,Qtyord,0 as Soldqty,nvl(Cdisc,0) as Cdisc,nvl(iexc_addl,0) as Iexc_Addl,nvl(sd,0) as sd ,nvl(ipack,0) as ipack,trim(nvl(cdrgno,'-')) as" +
                        " olineno,upper(trim(nvl(sale_rep,'-'))) as sale_rep,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from somas where " + ord_br_Str + " and type='" + lbl1a.Text.Substring(0, 2) + "' and " +
                        "trim(acode)='" + txtlbl4.Text + "' and trim(icat)!='Y' and trim(app_by)!='-' union all SELECT to_ChaR(podate,'YYYYMMDD')||'-'||ponum||'-'||trim(Icode) as fstr,null as pordno,null " +
                        "as pordno,null as Iname,trim(Icode) as ERP_code,null as Cpartno,0 as Irate,0 as Qtyord,iqtyout as Soldqty,0 as Cdisc,0 as iexc_Addl,0 as sd,0 as ipack,nvl(revis_no,'-') AS linno," +
                        "null as sale_rep  ,nvl(desc_,'-') AS DESC_,thru AS PAY_TERM from ivoucher where branchcd='" + frm_mbr + "' and type='" + lbl1a.Text.Substring(0, 2) + "' and trim(acode)='" + txtlbl4.Text + "')" +
                        " a,item b where trim(a.erp_code)=trim(B.icode) group by a.olineno,a.fstr,a.ERP_code,b.unit,b.hscode,b.cpartno,b.packsize,a.desc_ having (case when sum(a.Qtyord) > 0 then" +
                        " sum(a.Qtyord)-sum(a.Soldqty) else max(a.irate) end)> 0";
                    cond = "AND SUBSTR(A.ICODE,1,2)='59' ";
                    if (frm_cocd == "KRSM" || frm_cocd == "TEJAXO") cond = "AND SUBSTR(A.ICODE,1,2)='98' ";
                    SQuery = SQuery + " union all " + "select a.ICODE AS FSTR,A.INAME as Item_Name,a.ICODE AS ERP_code,A.CPARTNO as Part_no,A.IRATE As Irate,0 as Balance_Qty,A.Unit,A.hscode,'-' as PO_No,'-' as PO_Date," +
                        "'-' as SO_link,0 as CDisc,0 as iexc_Addl,0 as frt_pu,0 as pkchg_pu,0 as Qty_Ord,0 as Sold_Qty,'-' olineno,A.packsize as std_pack,'-' as Sale_Rep,'-' desc_,'-' PAY_TERM from " +
                        "ITEM A WHERE LENGTH(TRIM(ICODE))>4 " + cond + "";
            ...
            case "Print":
            case "Atch":
                Type_Sel_querywith4f();
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "Atch_E" || btnval == "COPY_OLD")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as Doc_no,to_char(a." + doc_df.Value + ",'dd/mm/yyyy')" +
                        " as Doc_Dt,b.Aname as Customer,b.Gst_No AS " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_TAXVAR") + ",a.Bill_tot,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt" +
                        ",to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tab_sale + " a,famst b where  a.branchcd='" + (sdWorking ? newBranchcd : frm_mbr) + "' " +
                        "and a.type='" + frm_vty + "' AND a." + doc_df.Value + " " + DateRange + " and  trim(a.acode)=trim(B.acodE) order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        if (frm_cocd == "SAIA")
        {
            if (frm_mbr.toDouble() > 50)
            {
                fgen.msg("-", "AMSG", "Invoice Creation Not Allowed in Depo Branch!!");
                return;
            }
        }
        string chk_curren = "";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (frm_cocd == "SAIA")
        {
            if (frm_mbr.toDouble() > 50)
            {
                fgen.msg("-", "AMSG", "Invoice Saving Not Allowed in Depo Branch!!");
                return;
            }
        }
        frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
        ...
        if (txtlbl24.Text.Trim().Length < 2 && Prg_Id != "F50106")
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + lbl24.Text;

        }
        if (frm_formID != "F50106")
        {
        ...
        if (reqd_nc > 0)
        {
            fgen.msg("-", "AMSG", "Dear " + frm_uname + " , " + reqd_nc + " Field(s) Require Input '13' Please fill " + reqd_flds);
            return;
        }
        ...
        //**************** Stock Check
        var allownegativestock = fgen.seek_iname(frm_qstr, frm_cocd, "select opt_enable from FIN_RSYS_OPT_PW where vchnum='002032' and type='OP' and branchcd = '" + frm_mbr + "'", "opt_enable");
        if (Prg_Id == "F50106" || allownegativestock.Trim() == "Y")
        {
    ...
    string checkStockQty()
    {
        ...
        foreach (GridViewRow gr in sg1.Rows)
        {
            if (gr.Cells[13].Text.ToString().Trim().Length > 4 && gr.Cells[3].Text.ToString().Trim() != "N")
            {
                drQty = dtQty.NewRow();
        ...
        else
        {
            ...
            switch (btnval)
            {
                case "New":
                    ...
                    sg4_dt.Dispose();
                    //-------------------------------------------
                    fgen.msg("-", "CMSG", "Do You Want to Copy From Existing Invoice ? (No for make it new)");
                    hffield.Value = "NEW_E";
                ...
                case "Atch":
                    ...
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Entry to Attach the Document", frm_qstr);
                    break;
                case "Edit_E":
                    //edit_Click 
                    if (col1 == "") return;
                ...
                case "COPY_OLD": 
                    ...
                    mv_col = frm_mbr + frm_vty + col1; 
                    #region Edit Start
                    SQuery = "Select a.*,to_char(A.refdate,'dd/mm/yyyy') as refdated,to_char(A.podate,'dd/mm/yyyy') as podtd,c.Aname,nvl(b.cpartno,'-') As Icpartno,nvl(b.unit,'-') as IUnit " +
                        "from " + frm_tab_ivch + " a,item b,famst c where trim(a.icode)=trim(b.icode) and trim(a.acode)=trim(c.acode) and " +
                        "a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + (sdWorking ? newBranchcd : frm_mbr) + mv_col.Substring(2, 18) + "' ORDER BY A.morder";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "fstr", mv_col.Substring(4, 16));
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (frm_vty == "45")
                            txtTCS.Text = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT nvl(params,0) as params from controls where id='D38'", "params");
                        if (hfA95.Value.Length < 2) txtTCS.Text = "0";

                        if (hfA96.Value.Contains(frm_vty)) txtTCS.Text = "0";
                        lblTCS.Text = txtTCS.Text;
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_h1"] = dt.Rows[i]["SALE_REP"].ToString().Trim();
                            sg1_dr["sg1_h2"] = "-";
                            sg1_dr["sg1_h3"] = "-";
                            sg1_dr["sg1_h4"] = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT NON_STK FROM ITEM WHERE TRIM(ICODE)='" + dt.Rows[i]["icode"].ToString().Trim() + "' ", "NON_STK");
                            sg1_dr["sg1_h5"] = "-";
                            sg1_dr["sg1_h6"] = "-";
                            ...
                            sg1_dr["sg1_t21"] = dt.Rows[i]["podtd"].ToString().Trim();
                            ...
                            sg1_dr["sg1_t21"] = dt.Rows[i]["refdated"].ToString().Trim();
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            chkso.Checked = dt.Rows[i]["amt_frt"].ToString().Trim() == "1" ? true : false;
                            ...
                            txtlbl21.Text = dt.Rows[i]["ins_co"].ToString().Trim();
                            ...
                            //added by Ram on 17012021 for No_pkg
                            txtlbl20.Text = dt.Rows[i]["no_bdls"].ToString().Trim();
                            if (frm_formID != "F50106")
                            {
                ...
                case "PICK_DA":
                    ...
                    if (doc_GST.Value == "N")
                    {
                        SQuery = "select max(a.mode_tpt) as dmode_tpt,max(a.thru) As dthru,max(a.freight) As dfreight ,c.unit,trim(a.acode) as acode,b.aname,b.staffcd as gst_pos,b.staten,'-' as desc_,trim(a.icode) As icode,sum(a.qtyord)-sum(a.chl_qty) as qtysupp,max(a.cdisc) as cdisc,max(a.ciname) as ciname,max(a.cpartno) As cpartno,a.ordno as ponum,to_Char(a.orddt,'dd/mm/yyyy') as podate,a.vchnum,to_chaR(a.vchdate,'dd/mm/yyyy') as Vchdated,0 as dirate,A.vchnum||'-'||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.icode)||trim(a.ordline) as refr,trim(a.ordline) as ordline,0 as num4,0 as num5,0 as num6 from (select ciname,cpartno,acode,ordno,orddt,PACKNO AS vchnum,PACKDATE AS vchdate,icode,QTYSUPP as qtyord,0 as chl_qty,cdisc,ordline,mode_tpt,thru,freight from DESPATCH where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(Acode)='" + txtlbl4.Text + "' union all select null as ciname,null as cpartno,acode,ponum,podate,tc_no,refdate,icode,0 as qtyord,iqtyout as chl_qty,0 as cdisc,revis_no,null as mode_tpt,null as thru,null as freight from " + frm_tab_ivch + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(Acode)='" + txtlbl4.Text + "') a ,famst b where trim(A.acode)=trim(B.acode) and a.vchnum||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode) in(" + col1 + ") group by c.unit,b.staffcd,b.aname,b.staten,trim(a.acode),trim(a.icode),a.ordno,a.orddt,a.vchnum,to_Char(a.vchdate,'dd/mm/yyyy'),A.vchnum||'-'||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.icode)||trim(a.ORDLINE),trim(a.ordline) having sum(a.qtyord)-sum(a.chl_qty)>0 order by a.vchnum";
                    }
                    else
                    {
                        SQuery = "select max(a.mode_tpt) as dmode_tpt,max(a.thru) As dthru,max(a.freight) As dfreight ,c.unit,trim(a.acode) as acode,b.aname,b.staffcd as gst_pos,b.staten,'-' as desc_,trim(a.icode) as icode,sum(a.qtyord)-sum(a.chl_qty) as qtysupp,max(a.cdisc) as cdisc,max(a.ciname) as ciname,max(a.cpartno) As cpartno,a.ordno as ponum,to_Char(a.orddt,'dd/mm/yyyy') as podate,a.vchnum,to_chaR(a.vchdate,'dd/mm/yyyy') as Vchdated,max(a.irate) as dirate,A.vchnum||'-'||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.icode)||trim(a.ordline) as refr,trim(a.ordline) as ordline,d.num4,d.num5,d.num6 from (select ciname,cpartno,acode,ordno,orddt,PACKNO AS vchnum,PACKDATE AS vchdate,icode,QTYSUPP as qtyord,0 as chl_qty,cdisc,ordline,irate,mode_tpt,thru,freight  from DESPATCH where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(Acode)='" + txtlbl4.Text + "' union all select null as ciname,null as cpartno,acode,ponum,podate,tc_no,refdate,icode,0 as qtyord,iqtyout as chl_qty,0 as cdisc,revis_no,0 as irate,null as mode_tpt,null as thru,null as freight  from " + frm_tab_ivch + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(Acode)='" + txtlbl4.Text + "') a ,famst b,item c,typegrp d where d.id='T1' and trim(A.acode)=trim(B.acode) and trim(c.hscode)=trim(d.acref) and trim(A.icode)=trim(c.icode) and a.vchnum||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.acode) in(" + col1 + ") group by c.unit,b.staffcd,b.aname,b.staten,trim(a.acode),trim(a.icode),a.ordno,a.orddt,a.vchnum,to_Char(a.vchdate,'dd/mm/yyyy'),A.vchnum||'-'||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.icode)||trim(a.ORDLINE),trim(a.ordline),d.num4,d.num5,d.num6 having sum(a.qtyord)-sum(a.chl_qty)>0 order by a.vchnum";
                    }
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl15.Text = dt.Rows[i]["dmode_tpt"].ToString().Trim();
                        txtlbl16.Text = dt.Rows[i]["dthru"].ToString().Trim();
                        txtlbl17.Text = dt.Rows[i]["dfreight"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            col11 = fgen.seek_iname(frm_qstr, frm_cocd, "select nvl(irate,0)||'~'||nvl(cdisc,0)||'~'||trim(pordno)||'~'||trim(nvl(sale_Rep,'-'))||'~'||to_char(porddt,'dd/mm/yyyy') as PP from somas where  " + br_Cond + " and trim(type)='" + frm_vty + "' and trim(ordno)='" + dt.Rows[i]["ponum"].ToString().Trim() + "' and trim(cdrgno)='" + dt.Rows[i]["ordline"].ToString().Trim() + "' and to_Char(orddt,'dd/mm/yyyy')='" + dt.Rows[i]["podate"].ToString().Trim() + "' and trim(acode)='" + txtlbl4.Text + "' and trim(icode)='" + dt.Rows[i]["Icode"].ToString().Trim() + "'", "PP");
                            ...
                            if (col11.Length > 1)
                            {
                                ...
                                if (txtlbl5.Text == "" || txtlbl5.Text == "-" || txtlbl5.Text == "0")
                                {
                                    ...
                                    txtlbl6.Text = col11.Split('~')[4].ToString();
                                }
                                sg1_dr["sg1_h1"] = col11.Split('~')[3].ToString();

                            }
                            else
                            {
                                sg1_dr["sg1_t4"] = dt.Rows[i]["dirate"].ToString().Trim();
                ...
                case "Atch_E":
                    ...
                    fgen.open_fileUploadPopup("Upload Document for " + lblheader.Text, frm_qstr);
                    break;
                case "TACODE":
                    ...
                    string last_inv;
                    last_inv = fgen.seek_iname(frm_qstr, frm_cocd, "select * from (SELECT vchnum FROM sale WHERE branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(acode)='" + txtlbl4.Text + "' order by vchdate desc) where rownum<2", "vchnum");
                    if (last_inv != "0")
                    {
                        last_inv = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT nvl(destin,'-')||'~'||nvl(mode_Tpt,'-')||'~'||nvl(ins_no,'-')||'~'||nvl(freight,'-')||'~'||nvl(insur_no,'-') as skfstr FROM sale WHERE branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and trim(acode)='" + txtlbl4.Text + "'  order by vchdate desc", "skfstr");
                        if (last_inv.Contains("~"))
                        {

                            txtlbl8.Text = last_inv.Split('~')[0].ToString();
                            txtlbl15.Text = last_inv.Split('~')[1].ToString();
                            txtlbl16.Text = last_inv.Split('~')[2].ToString();
                            txtlbl17.Text = last_inv.Split('~')[3].ToString();
                            txtlbl18.Text = last_inv.Split('~')[4].ToString();
                    ...
                    if (frm_formID != "F50106")
                    {
                        ...
                        if (doc_DAINV.Value == "Y")
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                make_qry_4_popup();
                                fgen.Fn_open_mseek("Select Dispatch Advice", frm_qstr);
                            }
                        }
                    }
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (doc_GST.Value == "N")
                        {
                            if (col1.Trim().Length == 8) SQuery = "select a.sale_Rep,a.po_no,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,0 as num4,0 as num5,0 as num6,0 as num7,a.olineno,a.std_pack from (" + pop_qry + ") a where trim(a.fstr) in ('" + col1 + "')";
                            else SQuery = "select a.sale_Rep,a.po_no,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,0 as num4,0 as num5,0 as num6,0 as num7,a.olineno,a.std_pack from (" + pop_qry + ") a where trim(a.fstr) in (" + col1 + ")";
                        }
                        else
                        {
                            ...
                            else SQuery = "select a.sale_Rep,a.po_no,a.Po_Date,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7,a.olineno,a.std_pack,a.desc_,a.PAY_TERM from (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in (" + col1 + ")";
                        }
                        if (Prg_Id == "F50106" || chkso.Checked)
                        {
                            if (col1.Trim().Length == 8) SQuery = "select a.sale_Rep,a.po_no,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7,a.olineno,a.std_pack from (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in ('" + col1 + "')";
                            else SQuery = "select a.sale_Rep,a.po_no,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7,a.olineno,a.std_pack from (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in (" + col1 + ")";
                        }
                        if (lbl1a.Text.Substring(0, 2) == "47")
                        {
                            if (col1.Trim().Length == 8) SQuery = "select a.sale_Rep,a.po_no,a.fstr,a.ERP_code as icode,a.Item_Name as iname,a.Part_no as cpartno,'-' as cdrgno,a.irate,a.iexc_addl,a.frt_pu,a.pkchg_pu,a.balance_qty,a.Cdisc,a.unit,a.hscode,b.num4,b.num5,b.num6,b.num7,a.olineno,a.std_pack from (" + pop_qry + ") a,typegrp b where trim(a.hscode)=trim(b.acref) and b.id='T1' and trim(a.fstr) in ('" + col1 + "')";
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            sg1_dr = sg1_dt.NewRow();
                            sg1_dr["sg1_srno"] = sg1_dt.Rows.Count + 1;
                            sg1_dr["sg1_h1"] = dt.Rows[d]["SALE_REP"].ToString().Trim();
                            sg1_dr["sg1_h2"] = dt.Rows[d]["iname"].ToString().Trim();
                            sg1_dr["sg1_h3"] = dt.Rows[d]["std_pack"].ToString().Trim();
                            sg1_dr["sg1_h4"] = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT NON_STK FROM ITEM WHERE TRIM(ICODE)='" + dt.Rows[d]["icode"].ToString().Trim() + "' ", "NON_STK");
                            sg1_dr["sg1_h5"] = "-";
                            sg1_dr["sg1_h6"] = "-";
                            sg1_dr["sg1_h7"] = "-";
                            sg1_dr["sg1_h8"] = "-";
                            ...
                            if (dt.Rows[d]["icode"].ToString().Trim().Substring(0, 4).Equals("9801"))
                            {
                                //int TotalPrice = sg1_dt.AsEnumerable().
                                //    Where(row => row.Field<decimal>("sg1_t7 + sg1_t8") ==
                                //    (fgen.Make_decimal(dt.Rows[d]["num4"].ToString().Trim()) + 
                                //    fgen.Make_decimal(dt.Rows[d]["num5"].ToString().Trim()) + 
                                //    fgen.Make_decimal(dt.Rows[d]["num6"].ToString().Trim())))
                                //    .Sum(row => row.Field<int>("sg1_t6"));
                                try
                                {

                                    decimal pfrate = fgen.Make_decimal(fgen.seek_iname(frm_qstr, frm_cocd, "select oprate3 from item where trim(icode)='" + dt.Rows[d]["icode"].ToString().Trim() + "'", "oprate3"));//item master se aayega
                                    var dtt = sg1_dt.AsEnumerable().
                                        Where(row => (fgen.Make_decimal(row.Field<string>("sg1_t7")) + fgen.Make_decimal
                                        (row.Field<string>("sg1_t8"))) == fgen.Make_decimal(dt.Rows[d]["num6"].ToString().Trim())).CopyToDataTable();
                                    decimal bamt = 0;
                                    for (int i = 0; i < dtt.Rows.Count; i++)
                                    {
                                        bamt += fgen.Make_decimal(dtt.Rows[i]["sg1_t6"].ToString());
                                    }

                                    decimal irate = Math.Round((bamt * pfrate) / 100, 2);
                                    //    DataTable table = new DataTable();
                                    //table.Columns.Add("basicamt", typeof(double));
                                    //table.Columns.Add("taxrate", typeof(string));
                                    //table.AcceptChanges();
                                    //for (int i = 0; i < sg1_dt.Rows.Count; i++)
                                    //{
                                    //    DataRow drow = table.NewRow();
                                    //    drow["basicamt"] = fgen.make_double(sg1_dt.Rows[i]["sg1_t6"].ToString());
                                    //    drow["taxrate"] = (fgen.make_double(sg1_dt.Rows[i]["sg1_t7"].ToString()) +
                                    //        fgen.make_double(sg1_dt.Rows[i]["sg1_t8"].ToString())).ToString();
                                    //    table.Rows.Add(drow);
                                    //}
                                    //var bamt = table.Compute("Sum(basicamt)"
                                    //    , "taxrate='" + (fgen.make_double(dt.Rows[d]["num4"].ToString().Trim()) +
                                    //    fgen.make_double(dt.Rows[d]["num5"].ToString().Trim()) +
                                    //    fgen.make_double(dt.Rows[d]["num6"].ToString().Trim())).ToString() + "' " +
                                    //    "and basicamt is not null");
                                    sg1_dr["sg1_t4"] = irate.ToString();
                                    sg1_dr["sg1_t3"] = "1";
                                }
                                catch (Exception ee) { }
                            }
                            //int rate = sg1_dt.Compute("sum("+Con(([]+")", "");
                            ...
                            try
                            {
                                sg1_dr["sg1_t9"] = dt.Rows[d]["desc_"].ToString().Trim();
                            }
                            catch { }
                            sg1_dr["sg1_t10"] = "-";
                            ...
                            try
                            {
                                ...
                                if (txtlbl21.Text.Trim().Length <= 1)
                                {
                                    txtlbl21.Text = dt.Rows[d]["PAY_TERM"].ToString().Trim();
                                }
                                sg1_dr["sg1_t21"] = dt.Rows[d]["Po_Date"].ToString().Trim();
                            }
                            catch { }
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[0].Text = dt.Rows[d]["SALE_REP"].ToString().Trim();
                        ...
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[3].Text = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT NON_STK FROM ITEM WHERE TRIM(ICODE)='" + dt.Rows[d]["icode"].ToString().Trim() + "' ", "NON_STK");
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select a.Vchnum as Inv_No,to_char(a.vchdate,'dd/mm/yyyy') as Dated,c.Aname as Customer,a.purpose  as Item_Name,a.exc_57f4 as Part_No,a.iqtyout as sale_Qty,a.Irate,a.ichgs as Disc,b.unit,b.hscode,a.Desc_,a.icode,a.ent_by,a.ent_Dt from " + frm_tab_ivch + " a, item b,famst c where a.branchcd='" + frm_mbr + "'  AND TRIM(A.TYPE) LIKE '4%'  /*and a.type='" + frm_vty + "'*/ and a." + doc_df.Value + " " + PrdRange + " and trim(a.icode)=trim(b.icode) and trim(a.acode)=trim(c.acode) and a.acode like '" + party_cd + "%' and a.icode like '" + part_cd + "%' order by a." + doc_df.Value + ",a." + doc_nf.Value + ",a.morder ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            ...
            else
            {
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        frm_vnum = "000000";
                        var PONO = txtlbl5.Text;
                        var PODATE = txtlbl6.Text;
                        ...
                        sal_code = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT acode FROM Type WHERE ID='V' and type1='" + lbl1a.Text + "'", "ACODE");
                        schg_code = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(opt_param) as param from FIN_RSYS_OPT where opt_id='W3001' and trim(opt_enable)='Y'", "param");
                        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
                        ...
                        if (sdWorking != true)
                        {
                            ...
                            if (frm_vty == "4S")
                            {
                                ...
                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), 1, par_code, sal_code, fgen.make_double(txtlbl27.Text.Trim(), 2) + fgen.make_double(txtlbl29.Text.Trim(), 2), 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, fgen.make_double(txtlbl27.Text.Trim(), 2) + fgen.make_double(txtlbl29.Text.Trim(), 2), 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                                if (code4s.Trim().Length > 4)
                                {
                                    srn += 1;
                                    fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, sal_code, code4s, fgen.make_double(txtlbl25.Text, 2), 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, fgen.make_double(txtlbl25.Text, 2), 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                                    srn += 1;
                                    fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, code4s, sal_code, 0, fgen.make_double(txtlbl25.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl25.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                                }
                            }
                            else
                            {
                                double dramtTemp = DrAmt = fgen.make_double(txtlbl31.Text.Trim(), 2);
                                if (schg_code.Trim().Length > 5)
                                {
                                    DrAmt = fgen.make_double(txtlbl31.Text.Trim(), 0);
                                    roundoffVal = Math.Round(dramtTemp - DrAmt, 2);
                                }

                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), 1, par_code, sal_code, DrAmt, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, fgen.make_double(txtlbl31.Text.Trim(), 2), 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                                srn += 1;
                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, sal_code, par_code, 0, fgen.make_double(txtlbl25.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl25.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");

                            }
                            srn += 1;
                            fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, tax_code, par_code, 0, fgen.make_double(txtlbl27.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl27.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                            if (tax_code2.Length > 0)
                            {
                                srn += 1;
                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, tax_code2, par_code, 0, fgen.make_double(txtlbl29.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl29.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                            }
                            if (fgen.make_double(txtTCSA.Text) > 0)
                            {
                                srn += 1;
                                if (frm_vty == "45") tax_code = fgen.seek_iname(frm_qstr, frm_cocd, "select params from controls where id='D37'", "params");
                                else tax_code = hfA95.Value;
                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, tax_code, par_code, 0, fgen.make_double(txtTCSA.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtTCSA.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                            }
                            if (schg_code.Trim().Length > 5)
                            {
                                srn += 1;
                                fgen.vSave(frm_qstr, frm_cocd, frm_mbr, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, schg_code, par_code, roundoffVal > 0 ? roundoffVal : 0, roundoffVal < 0 ? -(roundoffVal) : 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl25.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
                            }
                        }
                        #endregion
                        ...
                        if (frm_cocd == "SAIA" && frm_uname != "FINTEAM")
                        {
                            ...
                            {
                                try
                                {   ///to send email to Customer with Invoice PDF attachment by ram on 17012021
                                    //sendMail();
                                }
                                catch { }
                            }
    ...
    public void sdSavingInvAndMRR()
    {
        ...
        string vacode = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT ACODE FROM TYPE WHERE ID='B' AND TYPE1='" + frm_mbr + "'", "ACODE");
        foreach (DataRow dr in OLDDS.Tables[0].Rows)
        {
            ...
            oporow2["REJ_RW"] = "0";
            oporow2["IQTYOUT"] = "0";
            oporow2["IRATE"] = getDiscountedRate(oporow2["icode"].ToString().Trim(), fgen.seek_iname(frm_qstr, frm_cocd, "SELECT IRATE FROM ITEM WHERE TRIM(ICODE)='" + oporow2["ICODE"].ToString().Trim() + "'", "IRATE"), "0");
        ...
        roundOffAmount = (fgen.make_double(txtlbl31.Text.Trim(), 2) - fgen.make_double(txtlbl31.Text.Trim(), 0)).toDouble(2);
        totalInvoiceValue = fgen.make_double(txtlbl31.Text.Trim(), 0);


        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), 1, par_code, sal_code, totalInvoiceValue, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, totalInvoiceValue, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
        int srn = 50;
        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, sal_code, par_code, 0, fgen.make_double(txtlbl25.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl25.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
        srn += 1;
        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, tax_code, par_code, 0, fgen.make_double(txtlbl27.Text, 2), frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, fgen.make_double(txtlbl27.Text, 2), "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
        ...
        {
            if (roundOffAmount < 0)
                fgen.vSave(frm_qstr, frm_cocd, newBranchcd, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, schg_code, par_code, 0, roundOffAmount * -1, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, roundOffAmount * -1, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
            else fgen.vSave(frm_qstr, frm_cocd, newBranchcd, frm_vty, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), srn, schg_code, par_code, roundOffAmount, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, roundOffAmount, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr, "01");
        }
        ...
        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 1, sal_code, col3, sdTotalBasic, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, sdTotalBasic, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 2, tax_code, col3, sdTotalCGST, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, sdTotalCGST, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
        if (tax_code2.Length > 0)
        {
            fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 3, tax_code2, col3, sdTotalSGST, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, sdTotalSGST, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
        }

        roundOffAmount = (sdTotalValue - sdTotalValue.toDouble(0)).toDouble(2);
        sdTotalValue = sdTotalValue.toDouble(0);
        if (roundOffAmount != 0)
        {
            if (roundOffAmount < 0)
                fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 4, schg_code, col3, roundOffAmount * -1, 0, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, roundOffAmount * -1, 0, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
            else fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 4, schg_code, col3, 0, roundOffAmount, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, roundOffAmount, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
        }

        fgen.vSave(frm_qstr, frm_cocd, newBranchcd, "50", frm_vnum1, Convert.ToDateTime(txtvchdate.Text.Trim()), 50, col3, sal_code, 0, sdTotalValue, frm_vnum, Convert.ToDateTime(txtvchdate.Text.Trim()), rmToVSave, 0, 0, 1, 0, sdTotalValue, "-", Convert.ToDateTime(txtvchdate.Text.Trim()), frm_uname, Convert.ToDateTime(vardate), lbl27.Text.Substring(0, 2), 0, billQty, "", "-", Convert.ToDateTime(vardate), "-", frm_tab_vchr);
        #endregion
    }
    //------------------------------------------------------------------------------------
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["store"] = (sg1.Rows[i].Cells[3].Text.Trim() == "N") ? "N" : "Y";
                oporow["rec_iss"] = "C";
                ...
                oporow["SALE_REP"] = sg1.Rows[i].Cells[0].Text.Trim();
                ...
                try
                {
                    po_dts = fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t21")).Text.Trim(), vardate);
                }
                catch (Exception)
                {
                ...
                oporow["podate"] = fgen.make_def_Date(((TextBox)sg1.Rows[i].FindControl("sg1_t16")).Text.Trim(), vardate);
    ...
    void save_fun2()
    {
        ...
        oporow2["amt_frt"] = chkso.Checked ? "1" : "0";
    ...
    void Type_Sel_query()
    {
        ...
        SQuery = "SELECT type1 AS FSTR,NAME,type1 AS CODE,Acode AS Acctg_code FROM type where id='V' and type1 like '4%' and type1 not in ('4F') order by type1";

        btnval = hffield.Value;
        if (btnval != "New")
        {
            SQuery = "select Fstr,Document_Name,Document_Series,count(*) as Document_Count from (SELECT distinct a.type as Fstr,b.NAME as Document_Name,a.TYPE as Document_Series,a.vchnum FROM " + frm_tab_sale + " a ,TYPE b WHERE a.branchcd='" + frm_mbr + "' and a.vchdate " + DateRange + " and trim(A.type)=trim(B.type1) and b.ID='V' and a.type like '4%') group by Fstr,Document_Name,Document_Series order by Document_Series ";
        }
    ...
    string getDiscountedRate(string ticode, string currRate, string tax)
    {
        ...
        {
            cond = "BRANCHCD='00'";
            if (!sdWorking) cond = "BRANCHCD='" + frm_mbr + "'";
            dtdisc = fgen.getdata(frm_qstr, frm_cocd, "SELECT TRIM(ICODE) AS ICODE,num1 as irate,num2 AS IRATE2,to_char(vchdate,'yyyymmdd') as vdd FROM SCRATCH2 WHERE " + cond + " AND TYPE='DS' ORDER BY ICODE,vdd desc ");
        ...
        string rateDiscount = fgen.seek_iname_dt(dtdisc, "ICODE='" + ticode + "' ", "IRATE", "vdd desc");
    ...
    void sendMail()
    {
        ...
        SQuery = "select distinct A.MORDER, 'N' as logo_yn, a.branchcd,a.cess_pu,a.type,d.ciname,d.cpartno as dpartno,to_char(a.podate,'Mon yyyy') as po_month," +
            "a.iexc_addl,trim(a.finvno)||' Dt.'||to_char(a.podate,'dd/mm/yyyy') as po,a.idiamtr as mrp,a.finvno,a.exc_57f4,a.iexc_Addl,A.exc_amt,a.vchnum,a.o_deptt,a.exc_rate," +
            "to_char(a.vchdate,'dd/mm/yyyy') as vchdate,a.acode,c.chlnum,'-' as sd_val,to_char(c.chldate,'dd/mm/yyyy') as chldate,b.addr1 as caddr1,b.addr2 as caddr2,b.addr3 as caddr3,b.addr4 as caddr4" +
            ",b.mobile as ctel,nvl(b.dlno,'-') as dlno,b.person as cperson,b.rc_num2 as cstno, b.rc_num as tinno,b.exc_num as pcstno, c.pono,to_char(c.podate,'dd/mm/yyyy') as podate,c.exc_not_no," +
            "c.no_bdls,c.mode_tpt,c.mo_vehi,c.insur_no,c.st_entform,c.ins_cert,c.grno,c.stform_no,c.mcomment,to_char(c.remvdate,'dd/mm/yyyy') as remvdate,c.remvtime,c.bill_qty,c.naration,c.st_type," +
            "c.st_rate,c.drv_name,c.drv_mobile,c.freight,c.weight,c.invtime,c.st31_form,c.ins_co,to_char(c.grdate,'dd/mm/yyyy') as grdate,to_char(c.stform_dt,'dd/mm/yyyy') as stform_dt,c.cscode," +
            "c.act_tpt_amt,c.ins_no,c.destin,c.pack_rate,c.tptbill_no,c.sta_rate,c.sta_amt,c.totdisc_amt,c.tsubs_amt,c.retention,c.bill_tot,c.amt_sttt,c.amt_stsc,c.amt_sale,c.amt_exc,c.rvalue," +
            "c.amt_job,c.st_amt,c.amt_rea,b.aname, a.location,a.srno,a.icode,a.purpose as iname,a.exc_57f4 as cpartno,a.irate,a.revis_no as cdrgno,a.finvno as pordno,a.ponum as ordno," +
            "to_char(a.podate,'dd/mm/yyyy') as orddt,a.pname as nsp_flag,a.approxval as bal,a.ichgs as cdisc,a.iamount,a.iqtyout as qty,a.desc_,a.fabtype as strt,a.mode_tpt as stcd,ipack as stk," +
            "a.no_bdls as pkg,a.ent_by,to_char(a.ent_dt,'dd/mm/yyyy') as ent_dt,a.btchno,to_char(a.rtn_date,'mm-yy') as expdt,to_char(a.rGPdate,'mm-yy') as mfgdt,d.unit,a.exc_RATE as cgst," +
            "a.exc_amt as cgst_val,a.cess_percent as sgst,a.cess_pu as sgst_val,a.iopr,d.hscode,b.gst_no as cgst_no,b.girno,b.staten,b.vencode,t.type1,t1.name,C.tcsamt,c.acvdrt,a.doc_tot " +
            "from ivoucher a,sale c,item d,type t1,famst b left join type t on trim(b.staten)=trim(t.name) and t.id='{' where trim(a.acode)=trim(b.acode) and trim(a.type)=trim(t1.type1) " +
            "and t1.id='V' and a.BRANCHCD||a.TYPE||TRIM(a.vchnum)||TO_CHAr(a.vchdate,'DDMMYYYY')= c.BRANCHCD||c.TYPE||TRIM(c.vchnum)||TO_CHAr(c.vchdate,'DDMMYYYY') and trim(A.icode)=trim(d.icode) " +
            "AND A.BRANCHCD='" + frm_mbr + "' and A.TYPE ='" + frm_vty + "' and TRIM(a.vchnum)||TO_CHAR(A.vchdate,'DD/MM/YYYY') in (" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + ") order by vchdate," +
            "a.vchnum,a.MORDER";
    ...
    bool checkControlsLinkedCorrectly()
    {
        ...
        if (opt == "Y")
        {
            if (doc_GST.Value == "GCC")
            {
                col1 = fgen.getOption(frm_qstr, frm_cocd, "W0083", "OPT_PARAM");
                if (col1.ToString().Length < 6)
                {
                    fgen.msg("-", "AMSG", "Sales GST Control Not Linked Correctly, Check Control No. W0083");
                    return false;
                }
            }
            else
            {
                ...
                if (col1.ToString().Length < 6)
                {
                    fgen.msg("-", "AMSG", "Sales GST Control Not Linked Correctly, Check Control No. W0077, W0078, W0079");
                    return false;
                }
            }
            ...
            //if (col1.ToString().Length < 6)
            //{
            //    fgen.msg("-", "AMSG", "Purchase GST Control Not Linked Correctly, Check Control No. W0080, W0081, W0082");
            //    return false;
            //}
        }
        else
    ...
    void editFunction(string fstrMvCol)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            for (i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sg1_dr["sg1_t15"] = dt.Rows[i]["revis_no"].ToString().Trim();
                sg1_dr["sg1_t16"] = dt.Rows[i]["podtd"].ToString().Trim();
                sg1_dr["sg1_t21"] = dt.Rows[i]["refdate"].ToString().Trim();
            ...
            sg1_dt.Dispose();
            //------------------------
            SQuery = "Select to_char(a.vchdate,'dd/mm/yyyy') as vchdt,to_char(a.remvdate,'dd/mm/yyyy') as rmvdtd,to_char(a.podate,'dd/mm/yyyy') as podtd,a.* from " + frm_tab_sale + " a where a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + fstrMvCol + "' ";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
            ...
            sg2_dt.Dispose();
            //------------------------
            SQuery = "Select nvl(a.udf_name,'-') as udf_name,nvl(a.udf_value,'-') as udf_value from udf_Data a where trim(a.par_tbl)='" + frm_tab_ivch + "' and trim(a.par_fld)='" + fstrMvCol + "' ORDER BY a.srno";
            dt = new DataTable();
            dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
            ...
            //------------------------
            SQuery = "Select a.icode,to_chaR(a.dlv_Date,'dd/mm/yyyy') As dlv_Date,nvl(a.budgetcost,0) as budgetcost,nvl(a.actualcost,0) as actualcost,a.jobcardrqd,b.iname,nvl(b.cpartno,'-') As cpartno,nvl(b.cdrgno,'-') as cdrgno,nvl(b.unit,'-') as Unit from budgmst a,item b where trim(a.icode)=trim(b.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + fstrMvCol + "' and 1=2 ORDER BY A.SRNO ";
            //union all Select '-' as icode,to_DatE(to_char(sysdate,'dd/mm/yyyy'),'dd/mm/yyyy') as dlv_Date,0 as budgetcost,'-' as iname,'-' As cpartno,'-' as cdrgno,nvl(b.unit,'-') as Unit from dual                         
