...
public partial class om_asset_stkr : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "Sticker Configuration";
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "wb_fa_stkr";
        switch (Prg_Id)
        {


            case "F61106":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "CA");
                typePopup = "N";
                frm_vty = "CA";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "New":
               // SQuery = "select CCMno||CCMdt  as fstr,CCMno as CCM_No,CCMdt as CCM_DT,Cust_Name,max(comp_type) as Module_name,max(Cdescr) as PRoduct,CCMdd from (Select upper(Cust_name) as Cust_Name,trim(CCMno) as CCMno,to_Char(CCMdt,'dd/mm/yyyy') as CCMdt,Comp_type,1 as dasg,1 as conf,to_Char(cCMdt,'yyyymmdd') as CCMdd,Cdescr from WB_CCM_LOG where branchcd='" + frm_mbr + "' and type='CC' and nvl(CCM_Close,'-')!='Y' union all  Select upper(Cust_name),trim(CCMno) as CCMno,to_Char(CCMdt,'dd/mm/yyyy') as cCMdt,null as Emod, 0 as dasg,0 as impl,to_Char(CCMdt,'yyyymmdd') as cssdd,null as lvertical from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='AC' ) group by CCMno||CCMdt,CCMno,CCMdt,Cust_Name,CCMdd  order by CCMdd,CCMno ";
                SQuery = "select trim(branchcd)||trim(acode)||trim(vchnum)||to_char(vchdate,'dd/MM/yyyy') as fstr,acode,assetname,grpcode,assetsupp,assetsuppadd from wb_fa_pur where branchcd='" + frm_mbr + "' and type='10' and vchdate " + DateRange + " ORDER BY VCHNUM,VCHDATE";
                //LDESCR
                //MODULE_NAME
                break;
            case "Edit":
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    //SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as CAC_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as CAC_Dt,a.Cust_Name as company,a.Comp_type ,a.Cdescr as Product,a.Cont_name ,a.CCMno as CCMno,to_char(a.CCMdt,'dd/mm/yyyy') as CCMdt,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='CA' order by vdd desc,a." + doc_nf.Value + " desc";
                    SQuery = "select distinct trim(a.branchcd)||trim(a.type)||trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as ENTRY_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as ENTRY_Dt,a.FIELD1,A.FIELD2,A.FIELD3,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='10' order by vdd desc,a." + doc_nf.Value + " desc";
                    break;
        }
    ...
    void newCase(string vty)
    {
        ...
        SQuery ="select * from wb_fa_stkr where branchcd='"+frm_mbr+"'and type='10'and vchdate "+DateRange+"";
        dt = fgen.getdata(frm_qstr,frm_cocd,SQuery);
        if (dt.Rows.Count > 0)
        {
            fgen.msg("-", "AMSG", "Dear " + frm_uname + " , You Currently Do Not Have Rights To Add New Entry For This Form. Entry Is already Configured. !!");
            return;
        }
        frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
        //frm_vty = vty;
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(" + doc_nf.Value + ") AS VCH FROM " + frm_tabname + " WHERE BRANCHCD='" + frm_mbr + "' and vchdate "+DateRange+"", 6, "VCH");

        //txtlbl4.Value= col1.Substring(2,6);
       // string assetname= fgen.seek_iname(frm_qstr,frm_cocd,"select a.assetname from wb_fa_pur a where trim(a.branchcd)||trim(a.acode)||trim(a.vchnum)||to_char(a.vchdate,'dd/MM/yyyy')='" + col1 + "'","assetname");
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        //{
        //    fgen.msg("-", "AMSG", "Please Enter Input From !!");
        //    return;
        //}
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    string mv_col;
                    mv_col = col1;
                    SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date from " + frm_tabname + " a where a.branchcd||a.type||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + mv_col + "' ORDER BY A.vchnum desc";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                       ...
                       // txtlbl9.Value = fgen.seek_iname(frm_qstr, frm_cocd, "select a.assetname from wb_fa_pur a where trim(a.acode)='" + dt.Rows[i]["assetid"].ToString().Trim() + "' and a.branchcd='" + frm_mbr + "' and a.vchdate " + DateRange + "", "assetname");
                        ddfield1.Value = dt.Rows[i]["field1"].ToString().Trim();
                        ddfield2.Value = dt.Rows[i]["field2"].ToString().Trim();
                        ddfield3.Value = dt.Rows[i]["field3"].ToString().Trim();
                        //ddnextfolo.Value = dt.Rows[i]["Next_Folo"].ToString().Trim();

                        //txtrmk.Text = dt.Rows[i]["ORemarks"].ToString().Trim();

                        //SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date,to_Char(a.app_Dt,'dd/mm/yyyy') As mapp_date from WB_CCM_LOG a where a.branchcd||a.type||trim(a.CCMno)||to_Char(a.cCMdt,'dd/mm/yyyy')='" + frm_mbr + "CC" + txtvchnum.Value + txtvchdate.Value + "' ORDER BY A.SRNO";
                        //dt = new DataTable();
                        //dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        //if (dt.Rows.Count > 0)
                        //{

                        //    txtlbl4.Value = dt.Rows[i]["Cust_NAME"].ToString().Trim();
                        //    ddModule.Value = dt.Rows[i]["comp_type"].ToString().Trim();
                        //    txtlbl8.Value = dt.Rows[i]["Cdescr"].ToString().Trim();

                        //    txtlbl9.Value = dt.Rows[i]["Compcatg"].ToString().Trim();
                        //    ddIssueType.Value = dt.Rows[i]["compOccr"].ToString().Trim();
                        ...
                        //    Text_cl_rmk.Text  = dt.Rows[i]["LRemarks"].ToString().Trim();
                        //}
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "";
            //SQuery = "SELECT a.CACNO as Action_NO,to_char(A.CACDT,'dd/mm/yyyy') as Action_Dt,a.CCMNO as CCM_NO,to_char(A.CCMDT,'dd/mm/yyyy') as CCM_Dt,a.Cust_name as Co_Name,a.Comp_type,a.Cdescr as Product,a.Compcatg as Grading,a.Oremarks as Our_Remark,a.Cont_name,a.Curr_stat,a.Act_mode,a.Next_Folo as Close_by,a.Ent_Dt,a.app_by,a.app_dt,to_chaR(a.CACDT,'YYYYMMDD') as CAC_DTd  FROM WB_CCM_ACT a WHERE a.branchcd='" + frm_mbr + "' and A.TYPE='CA' AND A.CACDT " + PrdRange + "  ORDER BY CAC_DTd,A.CACNO";
            SQuery = "select a.vchnum as Entry_no, a.field1 as field1,a.field2 as field2, a.field3 as field3, a.ent_by as entry_by ,to_char(a.ent_DT,'dd/MM/yyyy') as Entry_date from wb_fa_stkr a where a.branchcd='" + frm_mbr + "' and a.type='10' and a.vchdate " + PrdRange + " order by a.vchnum,a.vchdate";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        ...
        else
        {
            ...
            string last_entdt;
            //checks
            last_entdt = fgen.seek_iname(frm_qstr, frm_cocd, "select to_char(max(" + doc_df.Value + "),'dd/mm/yyyy') as ldt from " + frm_tabname + " where branchcd='" + frm_mbr + "'", "ldt");
                ...
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        if (edmode.Value == "Y")
                        {
                            ...
                            fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " where branchcd='DD' and type||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + ddl_fld2.Substring(2, 18) + "'");
                        ...
                        //if (agt_email.Trim().Length <= 3) agt_email = "pkgupta@Tejaxo.in";

                        //html_body = html_body + "Please note Task No : " + frm_vnum + " For " + txtlbl4.Value.ToUpper().Trim() + "<br>";
                        //html_body = html_body + "This Task is For CSS No : " + txtvchnum.Value.Trim() + " Dtd " + txtvchdate.Value.Trim() + "<br>";
                        //html_body = html_body + "This Task is Expected to be done within : " + ddresolution.Value + " Hrs" + "<br>";
                        //html_body = html_body + "Please contact of any clarification ASAP , <br>";
                        //html_body = html_body + "Aiming for Best Customer Support, <br>";
                        //html_body = html_body + "Team Tejaxo <br>";

                        //fgen.send_mail(frm_cocd, "Tejaxo ERP", agt_email , "", "", "Task # "+ frm_vnum+" for "+ txtlbl4.Value.ToUpper().Trim() +" Assigned " , html_body);
    ...
    void save_fun()
    {
        ...
        oporow["field1"] = ddfield1.Value.Trim();
        oporow["field2"] = ddfield2.Value.Trim();
        oporow["field3"] = ddfield3.Value.Trim();
        ...
        //fgen.execute_cmd(frm_qstr, frm_cocd, "update wb_CCM_LOG set CURR_STAT='" + ddfield1.Value.Trim() + "',last_Action='" + ddCmode.Value.Trim() + "',last_Actdt='" + txtDFdt.Value.Trim() + "' where branchcd||type||trim(CCMNO)||to_char(CCMDT,'dd/mm/yyyy')='" + frm_mbr + "CC" + txtvchnum.Value + txtvchdate.Value + "'");
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F60106":
                SQuery = "SELECT 'LA' AS FSTR,'Lead Action Logging' as NAME,'LA' AS CODE FROM dual";
    ...
    protected void btnView_Click(object sender, ImageClickEventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(this, this.GetType(), "PopUP", "OpenSingle('" + "c:/tej_erp/" + filePath.Replace("\\", "/") + "','90%','90%','Tejaxo Viewer');", true);
    }
