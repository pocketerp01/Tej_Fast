...
public partial class om_pinv_entry : System.Web.UI.Page
{
    ...
    void setColHeadings()
    {
        ...
        for (int sR = 0; sR < sg1.Columns.Count; sR++)
        {
            ...
            for (int K = 0; K < sg1.Rows.Count; K++)
            {
                ...
                if ((frm_formID == "F70116") && frm_cocd != "DESH" && frm_cocd != "TEJAXO")
                {
                    if (hf150.Value != "")
                    {
                ...
                if (frm_formID == "F70108" || frm_formID == "F70110" || frm_formID == "F70116")
                    ((TextBox)sg1.Rows[K].FindControl("sg1_t4")).Attributes.Remove("readonly");
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                cond = "";
                if (Prg_Id == "F70112" || Prg_Id == "F70116")
                {
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (frm_cocd != "SGRP")
        {
            ...
            {
                if ((Convert.ToDateTime(last_entdt) > Convert.ToDateTime(txtvchdate.Text.ToString())) && frm_cocd != "KRSM")
                {
                    Checked_ok = "N";
            ...
            if (Convert.ToDateTime(txtvchdate.Text.ToString()) > Convert.ToDateTime(last_entdt) && edmode.Value == "N" && frm_cocd != "KRSM")
            {
                Checked_ok = "N";
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_h3"] = "-";
                            sg1_dr["sg1_h4"] = "0";
                            sg1_dr["sg1_h5"] = dt.Rows[d]["icode"].ToString().Trim();
                            sg1_dr["sg1_h6"] = dt.Rows[d]["iname"].ToString().Trim();
                            sg1_dr["sg1_h7"] = "-";
                            sg1_dr["sg1_h8"] = "-";
                            sg1_dr["sg1_h9"] = "-";
                            sg1_dr["sg1_h10"] = "-";
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "Select a.Vchnum as Inv_No,to_char(a.vchdate,'dd/mm/yyyy') as Dated,c.Aname as Supplier,a.purpose  as Item_Name,a.exc_57f4 as Part_No,a.iqtyout as sale_Qty,a.Irate,a.ichgs as Disc" +
    ...
    protected void sg1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":
                if (frm_formID == "F70116" && frm_cocd != "DESH")
                {
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["store"] = (frm_vty == "58" || frm_vty == "59") ? "Y" : "N";
                oporow["rec_iss"] = "C";
                ...
                {
                    oporow["iqtyout"] = 0;
                    oporow["iqtyin"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().toDouble();
                }
                if (frm_vty == "59")
                {
                    oporow["iqtyin"] = 0;
                    oporow["iqtyout"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().toDouble();
                }
    ...
    protected void btnPMRR_Click(object sender, EventArgs e)
    {
        string header_n = "MRR Passing Report";
        SQuery = "SELECT  '-' as fstr,TRIM(A.BRANCHCD) AS BRANCHCD,A.VCHNUM AS MRRNO,A.TYPE,TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS MRRDATE" +
            ",trim(A.ACODE) as acode,trim(B.ANAME) as aname, A.INVNO,A.INVDATE, A.O_DEPTT " +
            ",A.DESC_,A.FINVNO,(case when nvl(trim(a.inspected),'-')='Y' then '(Insp.'||trim(a.pname)||')'||trim(A.finvno) else '-' end) as inspby" +
            ",TRIM(B.ADDR1)||TRIM(B.ADDR2) AS ADRES " +
            "FROM IVOUCHER A,FAMST B WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND A.BRANCHCD='" + frm_mbr + "' " +
            "and a.store not in ('R','W') and nvl(trim(a.inspected),'-')='Y' and trim(A.finvno)='-' and a.srno='1'  ORDER BY A.MORDer,a.vchnum desc";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        fgen.Fn_open_rptlevel("All Pending MRRs Till Date", frm_qstr, "");
    }
}