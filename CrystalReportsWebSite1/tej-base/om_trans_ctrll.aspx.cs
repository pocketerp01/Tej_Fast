...
public partial class om_trans_ctrll : System.Web.UI.Page
{
    ...
    public void enablectrl()
    {
        ...
        ddcamtype.Disabled = false;
        ddpurpose.Disabled = false;
        dddurn.Disabled = false;
    }
    //------------------------------------------------------------------------------------
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "PERSON":
                ...
                SQuery = "SELECT Contactno as fstr ,USERNAME AS User_Code,Emailid,Contactno FROM EVAS WHERE userid<'000052' and NVL(USERNAME,'-')!='-' and UPPER(TRIM(username))='" + txtlbl4.Value.ToUpper().Trim() + "' ORDER BY USERNAME";
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as CTRL_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as CTRL_Dt,a.CTRL_Type,a.CTRL_spec,a.Acode as Team_member,a.cont_name as person,a.cont_no as contact_no, a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='" + lbl1a_Text + "' " + cond + " order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        ddcamtype.Value = dt.Rows[i]["CTRL_type"].ToString().Trim();
                        txtlbl8.Value = dt.Rows[i]["CTRL_spec"].ToString().Trim();
                        ddpurpose.Value = dt.Rows[i]["CTRL_purpose"].ToString().Trim();
                        //dddurn.Value = dt.Rows[i]["cam_durn"].ToString().Trim();
                ...
                try
                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "Trans No " + frm_vnum + "'13'We appreciate your Entry to Finsys");
                        }
                        else
                        {
    ...
    void save_fun()
    {
        ...
        oporow["CTRL_type"] = ddcamtype.Value;
        oporow["CTRL_spec"] = txtlbl8.Value.ToUpper().Trim();

        oporow["CTRL_purpose"] = ddpurpose.Value;
        //oporow["cam_durn"] = dddurn.Value;
