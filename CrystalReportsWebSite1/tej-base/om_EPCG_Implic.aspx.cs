...
public partial class om_EPCG_Implic : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        lblheader.Text = "EPCG Import Lic Adjustment";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "BTN_19":
                break;
            case "LICNO":
                //SQuery = "Select Distinct a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.licno) as fstr,a.licno as licence_no,to_char(licdt,'dd/mm/yyyy') as licence_dt, trim(a.vchnum) as doc_no,to_char(a.vchdate,'dd/mm/yyyy') as doc_dt,a.ciname as description,a.acode as hsn_code,a.qtyin as qty,to_char(vchdate,'yyymmdd') as vdd from wb_licrec a where a.branchcd!='DD' and a.type='10' order by vdd ";
                SQuery = "Select Distinct a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.licno)||trim(ciname)||TRIM(SRNO) as fstr,a.licno as licence_no,to_char(licdt,'dd/mm/yyyy') as licence_dt,a.ciname as description,to_char(a.impvalid,'dd/mm/yyyy') as validity,a.imp_val as import_value,a.imp_qty as import_qty,to_char(a.expvalid,'dd/mm/yyyy') as last_date,to_char(vchdate,'yyyymmdd') as vdd from wb_licrec a where a.branchcd='" + frm_mbr + "' and a.type='11' order by vdd,licence_no";
                break;
            ...
            case "SG1_ROW_ADD_E":
                ...
                if (frm_cocd == "SAGM")
                {
                    SQuery = "select trim(fstr) as fstr, trim(bill_Of_entry) as bill_Of_entry ,to_char(refdate,'dd/mm/yyyy') as bill_Of_date,trim(icode) as icode,iqtyin  from (select distinct trim(refnum)||to_char(refdate,'dd/mm/yyyy')||trim(icode) as fstr, trim(refnum) as bill_Of_entry,refdate,icode ,iqtyin , 1 as qty from ivoucher where branchcd='" + frm_mbr + "' and type='07' and trim(refnum)!='-' and substr(trim(icode),1,1)='5' union all select distinct trim(billno)||to_char(bill_dt,'dd/mm/yyyy')||trim(icode) as fstr,trim(billno) as bill_Of_entry,bill_dt,icode,qtyin ,-1 as qty from wb_licrec where branchcd='" + frm_mbr + "' and type='" + frm_vty + "') where bill_Of_entry||TO_CHAR(refdate,'DD/MM/YYYY') not in (" + col1 + ") and refdate between to_date('" + txtlicdt.Text + "','dd/mm/yyyy') and to_date('" + txtlbl5.Text + "','dd/mm/yyyy') group by trim(fstr),trim(icode), trim(bill_Of_entry),refdate,iqtyin having sum(qty)>0 ORDER BY FSTR";
                }
                else
                {
                    SQuery = "select trim(fstr) as fstr, trim(bill_Of_entry) as bill_Of_entry ,trim(refdate) as bill_Of_date,trim(icode) as icode,iqtyin  from (select distinct trim(b.be_refno)||b.be_refdt||trim(a.icode) as fstr,trim(b.be_refno) as bill_Of_entry,b.be_refdt as refdate,a.icode,a.iqtyin,1 as qty from ivoucher a,ivchctrl b where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')=b.branchcd||b.type||trim(b.vchnum)||to_char(b.vchdate,'dd/mm/yyyy') and a.branchcd='" + frm_mbr + "' and a.type='07' and NVL(trim(b.be_refno),'-')!='-' and substr(trim(icode),1,1)='5' union all select distinct trim(billno)||to_char(bill_dt,'dd/mm/yyyy')||trim(icode) as fstr,trim(billno) as bill_Of_entry,to_char(bill_dt,'dd/mm/yyyy') as refdate,icode,qtyin ,-1 as qty from wb_licrec where branchcd='" + frm_mbr + "' and type='" + frm_vty + "') where bill_Of_entry||refdate not in (" + col1 + ") and to_date(refdate,'dd/mm/yyyy') between to_date('" + txtlicdt.Text + "','dd/mm/yyyy') and to_date('" + txtlbl5.Text + "','dd/mm/yyyy') group by trim(fstr),trim(icode),trim(bill_Of_entry),trim(refdate),iqtyin having sum(qty)>0 ORDER BY FSTR";
                }
                break;
            ...
            case "Print_E":
                SQuery = "select distinct a.branchcd||a.type||trim(a.licno)||to_char(a.licdt,'dd/mm/yyyy') as fstr,a.licno as licence_no,to_char(a.licdt,'dd/mm/yyyy') as licence_dt,to_char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a WHERE  a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' ORDER BY vdd DESC";
                SQuery = "select distinct a.branchcd||a.type||trim(a.licno)||to_char(a.licdt,'dd/mm/yyyy') as fstr,a.licno as licence_no,to_char(a.licdt,'dd/mm/yyyy') as licence_dt,trim(a.term) as item_name,to_char(a.vchdate,'yyyymmdd') as vdd from " + frm_tabname + " a WHERE  a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' and vchdate " + DateRange + " ORDER BY vdd DESC";
                break;

            default:
                frm_vty = fgenMV.Fn_Get_Mvar(frm_qstr, "U_VTY");
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    SQuery = "SELECT distinct trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy') as fstr,a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_dt,trim(a.licno) as licence_no,to_char(a.licdt,'dd/mm/yyyy') as licence_dt,TRIM(CINAME) AS ITEM_DESC,a.ent_by,to_char(a.ent_dt,'dd/mm/yyyy') as ent_Dt, to_char(a.vchdate,'yyyymmdd') as vdd FROM " + frm_tabname + " A WHERE A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE  " + DateRange + " ORDER BY vdd desc,A.VCHNUM DESC";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        //{
        //    fgen.msg("-", "AMSG", "Please Check Your Quantity!! '13' Balance Quantity Cnanot be Less than BOE Qty "); return;
        //}

        if (fgen.make_double(txt_exp_oblig_imp.Text.Trim()) > fgen.make_double(txtlbl7.Text.Trim()))
        {
            fgen.msg("-", "AMSG", "Export Obligation as per Import can't be more than Export Obligation!!");
            return;
        }
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            ...
            {
                fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Please Fill Exchange_Rate At Line No. " + sg1.Rows[i].Cells[12].Text.Trim());
                return;
            }
            if (((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim() == "-")
            {
                fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Please Fill Duty Saved(USD) At Line No. " + sg1.Rows[i].Cells[12].Text.Trim());
                return;
            }
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl7.Text = dt.Rows[0]["num1"].ToString().Trim();
                        txtlbl7a.Text = dt.Rows[0]["num2"].ToString().Trim();
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t4"] = dt.Rows[i]["num4"].ToString().Trim();
                            sg1_dr["sg1_t5"] = dt.Rows[i]["val_usd"].ToString().Trim();
                            sg1_dr["sg1_t6"] = dt.Rows[i]["obsv1"].ToString().Trim();
                            sg1_dr["sg1_t7"] = "";
                            sg1_dr["sg1_t8"] = fgen.seek_iname(frm_qstr, frm_cocd, "select trim(iname) as item_name from item where trim(icode)='" + dt.Rows[i]["icode"].ToString().Trim() + "'", "item_name");
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        sg1_add_blankrows();
                        ViewState["sg1"] = sg1_dt;
                ...
                case "LICNO":
                    ...
                    SQuery = "Select a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.licno)||TRIM(SRNO) as fstr,to_char(a.impvalid,'dd/mm/yyyy') as impvalid,A.VAL_USD,a.num1,a.num2,a.licno as lic_no,to_char(a.licdt,'dd/mm/yyyy') as lic_dt,a.QTYIN,a.cif_val as val,a.ciname,to_char(a.vchdate,'yyymmdd') as vdd from wb_licrec a where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')||trim(a.licno)||trim(ciname)||TRIM(SRNO)='" + col1 + "' order  by vdd";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //txtdesc.Text = dt.Rows[0]["desc_"].ToString().Trim();
                        //txtlbl5.Text = dt.Rows[0]["hsn_code"].ToString().Trim();
                        txtcurrqty.Text = dt.Rows[0]["QTYIN"].ToString().Trim();
                        txtcurrval.Text = dt.Rows[0]["VAL_USD"].ToString().Trim(); //VAL_USD
                        ...
                        mq0 = "select balqty from wb_licrec where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' and licno='" + txtlbl4.Text + "'";
                ...
                case "SG1_ROW_ADD_E":
                    if (col1.Length <= 0) return;
                    dt = new DataTable();
                    SQuery = "SELECT trim(a.refnum) as refnum,to_char(a.refdate,'dd/mm/yyyy') as refdate,a.icode,a.iqtyin,a.doc_tot as iamount,i.iname from ivoucher a,item i where trim(a.icode)=trim(i.icode) and a.branchcd='" + frm_mbr + "' and a.type='07' and trim(a.refnum)!='-' and trim(a.refnum)||to_char(a.refdate,'dd/mm/yyyy')||trim(a.icode) ='" + col1 + "' ORDER BY refnum";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        ...
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t2")).Text = (fgen.make_double(fgen.seek_iname_dt(dt2, "entry_no_bill='" + dt.Rows[d]["refnum"].ToString().Trim() + "' and entry_dt_bill='" + dt.Rows[d]["refdate"].ToString().Trim() + "' and icode='" + dt.Rows[d]["icode"].ToString().Trim() + "'", "cifval"))).ToString();
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t8")).Text = dt.Rows[d]["iname"].ToString().Trim();
                    }
                    setColHeadings();
                    break;
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (frm_cocd == "SAGM")
                        {
                            SQuery = "SELECT trim(a.refnum) as refnum,to_char(a.refdate,'dd/mm/yyyy') as refdate,a.icode,a.iqtyin,a.doc_tot as iamount,i.iname from ivoucher a,item i where trim(a.icode)=trim(i.icode) and a.branchcd='" + frm_mbr + "' and a.type='07' and trim(a.refnum)!='-' and trim(a.refnum)||to_char(a.refdate,'dd/mm/yyyy')||trim(a.icode) in (" + col1 + ") ORDER BY refnum";
                        }
                        else
                        {
                            SQuery = "select trim(b.be_refno) as refnum,b.be_refdt as refdate,a.icode,a.iqtyin,a.doc_tot as iamount,i.iname from ivoucher a,ivchctrl b,item i where a.branchcd||a.type||trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy')=b.branchcd||b.type||trim(b.vchnum)||to_char(b.vchdate,'dd/mm/yyyy') and trim(a.icode)=trim(i.icode) and a.branchcd='" + frm_mbr + "' and a.type='07' and trim(b.be_refno)||b.be_refdt||trim(a.icode) in (" + col1 + ")";
                        }
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_t2"] = (fgen.make_double(fgen.seek_iname_dt(dt2, "entry_no_bill='" + dt.Rows[d]["refnum"].ToString().Trim() + "' and entry_dt_bill='" + dt.Rows[d]["refdate"].ToString().Trim() + "' and icode='" + dt.Rows[d]["icode"].ToString().Trim() + "'", "cifval"))).ToString();
                            sg1_dr["sg1_t3"] = "";
                            sg1_dr["sg1_t4"] = "";
                            sg1_dr["sg1_t5"] = "";
                            sg1_dr["sg1_t6"] = "";
                            sg1_dr["sg1_t7"] = "";
                            sg1_dr["sg1_t8"] = dt.Rows[d]["iname"].ToString().Trim();
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                    }
                    sg1_add_blankrows();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "sELECT distinct trim(a.Vchnum) as Entry_no,to_char(a.Vchdate,'dd/mm/yyyy') as Entry_Dt,a.billno,TO_CHAR(a.bill_Dt,'DD/MM/YYYY') AS BILL_DT,a.licno as licence_no,to_char(licdt,'dd/mm/yyyy') as licence_dt,trim(a.icode) as item_code,trim(b.iname) as item_name,A.IMP_QTY,A.IMP_VAL,a.num4 as exchange_rate,a.val_usd as duty_saved_usd,a.Ent_by,a.Ent_Dt,to_char(a.vchdate,'yyyymmdd') as vdd FROM " + frm_tabname + " a,item b WHERE trim(a.icode)=trim(b.icode) and a.BRANCHCD='" + frm_mbr + "' AND a.TYPE='" + frm_vty + "' AND a.VCHDATE  " + PrdRange + " ORDER BY vdd DESC,entry_No DESC";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 1)
            {
                ...
                oporow["num1"] = txtlbl7.Text.Trim().ToUpper();
                oporow["refdate"] = txtlbl5.Text.Trim().ToUpper();//lic ending dt
                oporow["num2"] = fgen.make_double(txtlbl7a.Text.Trim().ToUpper());
                ...
                oporow["QTYOUT"] = fgen.make_double(txtlbl7a.Text.Trim()) * fgen.make_double(txt_duty_Saved_usd.Text.Trim());//for export_obligation as per import
                ...
                oporow["num4"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper()); //duty save
                oporow["val_usd"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim().ToUpper()); //duty save
                oporow["obsv1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.Trim().ToUpper(); //unit on qty
                oporow["obsv2"] = "-";
                oporow["obsv3"] = "-";
                oporow["obsv4"] = "-";
                oporow["obsv5"] = "-";
    ...
    public void Cal()
    {
        ...
        for (int sg1r = 0; sg1r < sg1.Rows.Count - 1; sg1r++)
        {
            ...
            min += fgen.make_double(((TextBox)sg1.Rows[sg1r].FindControl("sg1_t5")).Text.Trim());
            txt_duty_Saved_usd.Text = min.ToString().Trim();
        }
    }
    //------------------------------------------------------------------------------------
}