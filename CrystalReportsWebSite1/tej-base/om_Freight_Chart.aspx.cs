...
//FRT_CHRT  dummy icon id on local

public partial class om_Freight_Chart : System.Web.UI.Page
{
    ...
    public void enablectrl()
    {
        ...
        sg2_add_blankrows();
        sg3_add_blankrows();
        btnshipline.Enabled = false;
        btnlbl7.Enabled = false; btnmbr.Enabled = false; btncust.Enabled = false;
    ...
    public void disablectrl()
    {
        ...
        btnlbl7.Enabled = true; btnprint.Disabled = true; btnlist.Disabled = true;
        btnmbr.Enabled = true; btncust.Enabled = true;
        btnshipline.Enabled = true;
    }
    //------------------------------------------------------------------------------------
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case  "CUST":
                SQuery = "SELECT distinct TRIM(A.ACODE) AS FSTR,TRIM(A.ACODE) AS ACODE,TRIM(B.ANAME) AS ANAME FROM  IVOUCHERP A,FAMST B WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='4F' AND A.VCHDATE " + DateRange + " ORDER BY FSTR";
                break;
            ...
            case "SG1_ROW_ADD_E":
                ...
                if (txtmbrcode.Text == "00")
                {
                    SQuery = "select a.branchcd||a.type||trim(a.invno)||a.invdate as fstr,a.invno,a.invdate ,trim(a.acode) as party_code,trim(b.aname) as party_name from (select distinct branchcd,type,trim(vchnum) as invno,to_char(vchdate,'dd/mm/yyyy') as invdate,trim(acode) as acode ,1 as qty from IVOUCHERP where branchcd!='DD' and type='4F' and vchdate " + PrdRange + " union all select distinct BRANCHCD,'4F' AS TYPE,trim(INVNO) as invno,to_char(INVDATE,'dd/mm/yyyy') as invdate,trim(acode) as acode,-1 AS QTY from wb_exp_frt WHERE BRANCHCD!='DD' AND TYPE='" + frm_vty + "' and vchdate " + PrdRange + ") a,famst b where trim(a.acode)=trim(b.acode)  " + col1 + " group by a.branchcd||a.type||trim(a.invno)||a.invdate ,invno,a.invdate ,trim(a.acode),trim(b.aname) having sum(qty)>0 order by fstr";
                }
                else
                {
                    SQuery = "select a.branchcd||a.type||trim(a.invno)||a.invdate as fstr,a.invno,a.invdate ,trim(a.acode) as party_code,trim(b.aname) as party_name from (select distinct branchcd,type,trim(vchnum) as invno,to_char(vchdate,'dd/mm/yyyy') as invdate,trim(acode) as acode ,1 as qty from IVOUCHERP where branchcd='" + frm_mbr + "' and type='4F' and vchdate " + PrdRange + " union all select distinct BRANCHCD,'4F' AS TYPE,trim(INVNO) as invno,to_char(INVDATE,'dd/mm/yyyy') as invdate,trim(acode) as acode,-1 AS QTY from wb_exp_frt WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='" + frm_vty + "' and vchdate " + PrdRange + ") a,famst b where trim(a.acode)=trim(b.acode)  " + col1 + " group by a.branchcd||a.type||trim(a.invno)||a.invdate ,invno,a.invdate ,trim(a.acode),trim(b.aname) having sum(qty)>0 order by fstr";
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txt_ocean.Text = dt.Rows[0]["CINAME"].ToString().Trim();                            
                        ...
                        //============================
                        txtrail_charg_rs.Text = dt.Rows[0]["num1"].ToString().Trim();
                        txtrail_charg_dlr.Text = dt.Rows[0]["num2"].ToString().Trim();
                        txt_locl_chrg_rs.Text = dt.Rows[0]["NUM3"].ToString().Trim();
                        txt_locl_chrg_dlr.Text = dt.Rows[0]["NUM4"].ToString().Trim();
                        txt_custm_clrnce_rs.Text = dt.Rows[0]["NUM5"].ToString().Trim();
                        txt_custm_clrnce_dlr.Text = dt.Rows[0]["NUM6"].ToString().Trim();
                        txt_ocean_fryt_rs.Text = dt.Rows[0]["NUM7"].ToString().Trim();
                        txt_ocean_fryt_dlr.Text = dt.Rows[0]["NUM8"].ToString().Trim();
                        txt_thc_chrg_rs.Text = dt.Rows[0]["NUM9"].ToString().Trim();
                        txt_thc_chrg_dlr.Text = dt.Rows[0]["NUM10"].ToString().Trim();
                        txt_doc_chg_rs.Text = dt.Rows[0]["NUM11"].ToString().Trim();
                        txt_doc_chg_dlr.Text = dt.Rows[0]["NUM12"].ToString().Trim();
                        txt_Ex_rate.Text = dt.Rows[0]["NUM13"].ToString().Trim();
                        txt_tot_fryt_dlr.Text = dt.Rows[0]["NUM14"].ToString().Trim();
                        txt_tot_fryt_rs.Text = dt.Rows[0]["NUM15"].ToString().Trim();
                       // txt_ratio.Text = dt.Rows[0]["NUM16"].ToString().Trim();
                        txt_oth_chg.Text = dt.Rows[0]["NUM17"].ToString().Trim();
                        txtshipno.Text = dt.Rows[0]["obsv10"].ToString().Trim();
                        txt_bl.Text = dt.Rows[0]["obsv3"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
          ...
          // SQuery = "select a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_date,A.container_no,A.CONT_SIZE ,B.NAME AS FORWARD_AGENT,trim(A.invno) as invoice_no,TO_CHAR(A.invdate,'DD/MM/YYYY') AS INVDATE,A.iamount as value,TO_CHAR(A.refdate,'DD/MM/YYYY') as shipment_date,A.cif_val as net_weight,A.forign_val as total_weight,A.iqtyin as no_of_pkgs,A.num1 as pkg_no_from,A.num2 as pkg_no_to,A.num3 as local_charge_inr,A.num4 as local_charge_fc,A.num5 as custom_clr_charges_inr,A.num6 as custom_clr_charges_fc,A.num7 as concor_rail_charges_inr,A.num8 as concor_rail_charges_fc,A.num9 as ocean_freight_inr,A.num10 as ocean_freight_fc,A.Num11 as thc_charges_inr ,A.num12 as thc_charges_fc,A.num13 as doc_charges_inr,A.num14 as doc_charges_fc,A.num15  as  other_charges_inr,A.num16 as other_charges_fc,A.num17 as ex_rate,A.num18 as total_freight_fc,A.num19 as total_freight_inr,to_char(a.vchdate,'yyyymmdd') as vdd from wb_exp_frt A, TYPEGRP B WHERE TRIM(A.obsv4) = TRIM(B.TYPE1) AND B.ID='^9' AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' and a.vchdate " + PrdRange + " order by vdd desc,entry_no desc ";
            SQuery = "select a.vchnum as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_date,A.container_no,A.CONT_SIZE ,a.obsv2 as ship_code,a.obsv11 AS FORWARD_AGENT,trim(A.invno) as invoice_no,TO_CHAR(A.invdate,'DD/MM/YYYY') AS INVDATE,A.iamount as value,TO_CHAR(A.refdate,'DD/MM/YYYY') as shipment_date,A.cif_val as net_weight,A.forign_val as total_weight,A.iqtyin as no_of_pkgs,A.num1 as rail_charges_rs,a.num2  as rail_charges_dollar,A.num3  as local_charge_rs,A.num4 as local_charge_dollar ,A.num5 as custom_clearance_rs,A.num6 as custom_clearance_dollar ,A.num7 as ocean_freight_rs ,A.num8 as ocean_freight_dollar ,A.num9 as thc_charge_rs ,A.num10 as thc_charge_dollar,A.Num11 as doc_charges_rs ,A.num12 as doc_charge_dollar,A.num13 as ex_rate ,A.num14 as total_freight_dollar ,A.num15 as total_freight_rs,A.num17 as other_charges,a.date1 as disp_date,a.date2 as suff_date,a.date3 as railout_date,a.date4 as sob_date,a.date5 as eta_Date,a.date6 as doc_Sent_date,a.date7 as do_date,a.date8 as ware_Date from wb_exp_frt A WHERE   A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' and a.vchdate " + PrdRange + " order by entry_no desc";
    ...
    protected void sg1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":
                if (txtmbrcode.Text.Trim().Length <= 1)
                {
                    fgen.msg("-", "AMSG", "Please Select Branch First !!");
                    return;
                }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            ...
            if (sg1.Rows[i].Cells[13].Text.Length > 2)
            {
                ...
                oporow["NUM1"] = Math.Round(fgen.make_double(txtrail_charg_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")), 4);
                oporow["NUM2"] = Math.Round(fgen.make_double(txtrail_charg_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                //local charges
                oporow["NUM3"] = Math.Round(fgen.make_double(txt_locl_chrg_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM4"] = Math.Round(fgen.make_double(txt_locl_chrg_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM5"] = Math.Round(fgen.make_double(txt_custm_clrnce_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM6"] = Math.Round(fgen.make_double(txt_custm_clrnce_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                //shipping line charges
                oporow["NUM7"] = Math.Round(fgen.make_double(txt_ocean_fryt_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM8"] = Math.Round(fgen.make_double(txt_ocean_fryt_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM9"] = Math.Round(fgen.make_double(txt_thc_chrg_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM10"] = Math.Round(fgen.make_double(txt_thc_chrg_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM11"] = Math.Round(fgen.make_double(txt_doc_chg_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM12"] = Math.Round(fgen.make_double(txt_doc_chg_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                ///ex rate
                oporow["NUM13"] = Math.Round(fgen.make_double(txt_Ex_rate.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM14"] = Math.Round(fgen.make_double(txt_tot_fryt_dlr.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
                oporow["NUM15"] = Math.Round(fgen.make_double(txt_tot_fryt_rs.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);
              //  oporow["NUM16"] = fgen.make_double(txt_ratio.Text.Trim());
                oporow["NUM17"] = Math.Round(fgen.make_double(txt_oth_chg.Text.Trim().Replace("NaN", "0").Replace("Infinity", "0")),4);

                oporow["CINAME"] = txt_ocean.Text.Trim().ToUpper();
               ...
               //===============
                ...
                oporow["SRNO"] = sg1.Rows[i].Cells[12].Text.Trim().ToUpper();
                ...
                oporow["obsv10"] = txtshipno.Text.Trim();
    ...
    {
        double coltot = 0;
        double qty1 = 0; double qty2 = 0; double qty3 = 0; double val1 = 0; double val2 = 0; double val3 = 0; double val4 = 0; double val5 = 0; double val6 = 0; double tot_fryt = 0;
        double doc = 0; double doc1 = 0; double tot_fryt_rs = 0;
        //==========local charges factory icd,tkd rs      
        qty2 = fgen.make_double(txt_Ex_rate.Text);
        qty1 = fgen.make_double(txt_locl_chrg_rs.Text);
        if (qty1 > 0)
        {
          
            qty3 = qty1 / qty2;
            txt_locl_chrg_dlr.Text = Math.Round(qty3, 2).ToString();
        }
        //=========CUSTOM CLEARANCE DOLLAR
        val1 = fgen.make_double(txt_custm_clrnce_rs.Text);
        if (val1 > 0)
        {
            val2 = val1 / qty2;
            txt_custm_clrnce_dlr.Text = Math.Round(val2, 2).ToString();
        }
        //=================concor rail Charges
        val3 = fgen.make_double(txtrail_charg_rs.Text);
        if (val3 > 0)
        {
            coltot = val3 / qty2;
            txtrail_charg_dlr.Text = Math.Round(coltot, 2).ToString();
        }
        //==========================THC CHRGES
        val4 = fgen.make_double(txt_thc_chrg_rs.Text);
        if (val4 > 0)
        {
            val5 = val4 / qty2;
            txt_thc_chrg_dlr.Text = Math.Round(val5, 2).ToString();
        }
        ///======================doucment charges
        doc = fgen.make_double(txt_doc_chg_rs.Text);
        if (doc > 0)
        {
            doc1 = doc / qty2;
            txt_doc_chg_dlr.Text = Math.Round(doc1, 2).ToString();
        }
        //========other charges 
        val6=(350) / qty2 + 32;
        if (val6 > 0)
        {
            txt_oth_chg.Text = Math.Round(val6, 2).ToString();
        }
        //=========total freight in dollar
        tot_fryt = qty3 + val2 + coltot + fgen.make_double(txt_ocean_fryt_dlr.Text) + val5 + doc1 + val6;
        txt_tot_fryt_dlr.Text = Math.Round(tot_fryt, 2).ToString();
         //===================total freight in rs
        tot_fryt_rs = (tot_fryt * qty2);
        txt_tot_fryt_rs.Text = Math.Round(tot_fryt_rs, 2).ToString();
    }
