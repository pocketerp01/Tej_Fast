...
public partial class om_SO_alert : System.Web.UI.Page
{
    string btnval, SQuery, col1, col2, col3, vardate, fromdt, todt, typePopup = "N";
    DataTable dt, dt2, dt3, dt4; DataRow oporow; DataSet oDS; DataSet oDS2; DataSet oDS3; DataSet oDS4; DataSet oDS5;
    int i = 0, z = 0;
    ...
    string frm_mbr, frm_tid, frm_vnum, frm_url, frm_qstr, frm_cocd, frm_uname, frm_PageName, frm_vty;
        ...
        else
        {
            ...
            if (frm_url.Contains("STR"))
            {
                if (Request.QueryString["STR"].Length > 0)
                {
                    ...
                    frm_CDT2 = fgenMV.Fn_Get_Mvar(frm_qstr, "U_Cdt1");
    ...
    public void set_Val()
    {
        ...
        frm_vty = "OA";
        lblheader.Text = "Alerts Related to Sales Order";


        fgenMV.Fn_Set_Mvar(frm_qstr, "U_TID", frm_vty);
        dt = new DataTable();

        //vipin :: to put this in load event to reduce re fresh each time
        dt = fgen.getdata(frm_qstr, frm_cocd, "select a.Order_Ref,b.aname,a.Acode,a.Reason ,a.delay_Days,a.Inform_to1 ,a.Inform_to2 ,a.Remarks ,a.doc_Ref,a.vchnum as Alert_No,to_Char(a.vchdate,'dd/mm/yyyy') as Alert_Dt,a.ent_by,a.ent_Dt from " + frm_tabname + " a,famst b  where trim(a.acode)=trim(B.acode) and a.branchcd='" + frm_mbr + "' and trim(a.type)='" + frm_vty + "' order by a.vchdate desc,a.vchnum desc");
        ...
        switch (btnval)
        {
            ...
            default:
                ...
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                {
                    set_Val();
                    frm_tid = fgenMV.Fn_Get_Mvar(frm_qstr, "U_TID");
                    //SQuery = "select Vchnum,deptt_name,mach_name,ent_by,ent_Dt from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' order by vchdate,trim(vchnum)";
                    SQuery = "select a.branchcd||a.type||trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as Opt_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as Opt_Dt,b.Aname,a.order_Ref,a.doc_Ref,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a,famst b where trim(A.acode)=trim(B.acode) and  a.branchcd='" + frm_mbr + "' and trim(a.type)='" + frm_vty + "' order by vdd desc,a." + doc_nf.Value + " desc";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtbox3.Value.Trim().Length < 2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + Label5.InnerHtml ;
        }


        if (fgen.make_double(txtbox7.Value.Trim()) <= 0)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + Label7.Text;
        }

        if (txtbox8.Value.Trim().Length<2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + Label8.InnerHtml;
        }


        if (txtbox9.Value.Trim().Length  <2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + Label9.Text;
        }
    ...
    protected void btndel_ServerClick(object sender, EventArgs e)
    {
        ...
        if (Prg_Id == "F70421" || Prg_Id == "F10123" || Prg_Id == "F10139A" || Prg_Id == "F10125" || Prg_Id == "F40252" || Prg_Id == "F40254" || Prg_Id == "F47125L")
        {
        }
        else
        {
    ...
    protected void btnlist_ServerClick(object sender, EventArgs e)
    {
        ...
        SQuery = "select SHFT_NAME, DEPTT_NAME, MACH_NAME, COMP_HRS, COMP_MIN, COMP_TYPE, REMARKS,Vchnum as Comp_no,to_char(a.vchdate,'dd/mm/yyyy') as Comp_Dt,a.ent_by  from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "' and trim(a.type)='" + frm_vty + "' and trim(a.ent_by)='" + frm_uname + "'  order by a.vchdate,a.vchnum";
    ...
    protected void btnprint_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            //SQuery = "SELECT '" + lblheader.Text + "' as header,'" + lbl3.InnerText + "' as h1,'" + Label5.InnerText + "' as h2,'" + Label2.Text + "' as h3, '" + Label3.Text + "' as h4, '" + Label4.Text + "'as h5 , A.TYPE1 AS CODE,A.NAME as name,a.acref as extra,a.lineno,a.acref3 as other_ref,'-' as col4  FROM TYPEGRP A  WHERE trim(a.ID)='" + frm_tid + "'   ORDER BY A.TYPE1";
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "D")
        {
            ...
            if (col1 == "Y")
            {
                ...
                fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " a where a.branchcd||trim(a.type)||trim(a." + doc_nf.Value + ")||to_char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr + frm_tid + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL1") + "'");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Del_E":
                    ...
                    edmode.Value = col1;
                    string mhd = "";
                    if (frm_formID == "F70174")
                    {

                        mhd = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT COUNT(*) AS CCNT FROM FAMST WHERE TRIM(BSSCH)='" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4").ToString().Trim().Replace("&amp", "") + "' ", "CCNT");
                        if (mhd.toDouble() > 0)
                        {
                            fgen.msg("-", "AMSG", "Deletion is not allowed. The ERP has found " + mhd + ", Level-4 Account codes linked to this Level-3 Code (" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4").ToString().Trim().Replace("&amp", "") + " - " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL5").ToString().Trim().Replace("&amp", "") + "), which you are trying to delete  !!");
                            return;
                        }
                    }
                    if (frm_formID == "F70177")
                    {
                        mhd = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT COUNT(*) AS CCNT FROM ITEM WHERE TRIM(HSCODE)='" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL6").ToString().Trim().Replace("&amp", "") + "' ", "CCNT");
                        if (mhd.toDouble() > 0)
                        {
                            fgen.msg("-", "AMSG", "Deletion is not allowed. The ERP has found " + mhd + ", Item codes linked to this HSN Code (" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL6").ToString().Trim().Replace("&amp", "") + " - " + fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL5").ToString().Trim().Replace("&amp", "") + "), which you are trying to delete  !!");
                            return;
                        }
                    }
                ...
                case "Edit_E":
                    ...
                    mv_col =  col1;
                    SQuery = "Select a.*,substr(a.type,1,2) as acgrp,to_Char(a.orig_Date,'dd/mm/yyyy') As orig_Dt from " + frm_tabname + " a where a.branchcd||trim(a.type)||trim(a." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy')='" + mv_col + "' ";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtbox3.Value = dt.Rows[i]["DOC_REF"].ToString().Trim();

                        txtbox4.Value = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where trim(AcodE)='"+ dt.Rows[i]["ACODE"].ToString().Trim() +"'", "aname"); 

                        txtbox5.Value = dt.Rows[i]["ORDER_REF"].ToString().Trim();
                        txtbox6.Value = dt.Rows[i]["ORIG_DT"].ToString().Trim();
                        txtbox7.Value = dt.Rows[i]["DELAY_DAYS"].ToString().Trim();

                        txtbox8.Value = dt.Rows[i]["REASON"].ToString().Trim();
                        txtbox9.Value = dt.Rows[i]["INFORM_TO1"].ToString().Trim();
                        txtbox10.Value = dt.Rows[i]["INFORM_TO2"].ToString().Trim();
                ...
                case "SOREF_POP":
                    if (col1.Length <= 0) return;
                        txtbox3.Value = col1;
                        txtbox4.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4").ToString().Trim();
                        txtbox5.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL5").ToString().Trim();
                        txtbox6.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL6").ToString().Trim();
                        break;
            ...
            if (col1 == "Y" && Checked_ok == "Y")
            {
                try
                {
                    ...
                    if (edmode.Value == "Y")
                    {
                        ...
                        fgen.execute_cmd(frm_qstr, frm_cocd, "update " + frm_tabname + " set branchcd='DD' where branchcd||trim(type)||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='" + frm_mbr+frm_vty+txtvchnum.Value +txtvchdate.Value  + "'");
                    ...
                    if (edmode.Value == "Y")
                    {
                        ...
                        fgen.execute_cmd(frm_qstr, frm_cocd, "delete from " + frm_tabname + " where branchcd||trim(type)||trim(" + doc_nf.Value + ")||to_char(" + doc_df.Value + ",'dd/mm/yyyy')='DD" + frm_vty + txtvchnum.Value + txtvchdate.Value + "'");
                    }
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            //fgen.send_mail(frm_cocd, "Finsys ERP", txtlbl5.Value, "", "", "SRF : Query has been logged " + frm_vnum, html_body);
    ...
    void save_fun()
    {
        ...
        oporow["DOC_REF"] = txtbox3.Value.ToUpper().Trim();
        
        SQuery = "update somas set ord_alert='" + txtbox8.Value.ToUpper().Trim()+ " : "+ frm_uname +" : "+vardate +"'  where branchcd||trim(type)||trim(ordno)||to_char(orddt,'dd/mm/yyyy')||trim(Acode)||trim(icode)='" + txtbox3.Value.ToUpper().Trim() + "'";
        
        fgen.execute_cmd(frm_qstr, frm_cocd, SQuery);
            
        oporow["ACODE"] = txtbox3.Value.ToUpper().Trim().Substring(20,7);
        oporow["ORDER_REF"] = txtbox5.Value.ToUpper().Trim();

        oporow["orig_date"] = txtbox6.Value.ToUpper().ToString();

        oporow["DELAY_DAYS"] = fgen.make_double(txtbox7.Value.ToUpper().Trim());
        oporow["REASON"] = txtbox8.Value.ToUpper().Trim();

        oporow["INFORM_TO1"] = txtbox9.Value.ToUpper().Trim();
        oporow["INFORM_TO2"] = txtbox10.Value.ToUpper().Trim();
    ...
    protected void btn_ord_Click(object sender, ImageClickEventArgs e)
    {
        
        hffield.Value = "SOREF_POP";
        make_qry_4_popup();
    ...
    protected void btninform1_Click(object sender, ImageClickEventArgs e)
    {

        hffield.Value = "INFORM1";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Code", frm_qstr);

    }

    protected void btninform2_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "INFORM2";
            make_qry_4_popup();
            fgen.Fn_open_sseek("Select Code", frm_qstr); 
    }
     protected void btncomp_Click(object sender, ImageClickEventArgs e)
    {
            hffield.Value = "COMP_POP";
            make_qry_4_popup();
            fgen.Fn_open_sseek("Select Code", frm_qstr); 
    }
    ///***********************************************Instructions
