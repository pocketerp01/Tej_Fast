...
public partial class om_ccm_log : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                lblheader.Text = "Customer Complain Logging";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "Print":
                ...
                break;
            case "USR":
                SQuery = "SELECT aname as FSTR ,aname AS Customer,acode AS Code,Addr1,Addr2,Staten FROM  famst where substr(Acode,1,2)='16' order by aname";
                break;
            case "CTYP":
                SQuery = "SELECT Name as FSTR ,Name AS Complaint_type,type1 AS Code FROM  typegrp where branchcd!='DD' and id='E1' order by Name";
                break;
            case "CATG":
                SQuery = "SELECT Name as FSTR ,Name AS Complaint_type,type1 AS Code FROM  typegrp where branchcd!='DD' and id='E3' order by Name";
                break;
            case "ITMC":
                SQuery = SQuery = "SELECT distinct a.icode as FSTR ,b.iname as Item_Name,to_char(a.vchdate,'dd/mm/yyyy')||'-'||a.vchnum as Inv_Ref,a.icode as ERP_code FROM  ivoucher a, item b where trim(A.icode)=trim(B.icodE) and a.branchcd='" + frm_mbr + "' and a.type like '4%' and UPPER(TRIM(a.acode))='" + doc_acode.Value + "' order by b.iname";
                break;
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            fgen.msg("-", "AMSG", "Product Name not entered !!");
            return;
        }
    ...
    protected void btnhideF_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "USR":
                    txtlbl4.Value = col1;
                    doc_acode.Value = col3;
                    break;
                case "CTYP":
                    txtlblctyp.Value = col1;
                    break;
                case "CATG":
                    txtlbl9.Value = col1;
                    break;
                ...
                case "PERSON":
                    txtlbl2.Value = col2;
                    txtlbl3.Value = col3;
                    txtlbl5.Value = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4").ToString().Trim().Replace("&amp", "");
                    break;
                case "Del":
                    if (col1 == "") return;
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        doc_acode.Value = dt.Rows[i]["acode"].ToString().Trim();
                        doc_icode.Value = dt.Rows[i]["icode"].ToString().Trim();

                        txtlblctyp.Value = dt.Rows[i]["comp_type"].ToString().Trim();
                        txtlbl8.Value = dt.Rows[i]["Cdescr"].ToString().Trim();

                        txtlbl9.Value = dt.Rows[i]["Compcatg"].ToString().Trim();
                        Lsubject.Value = dt.Rows[i]["compOccr"].ToString().Trim();
                        ...
                        txtinv.Text = dt.Rows[i]["INV_REF"].ToString().Trim();
                        txtrmk.Text = dt.Rows[i]["LRemarks"].ToString().Trim();

                        txtWrkRmk.Text = dt.Rows[i]["ORemarks"].ToString().Trim();
                        ddWorkDone.Value = dt.Rows[i]["CCM_mtg"].ToString().Trim();
                        ...
                        if (dt.Rows[i]["CCM_mtg"].ToString().Trim() == "N") ddWorkDone.Value = "NO";
                ...
                try
                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "CCM No " + frm_vnum + "'13'Customer Support should analyse then same within next 2-3 working days. You can track your service request through Portal also.");
                        }
                        else
                        {
    ...
    void save_fun()
    {
        ...
        oporow["ACODE"] = doc_acode.Value ;
        oporow["ICODE"] = doc_icode.Value;

        oporow["comp_type"] = txtlblctyp.Value.ToUpper().Trim();
        oporow["Cdescr"] = txtlbl8.Value.ToUpper().Trim();

        oporow["Compcatg"] = txtlbl9.Value;
        oporow["compOccr"] = Lsubject.Value;
        ...
        oporow["INV_REF"] = txtinv.Text;
        oporow["LRemarks"] = txtrmk.Text;

        oporow["ORemarks"] = txtWrkRmk.Text.Trim();
        oporow["CCM_mtg"] = ddWorkDone.Value.Substring(0, 1);
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F61101":
                SQuery = "SELECT 'CC' AS FSTR,'CCM Logging' as NAME,'CC' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "CC");
                break;
    ...
    protected void btnCtyp_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "CTYP";
        make_qry_4_popup();
    ...
    protected void btnCatg_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "CATG";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Complaint Category", frm_qstr);
    }

    protected void btnitmc_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "ITMC";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Item Category", frm_qstr);
    }
