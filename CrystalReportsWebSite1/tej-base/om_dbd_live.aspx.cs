...
public partial class om_dbd_live : System.Web.UI.Page
{
    ...
    void printGraph()
    {
        ...
        dt = fgen.getdata(frm_qstr, frm_cocd, "SELECT trim(a.obj_caption)||'@'||trim(a.obj_SQL) as db_query,a.OBJ_READONLY as graphtype FROM dbd_MW_config a where a.branchcd!='DD' and A.FRM_NAME='" + Prg_Id.Substring(0,3) + "000' order by a.srno ");
        ViewState["dt"] = dt;
        if (dt.Rows.Count > 0)
        {
            dispGrph(dt);
