...
public partial class om_unclaimed : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        frm_tabname = "WB_DOSDOC";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "10");
        lblheader.Text = "Un Claimed Amt";
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_ADD_E":
                ...
                //SQuery = "select DISTINCT A.ACODE AS FSTR, TRIM(A.ACODE) AS PARTY_CODE,TRIM(B.ANAME) AS PARTY_NAME from recdata A , FAMST B  where TRIM(A.ACODE)=TRIM(B.ACODE) and a.branchcd='" + frm_mbr + "' AND TRIM(SUBSTR(A.ACODE,1,2)) IN ('16','18') and TRIM(A.ACODE) not in (" + col1 + ") order by party_name";
                SQuery = "SELECT TRIM(A.ACODE) AS FSTR,TRIM(A.ACODE) AS ACODE,TRIM(B.ANAME) AS PARTY_NAME FROM (SELECT DISTINCT TRIM(A.ACODE)||TRIM(A.INVNO)||TO_CHAR(A.INVDATE,'DD/MM/YYYY') AS FSTR, TRIM(A.ACODE) AS ACODE,A.INVNO,A.INVDATE, 1 AS QTY FROM RECDATA A,FAMST F WHERE TRIM(A.ACODE)=TRIM(F.ACODE) AND A.BRANCHCD='" + frm_mbr + "' AND SUBSTR(TRIM(A.ACODE),1,2) IN ('16','18') UNION ALL SELECT TRIM(COL1) AS FSTR, ACODE,INVNO,INVDATE,-1 AS QTY FROM WB_DOSDOC WHERE BRANCHCD='" + frm_mbr + "' AND TYPE='10') A, FAMST B WHERE TRIM(A.ACODE)=TRIM(B.ACODE) AND TRIM(A.ACODE) not in (" + col1 + ") GROUP BY TRIM(A.ACODE),TRIM(B.ANAME) HAVING SUM(A.QTY)>0  ORDER BY PARTY_NAME";
                break;
            ...
            default:
                ...
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "COPY_OLD")
                    SQuery = "SELECT distinct trim(a.vchnum)||to_char(a.vchdate,'dd/mm/yyyy') as fstr,trim(a.vchnum) as entry_no,to_char(a.vchdate,'dd/mm/yyyy') as entry_dt,trim(a.acode) as party_code,trim(b.aname) as party_name,trim(a.invno) as invno,to_char(invdate,'dd/mm/yyyy') as invdate,a.ent_by,to_char(a.ent_dt,'dd/mm/yyyy') as ent_Dt FROM " + frm_tabname + " A,famst b WHERE trim(a.acode)=trim(b.acode) and A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='" + frm_vty + "' AND A.VCHDATE  " + DateRange + " ORDER BY entry_no DESC";
                break;
        }
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if(sg1.Rows.Count<=1)
        {
            fgen.msg("-", "AMSG", "Dear " + frm_uname + ", Please Select atleast one Party in Grid before Saving !!");
            return;
        }
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,trim(b.aname) as aname from " + frm_tabname + " a,famst b where trim(a.acode)=trim(b.acode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t9"] = dt.Rows[i]["LEGAL"].ToString().Trim();
                            sg1_dr["sg1_t10"] = dt.Rows[i]["DOSSIER"].ToString().Trim();
                            sg1_dr["sg1_t11"] = dt.Rows[i]["BG"].ToString().Trim();
                            sg1_dt.Rows.Add(sg1_dr);
                        }
                        sg1_add_blankrows();
                        ViewState["sg1"] = sg1_dt;
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        dt = new DataTable();
                        SQuery = "select DISTINCT A.ACODE AS FSTR, TRIM(A.ACODE) AS PARTY_CODE,TRIM(B.ANAME) AS PARTY_NAME,a.invno,to_char(a.invdate,'dd/mm/yyyy') as invdate from recdata A , FAMST B  where TRIM(A.ACODE)=TRIM(B.ACODE) and a.branchcd='" + frm_mbr + "' AND TRIM(A.ACODE) IN (" + col1 + ") and a.net>0 order by party_name";
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count <= 0)
                        {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_h9"] = "-";
                            sg1_dr["sg1_h10"] = "-";
                            sg1_dr["sg1_f1"] = dt.Rows[d]["PARTY_CODE"].ToString().Trim();
                            sg1_dr["sg1_f2"] = dt.Rows[d]["PARTY_NAME"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select A.SRNO,TRIM(A.VCHNUM) AS ENTRY_NO, TO_CHAR(A.VCHDATE,'DD/MM/YYYY') AS ENTRY_DATE,TRIM(A.ACODE) AS PARTY_CODE,TRIM(B.ANAME) AS PARTY_NAME, TRIM(INVNO) AS BILL_NO,TO_CHAR(A.INVDATE,'DD/MM/YYYY') AS BILL_DATE,LEGAL,DOSSIER,BG  from wb_dosdoc A, FAMST B where TRIM(A.ACODE)=TRIM(B.ACODE) AND A.branchcd='" + frm_mbr + "' and A.type='10' AND A.VCHDATE " + PrdRange + " ORDER BY VCHNUM ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[14].HeaderStyle.Width = 350;
            sg1.Columns[15].HeaderStyle.Width = 120;
            sg1.Columns[16].HeaderStyle.Width = 120;
            sg1.Columns[17].HeaderStyle.Width = 110;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 1)
            {
                ...
                oporow["LEGAL"] = ((DropDownList)sg1.Rows[i].FindControl("sg1_t9")).SelectedValue.Trim().ToUpper();
                oporow["DOSSIER"] = ((DropDownList)sg1.Rows[i].FindControl("sg1_t10")).SelectedValue.Trim().ToUpper();
                oporow["BG"] = ((DropDownList)sg1.Rows[i].FindControl("sg1_t11")).SelectedValue.Trim().ToUpper();
                oporow["COL1"] = sg1.Rows[i].Cells[13].Text.Trim().ToUpper() + sg1.Rows[i].Cells[15].Text.Trim().ToUpper() + sg1.Rows[i].Cells[16].Text.Trim().ToUpper();
                if (edmode.Value == "Y")
                {
    ...
    //------------------------------------------------------------------------------------
    //CREATE TABLE WB_dosdoc (BRANCHCD CHAR(2),TYPE CHAR(2),VCHNUM CHAR(6),VCHDATE DATE ,ACODE CHAR(10),INVNO VARCHAR2(20),INVDATE DATE,SRNO CHAR(4),LEGAL VARCHAR2(2),DOSSIER VARCHAR2(2),BG VARCHAR2(2),COL1 VARCHAR2(50),ENT_BY VARCHAR2(10) NOT NULL,ENT_DT DATE NOT NULL,EDT_BY VARCHAR2(10) NOT NULL,EDT_DT DATE NOT NULL);
}