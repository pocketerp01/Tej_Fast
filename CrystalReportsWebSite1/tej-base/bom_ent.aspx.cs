...
public partial class bom_ent : System.Web.UI.Page
{
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                //pop1
                string pquery;
                pquery = "select trim(icode) as icode,sum(cnt) as tot from (select icode,1 as cnt from item where length(trim(nvl(deac_by,'-')))<=1 and length(trim(icode))>4 ) group by trim(icode) having sum(cnt)>0 ";
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            if (sg1.Rows[i].Cells[13].Text.Trim().Length > 2 && fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text) > 0 && fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text) > 0)
            {
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    //SQuery = "Select a.*,b.text from " + frm_tabname + " a left outer join ico_tab b on trim(a.frm_name)=trim(b.id) where a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    dt = new DataTable();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.vchnum as BOM_no,to_char(a.vchdate,'dd/mm/yyyy') as BOM_Dt,b.IName as Product_Name,b.Cpartno,c.iName as Child_Name,c.Cpartno as Child_partno,a.ibqty as Child_qty,c.unit,a.Icode,a.Ibcode,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd,a.vchdate,a.Srno from " + frm_tabname + " a,Item b,Item c where trim(A.icode)=trim(B.Icode) and trim(A.ibcode)=trim(c.Icode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vchdate " + PrdRange + " order by vdd desc,a.vchnum desc,a.srno";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                if (i > 20)
                                {
                                    ...
                                    frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "select max(" + doc_nf.Value + ")+" + 1 + " as vch from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' ", 6, "vch");
                                    pk_error = "N";
                                    i = 0;
                                }
                                i++;
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["ibcode"] = sg1.Rows[i].Cells[13].Text.Trim();
                ...
                oporow["iomachine"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text);
