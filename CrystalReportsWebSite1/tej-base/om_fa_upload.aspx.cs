...
public partial class om_fa_upload : System.Web.UI.Page
{
    ...
    void save_fun()
    {
        ...
        //for checking assetman
        //DataTable mg_dt= new DataTable();
        //mg_dt = fgen.getdata(frm_qstr, frm_cocd, "select trim(acode) as fstr, assetsupp,invno,to_char(invdate,'dd/mm/yyyy')  as invdate, to_char(cutoffdt,'dd/mm/yyyy') as cutoffdt, totlife, ballife,to_char(purdt,'dd/mm/yyyy') as purdt,to_char(lastdpdt,'dd/mm/yyyy') as lastdpdt,deprpday,locn from assetman");
        ...
        {                      
            ...
            foreach (DataRow gr1 in dtW.Rows)
            {
                ...
                try
                {
                    ...
                    oporow["ASSETID"] = gr1["ASSETID"].ToString().ToUpper().Trim().Replace("FIN", "");
                    oporow["GRPCODE"] = gr1["GROUPCODE"].ToString().ToUpper().Trim();
                    oporow["GRP"] = "-";
                    oporow["Col1"] = gr1["OLD_TAG"].ToString().Trim().ToUpper().Trim();
                    oporow["ASSETNAME"] = gr1["NAMEOFASSET"].ToString().ToUpper().Trim();
                    oporow["ASSETSUPP"] = gr1["SUPPLYBY"].ToString().ToUpper().Trim();
                    //oporow["ASSETSUPP"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["OLD_TAG"].ToString().Trim().ToUpper().Trim() + "'", "assetsupp");
                    //oporow["ASSETSUPPADD"] = gr1["SUPPLYADD"].ToString().ToUpper().Trim();
                    oporow["ASSETSUPPADD"] = "-";
                    oporow["INVNO"] = gr1["INVOICENO"].ToString().ToUpper().Trim();
                    //oporow["INVNO"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["OLD_TAG"].ToString().Trim().ToUpper().Trim() + "'", "invno");
                    oporow["invdate"] = fgen.make_def_Date(Convert.ToDateTime(gr1["invoicedate"].ToString().Trim()).ToString("dd/MM/yyyy"), vardate);

                   // oporow["invdate"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper().Trim() + "'", "invdate");

                    oporow["voucherlink"] = frm_mbr + gr1["pur_type"].ToString().ToUpper().Trim() + oporow["INVNO"].ToString().ToUpper().Trim() + Convert.ToDateTime(oporow["invdate"].ToString().Trim()).ToString("dd/MM/yyyy");


                    oporow["LOCN"] = gr1["LOCATION"].ToString().Trim().ToUpper();
                    //oporow["LOCN"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["OLD_TAG"].ToString().Trim().ToUpper().Trim() + "'", "locn");
                    oporow["DOM_IMP"] = gr1["DOM_IMP"].ToString().Trim().ToUpper();
                    oporow["BASICCOST"] = fgen.make_double(gr1["BASICCOST"].ToString().Trim());
                    oporow["INSTALL_COST"] = fgen.make_double(gr1["INSTALLCOST"].ToString().Trim());
                    oporow["CUSTOM_DUTY"] = fgen.make_double(gr1["CUSTOMDUTY"].ToString().Trim());
                    oporow["OTHER_CHRGS"] = fgen.make_double(gr1["OTHERCHRGES"].ToString().Trim());
                    oporow["INSTDT"] = fgen.make_def_Date(Convert.ToDateTime(gr1["INSTALLDT"].ToString().Trim()).ToString("dd/MM/yyyy"), vardate);
                    oporow["OP_DEP"] = fgen.make_double(gr1["OPENINGDEP"].ToString().Trim());
                    // oporow["QUANTITY"] = fgen.make_double(gr1["QUANTITY"].ToString().Trim());
                    oporow["QUANTITY"] = 1;
                    oporow["TANGIBLE"] = gr1["TANGIBLE"].ToString().ToUpper().Trim();
                    oporow["PURENTRY"] = "Y";
                    oporow["ADDEP"] = "N";
                    oporow["LIFE"] = fgen.make_double(gr1["LIFE"].ToString().Trim());
                   // oporow["LIFE"] = fgen.make_double(fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper().Trim() + "'", "totlife")) / 365;
                    oporow["original_cost"] = fgen.make_double(gr1["BASICCOST"].ToString().Trim()) + fgen.make_double(gr1["INSTALLCOST"].ToString().Trim()) + fgen.make_double(gr1["CUSTOMDUTY"].ToString().Trim()) + fgen.make_double(gr1["OTHERCHRGES"].ToString().Trim());
                    oporow["depableval"] = fgen.make_double(oporow["original_cost"].ToString()) - fgen.make_double(gr1["OPENINGDEP"].ToString().Trim());
                    oporow["residval"] = fgen.make_double(gr1["RESIDUAL_VALUE"].ToString().Trim());
                    oporow["USED_LIFE"] = fgen.make_double(gr1["USED_LIFE"].ToString().Trim());
                   // oporow["USED_LIFE"] = (Convert.ToDateTime("31/03/2018") - Convert.ToDateTime(fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper() + "'", "purdt"))).TotalDays;
                    DateTime enddate = Convert.ToDateTime(gr1["INSTALLDT"].ToString().Trim()).AddYears(Convert.ToInt32(gr1["LIFE"].ToString().Trim()));
                    oporow["life_end"] = Convert.ToDateTime(enddate).AddDays(-1);
                    //oporow["life_end"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper().Trim() + "'", "lastdpdt");
                    DateTime lifenddt = Convert.ToDateTime(oporow["life_end"].ToString().Trim());
                    DateTime instaldt = Convert.ToDateTime(gr1["INSTALLDT"].ToString().Trim());
                    Double vtotlife = (lifenddt - instaldt).TotalDays;
                    oporow["totlife"] = vtotlife + 1;
                    //oporow["totlife"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper().Trim() + "'", "totlife");
                    oporow["deprpday"] = Math.Round(((Convert.ToDouble(oporow["original_cost"]) - Convert.ToDouble(oporow["residval"])) / (Convert.ToInt32(oporow["totlife"]) * Convert.ToInt32(oporow["QUANTITY"]))), 2).ToString();
                    //double balance = Convert.ToInt64(oporow["totlife"]) - Convert.ToInt64(fgen.make_double(gr1["USED_LIFE"].ToString().Trim()));// same value as balance life
                    double balance = Convert.ToInt64(oporow["totlife"]) - Convert.ToInt64(fgen.make_double(oporow["USED_LIFE"].ToString().Trim()));// same value as balance life
                    //oporow["deprpday"] = fgen.seek_iname_dt(mg_dt, "fstr='" + gr1["ASSETID"].ToString().Trim().ToUpper().Trim() + "'", "deprpday");
                    if (balance < 0)
                    {
                        oporow["BALLIFE"] = "0";
                    }
                    ...
                    {
                        oporow["BALLIFE"] = balance.ToString();
                    }


                    oporow["OWNER"] = gr1["OWNER"].ToString().ToUpper().Trim();
                    oporow["WARRANTY"] = gr1["WARRANTY_DAYS"].ToString().Trim();
                    int dtCorrect = fgen.ChkDate(gr1["WARRANTY_DATE"].ToString().Trim());
                    if (dtCorrect == 1) oporow["WARRANTY_DT"] = gr1["WARRANTY_DATE"].ToString().Trim();
                    else oporow["WARRANTY_DT"] = "-";
                    oporow["DCODE"] = gr1["DEPARTMENT_CODE"].ToString().Trim();
                    oporow["AMC"] = gr1["AMC"].ToString().Trim();
    ...
    protected void btnupload_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile)
        {
            ...
            try
            {
                ...
                dtn.Columns.Add("ASSETID", typeof(string));
                dtn.Columns.Add("GROUPCODE", typeof(string));
                dtn.Columns.Add("NAMEOFASSET", typeof(string));
                dtn.Columns.Add("SUPPLYBY", typeof(string));
                dtn.Columns.Add("SUPPLYADD", typeof(string));
                dtn.Columns.Add("INVOICENO", typeof(string));
                dtn.Columns.Add("INVOICEDATE", typeof(string));
                dtn.Columns.Add("LOCATION", typeof(string));
                dtn.Columns.Add("DOM_IMP", typeof(string));
                dtn.Columns.Add("BASICCOST", typeof(double));
                dtn.Columns.Add("INSTALLCOST", typeof(double));
                dtn.Columns.Add("CUSTOMDUTY", typeof(double));
                dtn.Columns.Add("OTHERCHRGES", typeof(double));
                dtn.Columns.Add("INSTALLDT", typeof(string));
                dtn.Columns.Add("OPENINGDEP", typeof(double));
                dtn.Columns.Add("QUANTITY", typeof(double));
                dtn.Columns.Add("TANGIBLE", typeof(string));
                dtn.Columns.Add("LIFE", typeof(double));
                dtn.Columns.Add("DEPPERDAY", typeof(double));
                dtn.Columns.Add("USED_LIFE", typeof(double));
                dtn.Columns.Add("RESIDUAL_VALUE", typeof(double));
                dtn.Columns.Add("OWNER", typeof(string));
                dtn.Columns.Add("DEPARTMENT_CODE", typeof(string));
                dtn.Columns.Add("WARRANTY_DAYS", typeof(string));
                dtn.Columns.Add("WARRANTY_DATE", typeof(string));
                dtn.Columns.Add("AMC", typeof(string));
                dtn.Columns.Add("PUR_TYPE", typeof(string));
                dtn.Columns.Add("OLD_TAG", typeof(string));
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    drn = dtn.NewRow();
                    drn["srno"] = count;
                    drn["assetid"] = dr[1].ToString().Trim().ToUpper();
                    if (dr[2] != null)
                    {
                        if (dr[2].ToString().Length > 1)
                           drn["groupcode"] = Convert.ToInt32(dr[2].ToString().Trim());
                        }
                    drn["nameofasset"] = dr[3].ToString().Trim().Replace("””", "").Replace("’’", "~").Replace("-", " ");
                    drn["supplyby"] = dr[4].ToString().Trim().Replace("””", "").Replace("’’", "~").Replace("-", " ");
                    drn["supplyadd"] = dr[5].ToString().Trim();
                    drn["invoiceno"] = dr[6].ToString().Trim();
                    drn["invoicedate"] = dr[7].ToString().Trim();
                    drn["location"] = dr[8].ToString().Trim();
                    ...
                    {
                        drn["dom_imp"] = dr[9].ToString().Trim();
                    }
                  
                    drn["basiccost"] =  Math.Round(fgen.make_double(dr[10].ToString().Trim()),2);
                    drn["installcost"] =  Math.Round(fgen.make_double(dr[11].ToString().Trim()),2);
                    drn["customduty"] =  Math.Round(fgen.make_double(dr[12].ToString().Trim()),2);
                    drn["otherchrges"] =  Math.Round(fgen.make_double(dr[13].ToString().Trim()),2);
                    //drn["originalcost"] = fgen.make_double(dr[14].ToString().Trim());
                   drn["installdt"]= dr[14].ToString().Trim();
                    // = fgen.make_def_Date(dr[14].ToString().Trim()); 
                    drn["openingdep"] = Math.Round(fgen.make_double(dr[15].ToString().Trim()), 2);
                    ...
                    {
                        drn["tangible"] = dr[17].ToString().Trim();
                    }
                    drn["life"] = fgen.make_double(dr[18].ToString().Trim());
                    drn["DEPPERDAY"] = Math.Round(fgen.make_double(dr[19].ToString().Trim()), 2);
                    drn["USED_LIFE"] = fgen.make_double(dr[20].ToString().Trim());
                    drn["RESIDUAL_VALUE"] = fgen.make_double(dr[21].ToString().Trim());
                    ...
                    {
                        drn["OWNER"] = dr[22].ToString().Trim();
                    }

                    drn["DEPARTMENT_CODE"] = dr[23].ToString().Trim();

                    if (dr[24].ToString().Trim() == "")
                    {
                        drn["WARRANTY_DAYS"] = "N";
                        drn["WARRANTY_DATE"] = "-";
                    ...
                    {
                        drn["WARRANTY_DAYS"] = dr[24].ToString().Trim();
                        drn["WARRANTY_DATE"] = dr[25].ToString().Trim();
                    }
                       
                    if (dr[26].ToString().Trim() == "")
                    {
                            drn["AMC"] = "N";
                    }
                        else
                     {
                            drn["AMC"] = dr[26].ToString().Trim();
                     }

                    if (dr[27].ToString().Trim() == "")
                    {
                        drn["PUR_TYPE"] = "-";
                    }
                    else
                    {
                        drn["PUR_TYPE"] = dr[27].ToString().Trim();
                    }
                    if (dr[28].ToString().Trim() == "")
                    {
                        drn["OLD_TAG"] = "-";
                    }
                    else
                    {
                        drn["OLD_TAG"] = dr[28].ToString().Trim();
                    } 
    ...
    protected void btnvalidate_ServerClick(object sender, EventArgs e)
    {
        ...
        for (int i1 = 0; i1 < dtn.Rows.Count; i1++)
        {
            ...
            chkgname = fgen.seek_iname_dt(dt2, "type1='" + dtn.Rows[i1]["groupcode"].ToString().Trim() + "'", "type1"); 
            if (chkgname == "0")
            {
                flag = 1;
