...
public partial class om_iss_entrybomreel : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        tab3.Visible = false;
        btnPost.Visible = false;
        if (fgenMV.Fn_Get_Mvar(frm_qstr, "U_POSTREEL") == "Y" && (lbl1a.Text == "31" || lbl1a.Text == "30" || lbl1a.Text == "11"))
        {
            tab3.Visible = true;
    ...
    DataTable FillBOM(DataTable inputdt, DataTable oudt)
    {
        foreach (DataRow dri in inputdt.Rows)
        {
            ...
            if (icode.Substring(0, 1) == "7" || icode.Substring(0, 1) == "8" || icode.Substring(0, 1) == "9")
            {
                string mq = " SELECT A.ibcode AS FSTR,D.INAME AS PARENT_ITEM,B.INAME,A.ibcode,(case when trim(nvl(a.LINKAGE,'N')) = 'Y' then 'Main' else 'Alt' end) as Item_type,b.cpartno,b.cdrgno,b.unit," +
                    "(a.ibqty/to_number(nvl(a.main_issue_no,'1')))*" + mqty + " as ibqty,c.Closing_Stk as bal,c.erpcode FROM ITEMOSP A,ITEM B,ITEM D,(select erpcode,Closing_Stk from (select sum(a.opening)||'~'||sum(a.cdr)||'~'||sum(a.ccr)||'~'||" +
                    ...
                    ",0 as op,sum(iqtyin) as cdr,sum(iqtyout) as ccr, 0 as aaa , 0 as aaa1,0 as aaa2 from IVOUCHER where branchcd='00' and TYPE LIKE '%' AND VCHDATE between to_date('" + fromdt + "','dd/mm/yyyy')" +
                    " and to_date('" + todt + "','dd/mm/yyyy')  and store='Y' GROUP BY trim(icode) ,branchcd) a group by a.icode)) c WHERE TRIM(A.icode)=TRIM(D.icode) AND TRIM(A.ibcode)=TRIM(B.icode) AND TRIM(A.ICODE)='" + icode + "' " +
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TICODE":
                ...
                break;
            case "ORD":
                SQuery = "SELECT DISTINCT TRIM(A.ORDNO)||'-'||TO_cHAR(A.ORDDT,'DD/MM/YYYY') AS FSTR,A.ORDNO AS WONO,TO_CHAR(A.ORDDT,'DD/MM/YYYY') AS WODT,B.INAME AS PRODUCT,A.ICODE,A.QTYORD AS WOQTY,a.ent_by,a.ent_dt FROM SOMAS A,ITEM B WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND " +
                    "A.BRANCHCD='" + frm_mbr + "' AND A.TYPE LIKE '4%' AND A.ORDDT " + DateRange + " AND NVL(A.APP_BY,'-') <> '-'";
                break;
            case "RWM":
                ...
                if (frm_cocd == "MASS" || frm_cocd == "MAST")
                {
                    SQuery = " SELECT A.ibcode AS FSTR,B.INAME,D.INAME AS PARENT_ITEM,A.ibcode,(case when trim(nvl(a.LINKAGE,'N')) = 'Y' then 'Main' else 'Alt' end) as Item_type," +
                        "b.cpartno,b.cdrgno,b.unit/*,a.ibqty/to_number(nvl(a.main_issue_no,'1')) as ibqty*/,(a.ibqty/to_number(nvl(a.main_issue_no,'1')))*" + fgen.make_double(txtlbl8.Text) + " as ibqty,c.Closing_Stk as bal,c.erpcode FROM ITEMOSP A,ITEM B,ITEM D," +
                        ...
                        "where branchcd='" + frm_mbr + "' and TYPE LIKE '%' AND VCHDATE between to_date('" + fromdt + "','dd/mm/yyyy') and to_date('" + todt + "','dd/mm/yyyy')  and store='Y' GROUP BY trim(icode) ,branchcd) a group by a.icode)) c" +
                        " WHERE TRIM(A.icode)=TRIM(D.icode) AND TRIM(A.ibcode)=TRIM(B.icode) AND TRIM(A.ICODE)='" + hf1.Value + "' and trim(b.icode)=trim(c.erpcode) and length(trim(nvl(b.deac_by,'-'))) <2 AND LENGTH(tRIM(b.ICODE))>=8 and trim(b.icode) not in ('-') ORDER BY A.IBCODE";
            ...
            .Select(g =>
               {
                   ...
                   row["PARENT_ITEM"] = g.Max(r => r.Field<string>("PARENT_ITEM"));
                   row["ibcode"] = g.Key.ibcode;
                   row["erpcode"] = g.Key.erpcode;
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "ORD":
                    ...
                    txtlblbd7a.Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL4");
                    txtlbl8B.Text = col2 + col3 + txtlblbd7.Text.Trim();
                    txtlbl2.Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL2");
                    txtlbl3.Text = fgenMV.Fn_Get_Mvar(frm_qstr, "U_COL3");
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_RWM", hf1.Value);
                    hffield.Value = "RWM";
                    make_qry_4_popup();
                    fgen.Fn_open_mseek("-", frm_qstr);
    ...
    protected void btnlblbd7_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "ORD";
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Order No. ", frm_qstr);
    }
    ...
    {
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        SQuery = "SELECT type1 AS FSTR,NAME,type1 AS CODE FROM type where id='M' and type1 like '3%' and type1='30' order by type1";
    }
    //------------------------------------------------------------------------------------
    void setGST()
    {
