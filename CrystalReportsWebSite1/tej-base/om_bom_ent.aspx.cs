...
public partial class om_bom_ent : System.Web.UI.Page
{
    ...
    protected void sg1_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (sg1.Rows[rowIndex].Cells[13].Text.Length > 2)
        {
            col1 = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT NVL(IMAGEF,'-') AS IMAGEF FROM ITEM WHERE ICODE='" + sg1.Rows[rowIndex].Cells[13].Text.Trim() + "' ", "IMAGEF");
            if (col1.Length > 2)
            {
                try
                {
