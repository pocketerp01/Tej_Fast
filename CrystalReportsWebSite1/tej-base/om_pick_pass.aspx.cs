...
public partial class om_pick_pass : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        lblheader.Text = "Pickling And Passivation Report";
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtPo_Qty.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill P.O. Qty");
            txtPo_Qty.Focus();
            return;
        }
        if (txtlbl7.Text == "-")
        ...
        if (txtITP.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill ITP No.");
            txtITP.Focus();
            return;
        }
        if (txtItem.Text == "-")
        ...
        if (txtStage.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Stage Of Testing");
            txtStage.Focus();
            return;
        }
        ...
        if (txtClientTag.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Client Tag No.");
            txtClientTag.Focus();
            return;
        }
        if (txtSurfacePrep.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Surface Preparation");
            txtSurfacePrep.Focus();
            return;
        }
        if (txtSurface_Temp.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Surface Temprature");
            txtSurface_Temp.Focus();
            return;
        }
        if (dd_Passivation.SelectedItem.Text == "PLEASE SELECT")
        {
            fgen.msg("-", "AMSG", "Please Select Passivation Technique");
            dd_Passivation.Focus();
            return;
        }
        ...
        if (txtNitric.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Method Of Application Nitric Acid");
            txtNitric.Focus();
            return;
        }
        if (txtChemical.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Chemical");
            txtChemical.Focus();
            return;
        }
        if (txtChemical1.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Chemical1");
            txtChemical1.Focus();
            return;
        }
        if (txtAppMix.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Duration For Application Of Mixture");
            txtAppMix.Focus();
            return;
        }
        ...
        if (txt_GridSurface.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Surface Preparation '13' (Tab 2)");
            txt_GridSurface.Focus();
            return;
        }
        if (txtPost.Text == "-")
        {
            fgen.msg("-", "AMSG", "Please Fill Post Cleaning '13' (Tab 2)");
            txt_GridSurface.Focus();
            return;
        }
        if (sg1.Rows.Count <= 1)
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    clearctrl();
                    FillDropDown();
                    SQuery = "Select a.*,to_chaR(a.ent_dt,'dd/mm/yyyy') as pent_Dt,f.aname,i.iname from " + frm_tabname + " a,famst f,item i where trim(a.acode)=trim(f.acode) and trim(a.icode)=trim(i.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        // txtItem.Text = dt.Rows[0]["INAME"].ToString().Trim(); // USER WANTS TO EDIT INAME THAT'S WHY IT IS SAVED IN FOOTNOTE FIELD
                        txtItem.Text = dt.Rows[0]["FOOTNOTE"].ToString().Trim();
                        ...
                        txtMaterial.Text = dt.Rows[0]["OBSV20"].ToString().Trim();
                        txtSurfacePrep.Text = dt.Rows[0]["COL3"].ToString().Trim();
                        txtChemical.Text = dt.Rows[0]["COL5"].ToString().Trim();
                        txtChemical1.Text = dt.Rows[0]["COL6"].ToString().Trim();
                        txtProject.Text = dt.Rows[0]["OBSV9"].ToString().Trim();
                        txtStage.Text = dt.Rows[0]["OBSV11"].ToString().Trim();
                        txtSurface_Temp.Text = dt.Rows[0]["OBSV13"].ToString().Trim();
                        txtNitric.Text = dt.Rows[0]["COL4"].ToString().Trim();
                        txt_GridSurface.Text = dt.Rows[0]["LINKFILE"].ToString().Trim();
                        ...
                        dd_Passivation.SelectedItem.Text = dt.Rows[0]["OBSV2"].ToString().Trim();
                        txtITP.Text = dt.Rows[0]["OBSV8"].ToString().Trim();
                        txtPo_Qty.Text = dt.Rows[0]["OBSV10"].ToString().Trim();
                        txtClientTag.Text = dt.Rows[0]["OBSV12"].ToString().Trim();
                        txtAppMix.Text = dt.Rows[0]["OBSV14"].ToString().Trim();
                        txtPost.Text = dt.Rows[0]["OBSV21"].ToString().Trim();
                        txtValve_Size_Rating.Text = dt.Rows[0]["OBSV19"].ToString().Trim();
                        txtComponent.Text = dt.Rows[0]["OBSV16"].ToString().Trim();
                        create_tab();
                        sg1_dr = null;
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t2"] = dt.Rows[i]["OBSV5"].ToString().Trim();
                            sg1_dr["sg1_t3"] = dt.Rows[i]["COL1"].ToString().Trim();
                            sg1_dr["sg1_t4"] = dt.Rows[i]["OBSV6"].ToString().Trim();
                ...
                case "WO":
                    if (col1.Length <= 0) return;
                    SQuery = "Select Distinct a.branchcd||a.type||trim(a.ordno)||to_char(a.orddt,'dd/mm/yyyy') as fstr,a.branchcd,a.work_ordno,b.aname as Customer,a.Pordno,a.org_invno as WO_NO,a.acode,a.icode,a.type,to_char(a.orddt,'dd/mm/yyyy') as orddt,a.ordno,a.weight,a.cdrgno,to_char(a.orddt,'yyyymmdd') as vdd,i.iname from Somas a,famst b,item i where trim(a.acodE)=trim(b.acodE) and trim(a.icode)=trim(i.icode) and a.branchcd||a.type||trim(a.ordno)||to_char(a.orddt,'dd/mm/yyyy')||trim(a.org_invno)||trim(a.work_ordno)||trim(a.icode)||trim(a.cdrgno)='" + col1 + "' order by vdd desc,a.ordno desc";
                    dt = new DataTable();
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        mq0 = "select VCHNUM,TO_CHAR(VCHDATE,'DD/MM/YYYY') AS VCHDATE,COL11 AS FACING,COL12 AS FLANGE_STD,COL22 AS DESIGN_STD,COL4 AS VALVE_MODEL,COL3 AS RATING,COL2 AS SIZE_MM,COL25 AS CLIENT_TAG,COL5 AS BODY,COL23 AS CLIENT_PO_ITEM_NO,COL3 AS RATING FROM SCRATCH WHERE BRANCHCD='" + dt.Rows[0]["BRANCHCD"].ToString().Trim() + "' AND TYPE='WO' AND COL26='" + dt.Rows[0]["CDRGNO"].ToString().Trim() + "' AND COL27='" + dt.Rows[0]["WO_NO"].ToString().Trim() + "' AND UPPER(COL28)='" + dt.Rows[0]["TYPE"].ToString().Trim() + "/" + dt.Rows[0]["ORDNO"].ToString().Trim() + " DT." + dt.Rows[0]["ORDDT"].ToString().Trim() + "' AND ICODE='" + dt.Rows[0]["ICODE"].ToString().Trim() + "'";
                        dt2 = new DataTable();
                        dt2 = fgen.getdata(frm_qstr, frm_cocd, mq0);

                        if (dt2.Rows.Count > 0)
                        {
                            // txtClientTag.Text = dt2.Rows[0]["CLIENT_TAG"].ToString().Trim(); // AS PER USER THEY WILL TYPE IT MANUALLY
                            txtMaterial.Text = dt2.Rows[0]["BODY"].ToString().Trim();
                            //txtPo_Qty.Text = dt2.Rows[0]["SIZE_MM"].ToString().Trim() + "-" + dt2.Rows[0]["CLIENT_PO_ITEM_NO"].ToString().Trim();
                            txtPo_Qty.Text = dt2.Rows[0]["SIZE_MM"].ToString().Trim(); // AS PER USER THEY WILL TYPE CLIENT_PO_ITEM_NO MANUALLY
                        ...
                        txtIcode.Text = dt.Rows[0]["icode"].ToString().Trim();
                        txtItem.Text = dt.Rows[0]["iname"].ToString().Trim();
                        FillGrid();
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        ...
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t4")).Text = "NO GREASE/DIRT/DUST";
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t5")).Text = "ACCEPTABLE";
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t6")).Text = "OK";
                    }
                    setColHeadings();
                    break;
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_t4"] = "NO GREASE/DIRT/DUST";
                            sg1_dr["sg1_t5"] = "ACCEPTABLE";
                            sg1_dr["sg1_t6"] = "OK";
                            sg1_dr["sg1_t7"] = "";
                            sg1_dr["sg1_t8"] = "";
                            sg1_dt.Rows.Add(sg1_dr);
    ...
    protected void sg1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            sg1.Columns[20].HeaderStyle.Width = 220;
            sg1.Columns[21].HeaderStyle.Width = 220;
            sg1.Columns[22].HeaderStyle.Width = 220;
            sg1.Columns[23].HeaderStyle.Width = 220;
            sg1.Columns[24].HeaderStyle.Width = 220;
            sg1.Columns[25].HeaderStyle.Width = 220;
        }
    }
    //------------------------------------------------------------------------------------
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 1; i++)
        {
            if (sg1.Rows[i].Cells[14].Text.Trim().Length > 1)
            {
                ...
                oporow["COL1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().ToUpper();
                oporow["COL2"] = txtProcedure.Text.Trim().ToUpper();
                oporow["COL3"] = txtSurfacePrep.Text.Trim().ToUpper();
                oporow["COL4"] = txtNitric.Text.Trim().ToUpper();
                oporow["COL5"] = txtChemical.Text.Trim().ToUpper();
                oporow["COL6"] = txtChemical1.Text.Trim().ToUpper();
                oporow["MRRNUM"] = "-";
                oporow["MRRDATE"] = "-";
                oporow["BTCHDT"] = "-";
                oporow["RESULT"] = "-";
                oporow["OBSV1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper();
                oporow["OBSV2"] = dd_Passivation.SelectedItem.Text.Trim().ToUpper();
                ...
                oporow["OBSV5"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper();
                oporow["OBSV6"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t4")).Text.Trim().ToUpper();
                oporow["OBSV7"] = "-";
                oporow["OBSV8"] = txtITP.Text.Trim().ToUpper();
                oporow["OBSV9"] = txtProject.Text.Trim().ToUpper();
                oporow["OBSV10"] = txtPo_Qty.Text.Trim().ToUpper();
                oporow["OBSV11"] = txtStage.Text.Trim().ToUpper();
                oporow["OBSV12"] = txtClientTag.Text.Trim().ToUpper();
                oporow["OBSV13"] = txtSurface_Temp.Text.Trim().ToUpper();
                oporow["OBSV14"] = txtAppMix.Text.Trim().ToUpper();
                oporow["OBSV15"] = "-";
                oporow["CONTPLAN"] = "-";
                oporow["SAMPQTY"] = 0;
                oporow["WONO"] = "-";
                ...
                oporow["OMAX"] = "-";
                oporow["OMIN"] = "-";
                oporow["LINKFILE"] = txt_GridSurface.Text.Trim().ToUpper();
                oporow["MFGDATE"] = "-";
                oporow["EXPDATE"] = "-";
                oporow["OBSV16"] = txtComponent.Text.Trim().ToUpper();
                oporow["OBSV17"] = "-";
                oporow["OBSV18"] = "-";
                oporow["OBSV19"] = txtValve_Size_Rating.Text.Trim().ToUpper();
                oporow["OBSV20"] = txtMaterial.Text.Trim().ToUpper();
                oporow["OBSV21"] = txtPost.Text.Trim().ToUpper();
                oporow["OBSV22"] = "-";
                oporow["OBSV23"] = "-";
                oporow["OBSV24"] = "-";
                oporow["OBSV25"] = "-";
                ...
                oporow["OBSV49"] = "-";
                oporow["OBSV50"] = "-";
                oporow["FOOTNOTE"] = txtItem.Text.Trim().ToUpper();
                if (edmode.Value == "Y")
                {
    ...
    {
        SQuery = "SELECT 'PLEASE SELECT' AS FSTR FROM DUAL UNION ALL SELECT 'SPRAY METHOD' AS FSTR FROM DUAL UNION ALL SELECT 'DIP METHOD' AS FSTR FROM DUAL";
        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);

        dd_Passivation.DataSource = dt;
        dd_Passivation.DataTextField = "fstr";
        dd_Passivation.DataValueField = "fstr";
        dd_Passivation.DataBind();

        txtProcedure.Text = "MFG PROCEDURE (QAD-W-39,ISSUE 02,REV.0)";
        txtSurfacePrep.Text = "AFTER SHOT BLASTING & AFTER MACHINIG (INTERNAL)";
        txtSurface_Temp.Text = "AMBIENT";
        txtNitric.Text = "MIXED WITH DEMINERALISED WATER";
        txtChemical.Text = "67 % HNO3 (NITRIC ACID) WEIGHT USED FOR PICKLING";
        txtChemical1.Text = "40% HF (HYDRO FLUORIC ACID) DEMINERALISED WATER BY USED FOR PASSIVATION";
        txt_GridSurface.Text = "OUTER AREA OF BODY AFTER SHOT BLASTED & INTERNAL AREA AFTER MACHINING";
        txtPost.Text = "BY DEMINERALISED WATER";
    }
    //------------------------------------------------------------------------------------
    protected void BlankDropDown()
    {
        dd_Passivation.Items.Clear();
    }
    //------------------------------------------------------------------------------------
