...
public partial class om_proc_map : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!Page.IsPostBack)
            {
                //plant wise stage mapping plan
                stag_pwise.Value = fgen.getOption(frm_qstr, frm_cocd, "W0091", "OPT_ENABLE").ToUpper();
                proc_pwise.Value = fgen.getOption(frm_qstr, frm_cocd, "W0093", "OPT_ENABLE").ToUpper();
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                ...
                string br_Cond = "";
                br_Cond = "branchcd!='DD'";
                if (stag_pwise.Value == "Y")
                {
                    br_Cond = "branchcd='" + frm_mbr + "'";
                }
                pquery = "select trim(icode) as icode,sum(cnt) as tot from (select icode,1 as cnt from item where length(trim(nvl(deac_by,'-')))<=1 and length(trim(icode))>4 union all select distinct icode,-1 as cnt from itwstage where " + br_Cond + " and type='" + lbl1a.Text + "') group by trim(icode) having sum(cnt)>0 ";
            ...
            case "SG1_ROW_ADD_E":
                ...
                if (proc_pwise.Value == "Y")
                { SQuery = "select type1 as fstr,name as proc_name,type1 as code from typegrp where branchcd='" + frm_mbr + "' and id='KK' " + col1 + " order by code"; }
                else
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        string chk_freeze = "";
        chk_freeze = fgen.Fn_chk_doc_freeze(frm_qstr, frm_cocd, frm_mbr, "W1003", txtvchdate.Text.Trim());
        if (chk_freeze == "1")
        {
    ...
    void newCase(string vty)
    {
        ...
        fgen.msg("-", "CMSG", "Do You Want to Use an Existing Routing to  Make the New Process Plan ? '13'It Will make The Process Very Fast'13'It Will Save Your Time of Re-Entry");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "COPY_OLD":
                    ...
                    clearctrl();
                    if (proc_pwise.Value == "Y")
                    { SQuery = "Select a.*,b.name,c.iname,c.cpartno,c.cdrgno,c.unit,to_char(a.ent_Dt,'dd/mm/yyyy') as pent_dt,to_char(a.edt_Dt,'dd/mm/yyyy') as papp_dt from " + frm_tabname + " a,typegrp b,item c where trim(a.stagec)=trim(b.type1) and b.branchcd='" + frm_mbr + "' and b.id='KK' and trim(a.icode)=trim(c.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO"; }
                    else
                ...
                case "Edit_E":
                    ...
                    clearctrl();
                    if (proc_pwise.Value == "Y")
                    { SQuery = "Select a.*,b.name,c.iname,c.cpartno,c.cdrgno,c.unit,to_char(a.ent_Dt,'dd/mm/yyyy') as pent_dt,to_char(a.edt_Dt,'dd/mm/yyyy') as papp_dt,p.mchname from " + frm_tabname + " a left join pmaint p on trim(a.opcode)=trim(p.acode)||'/'||trim(p.srno) and p.branchcd='" + frm_mbr + "' and p.type='10',typegrp b,item c where trim(a.stagec)=trim(b.type1) and b.branchcd='" + frm_mbr + "' and b.id='KK' and trim(a.icode)=trim(c.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO"; }
                    else
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (proc_pwise.Value == "Y")
                        {
                            if (col1.Length > 2) SQuery = "select type1,name,0 as rate from typegrp where branchcd='" + frm_mbr + "' and id='KK' and trim(type1) in (" + col1 + ") order by type1";
                            else SQuery = "select type1,name,0 as rate from typegrp where branchcd='" + frm_mbr + "' and id='KK' and trim(type1)='" + col1 + "' order by type1";
                        }
                        else
                        {
                ...
                case "SG1_ROW_ADD_E":
                    ...
                    if (proc_pwise.Value == "Y")
                    { SQuery = "select type1,name,rate from typegrp where branchcd='" + frm_mbr + "' and id='KK' and trim(type1)='" + col1 + "' order by type1"; }
                    else
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            if (proc_pwise.Value == "Y")
            { SQuery = "select '-' as fstr,'-' as gstr,b.IName as Product_Name,b.Cpartno,c.Name as Process_Name,a.Mtime as Set_up_time,a.Mtime1 as Std_Prodn_time,a.vchnum as Mapping_no,to_char(a.vchdate,'dd/mm/yyyy') as Map_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd,a.vchdate,a.Srno from " + frm_tabname + " a,Item b,typegrp c where c.id='KK' and trim(A.stagec)=trim(c.type1) and trim(A.icode)=trim(B.Icode) and a.branchcd='" + frm_mbr + "' and c.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vchdate " + PrdRange + " and a.icode like '" + party_cd + "%' and a.icode like '" + part_cd + "%' order by vdd desc,a.vchnum desc,a.srno"; }
            else
            { SQuery = "select '-' as fstr,'-' as gstr,b.IName as Product_Name,b.Cpartno,c.Name as Process_Name,a.Mtime as Set_up_time,a.Mtime1 as Std_Prodn_time,a.vchnum as Mapping_no,to_char(a.vchdate,'dd/mm/yyyy') as Map_Dt,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,to_Char(a.vchdate,'yyyymmdd') as vdd,a.vchdate,a.Srno from " + frm_tabname + " a,Item b,type c where c.id='K' and trim(A.stagec)=trim(c.type1) and trim(A.icode)=trim(B.Icode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and a.vchdate " + PrdRange + " and a.icode like '" + party_cd + "%' and a.icode like '" + part_cd + "%' order by vdd desc,a.vchnum desc,a.srno"; }



            fgen.drillQuery(0, SQuery, frm_qstr, "6#7#", "3#4#5#6#", "300#200#100#100#");
        ...
        else
        {
            ...
            {
                Checked_ok = "N";
                fgen.msg("-", "AMSG", "Dear " + frm_uname + " , Process Mapping Already done for This Item on Doc. No. " + last_entdt + " ,Please Check !!");
            }
            //checks
            if (edmode.Value == "Y")
            {
        ...
        switch (var)
        {
            ...
            case "SG1_ROW_ADD":
                if (index < sg1.Rows.Count - 1)
                {
                    ...
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_CMD_FROM", hffield.Value);
                    make_qry_4_popup();
                    fgen.Fn_open_sseek("Select Stages", frm_qstr);
                }
                else
                {
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 1)
            {
                ...
                oporow["SRNO"] = i + 1;
                oporow["icode"] = txtlbl4.Text.Trim().ToUpper();
                oporow["stagec"] = sg1.Rows[i].Cells[13].Text.Trim().ToUpper();
                oporow["stg_names"] = sg1.Rows[i].Cells[14].Text.Trim().ToUpper();

                oporow["mtime"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.Trim().ToUpper());
                oporow["mtime1"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.Trim().ToUpper());
                oporow["opcode"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.Trim().ToUpper());
                oporow["remarks"] = (((TextBox)sg1.Rows[i].FindControl("sg1_t5")).Text.Trim().ToUpper());
                oporow["Tshots"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.Trim().ToUpper());
                oporow["pmcount"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t6")).Text.Trim().ToUpper());
                oporow["pcpshot"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.Trim().ToUpper());
                oporow["hcut"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t7")).Text.Trim().ToUpper());
                oporow["fm_Fact"] = fgen.make_double(((TextBox)sg1.Rows[i].FindControl("sg1_t8")).Text.Trim().ToUpper());
                oporow["substg"] = "-";
                oporow["MTIME2"] = 0;
                oporow["MACHCD"] = "-";
                oporow["SCRPCD"] = "-";
    ...
    protected void btnPendItems_Click(object sender, EventArgs e)
    {
        frm_vty = "10";
    ...
    protected void btnChkStgMap_Click(object sender, EventArgs e)
    {
        frm_vty = "10";

        SQuery = "SELECT trim(a.icode) as fstr,'-' as gstr,A.ICODE AS ERPCODE,B.INAME AS PRODUCT,B.CPARTNO,B.UNIT,COUNT(A.ICODE) AS No_of_stages FROM " + frm_tabname + " A,ITEM B WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='10' GROUP BY A.ICODE,B.INAME,B.CPARTNO,B.UNIT ORDER BY A.ICODE ";
        fgen.drillQuery(0, SQuery, frm_qstr, "2#", "3#4#5#6#", "400#300#100#100#");

        SQuery = "SELECT trim(a.icodE) as fstr,trim(a.icodE) as gstr,B.INAME AS PRODUCT,B.CPARTNO,A.stg_names,a.mtime as Set_up_mins,a.mtime1 as Prodn_1K_mins,a.stagec,B.UNIT,A.ICODE AS ERPCODE,a.srno AS CNT FROM " + frm_tabname + " A,ITEM B WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='10' ORDER BY A.ICODE,a.srno ";
        fgen.drillQuery(1, SQuery, frm_qstr, "1#", "3#7", "400#300");

        fgen.Fn_DrillReport("List of " + lblheader.Text.Trim() + " ", frm_qstr);
    }
    protected void btnChkDuplicate_Click(object sender, EventArgs e)
    {
        frm_vty = "10";

        SQuery = "SELECT trim(a.icode) as fstr,'-' as gstr,B.INAME AS PRODUCT,B.CPARTNO,COUNT(A.ICODE) AS No_of_Stages,B.UNIT,A.ICODE AS ERPCODE FROM " + frm_tabname + " A,ITEM B WHERE TRIM(A.ICODE)=TRIM(B.ICODE) AND A.BRANCHCD='" + frm_mbr + "' AND A.TYPE='10' GROUP BY A.ICODE,B.INAME,B.CPARTNO,B.UNIT ORDER BY A.ICODE ";
        fgen.drillQuery(0, SQuery, frm_qstr, "1#", "3#7", "400#300");
        fgen.Fn_DrillReport("List of " + lblheader.Text.Trim() + " ", frm_qstr);


        //fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        //fgen.Fn_open_rptlevel("No of Stage Mapping for Each Item", frm_qstr);
    }
}