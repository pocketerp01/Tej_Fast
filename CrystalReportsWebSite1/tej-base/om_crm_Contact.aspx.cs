...
public partial class om_crm_Contact : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        if (Prg_Id == "F15214")
        {
            lblheader.Text = "Purchase Contacts Master";
            frm_tabname = "csmst_purch";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "P_");
        }
        else
        {
            lblheader.Text = "CRM Contacts Master";
            frm_tabname = "csmst_crm";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "C_");
        }
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            case "MGRBUT":
                ...
                SQuery = "select name as fstr,name as Contact_Type ,type1 as code from typegrp where branchcd!='DD' and id='SU' order by name ";
                break;           
            case "STATBUT":
                ...
                { SQuery = "select name as fstr ,name as State_Name ,type1 as code from typegrp where id='ES' order by Name"; }
                break;

            case "COUNTBUT":
                SQuery = "select name as fstr ,name as State_Name ,type1 as code from typegrp where id='CN' order by Name";
                break;
            ...
            case "CUSTBUT":
                    ...
                    if (Prg_Id == "F15214")
                    {
                        SQuery = "SELECT ACODE AS FSTR,ANAME AS PARTY,ACODE AS CODE,ADDR1,ADDR2,staten as state,Pay_num FROM FAMST where substr(acode,1,2)in ('02','06') and length(Trim(nvl(deac_by,'-')))<=1 ORDER BY aname ";
                    }
                    else
                    {
                        SQuery = "SELECT ACODE AS FSTR,ANAME AS PARTY,ACODE AS CODE,ADDR1,ADDR2,staten as state,Pay_num FROM FAMST where substr(acode,1,2)in ('02','16') and length(Trim(nvl(deac_by,'-')))<=1 ORDER BY aname ";
                    }
            ...
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E" || btnval == "Atch_E")
                    Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
                    SQuery = "select A.acode as fstr,a.Aname as Contact_Name,a.PERSON,A.Addr1 as Address_l1,a.acode as Cont_Acode,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as entry_Dt,a.addr2 as Address_l2,a.Edt_by,a.edt_Dt,a.ent_Dt from " + frm_tabname + " a where  a.branchcd!='DD' order by A.aname ";
                //SQuery = "select Acode as Fstr,Aname as Contact_company,Acode as Contact_Code,Person as Contact_Name,Addr1,Addr2,tdsnum,tcsnum from "+ frm_tabname +" where branchcd!='DD' order by aname";
    ...
    void newCase(string vty)
    {
        ...
        frm_vnum = fgen.next_no(frm_qstr, frm_cocd, "SELECT MAX(substr(" + doc_nf.Value + ",2,6)) AS VCH FROM " + frm_tabname + " WHERE branchcd='00' ", 5, "VCH");

        txt_ConsigneeCode.Value = frm_vty.Substring(0,1)+frm_vnum;  
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + "Contact Address";
        }
        if (txt_stat_name.Value.Trim().Length < 2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + "Contact State";
        }
        if (txt_count_name.Value.Trim().Length < 2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + "Contact Country";
        }

        if (txt_telephonenno.Value.Trim().Length < 2)
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + "Contact Telephone";
        }
        if (txt_email.Value.Trim().Length < 2)
        {
            reqd_nc = reqd_nc + 1;
        ...
        if (txt_type.Value.Trim().Length< 2 )
        {
            reqd_nc = reqd_nc + 1;
            reqd_flds = reqd_flds + " / " + "Type";
        }
    ...
    {
        SQuery = "select a.Aname as Contact_Company,a.Acode as Contact_Code,a.Person as Contact_Name,a.Addr1,a.Addr2,a.Staten,a.GST_NO,a.PERSON,a.MOBILE,trim(b.aname) as Ref_Customer,a.tdsnum as lead_type,a.Ent_by,a.Ent_Dt,a.edt_by,a.edt_dt from " +frm_tabname +" a left outer join famst b on trim(a.tcsnum)=trim(b.acode)  where a.branchcd='" + frm_mbr + "' and ent_by ='" + frm_uname + "'order by a.aname";
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
    ...
    protected void btnprint_ServerClick(object sender, EventArgs e)
    {
        ...
        string header_n = "CRM Contacts List";
        SQuery = "SELECT '" + header_n + "' as header,A.ACODE,trim(A.ANAME) as aname,A.ENT_BY,to_char(A.ENT_DT,'dd/mm/yyyy') as ENT_DT,a.Person as Contact_Name,trim(a.email) as email,a.MOBILE,trim(b.aname) as Ref_Customer FROM "+ frm_tabname +" a left outer join famst b on trim(a.tcsnum)=trim(b.acode) WHERE a.branchcd='" + frm_mbr + "' and trim(a.ent_by)='" + frm_uname + "' ORDER BY A.ANAME";
        fgen.Print_Report(frm_cocd, frm_qstr, frm_mbr, SQuery, "std_PartyMastercrm", "std_PartyMastercrm");
    }
    //------------------------------------------------------------------------------------
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Atch_E":
                    ...
                    if (frm_tabname == "csmst_purch")
                    {
                        mcol1 = col1 + mcol7 + "PCONT";
                    }
                    else
                    {
                        mcol1 = col1 + mcol7 + "SCONT";
                    }
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txt_count_name.Value = dt.Rows[i]["COUNTRYN"].ToString().Trim();
                        ...
                        txt_dealin.Value = dt.Rows[i]["DEALING_IN"].ToString().Trim();
                        txt_ourref.Value = dt.Rows[i]["CONTACT_OF"].ToString().Trim();
                ...
                case "COUNTBUT":
                    if (col1.Length <= 0) return;
                    txt_count_name.Value = col1.Trim();
                    //btnlbl7.Focus();
                    break;
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select Aname as Contact_Company,Acode as Contact_Code,Person as Contact_Name,Addr1,Addr2,Staten,GST_NO,GIRNO,PERSON,MOBILE,VENCODE as Parent_Cust,tdsnum,tcsnum from "+ frm_tabname +" where branchcd!='DD' order by aname";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
            fgen.Fn_open_rptlevel("List of " + lblheader.Text.Trim() + " ", frm_qstr);
            hffield.Value = "-";
        }
                ...
                try
                {
                    ...
                    else
                    {
                        if (save_it == "Y")
                        {
                            ...
                            fgen.msg("-", "AMSG", "Contact Number " + txt_ConsigneeCode.Value + "'13' has been Saved.");
                            //updateMessage();
                    ...
                    "<td><b>Consignee Master Code</b></td><td><b>Contact Master Name</b></td><td><b>User Name</b></td><td><b>Activity Date</b></td><td><b>ID</b></td>");
    ...
    void save_fun()
    {
        ...
        oporow["cstaffcd"] = txt_range.Value.ToUpper().Trim();
        oporow["pname"] = "-";
        ...
        oporow["countryn"] = txt_count_name.Value.ToUpper().Trim(); 
        ...
        oporow["DEALING_IN"] = txt_dealin.Value.ToUpper().Trim();
        oporow["CONTACT_OF"] = txt_ourref.Value.ToUpper().Trim();
    ...
    protected void btn_customer_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Reference Account Code", frm_qstr);
    }
    ...
    protected void btn_count_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "COUNTBUT";
        make_qry_4_popup();
