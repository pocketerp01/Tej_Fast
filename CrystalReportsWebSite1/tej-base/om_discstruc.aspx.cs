...
public partial class om_discstruc : System.Web.UI.Page
{
    ...
    public void set_Val()
    {
        ...
        fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "DS");

        fgenMV.Fn_Set_Mvar(frm_qstr, "U_TABNAME", frm_tabname);
        typePopup = "N";

        if (frm_mbr == "08" || frm_mbr == "09")
            tdParty.Visible = true;
        else tdParty.Visible = false;
    }
    //------------------------------------------------------------------------------------
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "TACODE":
                //pop1
                SQuery = "SELECT DISTINCT a.Acode as FStr,b.Aname as Customer,a.Acode,b.Addr1,b.Addr2,b.GST_No,b.Staten from SOMAS a,famst b where trim(A.acodE)=trim(B.acode) AND A.BRANCHCD='" + frm_mbr + "' AND a.TYPE LIKE '4%'  and length(trim(nvl(b.deac_by,'-'))) <2 AND trim(a.icat)!='Y' AND TRIM(NVL(a.APP_BY,'-'))!='-' order by b.Aname ";
                break;
            case "TICODE":
                //pop2
                ...
                break;
            case "ACODE":
                string statename = fgen.seek_iname(frm_qstr, frm_cocd, "SELECT STATEnM FROM TYPE WHERE ID='B' AND TYPE1='" + frm_mbr + "'", "STATEnM");
                SQuery = "SELECT ACODE AS FSTR,ANAME AS PARTY,ACODE AS CODE,ADDR1,ADDR2,ADDR3,staten as state,Pay_num FROM FAMST where substr(acode,1,2)in ('16') and length(Trim(nvl(deac_by,'-')))<=1 and length(Trim(nvl(STATEN,'-')))>1 and trim(upper(STATEN))='" + statename.Trim().ToUpper() + "' ORDER BY aname ";

                SQuery = "SELECT DISTINCT BANK_ACNO AS FSTR,BANK_ACNO AS CUSTOMER_cLASS,'CUSTOMER CLASS TO SELECT' AS COL1 FROM FAMST WHERE TRIM(NVL(BANK_ACNO,'-'))!='-' ORDER BY FSTR";
                break;
            case "SG1_ROW_ADD":
            case "SG1_ROW_ADD_E":
                col1 = "";
                ...
                SQuery = "SELECT DISTINCT trim(A.Icode)||a.ordno||to_char(a.orddt,'dd/mm/yyyy') AS FSTR,B.Iname AS Item_Name,B.Cpartno,A.Icode,a.Ordno,to_char(a.Orddt,'dd/mm/yyyy') as Ord_dt,b.Cdrgno,b.unit,to_char(a.Orddt,'yyyymmdd') as Ordt FROM SOMAS a , Item B where TRIM(a.ICODE)=TRIM(B.ICODE) AND a.branchcd='" + frm_mbr + "' AND a.acode='" + txtlbl4.Text + "' and length(Trim(b.deac_by))<2  and length(Trim(b.icode))>4 " + col1 + " and trim(nvl(a.app_by,'-'))!='-' and trim(a.icat)!='Y' ORDER BY b.Iname,Ordt,a.ordno ";
                SQuery = "SELECT DISTINCT A.ICODE AS FSTR,A.INAME AS PRODUCT,A.CPARTNO AS PARTNO,A.UNIT,A.ICODE AS ERPCODE FROM ITEM A WHERE LENGTH(TRIM(A.ICODE))>4 AND SUBSTR(A.ICODE,1,1)='9' ORDER BY A.ICODE ";

                if (frm_mbr == "08" || frm_mbr == "09")
                {
                    SQuery = "SELECT DISTINCT ABC_CLASS AS FSTR,ABC_CLASS PRODUCT_CLASS,'-' AS COL1 FROM ITEM WHERE LENGTH(TRIM(ICODE))>4 AND SUBSTR(ICODE,1,1)='9' AND TRIM(NVL(ABC_CLASS,'-'))!='-' ORDER BY FSTR";
                }
                break;
            case "sg1_t4":
                SQuery = "SELECT DISTINCT trim(mat1) AS FSTR,mat1 PRODUCT_CLASS,'-' AS COL1 FROM ITEM WHERE LENGTH(TRIM(ICODE))>4 AND SUBSTR(ICODE,1,1)='9' AND TRIM(NVL(mat1,'-'))!='-' ORDER BY FSTR";
                break;
            case "SG3_ROW_ADD":
            case "SG3_ROW_ADD_E":
                //pop3
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        //foreach (GridViewRow gr in sg1.Rows)
        //{
        //    //if(((TextBox)gr.FindControl("sg1_t1"))
        //}
    ...
    string checkGridQty()
    {
        ...
        foreach (DataRow drQty1 in dtQty1.Rows)
        {
            ...
            string chk_itc;
            chk_itc = drQty1["fstr"].ToString().Trim().Substring(0, 8);
            col1 = fgen.seek_iname(frm_qstr, frm_cocd, "Select sum(qtyord) as qtyord from somas where branchcd='" + frm_mbr + "' and type like '4%' and trim(acode)='" + txtlbl4.Text.Trim() + "' and trim(icode)||'-'||trim(ordno)||to_char(orddt,'dd/mm/yyyy')='" + drQty1["fstr"].ToString().Trim() + "'", "qtyord");
            if (fgen.make_double(sm.ToString()) > fgen.make_double(col1) && fgen.make_double(col1) > 0)
            {
                mqry = "select (a.Qtyord)-(a.Soldqty) as Bal_Qty from (select fstr,ERP_code,sum(Qtyord) as Qtyord,sum(Soldqty) as Soldqty  from (SELECT trim(Icode)||ordno||'-'||to_char(orddt,'dd/mm/yyyy') as fstr,trim(Icode) as ERP_code,Qtyord,0 as Soldqty,0 as prate from Somas where branchcd='" + frm_mbr + "' and type like '4%' and trim(icat)!='Y' and trim(acode)='" + txtlbl4.Text.Trim() + "' and orddt>=to_Date('01/04/2017','dd/mm/yyyy')  union all SELECT trim(Icode)||ponum||'-'||trim(podate) as fstr,trim(Icode) as ERP_code,0 as Qtyord,total as qtyord ,0 as irate from schedule where branchcd='" + frm_mbr + "' and type like '46%' and vchdate>=to_Date('01/04/2017','dd/mm/yyyy') and trim(acode)='" + txtlbl4.Text.Trim() + "' and trim(vchnum)||to_Char(vchdate,'dd/mm/yyyy')!='" + txtvchnum.Text + txtvchdate.Text + "')  group by fstr,ERP_code having sum(Qtyord)-sum(Soldqty)>0  )a,item b where trim(a.erp_code)=trim(B.icode) and a.fstr='" + drQty1["fstr"].ToString().Trim() + "' order by B.Iname,trim(a.fstr)";
            ...
            if (fgen.make_double(sm.ToString()) > fgen.make_double(col1) && fgen.make_double(col1) > 0)
            {
                ...
                string itm_name;
                itm_name = fgen.seek_iname(frm_qstr, frm_cocd, "select iname from ITEM where SUBSTR(ICODE,1,8)='" + chk_itc + "' ", "iname");
        ...
        else
        {
            ...
            switch (btnval)
            {
                ...
                case "Edit_E":
                    ...
                    if (col1 == "") return;
                    clearctrl();
                    SQuery = "Select a.*,b.iname,b.cpartno,b.unit from " + frm_tabname + " a,item b where trim(a.icodE)=trim(b.icode) and a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    if (frm_mbr == "08" || frm_mbr == "09")
                    {
                        SQuery = "Select a.*,'-' as iname,'-' as cpartno,'-' as unit from " + frm_tabname + " a where a.branchcd||a.type||trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy')='" + frm_mbr + frm_vty + col1 + "' ORDER BY A.SRNO";
                    }
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtlbl3.Text = Convert.ToDateTime(dt.Rows[i]["ent_Dt"].ToString().Trim()).ToString("dd/MM/yyyy");

                        txtAcode.Text = dt.Rows[i]["acode"].ToString().Trim();
                        txtAname.Text = fgen.seek_iname(frm_qstr, frm_cocd, "select aname from famst where trim(acode)='" + txtAcode.Text.Trim() + "'", "aname");
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            sg1_dr["sg1_t3"] = dt.Rows[i]["NUM3"].ToString().Trim();
                ...
                case "Print_E":
                    ...
                    SQuery = "Select a.vchnum as entry_no,to_char(A.vchdate,'dd/mm/yyyy') as entry_dt,a.icode as erpcode,b.iname,b.cpartno,b.unit,b.irate as present_mrp,a.num1 as vb_to_sapl,a.num2 as sapl_to_dealer,a.num3 as mrp from " + frm_tabname + " a,item b where trim(a.icodE)=trim(b.icode) and a.branchcd||a.type='" + frm_mbr + frm_vty + "' and trim(a.vchnum)||to_Char(a.vchdate,'dd/mm/yyyy') in (" + col1 + ") ORDER BY A.SRNO";
                    fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
                ...
                case "SG1_ROW_ADD":
                    ...
                    if (ViewState["sg1"] != null)
                    {
                        ...
                        if (col1.Contains("'")) SQuery = "SELECT DISTINCT A.ICODE AS FSTR,A.INAME AS PRODUCT,A.CPARTNO AS PARTNO,A.UNIT,A.ICODE AS ERPCODE,A.IRATE FROM ITEM A WHERE LENGTH(TRIM(A.ICODE))>4 AND SUBSTR(A.ICODE,1,1)='9' AND A.ICODE in (" + col1 + ") ";
                        else SQuery = "SELECT DISTINCT A.ICODE AS FSTR,A.INAME AS PRODUCT,A.CPARTNO AS PARTNO,A.UNIT,A.ICODE AS ERPCODE,A.IRATE FROM ITEM A WHERE LENGTH(TRIM(A.ICODE))>4 AND SUBSTR(A.ICODE,1,1)='9' AND A.ICODE='" + col1 + "' ";

                        if (frm_mbr == "08" || frm_mbr == "09")
                        {
                            SQuery = "SELECT DISTINCT ABC_CLASS AS FSTR,ABC_CLASS AS PRODUCT,ABC_CLASS AS PARTNO,ABC_CLASS UNIT,ABC_CLASS ERPCODE,0 AS IRATE FROM ITEM WHERE LENGTH(TRIM(ICODE))>4 AND SUBSTR(ICODE,1,1)='9' AND ABC_CLASS in (" + col1 + ") ";
                        }
                        ...
                        for (int d = 0; d < dt.Rows.Count; d++)
                        {
                            ...
                            sg1_dr["sg1_f1"] = dt.Rows[d]["ERPCODE"].ToString().Trim();
                            sg1_dr["sg1_f2"] = dt.Rows[d]["PRODUCT"].ToString().Trim();
                            sg1_dr["sg1_f3"] = dt.Rows[d]["PARTNO"].ToString().Trim();
                ...
                case "sg1_t4":
                    if (col1.Length > 0)
                    {
                        ((TextBox)sg1.Rows[Convert.ToInt32(hf1.Value)].FindControl("sg1_t4")).Text = col1;
                    }
                    break;
                case "SG1_ROW_ADD_E":
                    if (col1.Length <= 0) return;
                    if (col1.Contains("'")) SQuery = "SELECT DISTINCT A.Icode AS FSTR,B.Iname AS Item_Name,A.Icode,a.irate,a.Ordno,a.Orddt,B.Cpartno,b.Cdrgno,b.unit,to_char(a.orddt,'dd/mm/yyyy') as orddtd,a.type FROM SOMAS a , Item B where TRIM(a.ICODE)=TRIM(B.ICODE) AND a.branchcd='" + frm_mbr + "' AND a.acode='" + txtlbl4.Text + "' and length(Trim(b.deac_by))<2  and length(Trim(b.icode))>4  and trim(nvl(a.app_by,'-'))!='-' and trim(a.icat)!='Y' and trim(A.Icode)||a.ordno||to_char(a.orddt,'dd/mm/yyyy') in (" + col1 + ") ORDER BY b.Iname,a.orddt ";
                    else SQuery = "SELECT DISTINCT A.Icode AS FSTR,B.Iname AS Item_Name,A.Icode,a.irate,a.Ordno,a.Orddt,B.Cpartno,b.Cdrgno,b.unit,to_char(a.orddt,'dd/mm/yyyy') as orddtd,a.type FROM SOMAS a , Item B where TRIM(a.ICODE)=TRIM(B.ICODE) AND a.branchcd='" + frm_mbr + "' AND a.acode='" + txtlbl4.Text + "' and length(Trim(b.deac_by))<2  and length(Trim(b.icode))>4  and trim(nvl(a.app_by,'-'))!='-' and trim(a.icat)!='Y' and trim(A.Icode)||a.ordno||to_char(a.orddt,'dd/mm/yyyy')='" + col1 + "' ORDER BY b.Iname,a.orddt ";
                    dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                    ...
                    for (int d = 0; d < dt.Rows.Count; d++)
                    {
                        ...
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[14].Text = dt.Rows[d]["item_name"].ToString().Trim();
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[15].Text = dt.Rows[d]["cpartno"].ToString().Trim();
                        sg1.Rows[Convert.ToInt32(hf1.Value)].Cells[16].Text = dt.Rows[d]["ordno"].ToString().Trim() + "-" + dt.Rows[d]["orddtd"].ToString().Trim();
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "select a.vchnum as Sch_no,to_char(a.vchdate,'dd/mm/yyyy') as Sch_Dt,b.aName as Supplier,c.Iname as Item_Name,C.Cpartno,a.Total,c.Unit,a.line_rmk as Remarks,a.Icode,a.ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as ent_dt,a.app_by,(Case when length(trim(nvl(a.app_by,'-')))<=1 then '-' else to_char(a.app_Dt,'dd/mm/yyyy') end) as app_dt,to_Char(a.vchdate,'yyyymmdd') as vdd,a.srno from " + frm_tabname + " a,famst b,item c where trim(A.acode)=trim(B.acode) and trim(A.icode)=trim(c.icode) and a.branchcd='" + frm_mbr + "' and a.type='" + frm_vty + "' and vchdate " + PrdRange + " and a.acode like '" + party_cd + "%' and a.icode like '" + part_cd + "%' order by vdd ,a.vchdate ,a.srno";
                ...
                if (Checked_ok == "Y")
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            for (i = 0; i < sg1.Rows.Count - 0; i++)
                            {
                                if (sg1.Rows[i].Cells[13].Text.Length > 0 && sg1.Rows[i].Cells[13].Text != "-")
                                {
                                    save_it = "Y";
                                    break;
                                }
    ...
    protected void btnlbl4_Click(object sender, ImageClickEventArgs e)
    {
        ...
        make_qry_4_popup();
        fgen.Fn_open_sseek("Select Customer (From Approved SO List)", frm_qstr);
    }
    ...
    void save_fun()
    {
        ...
        for (i = 0; i < sg1.Rows.Count - 0; i++)
        {
            if (sg1.Rows[i].Cells[13].Text.Length > 0 && sg1.Rows[i].Cells[13].Text != "-")
            {
                ...
                oporow["NUM1"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t1")).Text.toDouble();
                oporow["NUM2"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t2")).Text.toDouble();
                oporow["NUM3"] = ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble();
                ...
                if (frm_vnum != "000000")
                {
                    if (((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble() > 0)
                        fgen.execute_cmd(frm_qstr, frm_cocd, "UPDATE ITEM SET IRATE='" + ((TextBox)sg1.Rows[i].FindControl("sg1_t3")).Text.toDouble() + "' WHERE TRIM(ICODE)='" + sg1.Rows[i].Cells[13].Text.Trim() + "' ");
                }
            }
        }
    }
            ...
            //if (txtacode.Value.Trim().Length > 2)
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    try
                    {
                        ...
                        SQuery = "SELECT DISTINCT ICODE,trim(ICODE)||'~'||INAME||'~'||trim(CPARTNO)||'~'||UNIT||'~'||IRATE AS VAL,CPARTNO,HSCODE,IRATE FROM ITEM WHERE SUBSTR(ICODE,1,1)='9' ORDER BY ICODE";
                        dt2 = new DataTable();
                        dt2 = fgen.getdata(frm_qstr, frm_cocd, SQuery);

                        foreach (DataRow dr in dt.Rows)
                        {
                            col2 = fgen.seek_iname_dt(dt2, "cpartno='" + dr[0].ToString().Trim() + "'", "VAL");
                            if (col2 != "0")
                            {
                                sg1_dr = sg1_dt.NewRow();
                                sg1_dr["sg1_srno"] = sg1_dt.Rows.Count + 1;
                                ...
                                sg1_dr["sg1_f1"] = col2.Split('~')[0];
                                sg1_dr["sg1_f2"] = col2.Split('~')[1];
                                sg1_dr["sg1_f3"] = col2.Split('~')[2];
                                sg1_dr["sg1_f4"] = "";
                                sg1_dr["sg1_f5"] = col2.Split('~')[3];
                                sg1_dr["sg1_t1"] = dr[1].ToString();
                                sg1_dr["sg1_t2"] = dr[2].ToString();
                                sg1_dr["sg1_t3"] = dr[3].ToString();
                ...
                sg1.DataBind();
                setColHeadings();
                fgen.msg("-", "AMSG", "Total Rows Imported : " + (sg1_dt.Rows.Count - 1).ToString());
            }
        }
    }
