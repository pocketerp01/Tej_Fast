...
public partial class om_css_asg : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                doc_addl.Value = "1";
                lblheader.Text = "CSS Assignment System";
    ...
    public void set_Val()
    {
        ...
        switch (Prg_Id)
        {
            ...
            case "P13003D1":
                ...
                break;
            case "F60106":
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "DF");
                typePopup = "N";
                frm_vty = "DF";
                ...
                ddModule.Disabled = true;
                txtlbl8.Disabled = true;
                txtlbl9.Disabled = true;
                ddIssueType.Disabled = true;
    ...
    public void make_qry_4_popup()
    {
        ...
        switch (btnval)
        {
            ...
            case "SG1_ROW_TAX":
                ...
                break;
            case "New":
                SQuery = "select CSSno||CSSdt||ccode as fstr,CSSno as CSS_No,CSSdt as CSS_DT,ccode as Client_Code,max(Emodule) as Module_name,(Case when sum(dasg)>0 then 'Yes' else 'NO' end ) as Pend_Asg,(Case when sum(conf)>0 then 'Yes' else 'NO' end ) as Pend_Conf,max(Req_type) as Req_type,max(Iss_Type) as Iss_Type,cssdd from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Req_type,Iss_Type,1 as dasg,1 as conf,to_Char(cssdt,'yyyymmdd') as cssdd from WB_CSS_LOG where branchcd='" + frm_mbr + "' and type='CS' and trim(nvl(Fapp_by,'-'))='-' and upper(nvl(dir_comp,'-'))!='Y' and cssdt>=to_DaTE('01/10/2017','dd/mm/yyyy') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, -1 as dasg,0 as impl,to_Char(cssdt,'yyyymmdd') as cssdd from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, 0 as dasg,-1 as impl,to_Char(cssdt,'yyyymmdd') as cssdd from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED'  ) group by CSSno||CSSdt||ccode,CSSno,CSSdt,ccode,cssdd having (sum(dasg)>0 or sum(conf)>0)  order by cssdd,CSSno ";
                
                SQuery = "select CSSno||CSSdt||ccode as fstr,CSSno as CSS_No,CSSdt as CSS_DT,ccode as Client_Code,max(Emodule) as Module_name,max(ent_by) as log_by,(Case when sum(dasg)>0 then 'Yes' else 'NO' end ) as Pend_Asg,max(Req_type) as Req_type,max(Iss_Type) as Iss_Type,cssdd from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Req_type,Iss_Type,1 as dasg,1 as conf,to_Char(cssdt,'yyyymmdd') as cssdd,ent_by from WB_CSS_LOG where branchcd='" + frm_mbr + "' and type='CS' and trim(nvl(Fapp_by,'-'))='-' and upper(nvl(dir_comp,'-'))!='Y' and cssdt>=to_DaTE('01/10/2017','dd/mm/yyyy') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, -1 as dasg,0 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' ) group by CSSno||CSSdt||ccode,CSSno,CSSdt,ccode,cssdd having (sum(dasg)>0 )  order by cssdd desc ,CSSno desc ";
                break;
            case "Edit":
            case "Del":
            case "Print":
                ...
                break;
            case "ASGN":
                SQuery = "Select a.username as fstr,a.username,a.erpdeptt,b.Name,a.Userid,a.emailid from evas a,type b where b.id='M' and trim(a.erpdeptt)=b.type1 and a.userid<'000052' and trim(a.username)!='-'  order by A.ERPDEPTT,a.username ";
                break;
            default:
                if (btnval == "Edit_E" || btnval == "Del_E" || btnval == "Print_E")
                    SQuery = "select distinct trim(A." + doc_nf.Value + ")||to_Char(a." + doc_df.Value + ",'dd/mm/yyyy') as fstr,a." + doc_nf.Value + " as DF_No,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as DF_Dt,a.ccode as company,a.eicon as S_Topic,a.asg_Agt as Assign_to,a.cssno as CSSno,to_char(a.cssdt,'dd/mm/yyyy') as CSSdt,a.Ent_by,to_char(a.ent_Dt,'dd/mm/yyyy') as Ent_dt,REMARKS,to_Char(a." + doc_df.Value + ",'yyyymmdd') as vdd from " + frm_tabname + " a where  a.branchcd='" + frm_mbr + "' and a.type='DF' order by vdd desc,a." + doc_nf.Value + " desc";
                break;
        }
    ...
    protected void btnnew_ServerClick(object sender, EventArgs e)
    {
        ...
        if (chk_rights == "Y")
        {
            ...
            hffield.Value = "New";
            fgen.msg("-", "CMSG", "Do you want to see Pending List'13'No for Complete List !!");
            //make_qry_4_popup();
    ...
    void newCase(string vty)
    {
        ...
        SQuery = "Select a.* from WB_CSS_LOG a where a.branchcd||a.type||trim(a.CSsNO)||to_Char(a.CSSDT,'dd/mm/yyyy')||TRIM(A.CCODE)='" + frm_mbr + "CS" + col1 + "' ORDER BY A.SRNO";
        dt = new DataTable();
        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
        if (dt.Rows.Count > 0)
        {
            i = 0;
            txtvchnum.Value = dt.Rows[0]["CSsNO"].ToString().Trim();
            txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["CSSDT"].ToString().Trim()).ToString("dd/MM/yyyy");

            //txtrmk.Text = dt.Rows[i]["remarks"].ToString().Trim();

            txtlbl4.Value = dt.Rows[i]["CCODE"].ToString().Trim();
            ddModule.Value = dt.Rows[i]["EMODULE"].ToString().Trim();
            txtlbl8.Value = dt.Rows[i]["EICON"].ToString().Trim();
            txtlbl9.Value = dt.Rows[i]["REQ_TYPE"].ToString().Trim();
            ...
            Text_cl_rmk.Text = dt.Rows[i]["remarks"].ToString().Trim();

            if (dt.Rows[i]["filepath"].ToString().Trim().Length > 1)
            {
                lblAtch.Text = dt.Rows[i]["filename"].ToString().Trim();
                hfAtchPath.Value = dt.Rows[i]["filepath"].ToString().Trim();
                divAtch.Visible = true;
            }
            else divAtch.Visible = false;
        }
        ...
        txtDFno.Value = frm_vnum;
        txtDFdt.Value = fgen.Fn_curr_dt(frm_cocd, frm_qstr);
    ...
    protected void btnsave_ServerClick(object sender, EventArgs e)
    {
        ...
        if (txtAssgn.Value.Trim().Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Enter Assignee !!");
            return;
        }
        if (txtrmk.Text.Trim().Length < 2)
        {
            fgen.msg("-", "AMSG", "Please Enter Remarks !!");
            return;
        }
        string mandField = "";
        ...
        else
        {
            ...
            switch (btnval)
            {
                case "New":
                    ...
                    if (col1 == "Y")
                    {
                        SQuery = "Select * from (select CSSno||CSSdt||ccode as fstr,CSSno as CSS_No,CSSdt as CSS_DT,ccode as Client_Code,max(Emodule) as Module_name,max(ent_by) as log_by,(Case when sum(dasg)>0 then 'Yes' else 'NO' end ) as Pend_Asg,(Case when sum(conf)>0 then 'Yes' else 'NO' end ) as Pend_Conf,max(Req_type) as Req_type,max(Iss_Type) as Iss_Type,cssdd from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Req_type,Iss_Type,1 as dasg,1 as conf,to_Char(cssdt,'yyyymmdd') as cssdd,ent_by from WB_CSS_LOG where branchcd='" + frm_mbr + "' and type='CS' and trim(nvl(Fapp_by,'-'))='-' and upper(nvl(dir_comp,'-'))!='Y' and cssdt>=to_DaTE('01/10/2017','dd/mm/yyyy') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, -1 as dasg,0 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, 0 as dasg,-1 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED'  ) group by CSSno||CSSdt||ccode,CSSno,CSSdt,ccode,cssdd having (sum(dasg)>0 and sum(conf)>0)) where upper(Pend_Asg)='YES'  order by css_dT,CSS_no ";
                        SQuery = "Select * from (select CSSno||CSSdt||ccode as fstr,CSSno as CSS_No,CSSdt as CSS_DT,ccode as Client_Code,max(Emodule) as Module_name,max(eicon) as subject,max(ent_by) as log_by,max(Req_type) as Req_type,max(Iss_Type) as Iss_Type,MAX(substr(REMARKS,1,60)||'...') AS REMARKS,(Case when sum(dasg)>0 then 'Yes' else 'NO' end ) as Pend_Asg,(Case when sum(conf)>0 then 'Yes' else 'NO' end ) as Pend_Conf,cssdd from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Req_type,Iss_Type,1 as dasg,1 as conf,to_Char(cssdt,'yyyymmdd') as cssdd,ent_by,REMARKS,eicon from WB_CSS_LOG where branchcd='" + frm_mbr + "' and type='CS' and trim(nvl(Fapp_by,'-'))='-' and upper(nvl(dir_comp,'-'))!='Y' and cssdt>=to_DaTE('01/10/2017','dd/mm/yyyy') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, -1 as dasg,0 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by,NULL AS REMARKS,null as eicon from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, 0 as dasg,-1 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by,NULL AS REMARKS,null as eicon from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED'  ) group by CSSno||CSSdt||ccode,CSSno,CSSdt,ccode,cssdd having (sum(dasg)>0 and sum(conf)>0)) where upper(Pend_Asg)='YES'  order by cssdd desc ,CSS_no desc ";
                    }
                    else
                        SQuery = "Select * from (select CSSno||CSSdt||ccode as fstr,CSSno as CSS_No,CSSdt as CSS_DT,ccode as Client_Code,max(Emodule) as Module_name,max(eicon) as subject,max(ent_by) as log_by,max(Req_type) as Req_type,max(Iss_Type) as Iss_Type,MAX(substr(REMARKS,1,60)||'...') AS REMARKS,(Case when sum(dasg)>0 then 'Yes' else 'NO' end ) as Pend_Asg,(Case when sum(conf)>0 then 'Yes' else 'NO' end ) as Pend_Conf,cssdd from (Select ccode,trim(cssno) as CSSno,to_Char(cssdt,'dd/mm/yyyy') as CSSdt,Emodule,Req_type,Iss_Type,1 as dasg,1 as conf,to_Char(cssdt,'yyyymmdd') as cssdd,ent_by,REMARKS,eicon from WB_CSS_LOG where branchcd='" + frm_mbr + "' and type='CS' and trim(nvl(Fapp_by,'-'))='-' and upper(nvl(dir_comp,'-'))!='Y' and cssdt>=to_DaTE('01/10/2017','dd/mm/yyyy') union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, -1 as dasg,0 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by,NULL AS REMARKS,null as eicon from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='ASSIGNED/ UNDER PROCESSING' union all Select ccode,trim(cssno) as cssno,to_Char(cssdt,'dd/mm/yyyy') as cssdt,null as Emod,null as Req_type,null as Iss_Type, 0 as dasg,-1 as impl,to_Char(cssdt,'yyyymmdd') as cssdd,null as ent_by,NULL AS REMARKS,null as eicon from " + frm_tabname + " where branchcd='" + frm_mbr + "' and type='" + frm_vty + "' AND UPPER(TRIM(CSS_STATUS))='CONFIRMATION REQUIRED'  ) group by CSSno||CSSdt||ccode,CSSno,CSSdt,ccode,cssdd having (sum(dasg)>0 and sum(conf)>0)) order by cssdd desc ,CSS_no desc ";
                ...
                case "Edit_E":
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtDFno.Value = dt.Rows[0]["" + doc_nf.Value + ""].ToString().Trim();
                        txtDFdt.Value = Convert.ToDateTime(dt.Rows[0]["" + doc_df.Value + ""].ToString().Trim()).ToString("dd/MM/yyyy");

                        txtrmk.Text = dt.Rows[i]["remarks"].ToString().Trim();

                        txtvchnum.Value = dt.Rows[i]["CSsNO"].ToString().Trim();
                        txtvchdate.Value = Convert.ToDateTime(dt.Rows[0]["CSSDT"].ToString().Trim()).ToString("dd/MM/yyyy");
                        txtlbl4.Value = dt.Rows[i]["CCODE"].ToString().Trim();
                        txtAssgn.Value = dt.Rows[i]["ASG_AGT"].ToString().Trim();
                        doc_addl.Value = dt.Rows[i]["ASG_DPT"].ToString().Trim();

                        ddStatus.Value = dt.Rows[i]["CSS_STATUS"].ToString().Trim();
                        txtrmk.Text = dt.Rows[i]["REMARKS"].ToString().Trim();

                        ddAnalysis.Value = dt.Rows[i]["ASG_ASYS"].ToString().Trim();
                        ddresolution.Value = dt.Rows[i]["priority"].ToString().Trim();

                        SQuery = "Select a.*,to_Char(a.ent_Dt,'dd/mm/yyyy') As ment_date,to_Char(a.app_Dt,'dd/mm/yyyy') As mapp_date from WB_CSS_LOG a where a.branchcd||a.type||trim(a.cssno)||to_Char(a.cssdt,'dd/mm/yyyy')='" + frm_mbr + "CS" + txtvchnum.Value + txtvchdate.Value + "' ORDER BY A.SRNO";
                        dt = new DataTable();
                        dt = fgen.getdata(frm_qstr, frm_cocd, SQuery);
                        if (dt.Rows.Count > 0)
                        {
    ...
    protected void btnhideF_s_Click(object sender, EventArgs e)
    {
        ...
        if (hffield.Value == "List")
        {
            ...
            SQuery = "SELECT a." + doc_nf.Value + " as Asg_NO,to_char(A." + doc_df.Value + ",'dd/mm/yyyy') as ASg_Dt,a.CSSNO as CSS_NO,to_char(A.CSSDT,'dd/mm/yyyy') as CSS_Dt,a.CCODE as Client_Code,a.Emodule as Module_Name,a.Asg_asys as Our_Analysis,a.Remarks,a.priority as Dlv_Hrs,a.Asg_Dpt,A.Asg_agt,a.CSS_Status ,a.Ent_Dt,a.Ent_by,to_chaR(a.DSRDT,'YYYYMMDD') as DSR_DT  FROM WB_CSS_ASG a WHERE a.branchcd='" + frm_mbr + "' and A.TYPE='DF' AND A.DSRDT " + PrdRange + "  ORDER BY DSR_DT,A.DSRNO";
            //SQuery = "Select a." + doc_nf.Value + " as Doc_no,to_char(a." + doc_df.Value + ",'dd/mm/yyyy') as Doc_Dt,a.Name,a.acode,a.Ment_by,a.Ment_Dt,a.Mapp_by,(Case when length(Trim(A.Mapp_by))<2 then null else a.Mapp_dt end) as App_Dt from " + frm_tabname + " a where a.branchcd='" + frm_mbr + "'  and a.type like '" + frm_vty + "%' order by a." + doc_df.Value + ",a." + doc_nf.Value + ",a.srno ";
            fgenMV.Fn_Set_Mvar(frm_qstr, "U_SEEKSQL", SQuery);
        ...
        else
        {
            ...
            if (col1 == "Y" && Checked_ok == "Y")
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (save_it == "Y")
                        {
                            ...
                            do
                            {
                                ...
                                pk_error = fgen.chk_pk(frm_qstr, frm_cocd, frm_tabname.ToUpper() + frm_mbr + frm_vty + frm_vnum + frm_CDT1, frm_mbr, frm_vty, frm_vnum, txtDFdt.Value.Trim(), "", frm_uname);
                                if (i > 20)
                                {
                    ...
                    agt_email = fgen.seek_iname(frm_qstr, frm_cocd, "select emailid from evas where upper(username)=upper('" + txtAssgn.Value.Trim() + "')", "emailid");

                    if (agt_email.Trim().Length <= 3) agt_email = "info@pocketdriver.in";

                    #region Mail Body

                    html_body = "";
                    html_body = html_body + "<html><body style='font-family: Arial, Helvetica, sans-serif; font-weight: 700; font-size: 13px; color: #474646'>";
                    html_body = html_body + "<br/>";
                    html_body = html_body + "<br/>";

                    string subject = txtlbl4.Value.Trim() + " : Customer Support Ticket [#" + txtvchnum.Value + "] ";

                    html_body = html_body + "Dear " + txtAssgn.Value + " [" + fgenMV.Fn_Get_Mvar(frm_qstr, "U_DEP_NAME") + "] , <br/><br/>";

                    html_body = html_body + "A Query has been assign to You CSS No. [#" + txtvchnum.Value.Trim() + "], By : " + frm_uname + " <br/>";
                    html_body = html_body + "Module : " + ddModule.Value.Trim() + "<br/>";
                    html_body = html_body + "Icon / Path : " + txtlbl8.Value.Trim() + "<br/>";
                    html_body = html_body + "Query was : " + txtrmk.Text.Trim() + "<br/><br/>";

                    html_body = html_body + "to be done by " + ddresolution.Value.ToString().Trim() + ". <br/>";

                    html_body = html_body + "Regards<br/>";
                    html_body = html_body + "Technical Support Team |  Pocketdriver Limited  |  9015-220-220 | <a href='http://www.Tejaxo.asia'>www.Tejaxo.asia</a> |";
                    html_body = html_body + "<br/>";
                    html_body = html_body + "<br>===========================================================<br>";
                    html_body = html_body + "This Email is Auto generated from the Finsys CSS. If you want a similar CSS for your<br/> ";
                    html_body = html_body + "company, contact us at  marketing@pocketdriver.in <br>";
                    html_body = html_body + "</body></html>";

                    col2 = fgenMV.Fn_Get_Mvar(frm_qstr, "U_EMAILID");
                    if (col2.Length > 1) { }
                    else col2 = "";

                    fgen.send_mail(frm_cocd, "Tejaxo ERP", agt_email, col2, "", subject, html_body);
                    #endregion
    ...
    void save_fun()
    {
        ...
        oporow[doc_df.Value] = txtDFdt.Value.Trim();

        oporow["CSSNO"] = txtvchnum.Value.Trim();
        oporow["CSSDT"] = txtvchdate.Value.Trim();

        oporow["SRNO"] = i;

        oporow["CCODE"] = txtlbl4.Value.ToUpper().Trim();
        oporow["EMODULE"] = ddModule.Value;

        oporow["eicon"] = txtlbl8.Value;

        oporow["Remarks"] = txtrmk.Text;

        oporow["ASG_DPT"] = doc_addl.Value.Trim();
        oporow["ASG_AGT"] = txtAssgn.Value.Trim();

        oporow["CSS_STATUS"] = ddStatus.Value.Trim();

        oporow["ESTATUS"] = "Y";

        if (ddStatus.Value.Trim() == "Confirmation Required")
        {
            oporow["ASG_ASYS"] = "CRM Support";
        }
        else
        {
            oporow["ASG_ASYS"] = ddAnalysis.Value;
        }
        oporow["priority"] = ddresolution.Value;
        ...
        fgen.execute_cmd(frm_qstr, frm_cocd, "update wb_CSS_LOG set last_Action='" + ddStatus.Value.Trim() + "',last_Actdt='" + txtDFdt.Value.Trim() + "' where branchcd||type||trim(CSSNO)||to_char(CSSDT,'dd/mm/yyyy')='" + frm_mbr + "CS" + txtvchnum.Value + txtvchdate.Value + "'");
    ...
    {
        Prg_Id = fgenMV.Fn_Get_Mvar(frm_qstr, "U_FORMID");
        SQuery = "SELECT Userid,Username,Userid AS CODE FROM evas WHERE trim(nvl(close_by,'-'))='-' order by Username";
    }
    ...
    void Type_Sel_query()
    {
        ...
        {
            case "F60106":
                SQuery = "SELECT 'DF' AS FSTR,'Support Request Assignment Logging' as NAME,'DF' AS CODE FROM dual";
                fgenMV.Fn_Set_Mvar(frm_qstr, "U_VTY", "DF");
                break;
    ...
    protected void btnAsgn_Click(object sender, ImageClickEventArgs e)
    {
        hffield.Value = "ASGN";
        make_qry_4_popup();
    ...
    {
        string filePath = hfAtchPath.Value.Substring(hfAtchPath.Value.ToUpper().IndexOf("UPLOAD"), hfAtchPath.Value.Length - hfAtchPath.Value.ToUpper().IndexOf("UPLOAD"));
    ...
    protected void btnDwnld_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            string filePath = hfAtchPath.Value.Substring(hfAtchPath.Value.ToUpper().IndexOf("UPLOAD"), hfAtchPath.Value.Length - hfAtchPath.Value.ToUpper().IndexOf("UPLOAD"));
