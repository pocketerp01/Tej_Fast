...
(function( $, window, document, undefined ) {
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			if ( cOption == '<' )
			{
				...
				if ( cNext == "'" || cNext == '"' )
				{
					...
					if ( sAttr.indexOf('.') != -1 )
					{
						...
						nNewNode.id = aSplit[0].substring(1, aSplit[0].length-1);
						nNewNode.className = aSplit[1];
					}
					else if ( sAttr.charAt(0) == "#" )
					{
						nNewNode.id = sAttr.substring(1, sAttr.length-1);
					}
					else
					{
			...
			{
				/* Length */
				featureNode = _fnFeatureHtmllen( oSettings );
			}
	...
	_api_register( 'page.len()', function ( len ) {
		// Note that we can't call this function 'len()' because `length`
